...
\echo '>> CPL-324 Tier C fix regression: optimistic-concurrency NULL-bypass on app.update_customer_booking_request_draft/app.submit_customer_booking_request/app.request_customer_booking_reschedule/app.request_customer_booking_cancellation -- a NULL p_expected_version is rejected with stale_version, the row proven byte-for-byte unchanged, then the real version succeeds (20260801260000)'
do $$
declare
  v_tenant1 uuid := (select id from app.tenants where slug = 'cbr1');
  v_account_alpha uuid := (select id from app.accounts where tenant_id = v_tenant1 and legal_name = 'Cbr1 Account Alpha');
  v_alpha_admin uuid := '00000000-0000-0000-0000-000000305010';
  v_draft app.customer_portal_booking_requests;
  v_after app.customer_portal_booking_requests;
begin
  -- update_customer_booking_request_draft
  v_draft := app.create_customer_booking_request_draft(v_tenant1, v_account_alpha, null, 'null-bypass probe', '{}'::jsonb, '{}'::jsonb, null, null, null, 'null-bypass-cbr-update', v_alpha_admin, 'alpha-admin');

  begin
    perform app.update_customer_booking_request_draft(v_draft.id, null, 'forged-update', '{}'::jsonb, '{}'::jsonb, null, null, null, v_alpha_admin, 'alpha-admin');
    raise exception 'assertion failed: expected stale_version for a NULL p_expected_version on update_customer_booking_request_draft';
  exception when others then if sqlerrm not like 'stale_version%' then raise; end if;
  end;

  select * into v_after from app.customer_portal_booking_requests where id = v_draft.id;
  if v_after.cargo_description <> v_draft.cargo_description or v_after.record_version <> v_draft.record_version then
    raise exception 'assertion failed: expected the booking request to be byte-for-byte unchanged after a rejected NULL-bypass update attempt, got %', v_after;
  end if;

  v_after := app.update_customer_booking_request_draft(v_draft.id, v_draft.record_version, 'real-version update', '{}'::jsonb, '{}'::jsonb, null, null, null, v_alpha_admin, 'alpha-admin');
  ...
  -- submit_customer_booking_request
  begin
    perform app.submit_customer_booking_request(v_after.id, null, v_alpha_admin, 'alpha-admin');
    raise exception 'assertion failed: expected stale_version for a NULL p_expected_version on submit_customer_booking_request';
  exception when others then if sqlerrm not like 'stale_version%' then raise; end if;
  end;

  if not exists (select 1 from app.customer_portal_booking_requests where id = v_after.id and status = 'draft' and record_version = v_after.record_version) then
    raise exception 'assertion failed: expected the booking request to be byte-for-byte unchanged after a rejected NULL-bypass submit attempt';
  end if;

  v_after := app.submit_customer_booking_request(v_after.id, v_after.record_version, v_alpha_admin, 'alpha-admin');
  ...
  -- request_customer_booking_reschedule
  begin
    perform app.request_customer_booking_reschedule(v_after.id, null, now() + interval '5 days', null, 'null-bypass probe', v_alpha_admin, 'alpha-admin');
    raise exception 'assertion failed: expected stale_version for a NULL p_expected_version on request_customer_booking_reschedule';
  exception when others then if sqlerrm not like 'stale_version%' then raise; end if;
  end;

  if not exists (select 1 from app.customer_portal_booking_requests where id = v_after.id and status = 'submitted' and record_version = v_after.record_version) then
    raise exception 'assertion failed: expected the booking request to be byte-for-byte unchanged after a rejected NULL-bypass reschedule attempt';
  end if;

  v_after := app.request_customer_booking_reschedule(v_after.id, v_after.record_version, now() + interval '5 days', null, 'real-version reschedule', v_alpha_admin, 'alpha-admin');
  if v_after.status <> 'reschedule_requested' then
    raise exception 'assertion failed: expected the real-version reschedule call to succeed, got %', v_after;
  end if;

  -- request_customer_booking_cancellation (reschedule_requested is not a
  -- cancellable status per design decision 5 -- reuse a fresh submitted
  -- draft instead, mirroring the same file's own pre-existing cancellation
  -- test fixture shape).
  v_draft := app.create_customer_booking_request_draft(v_tenant1, v_account_alpha, null, 'cancel probe', '{}'::jsonb, '{}'::jsonb, null, null, null, 'null-bypass-cbr-cancel', v_alpha_admin, 'alpha-admin');

  begin
    perform app.request_customer_booking_cancellation(v_draft.id, null, 'null-bypass probe', v_alpha_admin, 'alpha-admin');
    raise exception 'assertion failed: expected stale_version for a NULL p_expected_version on request_customer_booking_cancellation';
  exception when others then if sqlerrm not like 'stale_version%' then raise; end if;
  end;

  if not exists (select 1 from app.customer_portal_booking_requests where id = v_draft.id and status = 'draft' and record_version = v_draft.record_version) then
    raise exception 'assertion failed: expected the booking request to be byte-for-byte unchanged after a rejected NULL-bypass cancellation attempt';
  end if;

  v_after := app.request_customer_booking_cancellation(v_draft.id, v_draft.record_version, 'real-version cancel', v_alpha_admin, 'alpha-admin');
  if v_after.status <> 'cancelled' then
    raise exception 'assertion failed: expected the real-version cancellation call to succeed, got %', v_after;
  end if;
end $$;
