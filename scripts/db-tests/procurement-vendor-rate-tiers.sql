...
\echo '>> ISS-2026-278 (Step 16 historical-issue-backlog remediation, resumed) regression: app.commit_vendor_rate_import_job composes app.assert_current_step_up_authorization(tenant, actor, ''PRC'', ''Import'') immediately after its own existing PRC:Import check -- a strict no-op for a tenant with no MFA policy configured, and a real block-then-unblock once the tenant opts (PRC, Import) into its own additional_high_risk_actions and completes a genuine step-up challenge'
do $$
declare
  v_tenant1 uuid := (select id from app.tenants where slug = 'ratetier1');
  v_admin1 uuid := '00000000-0000-0000-0000-000000035101';
...
begin
  ...
  v_source_file1 := app.initiate_file_upload(v_tenant1, 'vendor_rate_import_source', 'import_source', gen_random_uuid(), 'vendor-rates-mfacheck-a.csv', 'text/csv', 1024, 'internal', false, null, null, null, 'idem-rt-import-mfacheck-src-a', v_admin1, 'admin');
  perform app.record_file_scan_result(v_source_file1.id, 'clean', 'test-scanner', v_admin1, 'admin');
  v_job1 := app.create_import_export_job(v_tenant1, 'import', 'vendor_rate_import', v_source_file1.id, '{}'::jsonb, 'idem-rt-import-mfacheck-job-a', v_admin1, 'admin');
  perform app.stage_import_rows(v_job1.job_id, jsonb_build_array(jsonb_build_object(
    'vendor_code', 'RT-MFACHECK-A', 'vendor_name', 'Mfa Check Vendor A', 'service_type', 'trucking',
    'origin_lane', 'Jakarta', 'destination_lane', 'Semarang', 'currency', 'IDR', 'base_amount', '500000'
  )), v_admin1, 'admin');
  select id into v_row1_id from app.import_staging_rows where job_id = v_job1.job_id and row_number = 1;
  perform app.validate_vendor_rate_import_row(v_row1_id, v_admin1, 'admin');
  v_committed := app.commit_vendor_rate_import_job(v_job1.job_id, false, v_admin1, 'admin');
  if v_committed.status <> 'completed' then
  ...
  -- (b) this tenant now additively opts (PRC, Import) into its own additional_high_risk_
  ...
  perform app.set_mfa_tenant_policy(v_tenant1, false, '["supreme_admin", "tenant_admin"]'::jsonb, 15, '[{"moduleCode": "PRC", "action": "Import"}]'::jsonb, v_supreme, 'supreme');

  v_source_file2 := app.initiate_file_upload(v_tenant1, 'vendor_rate_import_source', 'import_source', gen_random_uuid(), 'vendor-rates-mfacheck-b.csv', 'text/csv', 1024, 'internal', false, null, null, null, 'idem-rt-import-mfacheck-src-b', v_admin1, 'admin');
  perform app.record_file_scan_result(v_source_file2.id, 'clean', 'test-scanner', v_admin1, 'admin');
  v_job2 := app.create_import_export_job(v_tenant1, 'import', 'vendor_rate_import', v_source_file2.id, '{}'::jsonb, 'idem-rt-import-mfacheck-job-b', v_admin1, 'admin');
  perform app.stage_import_rows(v_job2.job_id, jsonb_build_array(jsonb_build_object(
    'vendor_code', 'RT-MFACHECK-B', 'vendor_name', 'Mfa Check Vendor B', 'service_type', 'trucking',
    'origin_lane', 'Jakarta', 'destination_lane', 'Bandung', 'currency', 'IDR', 'base_amount', '500000'
  )), v_admin1, 'admin');
  select id into v_row2_id from app.import_staging_rows where job_id = v_job2.job_id and row_number = 1;
  perform app.validate_vendor_rate_import_row(v_row2_id, v_admin1, 'admin');
  ...
  v_raised := false;
  begin
    perform app.commit_vendor_rate_import_job(v_job2.job_id, false, v_admin1, 'admin');
  ...
  v_challenge := app.request_mfa_step_up_challenge(v_tenant1, 'PRC', 'Import', v_admin1, 'admin');
  perform app.verify_mfa_step_up_challenge(v_challenge.id, v_admin1, 'admin');

  v_committed := app.commit_vendor_rate_import_job(v_job2.job_id, false, v_admin1, 'admin');
  if v_committed.status <> 'completed' then
  ...
  raise notice 'PASS: app.commit_vendor_rate_import_job (ISS-2026-278, resumed) is a strict no-op for a tenant with no MFA policy configured, blocks with mfa_step_up_required once the tenant opts (PRC, Import) into its own additional_high_risk_actions, and succeeds again once a genuine step-up challenge is requested and verified';
end;
$$;
