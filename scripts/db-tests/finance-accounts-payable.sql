...
begin
  ...
  select * into v_row from app.get_finance_ap_exposure_summary(v_tenant_a, v_vendor_id, '00000000-0000-0000-0000-000000029502') where currency = 'USD';
  if v_row.open_count < 3 then
    raise exception 'assertion failed: expected at least 3 open/partial/held USD items for tenant A''s own vendor, got %', v_row;
  end if;
...
begin
  v_tenant_a := (select id from app.tenants where slug = 'acmeapa');
  v_vendor_id := (select id from app.master_records where tenant_id = v_tenant_a and code = 'VEND-AP-1');
  v_source_id := pg_temp.iss319_mint_vendor_bill(v_tenant_a, (select id from app.accounts where tenant_id = v_tenant_a), v_vendor_id, '00000000-0000-0000-0000-000000029502', 'financemanagera', 'iss319-ap-b4-eur');

  perform app.post_finance_ap_open_item(v_tenant_a, null, v_vendor_id, 'vendor_bill', v_source_id, 'EUR', 200, '2026-03-12'::date, '2026-04-11'::date, '00000000-0000-0000-0000-000000029502', 'financemanagera');

  select * into v_row_eur from app.get_finance_ap_exposure_summary(v_tenant_a, v_vendor_id, '00000000-0000-0000-0000-000000029502') where currency = 'EUR';
  ...
  select * into v_row_usd from app.get_finance_ap_exposure_summary(v_tenant_a, v_vendor_id, '00000000-0000-0000-0000-000000029502') where currency = 'USD';
  ...
  select * into v_rate from app.create_finance_exchange_rate_draft(v_tenant_a, 'spot', 'USD', 'IDR', 15700, 'manual', '2026-01-01'::timestamptz, null, '00000000-0000-0000-0000-000000029502', 'financemanagera');
  perform app.approve_finance_exchange_rate(v_rate.id, v_rate.record_version, '00000000-0000-0000-0000-000000029502', 'financemanagera');
  ...
  select count(*) into v_row_count from app.get_finance_ap_exposure_summary(v_tenant_a, v_vendor_id, '00000000-0000-0000-0000-000000029502');
