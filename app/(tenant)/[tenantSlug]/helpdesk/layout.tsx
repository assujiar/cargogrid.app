 ...
 * independently of this guard). Reuses `resolveTicketAccessForRequest`, the
 * same guard the `helpdesk/page.tsx` route itself already uses.
 */
export default async function HelpdeskModuleLayout({
  children,
  params,
}: {
  children: ReactNode;
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-3">
        <span className="text-sm font-semibold text-neutral-900">CargoGrid — {access.tenant.slug} — Helpdesk</span>
        <div className="flex items-center gap-4">
          <TenantPortalNav tenantSlug={access.tenant.slug} current="helpdesk" />
