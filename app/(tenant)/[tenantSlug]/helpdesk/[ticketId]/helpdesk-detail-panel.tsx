...
// Client-side convenience mirror of the requester_allowed subset of app.
// ticket_status_transitions (cancel, reopen) -- the SERVER (app.
// transition_ticket_status, via app._ticket_transition_authority /
// app._is_ticket_requester_party) is the real, enforcing source of truth.
function nextTenantActions(status: TicketStatus): readonly { toStatus: TicketStatus; label: string; requiresReason: boolean }[] {
  if (status === "new" || status === "open" || status === "pending" || status === "on_hold") {
    return [{ toStatus: "cancelled", label: "Cancel case", requiresReason: true }];
  }
  if (status === "resolved" || status === "closed") {
    return [{ toStatus: "open", label: "Reopen case", requiresReason: true }];
  }
  return [];
}

type BoundAction = (prevState: HelpdeskActionState, formData: FormData) => Promise<HelpdeskActionState>;

function MessageBubble({ message }: { message: HelpdeskTicketMessageRow }) {
  return (
...
export function HelpdeskDetailPanel({
  detail,
  messages,
  replyAction,
  transitionAction,
  ...
  messages: readonly HelpdeskTicketMessageRow[];
  replyAction: BoundAction;
  transitionAction: (toStatus: TicketStatus) => BoundAction;
}) {
  const actions = nextTenantActions(detail.status);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-xs text-neutral-500">Only messages CargoGrid support intends for you are shown here — internal Platform notes are never visible on this page.</p>
