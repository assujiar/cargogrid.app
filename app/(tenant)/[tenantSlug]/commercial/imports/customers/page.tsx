...
import {
  bootstrapCustomerImportAction,
  uploadCustomerImportSourceAction,
  stageAndValidateCustomerImportRowsAction,
  commitCustomerImportAction,
} from "./actions.ts";
import {
  BootstrapCustomerImportForm,
  UploadCustomerImportSourceForm,
  StageAndValidateCustomerImportRowsForm,
  CommitCustomerImportForm,
} from "./customer-import-forms.tsx";
...
/**
 * Customer import wizard (CG-AUDIT-2026-09-02 A4, fifth import schema after
 * finance_opening_balance_import, employee_import, vendor_import, and
 * vendor_rate_import). A real, multi-step state machine reflecting the actual
 * underlying job lifecycle (bootstrap -> upload+scan -> stage+validate -> review
 * -> commit), a near-mechanical port of procurement/imports/vendors/page.tsx's
 * own structure -- see that page's doc comment for why the shape isn't
 * simplified further.
 *
 * Linked from the accounts page's own header, unlike finance's standalone
 * unlinked opening-balances page -- Commercial's own natural integration point (a
 * bulk-onboarding entry point next to the account list itself, which today has no
 * create-account form of its own -- accounts are otherwise only ever created by
...
export default async function CustomerImportPage({
  ...
  const rowColumns: readonly DataTableColumn<ImportStagingRow>[] = [
    ...
    { key: "taxId", header: "Tax ID", render: (row) => (row.rawPayload.tax_id as string | undefined) ?? "—" },
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-text-primary">Customer import</h1>
        <p className="text-sm text-text-secondary">
          Bulk-loads customer accounts from a roster CSV file. Real, staged, row-validated data -- each committed row either creates a real account or links to an existing one matched by legal name/tax ID, never a duplicate.
        </p>
      </div>
      ...
      {loadFailed ? (
        ...
        <EmptyState
          ...
          description="This organization has not yet published the file-upload rules and column definition customer imports require. This is a one-time setup."
          primaryAction={<BootstrapCustomerImportForm action={bootstrapCustomerImportAction.bind(null, tenantSlug)} />}
        />
      ) : jobLoadError ? (
        ...
        <section className="rounded-md border border-neutral-200 p-4">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            Columns: legal_name (required), trade_name, tax_id, billing_line1, billing_city, billing_region, billing_postal_code, billing_country.
          </p>
          <div className="mt-2">
            <UploadCustomerImportSourceForm action={uploadCustomerImportSourceAction.bind(null, tenantSlug)} />
          </div>
        </section>
      ) : (
        <>
          ...
          {job.status === "completed" ? (
            <SuccessState
              ...
              description={`Committed with ${job.validRowCount} valid row(s) and ${job.invalidRowCount} invalid row(s) skipped. A row matching an existing customer by legal name/tax ID was linked to it rather than duplicated.`}
            />
            ...
            <section className="rounded-md border border-neutral-200 p-4">
              ...
              <div className="mt-2">
                <StageAndValidateCustomerImportRowsForm action={stageAndValidateCustomerImportRowsAction.bind(null, tenantSlug, job.jobId)} />
              </div>
            </section>
          ) : pendingCount > 0 ? (
            <section className="rounded-md border border-neutral-200 p-4">
              ...
              <section className="rounded-md border border-neutral-200 p-4">
                ...
                <p className="mt-1 text-xs text-text-secondary">
                  {invalidCount > 0
                    ...
                    : "Every row is valid. Committing creates or links one account per row."}
                </p>
                <div className="mt-2">
                  <CommitCustomerImportForm action={commitCustomerImportAction.bind(null, tenantSlug, job.jobId)} invalidRowCount={invalidCount} />
                </div>
              </section>
            </>
          )}
