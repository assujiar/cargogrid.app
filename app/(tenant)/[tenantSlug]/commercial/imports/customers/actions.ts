...
/**
 * Customer import Server Actions (CG-AUDIT-2026-09-02 A4, fifth import schema after
 * finance_opening_balance_import, employee_import, vendor_import, and
 * vendor_rate_import). Every RPC this file calls already existed, fully
 * implemented and tested (app.create_import_export_job/app.stage_import_rows/
 * app.validate_customer_import_row/app.commit_customer_import_job, plus PLT-121's
 * generic config-draft primitives for the one-time per-tenant bootstrap) -- this
 * file is a near-mechanical port of procurement/imports/vendors/actions.ts's own
 * trio, swapping the vendor-profile validate/commit adapters for their
 * customer/account equivalents.
 *
 * Unlike vendor_import, the import_export:customer_import SCHEMA was already
 * registered as a real global catalog row directly by
 * 20260830120000_create_customer_and_item_import_adapters.sql (the same migration
 * that ALSO registers item_import's own schema kind -- customer_import and
 * item_import are siblings). Only the master_data_import_source DOCUMENT TYPE
 * (shared by both schemas) was never registered by any real migration, only by a
 * db-test fixture, the exact vendor_import_source gap repeated a fourth time --
 * fixed by 20260917040000_register_master_data_import_source_document_type.sql.
 *
 * server/mutations/account.ts had ZERO wrapper for
 * validate_customer_import_row/commit_customer_import_job at all (a from-scratch
 * build, like vendor_import's own slice, not a small parity-gap patch like
 * vendor_rate_import's) -- both new functions reuse the generic PLT-131 parsers
 * directly.
 *
 * app.commit_customer_import_job's own authority composition matches
 * vendor_import's: BOTH app.is_support_grant_authority AND COM:Import, additive,
 * never either alone -- this action's own gate (resolveCommercialAccessForRequest)
 * is coarser and deliberately does not stand in for either check. Commit is
 * create-or-link, not flag-for-review: a duplicate-fingerprint match resolves to
 * the existing account and is counted as linked, never blocked -- unless that
 * account is under legal hold (import_blocked_legal_hold), which aborts the whole
 * commit.
 *
...
import { validateCustomerImportRow, commitCustomerImportJob, AccountMutationError, type AccountMutationRpcClient } from "../../../../../../server/mutations/account.ts";
import type { BackgroundJobMutationRpcClient } from "../../../../../../server/mutations/background-job.ts";

const SCHEMA_CODE = "customer_import";
...
const CUSTOMER_IMPORT_COLUMNS = [
  ...
  { key: "tax_id", label: "Tax ID", required: false, data_type: "text" },
  { key: "billing_line1", label: "Billing address line 1", required: false, data_type: "text" },
  { key: "billing_city", label: "Billing city", required: false, data_type: "text" },
  { key: "billing_region", label: "Billing region", required: false, data_type: "text" },
  { key: "billing_postal_code", label: "Billing postal code", required: false, data_type: "text" },
  { key: "billing_country", label: "Billing country", required: false, data_type: "text" },
] as const;
...
const OK: CustomerImportActionState = { error: null };
const NO_ACCESS: CustomerImportActionState = { error: "You don't have access to this organization's Commercial workspace." };

type UnifiedClient = ConfigMutationRpcClient & DocumentMutationRpcClient & ImportExportMutationRpcClient & ImportExportQueryRpcClient & AccountMutationRpcClient & StorageUploadClient;
...
/** One-time per-tenant setup: publishes both the document-type definition (file-upload rules) and the import schema's column definition -- everything a tenant needs before its first customer-roster upload. Idempotent: createConfigDraft returns any already-pending draft rather than a new one, so re-running this after a partial failure safely resumes. */
export async function bootstrapCustomerImportAction(tenantSlug: string, _prevState: CustomerImportActionState, _formData: FormData): Promise<CustomerImportActionState> {
  ...
  try {
    ...
    await setConfigItems(client, {
      ...
      items: [{ key: "columns", value: [...CUSTOMER_IMPORT_COLUMNS], canonicalRef: null }],
      actorAuthUserId: access.authUserId,
      actorLabel: access.authUserId,
    });
    ...
    if (error instanceof ConfigMutationError || error instanceof DocumentMutationError || error instanceof ImportExportMutationError) {
      return { error: `Could not set up customer imports: ${error.message}` };
    }
    throw error;
  }

  revalidatePath(`/${tenantSlug}/commercial/imports/customers`);
  return OK;
}
...
export async function uploadCustomerImportSourceAction(tenantSlug: string, _prevState: CustomerImportActionState, formData: FormData): Promise<CustomerImportActionState> {
  const access = await resolveCommercialAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    const storeError = await storeFileBytesAndEnqueueScan(client, toBackgroundJobClient(supabase), uploaded, file, access.tenant.id, access.authUserId, "customer roster source file");
    if (storeError) {
      return storeError;
    }
  ...
  redirect(`/${tenantSlug}/commercial/imports/customers?job=${createdJobId}`);
}
...
export async function stageAndValidateCustomerImportRowsAction(tenantSlug: string, jobId: string, _prevState: CustomerImportActionState, _formData: FormData): Promise<CustomerImportActionState> {
  const access = await resolveCommercialAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    for (const row of stagedRows) {
      if (row.validationStatus === "pending") {
        await validateCustomerImportRow(client, { stagingRowId: row.id, actorAuthUserId: access.authUserId, actorLabel: access.authUserId });
      }
    }
  } catch (error) {
    if (error instanceof ImportExportMutationError || error instanceof ImportExportQueryError || error instanceof AccountMutationError) {
...
/** Requires BOTH tenant_admin/Supreme authority AND COM:Import, in-body -- this action's own gate (resolveCommercialAccessForRequest) is coarser and deliberately does not stand in for either check. Passes the caller's own client IP so the tenant's IP allowlist (if configured) is genuinely enforced. Create-or-link: a row whose legal name/tax ID matches an existing account links to it rather than duplicating -- unless that account is under legal hold, which aborts the whole commit. */
export async function commitCustomerImportAction(tenantSlug: string, jobId: string, _prevState: CustomerImportActionState, formData: FormData): Promise<CustomerImportActionState> {
  const access = await resolveCommercialAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  const client = toUnifiedClient(createSupabaseServiceRoleClient());
  try {
    await commitCustomerImportJob(client, { jobId, allowPartial, actorAuthUserId: access.authUserId, actorLabel: access.authUserId, clientIp });
  } catch (error) {
