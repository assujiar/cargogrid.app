...
/**
 * Integrations module shell (audit remediation A1). Was a bare
 ...
 * tenant member" guard `integrations/page.tsx` itself already uses.
 */
export default async function IntegrationsModuleLayout({
  children,
  params,
}: {
  children: ReactNode;
      ...
      <main id="main-content" tabIndex={-1} className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center gap-3 px-4 text-center">
        ...
        <p className="text-sm text-neutral-600">
          {access.status === "tenant_suspended"
            ...
            : "You don't have access to this organization's Integrations workspace."}
        </p>
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-3">
        <span className="text-sm font-semibold text-neutral-900">CargoGrid — {access.tenant.slug} — Integrations</span>
        <div className="flex items-center gap-4">
          <TenantPortalNav tenantSlug={access.tenant.slug} current="integrations" />
