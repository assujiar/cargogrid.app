...
import { VENDOR_RATE_IMPORT_SCHEMA_CODE } from "../../../../../../server/contracts/procurement-rate/procurement-rate.ts";
...
import {
  bootstrapVendorRateImportAction,
  uploadVendorRateImportSourceAction,
  stageAndValidateVendorRateImportRowsAction,
  commitVendorRateImportAction,
} from "./actions.ts";
import {
  BootstrapVendorRateImportForm,
  UploadVendorRateImportSourceForm,
  StageAndValidateVendorRateImportRowsForm,
  CommitVendorRateImportForm,
} from "./vendor-rate-import-forms.tsx";
...
/**
 * Vendor rate import wizard (CG-AUDIT-2026-09-02 A4, fourth import schema after
 * finance_opening_balance_import, employee_import, and vendor_import). A real,
 * multi-step state machine reflecting the actual underlying job lifecycle
 * (bootstrap -> upload+scan -> stage+validate -> review -> commit), a
 * near-mechanical port of procurement/imports/vendors/page.tsx's own structure --
 * see that page's doc comment for why the shape isn't simplified further.
 *
 * Linked from the vendor rate directory page's own header, unlike finance's
 * standalone unlinked opening-balances page -- Procurement's own natural
 * integration point (a bulk-onboarding entry point next to the one-at-a-time
 * create form).
 */
export default async function VendorRateImportPage({
  params,
  searchParams,
}: {
  ...
  const rowColumns: readonly DataTableColumn<ImportStagingRow>[] = [
    ...
    { key: "vendor", header: "Vendor", render: (row) => (row.rawPayload.vendor_name as string | undefined) ?? "—" },
    { key: "lane", header: "Lane", render: (row) => `${(row.rawPayload.origin_lane as string | undefined) ?? "—"} → ${(row.rawPayload.destination_lane as string | undefined) ?? "—"}` },
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-xl font-semibold text-text-primary">Vendor rate import</h1>
        <p className="text-sm text-text-secondary">
          Bulk-loads vendor rates and pricelists from a CSV file, including up to 3 weight/volume tier slots per row. Real, staged, row-validated data -- each committed row creates one real rate version, landing as pending_approval exactly like a manually-created one.
        </p>
      </div>
      ...
      {loadFailed ? (
        ...
        <EmptyState
          ...
          description="This organization has not yet published the file-upload rules and column definition vendor rate imports require. This is a one-time setup."
          primaryAction={<BootstrapVendorRateImportForm action={bootstrapVendorRateImportAction.bind(null, tenantSlug)} />}
        />
      ) : jobLoadError ? (
        ...
        <section className="rounded-md border border-neutral-200 p-4">
          ...
          <p className="mt-1 text-xs text-text-secondary">
            Columns: vendor_code, vendor_name, vendor_master_code, service_type, mode, origin_lane, destination_lane, equipment_type, currency, base_amount, minimum_amount, lead_time_days, capacity_terms, plus tier1/tier2/tier3_weight_min/weight_max/volume_min/volume_max/amount/minimum_charge.
          </p>
          <div className="mt-2">
            <UploadVendorRateImportSourceForm action={uploadVendorRateImportSourceAction.bind(null, tenantSlug)} />
          </div>
        </section>
      ) : (
        <>
          ...
          {job.status === "completed" ? (
            <SuccessState title="Import completed" description={`Committed with ${job.validRowCount} valid row(s) and ${job.invalidRowCount} invalid row(s) skipped. Every imported rate lands as pending_approval -- approve each one on its own detail page before it can be selected.`} />
            ...
            <section className="rounded-md border border-neutral-200 p-4">
              ...
              <div className="mt-2">
                <StageAndValidateVendorRateImportRowsForm action={stageAndValidateVendorRateImportRowsAction.bind(null, tenantSlug, job.jobId)} />
              </div>
            </section>
          ) : pendingCount > 0 ? (
            <section className="rounded-md border border-neutral-200 p-4">
              ...
              <section className="rounded-md border border-neutral-200 p-4">
                ...
                <p className="mt-1 text-xs text-text-secondary">
                  {invalidCount > 0
                    ...
                    : "Every row is valid. Committing creates one real rate version per row, pending approval."}
                </p>
                <div className="mt-2">
                  <CommitVendorRateImportForm action={commitVendorRateImportAction.bind(null, tenantSlug, job.jobId)} invalidRowCount={invalidCount} />
                </div>
              </section>
            </>
          )}
