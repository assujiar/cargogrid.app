  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={`map-line-po-${line.id}`} label={<span className="sr-only">PO line id</span>}>
        ...
        <FormField id={`map-line-rate-${line.id}`} label={<span className="sr-only">Rate version id</span>}>
...
function RaiseDisputeForm({ matchLineId, raiseDisputeAction }: { matchLineId: string | null; raiseDisputeAction: BoundFormAction }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={`${reactId}-reason`} label={<span className="sr-only">Dispute reason</span>}>
        ...
        <FormField id={`${reactId}-amount`} label={<span className="sr-only">Disputed amount</span>}>
          <NumberInput id={`${reactId}-amount`} name="disputedAmount" step="0.01" placeholder="Disputed amount" invalid={Boolean(state.error)} aria-describedby={describedBy} className="w-32 text-xs" />
        </FormField>
...
function RespondDisputeForm({ dispute, respondDisputeAction }: { dispute: VendorBillMatchDispute; respondDisputeAction: BoundFormAction }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={`respond-dispute-text-${dispute.id}`} label={<span className="sr-only">Vendor&apos;s response</span>}>
...
function ResolveDisputeForm({ dispute, resolveDisputeAction }: { dispute: VendorBillMatchDispute; resolveDisputeAction: BoundFormAction }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      ...
      <input type="hidden" name="expectedVersion" value={dispute.recordVersion} />
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={`resolve-dispute-decision-${dispute.id}`} label={<span className="sr-only">Decision</span>}>
        ...
        <FormField id={`resolve-dispute-note-${dispute.id}`} label={<span className="sr-only">Resolution note</span>}>
...
function RequestExceptionForm({ requestExceptionAction }: { requestExceptionAction: BoundFormAction }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={reactId} label={<span className="sr-only">Exception reason</span>}>
...
function DecideExceptionForm({ approval, decideExceptionAction }: { approval: VendorBillMatchExceptionApproval; decideExceptionAction: BoundFormAction }) {
  ...
  return (
    <form action={formAction} className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap items-center gap-1">
        <FormField id={`decide-exception-decision-${approval.id}`} label={<span className="sr-only">Decision</span>}>
        ...
        <FormField id={`decide-exception-note-${approval.id}`} label={<span className="sr-only">Decision note</span>}>
  ...
  return (
    <details className="rounded-md border border-neutral-200 p-3">
      ...
      <form action={formAction} className="mt-2 flex flex-col gap-2">
        <FormField id="re-evaluate-po" label={<span className="sr-only">Purchase order id</span>}>
              ...
              {lines.map((line) => (
                <tr key={line.billLineId} className="border-t border-neutral-200">
                  ...
                  <td className="px-2 py-1">
                    <FormField id={`re-evaluate-qty-${line.billLineId}`} label={<span className="sr-only">Vendor quantity, line {line.lineNo}</span>}>
                      <NumberInput id={`re-evaluate-qty-${line.billLineId}`} name={`quantity_${line.billLineId}`} step="0.0001" defaultValue={line.vendorStatedQuantity ?? undefined} className="w-20" invalid={Boolean(state.error)} aria-describedby={describedBy} />
                    </FormField>
                  </td>
                  <td className="px-2 py-1">
                    <FormField id={`re-evaluate-uom-${line.billLineId}`} label={<span className="sr-only">Vendor UOM, line {line.lineNo}</span>}>
                  ...
                  <td className="px-2 py-1">
                    <FormField id={`re-evaluate-rate-${line.billLineId}`} label={<span className="sr-only">Vendor rate, line {line.lineNo}</span>}>
                      <NumberInput id={`re-evaluate-rate-${line.billLineId}`} name={`rate_${line.billLineId}`} step="0.0001" defaultValue={line.vendorStatedRate ?? undefined} className="w-20" invalid={Boolean(state.error)} aria-describedby={describedBy} />
                    </FormField>
                  </td>
                  <td className="px-2 py-1">
                    <FormField id={`re-evaluate-amount-${line.billLineId}`} label={<span className="sr-only">Vendor amount, line {line.lineNo}</span>}>
                      <NumberInput id={`re-evaluate-amount-${line.billLineId}`} name={`amount_${line.billLineId}`} step="0.01" required defaultValue={line.vendorStatedAmount ?? undefined} className="w-24" invalid={Boolean(state.error)} aria-describedby={describedBy} />
                    </FormField>
                  </td>
                </tr>
              ))}
      ...
      <section className="flex flex-wrap items-start gap-4">
        ...
        {matchCase.overallStatus !== "cancelled" && matchCase.overallStatus !== "matched" ? (
          <form action={cancelFormAction} className="flex flex-col gap-1">
            <FormField id="cancel-case-reason" label={<span className="sr-only">Cancel reason</span>}>
