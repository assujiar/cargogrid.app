...
/**
 * Dashboards module shell (audit remediation A1). Was a bare `<TenantMain>`
 * pass-through with no access check and no way to reach any other module
 * from here; now matches the guard-then-chrome shape `admin/layout.tsx` and
 * `commercial/layout.tsx` already established (`docs/architecture/
 * 09_UX_DESIGN_SYSTEM_WORKSTREAM.md` §2.1: the route group is a UX boundary
 * only, every page nested below still enforces its own RLS/RBAC
 * independently of this guard). Reuses `resolveCommercialAccessForRequest`,
 * the same domain-agnostic "any active tenant member" guard
 * `dashboards/page.tsx` itself already uses.
 */
export default async function DashboardsModuleLayout({
  children,
  params,
}: {
  children: ReactNode;
      ...
      <main id="main-content" tabIndex={-1} className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center gap-3 px-4 text-center">
        ...
        <p className="text-sm text-neutral-600">
          {access.status === "tenant_suspended"
            ...
            : "You don't have access to this organization's Dashboards workspace."}
        </p>
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="flex items-center justify-between border-b border-neutral-200 bg-neutral-50 px-6 py-3">
        <span className="text-sm font-semibold text-neutral-900">CargoGrid — {access.tenant.slug} — Dashboards</span>
        <div className="flex items-center gap-4">
          <TenantPortalNav tenantSlug={access.tenant.slug} current="dashboards" />
