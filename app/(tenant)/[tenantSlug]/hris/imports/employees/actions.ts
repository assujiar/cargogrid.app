...
/**
 * Employee import Server Actions (CG-AUDIT-2026-09-02 A4, second import schema
 * after finance_opening_balance_import -- see this session's backlog entry for the
 * selection rationale). Every RPC this file calls already existed, fully
 * implemented and tested (app.create_import_export_job/app.stage_import_rows/
 * app.validate_employee_import_row/app.commit_employee_import_job, plus PLT-121's
 * generic config-draft primitives for the one-time per-tenant bootstrap) -- this
 * file is a near-mechanical port of
 * finance/imports/opening-balances/actions.ts's own trio, swapping the
 * finance-specific validate/commit adapters for their HRS equivalents.
 *
 * Unlike finance_opening_balance_source, the `employee_document` document type
 * and `employee_import` schema are both already registered as GLOBAL catalog
 * rows directly by 20260730830000_create_hris_employee_master.sql -- but each
 * tenant must still separately publish its own `document:employee_document` and
 * `import_export:employee_import` config VERSIONS (column definitions / upload
 * rules) before a real upload or stage can succeed, the identical per-tenant
 * onboarding step scripts/db-tests/hris-employee-master.sql's own fixture setup
 * performs. No new migration was needed for this schema.
 *
...
import { validateEmployeeImportRow, commitEmployeeImportJob, EmployeeMutationError, type EmployeeMutationRpcClient } from "../../../../../../server/mutations/employee.ts";
import type { BackgroundJobMutationRpcClient } from "../../../../../../server/mutations/background-job.ts";

const SCHEMA_CODE = "employee_import";
const DOCUMENT_TYPE_CODE = "employee_document";

const EMPLOYEE_IMPORT_COLUMNS = [
  { key: "employee_number", label: "Employee Number", required: false, data_type: "text" },
  { key: "full_name", label: "Full Name", required: true, data_type: "text" },
  { key: "employment_type", label: "Employment Type", required: true, data_type: "text" },
  { key: "work_email", label: "Work Email", required: false, data_type: "email" },
  { key: "personal_email", label: "Personal Email", required: false, data_type: "email" },
  { key: "personal_phone", label: "Personal Phone", required: false, data_type: "text" },
  { key: "company_org_unit_code", label: "Company Code", required: false, data_type: "text" },
  { key: "branch_org_unit_code", label: "Branch Code", required: false, data_type: "text" },
  { key: "department_org_unit_code", label: "Department Code", required: false, data_type: "text" },
  { key: "position_title", label: "Position Title", required: false, data_type: "text" },
] as const;

export interface EmployeeImportActionState {
  readonly error: string | null;
}

const OK: EmployeeImportActionState = { error: null };
const NO_ACCESS: EmployeeImportActionState = { error: "You don't have access to this organization's HR workspace." };

type UnifiedClient = ConfigMutationRpcClient & DocumentMutationRpcClient & ImportExportMutationRpcClient & ImportExportQueryRpcClient & EmployeeMutationRpcClient & StorageUploadClient;
...
/** One-time per-tenant setup: publishes both the document-type definition (file-upload rules) and the import schema's column definition -- everything a tenant needs before its first employee-roster upload. Idempotent: createConfigDraft returns any already-pending draft rather than a new one, so re-running this after a partial failure safely resumes. */
export async function bootstrapEmployeeImportAction(tenantSlug: string, _prevState: EmployeeImportActionState, _formData: FormData): Promise<EmployeeImportActionState> {
  ...
  try {
    ...
    await setConfigItems(client, {
      ...
      items: [{ key: "columns", value: [...EMPLOYEE_IMPORT_COLUMNS], canonicalRef: null }],
      actorAuthUserId: access.authUserId,
      actorLabel: access.authUserId,
    });
    ...
    if (error instanceof ConfigMutationError || error instanceof DocumentMutationError || error instanceof ImportExportMutationError) {
      return { error: `Could not set up employee imports: ${error.message}` };
    }
    throw error;
  }

  revalidatePath(`/${tenantSlug}/hris/imports/employees`);
  return OK;
}
...
export async function uploadEmployeeImportSourceAction(tenantSlug: string, _prevState: EmployeeImportActionState, formData: FormData): Promise<EmployeeImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    const storeError = await storeFileBytesAndEnqueueScan(client, toBackgroundJobClient(supabase), uploaded, file, access.tenant.id, access.authUserId, "employee roster source file");
    if (storeError) {
      return storeError;
    }
  ...
  redirect(`/${tenantSlug}/hris/imports/employees?job=${createdJobId}`);
}
...
export async function stageAndValidateEmployeeImportRowsAction(tenantSlug: string, jobId: string, _prevState: EmployeeImportActionState, _formData: FormData): Promise<EmployeeImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    for (const row of stagedRows) {
      if (row.validationStatus === "pending") {
        await validateEmployeeImportRow(client, { stagingRowId: row.id, actorAuthUserId: access.authUserId, actorLabel: access.authUserId });
      }
    }
  } catch (error) {
    if (error instanceof ImportExportMutationError || error instanceof ImportExportQueryError || error instanceof EmployeeMutationError) {
...
/** Requires HRS:Import authority, in-body -- this action's own gate (resolveHrisAccessForRequest) is coarser and deliberately does not stand in for it. Passes the caller's own client IP so the tenant's IP allowlist (if configured) is genuinely enforced, matching commitOpeningBalanceImportAction's own precedent. */
export async function commitEmployeeImportAction(tenantSlug: string, jobId: string, _prevState: EmployeeImportActionState, formData: FormData): Promise<EmployeeImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  const client = toUnifiedClient(createSupabaseServiceRoleClient());
  try {
    await commitEmployeeImportJob(client, { jobId, allowPartial, actorAuthUserId: access.authUserId, actorLabel: access.authUserId, clientIp });
  } catch (error) {
