...
/**
 * Leave opening balance import Server Actions (CG-AUDIT-2026-09-02 A4, tenth
 * import schema). Every RPC this file calls already existed, fully
 * implemented and tested (app.create_import_export_job/app.stage_import_rows/
 * app.validate_leave_opening_balance_import_row/
 * app.commit_leave_opening_balance_import_job, plus PLT-121's generic
 * config-draft primitives for the one-time per-tenant bootstrap) -- this
 * file is a near-mechanical port of hris/imports/timesheet/actions.ts's own
 * trio, swapping the timesheet-specific validate/commit adapters for their
 * leave-opening-balance equivalents.
 *
 * Unlike attendance_device_import/timesheet_import (both already fully
 * catalogued), the `leave_opening_balance_import` SCHEMA kind was already a
 * real, global app.import_export_schemas row
 * (20260831260000_create_inventory_and_leave_opening_balance_import_adapters.sql),
 * but no DOCUMENT TYPE for the raw source file existed anywhere -- not even
 * in a db-test fixture (scripts/db-tests/hris-leave-permit-business-trip.sql's
 * own bootstrap reuses the generic, COM-owned master_data_import_source
 * document type rather than a dedicated one). This session registered a
 * dedicated, HRS-owned `leave_opening_balance_import_source` document type
 * (20260917050000_register_leave_opening_balance_import_source_document_type.sql),
 * mirroring timesheet_import_source's/attendance_device_import_source's own
 * one-document-type-per-schema precedent rather than sharing a generic one.
 * Each tenant must still separately publish its own
 * `document:leave_opening_balance_import_source` and
 * `import_export:leave_opening_balance_import` config VERSIONS before a
 * real upload or stage can succeed -- this file's own bootstrap action.
 *
 * IMPORTANT, surfaced in this wizard's own copy: this is a one-time cutover
 * load, not a routine batch import. app.load_opening_leave_balance's own
 * idempotency key is derived from the staging row's own id, so re-running
 * the SAME job is a safe no-op, but re-uploading a corrected CSV posts a
 * brand-new ledger entry on top of a wrong one rather than overwriting it --
 * this import can never correct a mistake by itself; correcting a wrong
 * opening balance requires the separate, deliberate leave-balance adjustment
 * path (app.adjust_leave_balance), outside this wizard entirely.
 *
 * commit_leave_opening_balance_import_job's own authority composition is the
 * richest of any A4 import schema so far: BOTH app.is_support_grant_authority
 * (Supreme Admin or tenant_admin) AND HRS:Import are required (additive,
 * never either-or) -- a plain HR staffer holding only HRS:Import is refused.
 *
...
import { validateLeaveOpeningBalanceImportRow, commitLeaveOpeningBalanceImportJob, LeaveMutationError, type LeaveMutationRpcClient } from "../../../../../../server/mutations/leave.ts";
...
const LEAVE_OPENING_BALANCE_IMPORT_COLUMNS = [
  { key: "employee_number", label: "Employee Number", required: true, data_type: "text" },
  { key: "leave_type_code", label: "Leave Type", required: true, data_type: "text" },
  { key: "units", label: "Units", required: true, data_type: "number" },
  { key: "as_of_date", label: "As Of", required: true, data_type: "date" },
  { key: "source_reference", label: "Source Reference", required: false, data_type: "text" },
] as const;

export interface LeaveOpeningBalanceImportActionState {
  readonly error: string | null;
}

const OK: LeaveOpeningBalanceImportActionState = { error: null };
const NO_ACCESS: LeaveOpeningBalanceImportActionState = { error: "You don't have access to this organization's HR workspace." };

type UnifiedClient = ConfigMutationRpcClient & DocumentMutationRpcClient & ImportExportMutationRpcClient & ImportExportQueryRpcClient & LeaveMutationRpcClient & StorageUploadClient;
...
/** One-time per-tenant setup: publishes both the document-type definition (file-upload rules) and the import schema's column definition -- everything a tenant needs before its first opening-balance cutover upload. Idempotent: createConfigDraft returns any already-pending draft rather than a new one, so re-running this after a partial failure safely resumes. */
export async function bootstrapLeaveOpeningBalanceImportAction(tenantSlug: string, _prevState: LeaveOpeningBalanceImportActionState, _formData: FormData): Promise<LeaveOpeningBalanceImportActionState> {
  ...
  try {
    ...
    await setConfigItems(client, {
      ...
      items: [{ key: "columns", value: [...LEAVE_OPENING_BALANCE_IMPORT_COLUMNS], canonicalRef: null }],
      actorAuthUserId: access.authUserId,
      actorLabel: access.authUserId,
    });
    ...
    if (error instanceof ConfigMutationError || error instanceof DocumentMutationError || error instanceof ImportExportMutationError) {
      return { error: `Could not set up leave opening balance imports: ${error.message}` };
    }
    throw error;
  }

  revalidatePath(`/${tenantSlug}/hris/imports/leave-opening-balance`);
  return OK;
}
...
export async function uploadLeaveOpeningBalanceImportSourceAction(tenantSlug: string, _prevState: LeaveOpeningBalanceImportActionState, formData: FormData): Promise<LeaveOpeningBalanceImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    const storeError = await storeFileBytesAndEnqueueScan(client, toBackgroundJobClient(supabase), uploaded, file, access.tenant.id, access.authUserId, "leave opening balance cutover source file");
    if (storeError) {
      return storeError;
    }
  ...
  redirect(`/${tenantSlug}/hris/imports/leave-opening-balance?job=${createdJobId}`);
}
...
export async function stageAndValidateLeaveOpeningBalanceImportRowsAction(tenantSlug: string, jobId: string, _prevState: LeaveOpeningBalanceImportActionState, _formData: FormData): Promise<LeaveOpeningBalanceImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    for (const row of stagedRows) {
      if (row.validationStatus === "pending") {
        await validateLeaveOpeningBalanceImportRow(client, { stagingRowId: row.id, actorAuthUserId: access.authUserId, actorLabel: access.authUserId });
      }
    }
  } catch (error) {
    if (error instanceof ImportExportMutationError || error instanceof ImportExportQueryError || error instanceof LeaveMutationError) {
...
/** Requires BOTH is_support_grant_authority AND HRS:Import, in-body -- this action's own gate (resolveHrisAccessForRequest) is coarser and deliberately does not stand in for it; a plain HR staffer holding only HRS:Import is refused here. Passes the caller's own client IP so the tenant's IP allowlist (if configured) is genuinely enforced, and so the tenant's conditional MFA step-up gate (if configured) is genuinely reachable, matching commitTimesheetImportAction's own precedent. */
export async function commitLeaveOpeningBalanceImportAction(tenantSlug: string, jobId: string, _prevState: LeaveOpeningBalanceImportActionState, formData: FormData): Promise<LeaveOpeningBalanceImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  const client = toUnifiedClient(createSupabaseServiceRoleClient());
  try {
    await commitLeaveOpeningBalanceImportJob(client, { jobId, allowPartial, actorAuthUserId: access.authUserId, actorLabel: access.authUserId, clientIp });
  } catch (error) {
