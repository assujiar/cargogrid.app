...
/**
 * Attendance device import Server Actions (CG-AUDIT-2026-09-02 A4, seventh
 ...
 * app.validate_attendance_device_import_row/app.commit_attendance_device_import_job,
 * plus PLT-121's generic config-draft primitives for the one-time per-tenant
 * bootstrap) -- this file is a near-mechanical port of
 * hris/imports/employees/actions.ts's own trio, swapping the employee-specific
 * validate/commit adapters for their attendance equivalents.
 *
 * Both the `attendance_device_import_source` document type and the
 * `attendance_device_import` schema are already registered as GLOBAL catalog
 * rows directly by 20260730900000_create_hris_attendance.sql -- but each
 * tenant must still separately publish its own `document:attendance_device_import_source`
 * and `import_export:attendance_device_import` config VERSIONS (column
 * definitions / upload rules) before a real upload or stage can succeed, the
 * identical per-tenant onboarding step scripts/db-tests/hris-attendance.sql's
 * own fixture setup performs. No new migration was needed for this schema.
 *
 * Unlike every prior A4 slice, this document type's own default_classification
 * is `confidential` (not `internal`) -- biometric/clock-event data tied to an
 * individual employee -- matching scripts/db-tests/hris-attendance.sql's own
 * fixture value verbatim.
 *
...
import { validateAttendanceDeviceImportRow, commitAttendanceDeviceImportJob, AttendanceMutationError, type AttendanceMutationRpcClient } from "../../../../../../server/mutations/attendance.ts";
...
const ATTENDANCE_DEVICE_IMPORT_COLUMNS = [
  { key: "employee_number", label: "Employee Number", required: true, data_type: "text" },
  { key: "event_type", label: "Event Type", required: true, data_type: "text" },
  { key: "event_at", label: "Event At", required: true, data_type: "text" },
  { key: "device_label", label: "Device", required: false, data_type: "text" },
] as const;

export interface AttendanceDeviceImportActionState {
  readonly error: string | null;
}

const OK: AttendanceDeviceImportActionState = { error: null };
const NO_ACCESS: AttendanceDeviceImportActionState = { error: "You don't have access to this organization's HR workspace." };

type UnifiedClient = ConfigMutationRpcClient & DocumentMutationRpcClient & ImportExportMutationRpcClient & ImportExportQueryRpcClient & AttendanceMutationRpcClient & StorageUploadClient;
...
/** One-time per-tenant setup: publishes both the document-type definition (file-upload rules) and the import schema's column definition -- everything a tenant needs before its first device-event upload. Idempotent: createConfigDraft returns any already-pending draft rather than a new one, so re-running this after a partial failure safely resumes. */
export async function bootstrapAttendanceDeviceImportAction(tenantSlug: string, _prevState: AttendanceDeviceImportActionState, _formData: FormData): Promise<AttendanceDeviceImportActionState> {
  ...
  try {
    ...
    await setConfigItems(client, {
      ...
      items: [{ key: "columns", value: [...ATTENDANCE_DEVICE_IMPORT_COLUMNS], canonicalRef: null }],
      actorAuthUserId: access.authUserId,
      actorLabel: access.authUserId,
    });
    ...
    if (error instanceof ConfigMutationError || error instanceof DocumentMutationError || error instanceof ImportExportMutationError) {
      return { error: `Could not set up attendance device imports: ${error.message}` };
    }
    throw error;
  }

  revalidatePath(`/${tenantSlug}/hris/imports/attendance-devices`);
  return OK;
}
...
export async function uploadAttendanceDeviceImportSourceAction(tenantSlug: string, _prevState: AttendanceDeviceImportActionState, formData: FormData): Promise<AttendanceDeviceImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    const storeError = await storeFileBytesAndEnqueueScan(client, toBackgroundJobClient(supabase), uploaded, file, access.tenant.id, access.authUserId, "attendance device event source file");
    if (storeError) {
      return storeError;
    }
  ...
  redirect(`/${tenantSlug}/hris/imports/attendance-devices?job=${createdJobId}`);
}
...
export async function stageAndValidateAttendanceDeviceImportRowsAction(tenantSlug: string, jobId: string, _prevState: AttendanceDeviceImportActionState, _formData: FormData): Promise<AttendanceDeviceImportActionState> {
  const access = await resolveHrisAccessForRequest(tenantSlug);
  if (access.status !== "allowed") return NO_ACCESS;
  ...
  try {
    ...
    for (const row of stagedRows) {
      if (row.validationStatus === "pending") {
        await validateAttendanceDeviceImportRow(client, { stagingRowId: row.id, actorAuthUserId: access.authUserId, actorLabel: access.authUserId });
      }
    }
  } catch (error) {
    if (error instanceof ImportExportMutationError || error instanceof ImportExportQueryError || error instanceof AttendanceMutationError) {
...
export async function commitAttendanceDeviceImportAction(tenantSlug: string, jobId: string, _prevState: AttendanceDeviceImportActionState, formData: FormData): Promise<AttendanceDeviceImportActionState> {
  ...
  const client = toUnifiedClient(createSupabaseServiceRoleClient());
  try {
    await commitAttendanceDeviceImportJob(client, { jobId, allowPartial, actorAuthUserId: access.authUserId, actorLabel: access.authUserId, clientIp });
  } catch (error) {
    if (error instanceof AttendanceMutationError) {
