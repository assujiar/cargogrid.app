/** RGL-402 (Penetration Test Evidence): route-level HTTP-layer coverage for POST /api/webhooks/finance-payment-gateway/[connectionId]. */
import { test, describe } from "node:test";
import assert from "node:assert/strict";
import { installRpcFetchStub } from "../v1/support/rpc-fetch-stub.ts";
import { POST } from "../../../app/api/webhooks/finance-payment-gateway/[connectionId]/route.ts";

function post(connectionId: string, headers: Record<string, string> = {}, body = '{"test":1}') {
  return POST(
    new Request(`http://localhost/api/webhooks/finance-payment-gateway/${encodeURIComponent(connectionId)}`, {
      method: "POST",
...
describe("POST /api/webhooks/finance-payment-gateway/[connectionId]", () => {
  test("a non-UUID connectionId -> 400 invalid, never reaches the ingest RPC (same bug class as third-party-gps, live-forced: was an uncaught 500)", async () => {
    ...
    try {
      ...
      assert.equal(stub.calls.some((c) => c.fn === "ingest_finance_payment_gateway_webhook_event"), false);
    } finally {
      stub.restore();
    }
  });
  ...
  test("a well-formed UUID connectionId with a valid ingest outcome -> 200 (fix does not regress the happy path)", async () => {
    const stub = installRpcFetchStub({
      ingest_finance_payment_gateway_webhook_event: { data: { ingest_status: "ok", event_id: "11111111-1111-4111-8111-111111111111" } },
    });
    try {
  ...
  test("a well-formed UUID connectionId with an invalid-signature outcome -> 401 (denial mapping still correct)", async () => {
    const stub = installRpcFetchStub({
      ingest_finance_payment_gateway_webhook_event: { data: { ingest_status: "invalid", event_id: null } },
    });
    try {
      const response = await post("22222222-2222-4222-8222-222222222222");
