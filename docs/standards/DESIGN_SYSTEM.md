...
**Update (Checkpoint 2, 2026-07-26, out-of-band — a separate, later "CargoGrid UI Modernization" instruction):** `components/tables/data-table.tsx` and `components/tables/pagination.tsx` were added, following this same section's own roadmap item 3. Full detail: `docs/design-system/02_COMPONENTS.md` §1, `docs/design-system/07_GAP_ANALYSIS_AND_ROADMAP.md` §8. The remaining ~60+-primitive catalogue this paragraph already disclosed as not-yet-implemented is still not implemented — this update narrows that count by exactly two, nothing more.

**Update (Checkpoint 4, 2026-07-26, out-of-band — the user explicitly asked to build everything checkpoint 3's inventory found missing, a deliberate one-time departure from this section's own "wait for a real consumer" default):** ~40 further primitives implemented across new `components/forms/` and `components/domain/` directories plus an extended `components/ui/` — full detail `docs/design-system/07_GAP_ANALYSIS_AND_ROADMAP.md` §10. Zero new npm dependencies (built on the `radix-ui` package already installed). Of the original ~70-item catalogue, only a handful remain not built, each for a specific named reason (a missing storage pipeline, a missing chart-library decision, or no real consumer/data-model to build the right shape against yet) — see `docs/design-system/02_COMPONENTS.md` §2 for the per-item status.
