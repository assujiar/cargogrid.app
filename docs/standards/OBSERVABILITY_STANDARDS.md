...
| ~~Health/readiness endpoints (`/api/health`, `/api/ready`)~~ — **built, `HDN-382` (`ISS-2026-252` correction)** | Built exactly per the contract this row had fixed in advance: `app/api/health/route.ts` returns `{status: "ok"}` with no dependency check (liveness, unauthenticated by design); `app/api/ready/route.ts` additionally calls `app.ping()` via the service-role client and returns `503` with `{status: "degraded", reason: ["database_unreachable"]}` on failure (readiness) — never a false "ok" (Prompt 93 §23 "misleading health blocks rollout"). No longer `NOT_RUN`; retained here as a record of the contract this row originally fixed. See `ISS-2026-253` for a still-open diagnosability gap in `/api/ready`'s own failure path (unlogged server-side). | Done — `HDN-382`, Observability Audit |
