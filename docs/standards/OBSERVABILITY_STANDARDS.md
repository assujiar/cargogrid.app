...
`scripts/observability/logger.test.ts` proves: (1) every emitted log line matches §3's required-field shape; (2) `redact()` actually redacts every sensitive-key-pattern match and never leaks the literal value, using the same fingerprint-based non-reversible evidence-readback pattern `scripts/env/redact.test.ts`-equivalent coverage already established; (3) severity ordering is correct and total; (4) `assertBoundedDimension()` accepts an allowlisted value and rejects/warns on an out-of-allowlist one; (5) a simulated sink failure (thrown error) does not propagate to the caller — the safe-degrade rule (§8) holds under real failure injection, not merely asserted in prose.
