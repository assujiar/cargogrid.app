...
**`GENERIC_HARDCODED_SECRET_ASSIGNMENT`'s key-name list is deliberately narrower than `scripts/observability/logger.ts`'s `SENSITIVE_KEY_PATTERN` and `scripts/product-analytics/analytics.ts`'s `PROHIBITED_PROPERTY_KEY_PATTERN` — a decision, not a gap (`CG-S5-PH0-021`, Prompt 100 Hardening, resolving `ISS-2026-008`).** Those two patterns cover PII-shaped keys (`npwp`, `bank`/`account_number`, `salary`, `payroll`, `email`, `phone`, `address`, `name`, `tax`) because their job is redacting/rejecting *runtime data* (structured log fields, analytics event properties) flowing through a data path this repository controls end-to-end. `check-secrets.ts`'s job is different: detecting *credential-shaped literals hardcoded into source code* — the industry-standard scope for a secret scanner (matching tools like gitleaks/trufflehog/detect-secrets, which likewise target key/token/password shapes, not arbitrary PII). A hardcoded NPWP or bank-account-number literal in source is a real risk, but it is a **data-classification** risk (a real PII value should never be a literal in tracked source at all, regardless of its key name) — the correct control for that is `scripts/data-classification/check-registry.ts` and code review, not widening a credential scanner into a second, overlapping PII scanner (which Prompt 100 §24's "no new capability" rule forbids). `scripts/verification/phase0-integration.test.ts` documents this boundary as an intentional, tested invariant, not an inconsistency to close.
...
## 7. Posted financial record protection matrix (`CG-S9-FIN-015`, Prompt 204 — Posted-Journal Integrity)

Every posted-state Finance table in this repository (`finance_invoices`, `finance_ar_open_items`, `finance_ap_open_items`, `finance_vendor_bills`, `finance_receipts`/`finance_receipt_allocations`, `finance_settlements`/`finance_settlement_allocations`, `finance_subledger_batches`/`finance_subledger_lines`, `finance_journals`/`finance_journal_lines`) has carried zero `UPDATE`/`DELETE` grant for `authenticated` since each one's own migration (`PLT-118`'s repository-wide convention, `ERR-2026-004`) — a normal-role direct-table mutation is already structurally impossible; every real mutation path is a `service_role`-only, authority-checked `plpgsql` function.

**`app.finance_journals`/`app.finance_journal_lines` additionally carry a table-level trigger** (`app.protect_posted_finance_journal`/`app.protect_posted_finance_journal_line`, `supabase/migrations/20260729180000_create_finance_posted_journal_integrity.sql`) that blocks *any* `UPDATE`/`DELETE` against an already-posted row — including a hypothetical `service_role`-level direct mutation outside the vetted RPC surface, a strictly stronger guarantee than the grant-only protection every other table above relies on — unless the acting identity holds a live `supreme_admin` principal membership (`app.is_supreme_admin`, `RPD-022`). This is the named epic's own bounded scope (Posted-**Journal** Integrity); the remaining posted-state tables are disclosed as relying on the pre-existing grant-only protection, not silently claimed to carry the same trigger-level guard.

**RPD-022 exercised**: when `app.is_supreme_admin` returns true and the touched row was already posted, the trigger still permits the mutation (never a tamper-proof claim) but captures a best-effort `app.capture_audit_event` row disclosing the bypass (actor, table, resource, before/after). This is a detective control, not a preventive one — the same accepted-risk shape RPD-022 already discloses everywhere else in this repository (§6 above): a Supreme Admin who also controls the database directly can always defeat any application-layer evidence, and this repository has never claimed otherwise.

**No absolute-immutability claim is made anywhere in this repository's own code, comments, or UI copy** for any Finance record — every "posted"/read-only UI state is worded as "a normal role cannot edit this record; correction uses governed reversal" (FIN-206), never "this record cannot be changed."
