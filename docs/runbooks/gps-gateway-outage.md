...
2. Check the durable local buffer (`services/gps-gateway/src/buffer.ts`, an append-only NDJSON file) for an unusually large backlog — the flush loop stops at the first failure and never reorders (line 63-86 of that file), so a large backlog after a Supabase-side recovery indicates the buffer itself needs draining, not a gateway-process problem (see `docs/runbooks/gps-ingestion-database-outage.md` for that case).
