...
| `docs/ai-agent-build-prompt-package/**` | Read-only execution plan | Never edited by a runtime agent — only read. **Narrowed once, by `ADR-0026`/`CON-016`**, for the five register/derived-metadata files in the rows below; every other file in the tree — all 324 prompt files, all 18 step READMEs, `00_PACKAGE_README.md`, `01_SOURCE_OF_TRUTH_MATRIX.md`, and the `01-agent-governance/`/`04-reusable-prompts/` templates — is unchanged |
| `docs/ai-agent-build-prompt-package/00-control/04_CONFLICT_REGISTER.md`, `05_REQUIREMENT_COVERAGE_MATRIX.md`, `06_PACKAGE_BUILD_STATUS.md`, `07_PROMPT_PACKAGE_MANIFEST.md`, `START_HERE.md` | Register/derived package metadata — `CAUTION`, not `FORBIDDEN`. `04`/`05` keep their stricter “decision-change protocol only” authority from the row below; the gate warns rather than blocks because a path check cannot express that condition | **Step 17 (Final Package Validation) only**, for mechanical, source-safe corrections with cited evidence, per `ADR-0026`/`CON-016`. Anything that would change *what a future agent builds* is a finding in `docs/build-log/final-package-validation/FINAL_GAP_RISK_REGISTER.md`, never an edit. Re-verify every correction with `pnpm run package:check`. No other step inherits this authority |
| `docs/ai-agent-build-prompt-package/00-control/02_CONFIRMED_DECISION_REGISTER.md`, `03_ASSUMPTION_REGISTER.md`, `04_CONFLICT_REGISTER.md`, `05_REQUIREMENT_COVERAGE_MATRIX.md` | Authoritative registers | Decision-change protocol only (`02_CONFIRMED_DECISION_REGISTER.md` §5). `ADR-0026` **is** that protocol being used, for `05` and for recording `CON-016` in `04`; `02` and `03` remain untouched by it |
