...
|---|---|---|---|---|---|---|
| `ISS-2026-002` | No single-writer discipline across concurrent agent branches | **Critical** (escalated, 5th occurrence) | `OPEN` — **BLOCKING** | Runtime agent / repo owner | Run each runtime step on one authoritative branch only; **enforcement still not adopted after 4 prior occurrences; 5th occurrence resulted in corrupted content committed to `main` (`ERR-2026-003`)** | **Yes — blocks all further Phase 0/Step 3-derived work until `main`'s `docs/architecture/14..16_*.md` are reconciled into single authoritative documents** |
