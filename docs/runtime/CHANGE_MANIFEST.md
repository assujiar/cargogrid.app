...
| Change type | MIGRATION — seven additive migrations: 13 dashboard functions hardened, the root RBAC evaluator patched, 16 CRM/ops functions hardened, an own-row RLS membership gap closed on 3 tables, a loyalty maker/checker guard added, 95 Finance functions converted `SECURITY INVOKER` → `SECURITY DEFINER`, a Finance journal RLS gate and self-approval guard added; TEST — 4 pre-existing Finance db-test fixtures corrected to use a second distinct approver, 2 new regression blocks added to `finance-journal.sql`; DOCS — build log, index, matrix, blocker ledger, 13 issue-ledger entries plus 3 updated, runtime ledgers |
...
| Status | **`COMPLETED`, not `VERIFIED`** — adversarially unreviewed. A Tier C review plus an independent full gate re-run must close it before `HDN-374` or any other lane may begin |
| Date | 2026-08-23 |
