...
### CHG-2026-198 — Tenant Isolation Audit: a cross-tenant read defect class live-forced and fixed at the root, a second same-shape finding deferred (Step 15, Prompt 372)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-004` (Prompt 372, Tenant Isolation Audit), package document `CG-AABPP-HDN-372` |
| Change type | MIGRATION — one additive migration, 9 functions hardened; TEST — 2 pre-existing db-test fixture regressions fixed; DOCS — build log, index, matrix, blocker ledger, 15 new issue-ledger entries, runtime ledgers |
...
| Build process | Four independent parallel adversarial investigations, each with its own disposable Postgres database built from all 306 migrations and its own live two-tenant fixture. The orchestrating session additionally queried the real live Supabase project directly (read-only) to settle a reachability question rather than assume it, and independently re-verified both the vulnerability and the fix live before and after applying the migration |
| Findings and disposition | **`HDN-BLK-011`/`ISS-2026-164` (High) — FIXED same checkpoint.** 9 `SECURITY DEFINER` functions granted to `authenticated` evaluated authority against a client-supplied actor UUID instead of the verified session identity (the `ATW-032` sweep's own "STABLE reads are exempt" premise is false for a function that bypasses RLS). Live-forced against `app.get_self_employee` (full PII) and the 12-function `app.resolve_customer_owner_account_scope`/`ATW-023` family (customer inventory data); code-verified for audit-trail and notification functions. 21 functions protected total once transitive propagation is counted. Fixed via `create or replace function` adding `assert_actor_is_session_identity`, 2 of 9 converted from `language sql` to `plpgsql` to carry it. **`HDN-BLK-012`/`ISS-2026-165` (High) — deferred to `HDN-373`.** 13 dashboard functions share the identical shape with no common root to fix once. **15 further Medium/Low findings** registered across `HDN-373`/`376`/`377`/`378`/`379`/`382` — none silently fixed or dropped |
| Files edited | `supabase/migrations/20260810000000_harden_tenant_isolation_actor_identity_gaps.sql` (new); `scripts/db-tests/rbac-enforcement.sql`, `scripts/db-tests/hris-employee-master.sql` (two genuine pre-existing test regressions this fix surfaced, both fixed); `docs/build-log/full-system-hardening/HDN-372.md` (new), `00_EXECUTION_INDEX.md`, `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md`; `docs/runtime/KNOWN_ISSUES.md` (15 new entries), `TASK_LEDGER.md`, `HANDOFF.md`, `CHANGE_MANIFEST.md`, `CARGOGRID_BUILD_STATUS.md` |
| Migration | **One, additive only** — `create or replace function` on 9 existing functions, no schema/table change, no applied migration edited |
| Risk | Low for the fix itself — additive, mechanical, precedented (identical pattern already proven for 449 other functions in this codebase), live-verified not to weaken any legitimate call path (own-identity control calls still succeed). The risk this checkpoint *closes*: a real, live-forced cross-tenant PII/inventory/audit-trail read, currently gated only by a pre-deployment Data-API exposure setting rather than any durable control |
| Gates | Three full db-test runs (the first two caught genuine pre-existing regressions, fixed in the same pass): `typecheck` 0 errors; `lint` 0 errors/337 warnings; `pnpm run test` **5394/5394**; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`** on the third and final run; all 7 governance gates clean |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's commit. The migration is additive only (`create or replace function`, no schema change) — reverting restores the pre-fix function bodies exactly |
| Status | **`COMPLETED`, not `VERIFIED`** — adversarially unreviewed. A Tier C review plus an independent full gate re-run must close it before `HDN-373` or any other lane may begin |
| Date | 2026-08-23 |
