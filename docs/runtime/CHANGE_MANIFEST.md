...
### CHG-2026-182 — Organization and Position Linkage (Phase 7, Prompt 275) — closes the full "lanjut 271-275" range

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-003` (Prompt 275, Organization and Position Linkage) — `VERIFIED` this checkpoint. Fourth and final task within the "lanjut 271-275" operator authorization (Prompts 271, 273, 274, 275), following `CG-S12-HRT-002` (Employee Master, `VERIFIED`). |
| Change type | DB/SERVICE/UI — 1 new additive migration (ADR-0023 Part A implementation + 3 new tables + ~24 RPCs + 1 additive column on `app.employees`), service layer (`server/{contracts,queries,mutations}/position/`), UI under `app/(tenant)/[tenantSlug]/hris/{positions,organization}/` and `app/(tenant)/[tenantSlug]/hris/employees/[masterRecordId]/positions/`, one additive edit to the existing employee detail panel |
| Authorization | The same "lanjut 271-275" operator instruction that authorized `CG-S11-PRC-022`/`CG-S12-HRT-001`/`CG-S12-HRT-002` — Prompt 275 was explicitly named in that range from the start |
| Build process | `Workflow`: Implement → iterative adversarial `psql`-driven db-testing → fix pass, mirroring HRT-274's own established discipline for this phase |
| Findings and disposition | 2 real defects found and fixed before commit, both live-reproduced by this checkpoint's own db-test suite, neither shipped: capacity enforcement double-counting an employee's own soon-to-be-closed predecessor on a same-position correction (would have made a capacity=1 position permanently unable to process a legitimate same-position correction for its own incumbent); a bare-column-reference ambiguity bug in `app.get_employee_manager_chain` (the identical bug class HRT-274's own build log already found and fixed three times in three different functions). No CRITICAL/HIGH finding — the RLS/RBAC/audit-projection discipline HRT-274's own review round hardened is reused verbatim (no new PII/masking surface exists in this domain to regress), and the capacity/overlap/cycle guards were designed with PRC-255's own overlap-handling precedent in hand from the start. |
| Highlights (implemented) | ADR-0023 Part A (`app.org_units.unit_type` widened to add `team`, `enforce_org_unit_parent_shape()` widened with a leaf-type guard) — this checkpoint's own chartered implementation of an already-ratified decision, regression-proven against `org-hierarchy.sql`'s own pre-existing assertions. `app.position_grades`/`app.positions` — HR-governed, tenant-scoped catalogue, deliberately not routed through `app.master_records`. `app.employee_position_assignments` — the effective-dated assignment/version table HRT-274 deferred, a genuinely new table (never a mutable column), two real `btree_gist` EXCLUDE constraints for overlap, position-scoped-advisory-locked capacity enforcement at approval time. `app.employees.position_id` — the additive current-pointer column HRT-274 recommended; `position_title` unchanged. A two-step propose (`HRS:Edit`)/decide (`HRS:Approve`) workflow adding zero new `app.permissions` rows. A real, non-fabricated impact-preview RPC disclosing (not inventing data for) the Payroll/Ticketing/approval-queue-org-scope systems that do not exist yet in this repository. Manager-cycle prevention formalized by keeping the existing bounded chain walk unchanged, with recorded reasoning, rather than building a materialized closure table. |
| Files added | `supabase/migrations/20260730840000_create_hris_organization_position_linkage.sql`; `server/contracts/position/position.ts`+`.test.ts`, `server/queries/position.ts`+`.test.ts`, `server/mutations/position.ts`+`.test.ts`; `app/(tenant)/[tenantSlug]/hris/positions/**`, `app/(tenant)/[tenantSlug]/hris/organization/**`, `app/(tenant)/[tenantSlug]/hris/employees/[masterRecordId]/positions/**`; `scripts/db-tests/hris-organization-position-linkage.sql`; `docs/build-log/phase-07/HRT-275.md` |
| Files edited | `app/(tenant)/[tenantSlug]/hris/employees/[masterRecordId]/employee-detail-panel.tsx` (additive callout + relabel only), `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-066` added, `ISS-2026-065` updated in place), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md` |
| Migration | 1 new additive migration — no already-applied migration edited, per standing convention. `CREATE OR REPLACE FUNCTION app.enforce_org_unit_parent_shape()` widens the existing trigger function in place (additive, reversible), not a table rewrite. |
| Risk | LOW throughout — both findings were caught by this checkpoint's own db-testing before commit, neither reached a shipped state; no PII/security surface exists in this domain (no compensation/salary/pay-band field anywhere, confirmed via `data-classification:check`) |
| Gates | Independently re-run fresh after every fix: `typecheck` 0 errors; `lint` 0 errors/185 warnings (185 vs. 180 baseline is pre-existing `no-html-link-for-pages` noise scaling with the 4 new page routes, zero findings in this checkpoint's own files, confirmed via a scoped `eslint` run); `pnpm run test` **3509/3509** (3473 prior + 36 new); `pnpm run db:test` **ALL PASSED**, 151 files/184 migrations; `next build` clean, all 4 new routes present; `security:audit`/`standards:check`/`docs:check`/`data-classification:check`/`security:check`/`threat-model:check` all clean |
| Commits | (this checkpoint's own commit) — branch `claude/lanjut-271-275-f4mz9a` |
| Rollback | `git revert` this checkpoint's commit; the migration is additive-only and reversible (narrow the `unit_type` CHECK constraint back, drop the 3 new tables and the additive `app.employees.position_id` column) — no live Supabase project or production data exists anywhere in this repository |
| Status | `VERIFIED` |
| Date | 2026-08-09 |
...
| Files edited | `AGENTS.md` (new "Execution cadence" section; instance version `0.2.0` → `0.3.0`; commit/checkpoint rules gain per-prompt-commit/per-batch-push/per-batch-collision-check; gate policy cross-reference), `docs/adr/README.md` (§5.2 candidate `ADR-CAND-ARCH-033`, §6 index row, count reconciliation), `docs/ai-agent-build-prompt-package/00-control/04_CONFLICT_REGISTER.md` (`CON-015`, version `0.1.1` → `0.1.2`), plus this manifest, `CARGOGRID_BUILD_STATUS.md`, `HANDOFF.md` |
| Prompt files | **Zero edited, and none needs to be.** All 430 remain valid as written: a capability prompt specifies *what* to build and *which* gates apply, never *how often* review runs. Verified before adopting the cadence — §30 names the gate set without a cadence, §33's "one recorded checkpoint" is satisfied by the batch close, §34/§35 carry no timing claim, and the only Phase 6 prompt mentioning adversarial review is Prompt 269 (hardening), already exempt from batching. **One genuine conflict found and resolved rather than glossed:** 166 capability prompts carry an identical §36 clause, "Only the execution index may release `<NEXT>` … after this task is `VERIFIED`", which cannot hold literally when a prompt sits at `COMPLETED` until its batch closes. Resolved as `CON-015` — released on `COMPLETED` **within** a batch, unchanged **across** a batch boundary — placing the override at precedence rank 3 (above the task prompt) so it is explicit, not an implementation artifact silently contradicting a prompt |
...
### CHG-2026-178 — Procurement RFQ / Vendor Comparison / Procurement Approval, first batch under `ADR-0021` (Phase 6, Prompts 257-259)

| Field | Value |
|---|---|
| Task/prompt | `CG-S11-PRC-008..010` (Prompts 257-259: Procurement RFQ, Vendor Comparison, Procurement Approval) — all `VERIFIED` this checkpoint. The FIRST batch executed under `ADR-0021`'s batched-review cadence (`docs/standards/BUILD_EXECUTION_PROTOCOL.md`) |
| Change type | DB/SERVICE/UI — 4 migrations (3 create + 1 harden), service layer (contracts/queries/mutations), and UI (`app/(tenant)/[tenantSlug]/procurement/{rfq,vendor-comparison,approvals}/`) |
| Authorization | A fresh, separate "lanjut execute prompt 257-260" operator instruction (2026-08-07) — distinct from the earlier, already-spent "lanjut prompt 256 sd 260" authorization that had been narrowed to Prompt 256 alone before any Prompt 257 work began |
| Build process | Background `Workflow`: sequential implement 257→258→259 (each with Tier A automated gates + Tier B `RECURRING_DEFECT_TAXONOMY.md` self-check recorded in its own build log), then one combined Tier C adversarial review (4 parallel lenses — spec-compliance; security/RLS/tenant isolation live-tested with forged/alternate-actor sessions against a real disposable Postgres; correctness/concurrency live-tested with real concurrent `psql` sessions; cross-prompt integration and data dependency, the lens new at `ADR-0021`) against the batch's combined diff, then one fix pass |
| Batch-cut trigger | The batch closed at 259, before Prompt 260: Prompt 259's own Tier B self-check found and fixed two defects live during its own `db:test` iteration matching Critical/High-shaped classes already closed once before for other capabilities (cross-tenant business-intelligence disclosure; identity-vs-authority gap) — per `BUILD_EXECUTION_PROTOCOL.md` §3.2 trigger 1, a batch-cut signal regardless of whether fixed before commit; the protocol working as designed |
| Findings and disposition | Tier C found **15 findings, 9 Critical/High**, all live-reproduced or directly re-derived. Fix pass re-verified every finding independently before acting; fixed **12** at root cause with propagation sweeps (no blanket `GRANT` added); disclosed **3** as new `KNOWN_ISSUES.md` entries (`ISS-2026-043`, `ISS-2026-044`, `ISS-2026-045`); rejected **2** as unconfirmed with counter-evidence recorded. Two novel classes added to `docs/standards/RECURRING_DEFECT_TAXONOMY.md`: `C-21` (inconsistent lock ordering between sibling functions — live-reproduced Postgres deadlock `40P01` between `app.close_rfq_for_comparison`/`app.submit_rfq_response`) and `C-22` (numeric threshold compared without currency normalization — live-confirmed ~15,000× USD/IDR gap) |
| Highlights (fixed) | **CRITICAL**: `vendor_comparison_offers`/`_offer_scores`/`vendor_comparisons`/`_events` granted unrestricted `SELECT` to `authenticated`, bypassing the `PRC:View cost` RPC gate — fixed by column-restricting grants and masking all four `_directory` views. **HIGH**: `evaluate_procurement_approval_requirement` missing caller-is-actor assertion (cross-tenant threshold disclosure) — fixed. **HIGH**: MFA reauth for privileged approvals, named in 7 spec sections, was entirely absent — fixed by wiring the `p_reauth_confirmed_at` pattern (`PRC-254`/`COM-157`) into all four decide-wrappers. **HIGH**: the `C-21` deadlock — fixed. **HIGH**: an idempotency-retry race in `create_procurement_exception_request` could orphan an undecidable `approval_requests` row — fixed with an advisory lock. **HIGH**: `decide_vendor_profile_review`'s unlocked read crashed on double-click retry — fixed with a row lock. **HIGH**: Prompt 259's widening of `vendor_rate_versions` leaked 2 governance columns into `COM-149`'s `select_vendor_rate` snapshot (the same class `PRC-255` already closed once) — fixed by stripping at write time. **MEDIUM**: all 13 new by-id read RPCs disclosed real owning `tenant_id` via error text pre-membership-check — fixed uniformly |
| Files added | `supabase/migrations/20260730640000_create_procurement_rfq.sql`, `20260730650000_create_procurement_vendor_comparison.sql`, `20260730660000_create_procurement_approval.sql`, `20260730670000_harden_procurement_batch_257_259_review_fixes.sql`; `docs/build-log/phase-06/PRC-257.md`, `PRC-258.md`, `PRC-259.md` |
| Files edited | `docs/standards/RECURRING_DEFECT_TAXONOMY.md` (`C-21`/`C-22` added), `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-043/044/045` added), `docs/build-log/phase-06/PROCUREMENT_VENDOR_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md`, plus service layer (`server/contracts|queries|mutations/rfq*`, `vendor-comparison*`, `procurement-approval*`) and UI under `app/(tenant)/[tenantSlug]/procurement/{rfq,vendor-comparison,approvals}/` |
| Migration | 3 new create migrations (RFQ, Vendor Comparison, Approval) + 1 corrective harden migration applying all 12 root-cause fixes — no already-applied migration edited, per standing convention |
| Risk | Started HIGH (9 Critical/High Tier C findings, including an unrestricted-`SELECT` cost-data leak and an absent MFA gate on financial approvals); closed LOW-MEDIUM — every Critical/High either fixed and independently re-verified, or disclosed with recorded reasoning in a tracked issue, none left open uncontained |
| Gates | Independently re-run fresh by the orchestrating session (not the fix agent's self-report), every number matching: `typecheck` 0 errors; `lint` 0 errors/140 pre-existing warnings (0 introduced); `pnpm run test` **3406/3406**; `pnpm run db:test` full 142-file suite **ALL PASSED**; `next build` clean, all 6 new routes present; `git:check-paths` 51 files checked, 0 forbidden, 1 caution (`KNOWN_ISSUES.md`, expected/additive); `security:check` clean; `security:audit` fails only on the pre-existing `ISS-2026-042` `js-yaml` advisory (untouched by this batch); `data-classification:check`/`threat-model:check` (25 entries)/`standards:check`/`docs:check`/`git:check` all clean |
| Commits | `7e70ea2` (257 implement), `d8066cf` (258 implement), `9298dce` (259 implement), `d2aaec2` (fix pass) — branch `claude/execute-prompt-257-260-6rrbge` |
| Rollback | `git revert` this checkpoint's commits; the corrective migration follows this repository's standing "applied migrations are never edited, a defect is repaired by a new migration" precedent |
| Status | `VERIFIED` |
| Date | 2026-08-07 |

### CHG-2026-179 — Purchase Order, second (single-prompt) batch under `ADR-0021` (Phase 6, Prompt 260) — closes the full 257-260 range

| Field | Value |
|---|---|
| Task/prompt | `CG-S11-PRC-011` (Prompt 260: Purchase Order) — `VERIFIED` this checkpoint. **Batch 2 of the "lanjut execute prompt 257-260" authorization — a single-prompt batch, its own Tier A/B then its own mandatory Tier C per `BUILD_EXECUTION_PROTOCOL.md` §3.3 (no batch, however small, skips Tier C). This closes the FULL 257-260 authorized range: all of Prompts 257, 258, 259, 260 now `VERIFIED` across two batches.** |
| Change type | DB/SERVICE/UI — 2 migrations (1 create + 1 harden), service layer (contracts/queries/mutations), and UI (`app/(tenant)/[tenantSlug]/procurement/purchase-orders/`) |
| Authorization | The same "lanjut execute prompt 257-260" operator instruction (2026-08-07) that authorized `CHG-2026-178` — 260 is explicitly named in that range, no fresh authorization needed |
| Build process | Background `Workflow`: implement (Tier A automated gates + Tier B `RECURRING_DEFECT_TAXONOMY.md` self-check recorded in its own build log), then its own Tier C adversarial review (4 parallel lenses against Prompt 260's own diff), then one fix pass |
| Findings and disposition | Tier C found **12 findings, 3 HIGH, 0 CRITICAL**, all live-reproduced or directly re-derived. Fix pass fixed **10** at root cause; confirmed **1** (`C-22` currency-blind threshold) as already-disclosed via `ISS-2026-045` (updated in place to name the third affected entity type, no duplicate issue); rejected **1** as unconfirmed after direct investigation (a vendor-read-visibility gap that does not exist — no vendor-portal identity concept exists anywhere in this repository, the same already-accepted PRC-257/258 scope boundary). One new class added to `docs/standards/RECURRING_DEFECT_TAXONOMY.md`: `C-23` (a named spec requirement silently dropped instead of joining the checkpoint's own disclosed-scope-boundary list) |
| Highlights (fixed) | **STANDOUT**: `app.cancel_purchase_order` and `app.decide_purchase_order_approval_step` locked `app.purchase_orders`/`app.approval_requests`/`app.approval_request_steps` in opposite order — live-reproduced as a genuine Postgres deadlock (SQLSTATE `40P01`), contradicting Prompt 260's own migration-header claim and Tier B self-report, both of which missed locks taken transitively inside the shared Platform Approval Engine (`PLT-123`). The first-pass fix (reordering `cancel_purchase_order` alone) was proven **insufficient** by a live two-process regression test the fix agent itself wrote, surfacing a second, deeper deadlock inside pre-existing, unmodified `PLT-123` code (`app.cancel_approval_request` vs `app.decide_approval_step` disagreeing on lock order) — fixed at that true shared root (reordering `cancel_approval_request` to match `decide_approval_step` exactly), verified deadlock-free across 5 consecutive clean `db:test` runs total. This single fix also closes the identical latent risk for already-`VERIFIED` `PRC-259`'s own `cancel_procurement_exception_request`/`decide_procurement_exception_approval_step` pairing. **MEDIUM**: all 6 PO lifecycle write RPCs echoed a foreign PO's real `tenant_id` to a zero-membership cross-tenant caller before checking membership — fixed uniformly, mirroring batch 1's own read-RPC fix pattern. **MEDIUM/LOW**: two new `CHECK` constraints (`total_amount = subtotal_amount + tax_amount`; service-period ordering) |
| Files added | `supabase/migrations/20260730680000_create_procurement_purchase_order.sql`, `20260730690000_harden_procurement_purchase_order_batch_260_review_fixes.sql`; `docs/build-log/phase-06/PRC-260.md` |
| Files edited | `docs/standards/RECURRING_DEFECT_TAXONOMY.md` (`C-23` added), `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-046/047/048/049` added, `ISS-2026-045` updated in place), `docs/build-log/phase-06/PROCUREMENT_VENDOR_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md`, plus service layer (`server/contracts|queries|mutations/purchase-order*`) and UI under `app/(tenant)/[tenantSlug]/procurement/purchase-orders/` |
| Migration | 1 new create migration (Purchase Order) + 1 corrective harden migration applying all 10 root-cause fixes — no already-applied migration edited, per standing convention |
| Risk | Started MEDIUM-HIGH (3 HIGH Tier C findings, including a genuine cross-function Postgres deadlock reaching into shared `PLT-123` engine code); closed LOW — every Critical/High either fixed and independently re-verified, or disclosed with recorded reasoning in a tracked issue, none left open uncontained |
| Gates | Independently re-run fresh by the orchestrating session (not the fix agent's self-report), every number matching: `typecheck` 0 errors; `lint` 0 errors/145 pre-existing warnings; `pnpm run test` **3426/3426**; `pnpm run db:test` full 143-file suite **ALL PASSED** (5 consecutive clean runs total, zero deadlocks); `next build` clean, both new routes present; `git:check-paths` 73 files checked across the FULL 257-260 range, 0 forbidden, 5 caution (all five `docs/runtime/` ledgers, expected/additive); `security:check` clean; `security:audit` fails only on the pre-existing `ISS-2026-042` `js-yaml` advisory (not a regression); `data-classification:check`/`threat-model:check` (25 entries)/`standards:check`/`docs:check`/`git:check` all clean |
| Commits | `8700062` (260 implement), `dd60b31` (fix pass) — branch `claude/execute-prompt-257-260-6rrbge` |
| Rollback | `git revert` this checkpoint's commits; the corrective migration follows this repository's standing "applied migrations are never edited, a defect is repaired by a new migration" precedent |
| Status | `VERIFIED` |
| Date | 2026-08-07 |

### CHG-2026-180 — Vendor Contract / Vendor Capacity and Availability / Vendor Assignment, third batch under `ADR-0021` (Phase 6, Prompts 261-263)

| Field | Value |
|---|---|
| Task/prompt | `CG-S11-PRC-012..014` (Prompts 261-263: Vendor Contract, Vendor Capacity and Availability, Vendor Assignment) — all `VERIFIED` this checkpoint. **Batch 3 of a fresh, separate "lanjut sd prompt 265" authorization, adaptive-sized at 3 (capped below the default 5 because batch 2's own Tier C found 3 Critical/High findings, per `BUILD_EXECUTION_PROTOCOL.md` §3.4).** |
| Change type | DB/SERVICE/UI — 3 create migrations + 1 corrective harden migration, service layer (contracts/queries/mutations), and UI under `app/(tenant)/[tenantSlug]/procurement/{vendor-contracts,vendor-capacity,vendor-assignments}/` |
| Authorization | A fresh, separate "lanjut sd prompt 265" operator instruction — distinct from the spent "lanjut execute prompt 257-260" authorization that closed at `CHG-2026-179` |
| Build process | `Workflow`: sequential implement 261→262→263 (each with Tier A automated gates + Tier B `RECURRING_DEFECT_TAXONOMY.md` self-check recorded in its own build log — each build log keeps its own `COMPLETED` header per this build's standing precedent, `VERIFIED` recorded only in `TASK_LEDGER.md`/`PROCUREMENT_VENDOR_EXECUTION_INDEX.md`, matching `PRC-257`-`PRC-260`'s own identical precedent), then one combined Tier C adversarial review (4 parallel lenses against the batch's combined diff), then one fix pass; independently re-verified by the orchestrating session with a fresh, from-scratch gate-suite re-run, not the fix agent's self-report |
| Findings and disposition | Tier C's four lenses (spec-compliance; security/RLS/tenant isolation live-tested with forged/alternate-actor sessions; correctness/concurrency live-tested with real concurrent `psql` sessions including a genuine deadlock race; cross-prompt integration and data dependency) found a substantial CONFIRMED, live-reproduced set, several independently cross-confirmed by two lenses at once — 4 HIGH, 3 MEDIUM fixed at root cause; 1 MEDIUM disclosed rather than fixed (`ISS-2026-050`). No new taxonomy class needed — every finding fit within the existing `C-01`/`C-02`/`C-04`/`C-05`/`C-15`/`C-19`/`C-20`/`C-23` classes |
| Highlights (fixed) | **HIGH** — `app.decide_vendor_contract_approval_step` vs `app.cancel_vendor_contract_draft` took opposite lock orders across `vendor_contracts`/`approval_requests`/`approval_request_steps`, a real two-process race hitting genuine Postgres `deadlock detected` (SQLSTATE `40P01`) in half of eight trials — fixed by reordering to lock the contract row first, verified deadlock-free across both possible race orderings with a real two-process regression test. **HIGH** — C-01 idempotency-tuple gaps in 3 creation RPCs (`create_vendor_contract_draft`, `create_vendor_capacity_offer_draft`, `propose_vendor_assignment_invitation`), each claiming "full tuple" comparison in its own Tier B self-check but comparing only a leading subset — fixed in all 3, plus `reassign_vendor_assignment` gained idempotency handling it never had at all. **HIGH** — `app.resolve_effective_vendor_contract`, built by 261 explicitly for 263 to consume, was never actually called by 263 (independently found by two lenses), and a supplied contract's effective-date window was never checked, only its status — fixed with auto-resolution (a genuine `OUT`-parameter signature addition, `DROP`+`CREATE`, re-granted `service_role`-only) and a real window check. **HIGH** — vendor capacity offers/reservations never checked vendor compliance hold or governing-contract validity, contradicting 262's own spec — fixed at both publish and reserve time, independently re-verified at each. **MEDIUM** — the SAME `ISS-2026-048` tenant-id-disclosure oracle batch 2 already fixed once reappeared in ~26 new write RPCs across all three of this batch's own migrations — fixed uniformly, `ISS-2026-048` updated in place. **MEDIUM** — `rate_version_id` carried zero vendor/tenant scope validation, unlike its sibling linked-evidence fields — fixed. **MEDIUM (C-19-adjacent)** — a `'held'` (never accepted) capacity reservation was treated as eligible even though confirmation only ever consumes `'accepted'` ones, a live-reproduced silent capacity leak — fixed |
| Files added | `supabase/migrations/20260730700000_create_procurement_vendor_contract.sql`, `20260730710000_create_procurement_vendor_capacity.sql`, `20260730720000_create_procurement_vendor_assignment.sql`, `20260730730000_harden_procurement_vendor_batch3_tier_c_review_fixes.sql`; `docs/build-log/phase-06/PRC-261.md`, `PRC-262.md`, `PRC-263.md` |
| Files edited | `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-050`/`051` added, `ISS-2026-048` updated in place), `docs/build-log/phase-06/PROCUREMENT_VENDOR_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md`, `scripts/db-tests/procurement-vendor-{contract,capacity,assignment}.sql` (+20 regression tests), plus service layer (`server/contracts|queries|mutations/vendor-contract*`, `vendor-capacity*`, `vendor-assignment*`) and UI under `app/(tenant)/[tenantSlug]/procurement/{vendor-contracts,vendor-capacity,vendor-assignments}/` |
| Migration | 3 new create migrations (Vendor Contract, Vendor Capacity, Vendor Assignment) + 1 corrective harden migration applying all root-cause fixes — no already-applied migration edited, per standing convention. **Process note**: the fix pass's first draft directly edited the three already-applied create migrations; this was caught by `pnpm run git:check-paths` reporting `FORBIDDEN` before any commit, and corrected by reconstructing the entire fix pass as the corrective migration instead, mirroring `20260730690000`'s own batch-2 precedent |
| Risk | Started HIGH (4 HIGH Tier C findings including a genuine cross-function Postgres deadlock and a dropped cross-prompt dependency); closed LOW — every Critical/High either fixed and independently re-verified, or disclosed with recorded reasoning in a tracked issue, none left open uncontained |
| Gates | Independently re-run fresh by the orchestrating session, every number matching: `typecheck` 0 errors; `lint` 0 errors/160 pre-existing warnings; `pnpm run test` **3426/3426**; `pnpm run db:test` full 146-file suite **ALL PASSED** (re-run repeatedly through the fix pass's own iteration, zero deadlocks across every race-test run); `next build` clean, all 6 new routes present; `git:check-paths` 10 files checked, 0 forbidden, 5 caution (all five `docs/runtime/` ledgers, expected/additive); `security:check`/`data-classification:check`/`threat-model:check` (25 entries)/`standards:check`/`docs:check`/`git:check` all clean; `security:audit` fails only on two pre-existing advisories, both confirmed unrelated to this batch and registered (`ISS-2026-042` `js-yaml`, already tracked; `ISS-2026-051` `nanoid`, newly registered this checkpoint, `package.json`/`pnpm-lock.yaml` confirmed untouched throughout) |
| Commits | `740ae6d` (261 implement), `c5fb389` (262 implement), `8113b1d` (263 implement), plus the fix-pass commit — branch `claude/prompt-mana-9j4etc` |
| Rollback | `git revert` this checkpoint's commits; the corrective migration follows this repository's standing "applied migrations are never edited, a defect is repaired by a new migration" precedent |
| Status | `VERIFIED` |
| Date | 2026-08-08 |

### CHG-2026-181 — Vendor Performance / Vendor Invoice Matching, fourth and final batch under `ADR-0021` (Phase 6, Prompts 264-265) — closes the full 261-265 range

| Field | Value |
|---|---|
| Task/prompt | `CG-S11-PRC-015..016` (Prompts 264-265: Vendor Performance, Vendor Invoice Matching) — both `VERIFIED` this checkpoint. **Batch 4, the second and final batch of the "lanjut sd prompt 265" authorization. This closes the FULL 261-265 authorized range: all five prompts now `VERIFIED` across two batches.** |
| Change type | DB/SERVICE/UI — 3 create/wiring migrations + 1 corrective harden migration, service layer (contracts/queries/mutations), UI under `app/(tenant)/[tenantSlug]/procurement/{vendor-performance,vendor-invoice-matching}/` |
| Authorization | The same "lanjut sd prompt 265" operator instruction that authorized `CHG-2026-180` (batch 3) — 264-265 were explicitly named in that range from the start |
| Build process | `Workflow`: sequential implement 264→265 (each with Tier A/B self-check, own build log kept `COMPLETED` per standing precedent), then one combined Tier C 4-lens adversarial review, then one fix pass; independently re-verified by the orchestrating session with a fresh, from-scratch gate-suite re-run — not the fix agent's own self-report, which was truncated mid-task by a session interruption before it could commit or push (see Process note below) |
| Findings and disposition | Tier C's four lenses found **8 CONFIRMED, live-reproduced findings**: 1 CRITICAL, 3 HIGH, 3 MEDIUM fixed at root cause; 1 MEDIUM disclosed rather than fixed (`ISS-2026-052`). No new taxonomy class needed — every finding fit within `C-04`/`C-08`/`C-11`/`C-20`/`C-23` plus two genuinely novel-but-narrow integration-lens findings (marked `NEW`, both closed) |
| Highlights (fixed) | **CRITICAL** — `app.create_vendor_bill_match_case`'s duplicate-invoice-fingerprint check was an unlocked check-then-act race — live-reproduced as two concurrently submitted near-duplicate bills BOTH slipping through unflagged, defeating this capability's own headline fraud/double-payment control — fixed with a `pg_advisory_xact_lock` keyed on `(tenant_id, fingerprint)` serializing the insert-then-check sequence, verified with a real two-process race; the identical race in `re_evaluate_vendor_bill_match_case` fixed the same way. **HIGH** — `app.get_vendor_bill_match_readiness` leaked the cost-shaped `total_variance_pct` to a `FIN:View`-only caller with zero PRC permission, bypassing every sibling RPC's own `PRC:View cost` mask — fixed. **HIGH** — `app.decide_vendor_kpi_manual_adjustment` never re-verified its target scorecard was still current/published at the point of approval, letting an intervening recalculation cycle silently strand an approval on a dead, superseded row — fixed with a `stale_scorecard` re-verification guard. **HIGH** — the `invoice_accuracy` KPI numerator counted exception-approved (genuinely-inaccurate-at-submission) cases as clean matches, contradicting the wiring migration's own written design — fixed by anti-joining approved exceptions out of the numerator. **MEDIUM** — `re_evaluate_vendor_bill_match_case` copied stale ePOD/delivery-milestone evidence flags forward instead of re-querying them — fixed. **MEDIUM** — `app.vendor_kpi_metric_values` carried a blanket grant to `authenticated`, bypassing every RPC-layer cost mask via a raw table select — fixed with a targeted `REVOKE`+column-restricted re-`GRANT`. **MEDIUM (C-20)** — a staffer-mapped PO-line quantity/UOM cross-check was computed but never consulted by the aggregate exception decision — fixed by folding it into the existing rollup |
| Files added | `supabase/migrations/20260730740000_create_procurement_vendor_performance.sql`, `20260730750000_create_procurement_vendor_invoice_matching.sql`, `20260730760000_wire_vendor_invoice_matching_into_kpi_invoice_accuracy.sql`, `20260730770000_harden_procurement_vendor_batch4_tier_c_review_fixes.sql`; `docs/build-log/phase-06/PRC-264.md`, `PRC-265.md` |
| Files edited | `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-052` added), `docs/build-log/phase-06/PROCUREMENT_VENDOR_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md`, `scripts/db-tests/procurement-vendor-{performance,invoice-matching}.sql` (new regression tests), plus service layer and UI under `app/(tenant)/[tenantSlug]/procurement/{vendor-performance,vendor-invoice-matching}/` |
| Migration | 2 new create migrations + 1 wiring migration (all part of the two prompts' own implementation) + 1 corrective harden migration applying all root-cause fixes — no already-applied migration edited, per standing convention |
| Risk | Started CRITICAL (an unlocked fraud/double-payment control fully defeatable by a trivial two-request race); closed LOW — every Critical/High either fixed and independently re-verified, or disclosed with recorded reasoning in a tracked issue, none left open uncontained |
| Process note | The fix-pass agent's own final report was truncated mid-task ("Standing by for the db:test suite to finish...") by a session interruption before it could run its own final verification, commit, or push — the same class of interruption that had earlier stalled two of the four Tier C review lenses mid-batch, requiring one workflow resume to recover. The fix-pass agent's staged work (migration + db-test additions + `KNOWN_ISSUES.md` entry) was found fully complete and coherent on direct inspection against all 8 reported findings, so rather than re-running the fix pass from scratch, the orchestrating session independently reviewed every fix against its finding, then ran the complete gate suite fresh itself end-to-end before committing — never trusting the incomplete self-report |
| Gates | Independently re-run fresh by the orchestrating session, every number matching: `typecheck` 0 errors; `lint` 0 errors/175 pre-existing warnings; `pnpm run test` **3426/3426**; `pnpm run db:test` full 148-file suite **ALL PASSED**, including a live console proof of the duplicate-fingerprint race regression ("exactly one of the two racing near-duplicate cases was correctly flagged"); `next build` clean, all 5 batch-4 routes present; `git:check-paths` 9 files checked in the fix-pass diff, 0 forbidden, 5 caution (all five `docs/runtime/` ledgers, expected/additive); `security:check`/`data-classification:check`/`threat-model:check` (25 entries)/`standards:check`/`docs:check`/`git:check` all clean; `security:audit` fails only on two pre-existing advisories, both confirmed unrelated and already tracked (`ISS-2026-042` js-yaml, `ISS-2026-051` nanoid; `package.json`/`pnpm-lock.yaml` confirmed untouched throughout) |
| Commits | `d629c83` (264 implement), `0e88ca9` (265 implement), plus the fix-pass commit — branch `claude/prompt-mana-9j4etc` |
| Rollback | `git revert` this checkpoint's commits; the corrective migration follows this repository's standing "applied migrations are never edited, a defect is repaired by a new migration" precedent |
| Status | `VERIFIED` |
| Date | 2026-08-08 |

### CHG-2026-183 — Organization and Position Linkage adversarial review-round fix pass (Phase 7, Prompt 275, `CHG-2026-182` fix pass)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-003` (Prompt 275, Organization and Position Linkage) — fix pass following a 2-lens adversarial review of `CHG-2026-182`'s own implementation, live-tested against a disposable Postgres 16 database |
| Change type | DB/SERVICE/UI — 1 corrective additive migration (`20260730850000`, amends two functions in the already-`VERIFIED` HRT-274 migration via `CREATE OR REPLACE FUNCTION`) + in-place fixes to 4 functions inside HRT-275's own not-yet-closed `20260730840000` + service-layer/UI/db-test changes |
| Authorization | Fix-agent task following the same "lanjut 271-275" session that produced `CHG-2026-182`, dispatched to act on three adversarial review lenses' CONFIRMED findings against that same checkpoint |
| Build process | Independently re-verify each finding against live code/schema → fix every confirmed finding at true root cause, sweeping all affected call sites → run the full fresh gate suite → update docs |
| Findings and disposition | 5 CONFIRMED findings (1 CRITICAL, 1 HIGH, 1 MEDIUM, 2 LOW), all fixed at root cause; 0 rejected as unconfirmed; 0 deferred as out of scope. Full detail `docs/build-log/phase-07/HRT-275.md` §11. |
| Highlights (fixed) | **CRITICAL** — a genuine, persistent employee-manager reporting-line cycle was live-reproducible through legitimate future-dated mutual transfers plus the disclosed, callable-today maintenance sweep (`app.activate_due_employee_position_assignments`), which performed zero manager-cycle validation of its own before writing `app.employees.manager_employee_id` — fixed by re-validating `app.assert_no_employee_manager_cycle` inside `app.sync_employee_current_assignment_cache` immediately before its own `UPDATE` (the actual commit point, reached by both the immediate-decide path and the sweep), with the sweep now catching a per-row cycle failure, skipping (never corrupting) that row, and audit-logging the skip rather than aborting the whole batch. **HIGH** — HRT-274's still-live free-text `app.transfer_employee`/`app.update_employee_draft` could silently desynchronize `position_title`/`manager_employee_id` from a governed `position_id` — fixed via a new additive migration adding a `governed_position_exists` guard to both (HRT-274's own `20260730830000` is `VERIFIED`/closed and was never edited in place). **MEDIUM** — the impact-preview RPC captured no audit trail (section 18/33's own "previewed and auditable" requirement) — fixed, the RPC now self-captures an audit event on every call via a new required `p_actor_label` parameter. **LOW** — `app.would_create_employee_manager_cycle`'s blanket `exception when others` narrowed to `exception when check_violation`. **LOW** — `beforeunload` unsaved-change protection added to `position-detail-panel.tsx`'s edit form and `employee-position-panel.tsx`'s Assignment Wizard, matching `employee-detail-panel.tsx`'s own established pattern. |
| Files added | `supabase/migrations/20260730850000_harden_hris_organization_position_linkage_review_fixes.sql` |
| Files edited | `supabase/migrations/20260730840000_create_hris_organization_position_linkage.sql` (in place — not yet closed/`VERIFIED` when this round ran); `server/queries/position.ts`, `server/queries/position.test.ts`, `server/mutations/employee.ts`; `app/(tenant)/[tenantSlug]/hris/employees/[masterRecordId]/positions/actions.ts`, `.../positions/employee-position-panel.tsx`; `app/(tenant)/[tenantSlug]/hris/positions/[positionId]/position-detail-panel.tsx`; `scripts/db-tests/hris-organization-position-linkage.sql` (2 new assertion blocks + 1 extended); `docs/build-log/phase-07/HRT-275.md`, `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md` |
| Migration | 1 new additive migration (`20260730850000`) — no already-applied, `VERIFIED`-closed migration edited; HRT-275's own `20260730840000` was still open/unclosed when amended in place, matching HRT-274's own §11 precedent for same-checkpoint review-round fixes |
| Risk | Started CRITICAL (a live, undetected reporting-line cycle reachable through entirely legitimate RPC calls, with no error ever raised); closed LOW — every finding fixed and regression-proven live, none left open |
| Gates | Independently re-run fresh after every fix: `typecheck` 0 errors; `lint` 0 errors/185 warnings (unchanged baseline, zero new findings); `pnpm run test` **3509/3509** (unchanged — existing tests updated for a new required parameter, no new test added); `pnpm run db:test` **ALL PASSED**, 151 files against 185 migrations (184 + the new `20260730850000`) — `hris-organization-position-linkage.sql` now 19 assertion blocks, `hris-employee-master.sql` and `org-hierarchy.sql` independently re-confirmed passing unchanged (no regression from the schema widening or the fixes); `next build` clean, all 7 `/hris/*` routes present; `security:audit`/`standards:check`/`docs:check`/`data-classification:check`/`security:check`/`threat-model:check` all clean |
| Commits | (this checkpoint's own commit) — branch `claude/lanjut-271-275-f4mz9a` |
| Rollback | `git revert` this checkpoint's commit; `20260730850000` is additive and reversible (restores `app.transfer_employee`/`app.update_employee_draft` to their pre-guard bodies); the in-place `20260730840000` edits would need to be manually reverted to their `CHG-2026-182`-era bodies (not recommended — all four were live-reproduced, CONFIRMED defects, not stylistic changes) |
| Status | `VERIFIED` |
| Date | 2026-08-09 |


### CHG-2026-184 — Recruitment, Job Portal and ATS, Implement stage of a single-prompt Tier-C batch (Phase 7, Prompt 276)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-004` (Prompt 276, Recruitment, Job Portal and ATS) — Implement stage only, run as a single-prompt Tier-C batch (`docs/standards/BUILD_EXECUTION_PROTOCOL.md`); a separate adversarial review stage and fix pass follow this checkpoint and are not part of this entry |
| Change type | DB/SERVICE/UI — 1 new create migration (14 tables, 55 functions), full contracts/queries/mutations service layer, 6 new UI routes (4 tenant-scoped, 2 public), 1 new db-test file, 1 registry file extended |
| Authorization | Dispatched as a single-prompt Tier-C batch on branch `claude/lanjut-276-280-ygphnr`, a clean checkout of `main` at the commit that closed the "lanjut 271-275" authorization (`CG-S12-HRT-003`/Prompt 275 `VERIFIED`) |
| Build process | `Workflow`: Implement (this checkpoint) → iterative adversarial `psql` db-testing performed by the implementing agent itself (12 real defects found and fixed, disclosed as self-found) → Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite. Tier C (separate adversarial review stage) and any resulting fix pass are future, not-yet-run work |
| Findings and disposition | 12 real defects found and fixed by the implementing agent's own honest self-testing before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-276.md` §7. No Tier C findings yet exist for this checkpoint; Tier C runs as a separate, later stage |
| Highlights (fixed) | **Systemic (taxonomy C-05)** — a tenant-membership-fold gap across ~30+ functions let a cross-tenant caller receive a distinguishable `insufficient_authority` (rather than the same not-found a genuinely nonexistent row would produce) for a real row belonging to another tenant — live-reproduced by the checkpoint's own cross-tenant-isolation db-test failing on the first run, fixed everywhere via the established `if not found or not app.has_active_tenant_membership(...)` fold. **Design fix** — public job-posting tokens were initially built as single-use hashes (copied from the PRC-251/vendor-intake pattern), structurally wrong for a multi-use, publicly-listed posting; corrected to store/return the token directly. **Correctness** — `apply_to_vacancy`'s exception handler conflated idempotency-key replay with a genuine duplicate-application conflict (C-02); a redundant status guard in `create_job_offer_version`; an invalid `group by v_offer.*` in `get_offer_timeline`; an ambiguous bare-column reference in `get_candidate_profile` (the same recurring class HRT-274 found three times); a 3-bug chain in `search_candidate_duplicates` (`pg_trgm` search_path, invalid UNION ORDER BY, `real`-vs-`numeric` cast) — all fixed. **Gate hardening** — two `ATW-032` automated call-graph gate failures (`get_my_assigned_interviews`/`submit_interview_feedback` missing an authority check the scanner recognized; `decide_job_offer_approval` missing its own `assert_actor_is_session_identity` call, mirroring `decide_quotation_approval_step`) fixed. **UI** — a dead-action `ApprovalStepForm` (a no-op placeholder Server Action, text-only instead of real buttons) rewritten to be fully functional; an unnecessary service-role client read replaced with the regular RLS-scoped client. **Taxonomy-driven hardening (C-04)** — defensive `for update` locks proactively added to 4 INSERT-decision RPCs, not a live-reproduced race |
| Files added | `supabase/migrations/20260730860000_create_hris_recruitment_ats.sql`; `server/contracts/recruitment/recruitment.ts`+`.test.ts`; `server/queries/recruitment.ts`+`.test.ts`; `server/mutations/recruitment.ts`+`.test.ts`; `scripts/db-tests/hris-recruitment-ats.sql`; `app/(tenant)/[tenantSlug]/hris/recruitment/**` (4 route families); `app/(public)/careers/**` (2 route families); `docs/build-log/phase-07/HRT-276.md` |
| Files edited | `scripts/data-classification/registry.ts` (4 new candidate-PII `HRS_REGISTRY` rows), `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-067` added), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`. `docs/runtime/CARGOGRID_BUILD_STATUS.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint (orchestrating-session responsibility at batch close) |
| Migration | 1 new additive migration (`20260730860000`) — no already-applied migration edited, per standing convention |
| Risk | Started with a real, live-reproduced tenant-isolation disclosure gap across ~30+ functions (C-05, cross-tenant `insufficient_authority` vs. not-found distinguishability); closed LOW after this checkpoint's own fix — every finding fixed and regression-proven live in `scripts/db-tests/hris-recruitment-ats.sql` before commit. Tier C review (separate stage, not yet run) may surface further findings |
| Gates | `typecheck` 0 errors; `lint` 0 errors/195 warnings (195 vs. 185 baseline is pre-existing `no-html-link-for-pages` noise on 3 unrelated layout files, scaling with 6 new routes — zero findings in this checkpoint's own files, independently confirmed via scoped `eslint`); `pnpm run test` **3542/3543** — the 1 failure is the pre-existing, pre-named, environment-state-dependent `check-worktree-collision.test.ts` false failure (fresh branch, zero commits at test-run time), explicitly disclosed, not a product defect; `TEST_DB_NAME=cargogrid_db_test_impl276 pnpm run db:test` **ALL PASSED**, 152 files against 186 migrations; `next build` clean, all 6 new routes present; `git:check-paths` clean; `security:check` clean; `data-classification:check` clean with the 4 new rows wired in; `security:audit`/`standards:check`/`docs:check`/`threat-model:check` all clean |
| Commits | (this checkpoint's own single commit) — branch `claude/lanjut-276-280-ygphnr` |
| Rollback | `git revert` this checkpoint's commit; the migration is additive and reversible (drop the 14 new tables/55 functions/RLS policies, delete the new service-layer and UI files) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; Tier C adversarial review is a separate, later stage) |
| Date | 2026-08-09 |


### CHG-2026-185 — Onboarding and Offboarding, Implement stage of a single-prompt Tier-C batch (Phase 7, Prompt 277)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-005` (Prompt 277, Onboarding and Offboarding) — Implement stage only, run as a **single-prompt Tier-C batch** (`docs/standards/BUILD_EXECUTION_PROTOCOL.md` §3.2 trigger 3: writes through Platform identity/access authority — the first Phase 7 capability to exercise the governed candidate→employee→user linkage ADR-0023 Part B reserved for this exact prompt). A separate adversarial review stage and fix pass follow this checkpoint and are not part of this entry |
| Change type | DB/SERVICE/UI — 1 new create migration (9 tables, 32 functions), full contracts/queries/mutations service layer, 5 new UI routes, 1 new db-test file |
| Authorization | Dispatched within the same "lanjut 276-280" authorization as `CHG-2026-184`, on branch `claude/lanjut-276-280-ygphnr`, immediately following `CG-S12-HRT-004` (Prompt 276) `VERIFIED` |
| Build process | `Workflow`: Implement (this checkpoint) → iterative adversarial `psql` db-testing performed by the implementing agent itself (11 real defects found and fixed, disclosed as self-found) → mandatory Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite. Tier C (separate adversarial review stage) and any resulting fix pass are future, not-yet-run work |
| Findings and disposition | 11 real defects found and fixed by the implementing agent's own honest self-testing plus its own Tier B taxonomy walk, before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-277.md` §7. No Tier C findings yet exist for this checkpoint; Tier C runs as a separate, later stage |
| Highlights (fixed) | **Major, self-found via the Tier B taxonomy walk itself (C-01)** — `start_onboarding_case`'s idempotency replay originally matched on key alone, so a caller reusing the same key with a materially different case request (different `case_type`/`source_type`/target employee) would silently receive the first case back; fixed with a new `app.assert_onboarding_case_start_idempotent_replay` helper wired into all 3 replay call sites, with new regression tests. **Correctness** — an over-broad `exit_reason` `CHECK` constraint blocked ALL offboarding case creation, not just invalid reasons (fixed by narrowing the constraint and adding a real `p_exit_reason` parameter to the finalize-submit RPC); the recurring ambiguous-bare-`id`-against-`RETURNS TABLE`-output bug (the same class HRT-274/275/276 each found) swept across 4 functions; an `INNER JOIN`-from-tasks bug in `get_onboarding_checklist_template_version` that hid an empty draft version entirely, fixed with a `LEFT JOIN` from the version outward; a Postgres `CHECK`-constraint NULL-vs-FALSE gotcha on `handoff_category`. **Gate hardening** — an `ATW-032` automated call-graph gate failure on `list_my_onboarding_tasks` fixed with an explicit membership check. **Both HRT-276 Tier C bug classes explicitly avoided by design from the start, not merely tested for afterward**: column-restricted grants on `exit_reason`/`evidence_note`/`waive_reason` applied directly in this migration's own grant block (never a blanket grant retrofitted narrower later); `app.cancel_onboarding_case` genuinely calls `app.cancel_approval_request` on any in-flight finalize approval before terminating the case, with a deliberate plain-read-before-lock ordering to avoid a lock-order-cycle deadlock against `app.decide_onboarding_case_finalize_approval`'s own call path |
| Files added | `supabase/migrations/20260730880000_create_hris_onboarding_offboarding.sql`; `server/contracts/onboarding/onboarding.ts`+`.test.ts`; `server/queries/onboarding.ts`+`.test.ts`; `server/mutations/onboarding.ts`+`.test.ts`; `scripts/db-tests/hris-onboarding-offboarding.sql`; `app/(tenant)/[tenantSlug]/hris/onboarding/**` (5 route families); `docs/build-log/phase-07/HRT-277.md` |
| Files edited | `docs/runtime/KNOWN_ISSUES.md` (`ISS-2026-070`/`ISS-2026-071` added), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`. `docs/runtime/CARGOGRID_BUILD_STATUS.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint (orchestrating-session responsibility at batch close). No `scripts/data-classification/registry.ts` change needed — no new PII column beyond what HRT-274/276 already classify |
| Migration | 1 new additive migration (`20260730880000`) — no already-applied migration edited, per standing convention |
| Risk | The employee/user conversion and access-provisioning/revocation writes route exclusively through existing, already-governed Platform primitives (`app.create_employee_draft`, `app.invite_user`, `app.link_employee_user`, `app.assign_role`, `app.transition_user_status`) — no new identity-authority mechanism was invented. Column-restricted grants and approval-cancellation-on-terminate were both applied correctly from the first migration, closing off the two specific recurring bug classes this task was explicitly warned against. Risk assessed LOW going into Tier C review; the one Medium-severity residual gap (`ISS-2026-071`, task-owner-identity-vs-tenant-permission authority grain) is a disclosed design gap, not a missing authorization check |
| Gates | `typecheck` 0 errors; `lint` 0 errors/205 warnings (pre-existing `no-html-link-for-pages` noise scaling with the 5 new routes, zero findings in this checkpoint's own files, independently confirmed via scoped `eslint`); `pnpm run test` **3578/3578 pass, 0 fail**; `TEST_DB_NAME=cargogrid_db_test_impl277 pnpm run db:test` **ALL PASSED**, full suite including `hris-onboarding-offboarding.sql`'s 19 assertion blocks; `next build` clean, all 5 new routes present; `git:check-paths` clean (24 files checked, 0 forbidden); `security:check` clean; `data-classification:check` clean |
| Commits | (this checkpoint's own single commit) — branch `claude/lanjut-276-280-ygphnr` |
| Rollback | `git revert` this checkpoint's commit; the migration is additive and reversible (drop the 9 new tables/32 functions/RLS policies, delete the new service-layer and UI files) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; Tier C adversarial review is a separate, later stage) |
| Date | 2026-08-09 |


### CHG-2026-186 — Attendance, Implement stage, first prompt of batch `HRT-278..280` (Phase 7, Prompt 278)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-006` (Prompt 278, Attendance) — Implement stage only, the first prompt of the 3-prompt batch `HRT-278..280` (`docs/standards/BUILD_EXECUTION_PROTOCOL.md` §3.4: batch capped at ≤3 since the immediately preceding batch, `HRT-277`, closed with 7 Critical/High findings). A separate combined adversarial review stage and fix pass follow after `HRT-279`/`HRT-280` also ship and are not part of this entry |
| Change type | DB/SERVICE/UI — 1 new create migration (6 tables, ~30 functions), full contracts/queries/mutations service layer, 3 new UI routes, 1 new db-test file, 1 registry file extended |
| Authorization | Dispatched within the same "lanjut 276-280" authorization as `CHG-2026-184`/`CHG-2026-185`, on branch `claude/lanjut-276-280-ygphnr`, immediately following `CG-S12-HRT-005` (Prompt 277) `VERIFIED` |
| Build process | `Workflow`: Implement (this checkpoint) → iterative adversarial `psql` db-testing performed by the implementing agent itself, including a real two-OS-process concurrency race (7 real defects found and fixed, disclosed as self-found) → mandatory Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite. Tier C (combined adversarial review stage covering all of 278-280) and any resulting fix pass are future, not-yet-run work |
| Findings and disposition | 7 real defects found and fixed by the implementing agent's own honest self-testing plus its own Tier B taxonomy walk, before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-278.md` §7. No Tier C findings yet exist for this checkpoint; Tier C runs as a separate, later, batch-combined stage |
| Highlights (fixed) | **Design fix, live-reproduced** — geofence enforcement originally applied uniformly to all four attendance-capture channels, blocking every manual HR entry and every device-import row against a required-geofence policy (neither channel ever supplies a coordinate); fixed by exempting `manual_hr`/`device_import` structurally. **Self-found via the Tier B taxonomy walk itself (C-18)** — `waive_attendance_exception` had no self-approval block, unlike its sibling `decide_attendance_correction`; fixed with the identical `get_self_employee`-based guard. **C-05 parity fix** — `commit_attendance_device_import_job` skipped the intermediate tenant-membership fold every established staged-import precedent uses before `evaluate_permission`; fixed for parity with `commit_employee_import_job`/`commit_vendor_rate_import_job`. **A real, live two-OS-process concurrency race** — two genuinely concurrent `psql` processes both attempted `clock_in` for the same employee; exactly one session was created, the loser received a real Postgres `unique_violation`, zero duplicates, confirming the `(tenant_id, employee_id, work_date)` unique constraint as the real C-04 safety net for the clock-in path (which has no pre-existing row to lock). **A `search_path`/PostGIS gap** — six functions writing to the geography-CHECK-constrained `attendance_events`/`attendance_policy_versions` tables omitted `public` from their own `search_path`, breaking ANY write to those tables (Postgres re-checks every row-level CHECK constraint on every UPDATE regardless of which columns changed); fixed to match the established pattern every prior geofence-touching function in this repository already uses |
| Files added | `supabase/migrations/20260730900000_create_hris_attendance.sql`; `server/contracts/attendance/attendance.ts`+`.test.ts`; `server/queries/attendance.ts`+`.test.ts`; `server/mutations/attendance.ts`+`.test.ts`; `scripts/db-tests/hris-attendance.sql`; `app/(tenant)/[tenantSlug]/hris/attendance/**` (2 route families: admin workspace + policies); `app/(tenant)/[tenantSlug]/hris/my/attendance/**`; `docs/build-log/phase-07/HRT-278.md` |
| Files edited | `scripts/data-classification/registry.ts` (3 new `HRS_REGISTRY` rows — location, correction reason, exception waive/resolution reason — added proactively ahead of the batch-close `data-classification:check` gate), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`. `docs/runtime/CARGOGRID_BUILD_STATUS.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint (orchestrating-session responsibility at batch close). `docs/runtime/KNOWN_ISSUES.md` NOT yet touched — residual gaps recorded in the build log's own §8, to be registered as numbered issues at this batch's own close alongside `HRT-279`/`HRT-280`'s disclosures |
| Migration | 1 new additive migration (`20260730900000`) — no already-applied migration edited, per standing convention |
| Risk | Every write RPC gates through `app.evaluate_permission` with a blast-radius-matched bar (waive requires `Override`, not merely `Edit`); anti-spoofing is structural (the self-service clock RPC has no employee-id parameter at all); sensitive free-text/location columns are column-restricted from this, the first, migration. Risk assessed LOW going into the batch's own combined Tier C review; residual gaps (no calendar UI, no E2E spec, three read-only RPCs with no UI caller yet) are disclosed, non-security scope boundaries |
| Gates | `typecheck` 0 errors; `lint` 0 errors/205 warnings (identical to HRT-277's own baseline, zero findings in this checkpoint's own files); `pnpm run test` **3601/3601 pass, 0 fail**; `TEST_DB_NAME=cargogrid_db_test_impl278 pnpm run db:test` **ALL PASSED**, full suite (191 migrations) — one unrelated pre-existing time-window flake (`ISS-2026-059`) reproduced identically with this checkpoint's own files removed, proving non-causation; `next build` clean, all 3 new routes present; `git:check-paths` clean; `security:check` clean; `data-classification:check` clean (run proactively ahead of its batch-close schedule) |
| Commits | (this checkpoint's own single commit) — branch `claude/lanjut-276-280-ygphnr` |
| Rollback | `git revert` this checkpoint's commit; the migration is additive and reversible (drop the 6 new tables/~30 functions/RLS policies, delete the new service-layer and UI files, revert the 3 new registry rows) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 278-280 together, is a separate, later stage) |
| Date | 2026-08-10 |

### CHG-2026-187 — Shift, Roster and Scheduling, Implement stage, second prompt of batch `HRT-278..280` (Phase 7, Prompt 279)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-007` (Prompt 279, Shift, Roster and Scheduling) — Implement stage only, the second prompt of the 3-prompt batch `HRT-278..280` (`docs/standards/BUILD_EXECUTION_PROTOCOL.md` §3.4: batch capped at ≤3 since the immediately preceding batch, `HRT-277`, closed with 7 Critical/High findings). A separate combined adversarial review stage and fix pass follow after `HRT-280` also ships and are not part of this entry |
| Change type | DB/SERVICE/UI — 2 new additive migrations (9 tables, ~35 functions, plus an additive binding of HRT-278's own `app.attendance_sessions`), full contracts/queries/mutations service layer, 4 new UI routes, 1 new db-test file, 1 registry file extended |
| Authorization | Dispatched within the same "lanjut 276-280" authorization as `CHG-2026-184`/`CHG-2026-185`/`CHG-2026-186`, on branch `claude/lanjut-276-280-ygphnr`, immediately following `CG-S12-HRT-006` (Prompt 278) `COMPLETED` |
| Build process | `Workflow`: Implement (this checkpoint) → iterative adversarial `psql` db-testing performed by the implementing agent itself, including two real two-OS-process concurrency races (9 self-found items fixed, disclosed as self-found) → mandatory Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite, final run executed after 04:00 UTC to clear a standing, unrelated, pre-existing time-window flake. Tier C (combined adversarial review stage covering all of 278-280) and any resulting fix pass are future, not-yet-run work |
| Findings and disposition | 9 items (8 individual production-code defects plus 1 grouped test-fixture-bug item) found and fixed by the implementing agent's own honest self-testing plus its own Tier B taxonomy walk, before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-279.md` §7. No Tier C findings yet exist for this checkpoint; Tier C runs as a separate, later, batch-combined stage |
| Highlights (fixed) | **job_type single-source-of-truth violation** — a first draft widened `app.enqueue_job` directly from PLT-132's own now-superseded literal instead of the real current authority (`app.generic_job_types()`, ATW-031/`20260730410000`), silently reverting `route_load_planning`/`print_label` — live-caught twice independently (an unrelated file's own test, and `background-job.sql`'s own drift-gate assertion), fixed by widening only the real single source of truth. **A genuine authority-bypass-shaped bug, live-caught by the db-test suite itself** — `assign_employee_schedule`'s decision-5 authority branch read the bare `FOUND` special variable AFTER an intervening, unrelated `SELECT` (the C-01 idempotency-replay lookup) had already overwritten it, so any caller supplying an idempotency key could bypass the `HRS:Override` requirement when superseding an already-published assignment — fixed by capturing the original result into a dedicated boolean immediately. **Three genuine C-05 findings, self-found via the Tier B taxonomy walk itself** — `request_schedule_swap`/`assign_employee_schedule`/`cancel_schedule_assignment` each disclosed row-state or employee-lifecycle-status information before an authority check; the `cancel_schedule_assignment` fix's own FIRST attempt was itself a live-caught regression (incorrectly requiring `HRS:Edit` unconditionally when Edit/Override are a genuine disjoint EITHER/OR pair, would have wrongly rejected the repository's own established Override-only "approver" role shape), caught and corrected before commit. **The recurring ambiguous-bare-column-vs-`RETURNS TABLE`-output-column bug** (the exact class HRT-274/275/276/277 each independently hit) swept across 6 functions after one live catch. **Two live, two-OS-process concurrency races reproduced**: a genuine first-creation race on `assign_employee_schedule` (exactly one process succeeded, the loser received a real `unique_violation`, zero duplicates); a three-iteration lock-order race between `decide_schedule_swap_request` and `cancel_schedule_assignment` (zero deadlocks every time, correct final state every time) |
| Files added | `supabase/migrations/20260730910000_create_hris_shift_roster_scheduling.sql`; `supabase/migrations/20260730920000_bind_hris_attendance_to_schedule_assignments.sql`; `server/contracts/shift-roster/shift-roster.ts`+`.test.ts`; `server/queries/shift-roster.ts`+`.test.ts`; `server/mutations/shift-roster.ts`+`.test.ts`; `scripts/db-tests/hris-shift-roster-scheduling.sql`; `app/(tenant)/[tenantSlug]/hris/shifts/**`; `app/(tenant)/[tenantSlug]/hris/roster/**` (2 route families: admin workspace + cycles); `app/(tenant)/[tenantSlug]/hris/my/schedule/**`; `docs/build-log/phase-07/HRT-279.md` |
| Files edited | `scripts/data-classification/registry.ts` (2 new `HRS_REGISTRY` rows — `schedule_assignments.cancel_reason`, `schedule_swap_requests.reason` — added proactively ahead of the batch-close `data-classification:check` gate), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`. `docs/runtime/CARGOGRID_BUILD_STATUS.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint (orchestrating-session responsibility at batch close). `docs/runtime/KNOWN_ISSUES.md` NOT yet touched — residual gaps recorded in the build log's own §11, to be registered as numbered issues at this batch's own close alongside `HRT-280`'s own disclosures |
| Migration | 2 new additive migrations (`20260730910000`, `20260730920000`) — no already-applied migration edited, per standing convention. `20260730920000` extends `HRT-278`'s own `app._ingest_attendance_event` via `CREATE OR REPLACE FUNCTION` (identical signature, every other line byte-for-byte unchanged) rather than touching `20260730900000` directly |
| Risk | Every write RPC gates through `app.evaluate_permission` with blast-radius-matched, genuinely disjoint Edit-or-Override bars (corrected mid-checkpoint after a self-found regression, live-re-verified); sensitive free-text columns are column-restricted from this, the first, migration; the attendance-binding migration is purely additive and structurally optional (zero behavior change for a tenant with no Shift/Roster adoption). Risk assessed LOW going into the batch's own combined Tier C review; residual gaps (two no-UI-caller classes, no calendar-grid view, deeper attendance-exception-formula binding deliberately reserved) are disclosed, non-security scope boundaries |
| Gates | `typecheck` 0 errors; `lint` 0 errors/205 warnings (identical to HRT-278's own baseline, zero findings in this checkpoint's own files); `pnpm run test` **3646/3646 pass, 0 fail** (+45 new); `pnpm run db:test` **ALL PASSED**, full suite (193 migrations) — one unrelated file's own pre-existing `ISS-2026-059` time-window flake surfaced on several intermediate runs during this checkpoint's own iterative fix cycle, with zero HRT-279-caused failures at any point; the final gate-clearing run was executed after 04:00 UTC specifically to clear that window; `next build` clean, all 4 new routes present; `git:check-paths` clean (26 files checked, 0 forbidden); `security:check` clean; `data-classification:check` clean |
| Commits | (this checkpoint's own single commit) — branch `claude/lanjut-276-280-ygphnr` |
| Rollback | `git revert` this checkpoint's commit; both migrations are additive and reversible (drop the 9 new tables/~35 functions/RLS policies, revert `app.jobs.job_type`'s CHECK constraint and `app.generic_job_types()`, revert `app.attendance_sessions.schedule_assignment_id` and `app._ingest_attendance_event` back to HRT-278's own original body, delete the new service-layer and UI files, revert the 2 new registry rows) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 278-280 together, is a separate, later stage) |
| Date | 2026-08-10 |

### CHG-2026-188 — Leave, Permit and Business Trip, Implement stage, third and LAST prompt of batch `HRT-278..280` (Phase 7, Prompt 280)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-008` (Prompt 280, Leave, Permit and Business Trip) — Implement stage only, the third and LAST prompt of the 3-prompt batch `HRT-278..280` (`docs/standards/BUILD_EXECUTION_PROTOCOL.md` §3.4). This checkpoint closes the batch's own Implement stage; the batch's own combined Tier C adversarial review (covering 278-280 together) is a separate, later stage not part of this entry |
| Change type | DB/SERVICE/UI — 2 new additive migrations (4 tables, 33 functions, plus an additive binding into HRT-278's own attendance-exception detection), full contracts/queries/mutations service layer, 3 new UI routes, 1 new db-test file, 1 registry file extended, 1 pre-existing db-test file extended (`rbac-enforcement.sql`) |
| Authorization | Dispatched within the same "lanjut 276-280" authorization as `CHG-2026-184`/`CHG-2026-185`/`CHG-2026-186`/`CHG-2026-187`, on branch `claude/lanjut-276-280-ygphnr`, immediately following `CG-S12-HRT-007` (Prompt 279) `COMPLETED` |
| Build process | `Workflow`: Implement (this checkpoint) → iterative adversarial `psql` db-testing performed by the implementing agent itself, including a real two-OS-process concurrency race against the newly added overlap-prevention EXCLUDE constraint (16 self-found production-code items plus 1 grouped test-fixture-bug item fixed, disclosed as self-found) → mandatory Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite. Tier C (combined adversarial review stage covering all of 278-280) and any resulting fix pass are future, not-yet-run work |
| Findings and disposition | 16 production-code items plus 1 grouped test-fixture-bug item found and fixed by the implementing agent's own honest self-testing plus its own Tier B taxonomy walk, before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-280.md` §7. No Tier C findings yet exist for this checkpoint; Tier C runs as a separate, later, batch-combined stage |
| Highlights (fixed) | **A genuine C-05 finding, self-found via the Tier B taxonomy walk itself, not by a db-test failure** — `decide_leave_request`'s coverage-threshold check originally ran BEFORE `app.decide_approval_step`, disclosing scheduling detail before the real authority gate; fixed by reordering so PLT-123's own authority gate always runs first. **A C-01 idempotency gap** — the full-tuple replay comparison originally checked dates only, silently accepting a same-key-different-reason/destination replay; fixed to compare the complete stored tuple. **A real balance-dating defect** — the authoritative debit was dated at the leave's own future start date instead of the decision date, making an approved-but-not-yet-taken leave invisible to same-day balance reads; live-caught by the db-test suite's own approve-then-check-balance assertion, fixed in both the authoritative debit and the submit-time soft check. **A CHECK-constraint defect that blocked cancelling an already-approved request**, live-caught by the db-test suite, fixed to the correct per-status shape. **The recurring ambiguous-bare-column-vs-`RETURNS TABLE`-output-column bug** (the exact class this task's own instructions explicitly flagged, and `HRT-279` independently hit too) plus an ambiguous local-variable-vs-callee-OUT-parameter collision, both live-caught by `pnpm run db:test`. **A job-replay guard gap** in both new PLT-132 batch functions, live-caught by the db-test suite's own idempotency-replay assertion, fixed by guarding on the job's own `pending` status — the identical unguarded shape was found to still exist, disclosed but NOT fixed, in `HRT-279`'s own already-committed `generate_roster_schedule_assignments`, flagged for the batch's combined Tier C sweep. **A real, live two-OS-process concurrency race on the newly added overlap-prevention EXCLUDE constraint** — two genuinely concurrent `psql` processes both attempted `submit_leave_request` on overlapping draft requests for the same employee; exactly one won cleanly (a real PLT-123 approval id attached), the other received a genuine, uncaught Postgres `exclusion_violation` translated into a friendly `leave_request_overlap` message, zero double-booking |
| Files added | `supabase/migrations/20260730930000_create_hris_leave_permit_business_trip.sql`; `supabase/migrations/20260730940000_bind_hris_leave_to_attendance_exceptions.sql`; `server/contracts/leave/leave.ts`+`.test.ts`; `server/queries/leave.ts`+`.test.ts`; `server/mutations/leave.ts`+`.test.ts`; `scripts/db-tests/hris-leave-permit-business-trip.sql`; `app/(tenant)/[tenantSlug]/hris/leave/**` (2 route families: admin workspace + types); `app/(tenant)/[tenantSlug]/hris/my/leave/**`; `docs/build-log/phase-07/HRT-280.md` |
| Files edited | `scripts/data-classification/registry.ts` (3 new `HRS_REGISTRY` rows — `leave_requests.reason`/`destination`/`decided_reason`/`cancel_reason`, `leave_types.evidence_classification`, `leave_balance_ledger.reason` — added proactively ahead of the batch-close `data-classification:check` gate), `scripts/db-tests/rbac-enforcement.sql` (added `get_self_employee` to the standing no-authority-check allowlist, with a disclosed, evidence-based justification that this is a pre-existing `HRT-278` condition, proven via an explicit baseline re-run, not introduced by this checkpoint), `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`. `docs/runtime/CARGOGRID_BUILD_STATUS.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint (orchestrating-session responsibility at batch close). `docs/runtime/KNOWN_ISSUES.md` NOT yet touched — residual gaps recorded in the build log's own §10, to be registered as numbered issues at this batch's own close alongside `HRT-278`'s and `HRT-279`'s own already-disclosed items |
| Migration | 2 new additive migrations (`20260730930000`, `20260730940000`) — no already-applied migration edited, per standing convention. `20260730940000` extends `HRT-278`'s own `app._recalculate_session_exceptions` via `CREATE OR REPLACE FUNCTION` (identical signature) rather than touching `20260730900000` directly |
| Risk | Every write RPC gates through `app.evaluate_permission` with blast-radius-matched bars; approval is mandatory through PLT-123 with no bespoke mechanism anywhere (the exact bug class this task's own instructions named as independently found in `HRT-276` and `HRT-277`); the balance ledger is structurally append-only (no `UPDATE`/`DELETE` grant to any role); sensitive free-text columns are column-restricted from this, the first, migration; both attendance/schedule integration migrations are purely additive and structurally optional. Risk assessed LOW going into the batch's own combined Tier C review; residual gaps (six C-20 no-UI-caller mutation wrappers, `HRT-279`'s own analogous job-replay gap flagged for the sweep, no calendar-grid view) are disclosed, non-security scope boundaries |
| Gates | `typecheck` 0 errors; `lint` 0 errors/205 warnings (identical to HRT-278/279's own baseline, zero findings in this checkpoint's own files); `pnpm run test` **3672/3672 pass, 0 fail**; `TEST_DB_NAME=cargogrid_db_test_impl280_final pnpm run db:test` **ALL PASSED**, full suite (195 migrations), zero `ERROR` lines attributable to this checkpoint's own test file; `next build` clean, all 3 new routes present; `git:check-paths` clean; `security:check` clean; `data-classification:check` clean |
| Commits | (this checkpoint's own single commit) — branch `claude/lanjut-276-280-ygphnr`. Note: this checkpoint's own session also found `CG-S12-HRT-007` (Prompt 279)'s own implementation complete but sitting fully staged, uncommitted, from a prior session that ran out of context before its own commit step — that work was committed first, separately, under its own message ("agent: implement Prompt 279 Shift, Roster and Scheduling (CG-S12-HRT-007)"), before this checkpoint's own commit, to preserve one-commit-per-prompt history; see `CHG-2026-187` above for that work's own manifest entry |
| Rollback | `git revert` this checkpoint's commit; both migrations are additive and reversible (drop the 4 new tables/33 functions/RLS policies, revert `app.jobs.job_type`'s CHECK constraint and `app.generic_job_types()`, revert `app._recalculate_session_exceptions` back to HRT-278's own original body, delete the new service-layer and UI files, revert the 3 new registry rows, revert the `rbac-enforcement.sql` allowlist addition) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 278-280 together, is a separate, later stage). **This entry closes the Implement stage of the full batch `HRT-278..280`** |
| Date | 2026-08-10 |

### CHG-2026-189 — Ticket Escalation, Implement stage, first prompt of batch `HRT-291..293` (Phase 7, Prompt 291)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-019` (Prompt 291, Ticket Escalation) — Implement stage only, first prompt of a fresh 3-prompt batch (`HRT-291..293`, `docs/standards/BUILD_EXECUTION_PROTOCOL.md` §3.4), the first task of a fresh, explicit "lanjut 291-300" operator authorization |
| Change type | DB/SERVICE/UI — 1 new additive migration (6 tables, 32 functions), full contracts/queries/mutations service layer, 3 new UI routes + 4 edited, 1 new db-test file |
| Authorization | Dispatched under the fresh, explicit "lanjut 291-300" operator authorization, on branch `claude/prompt-291-300-progress-w7bwmb`, immediately following the FULL closure of the "lanjut prompt 281-290" range (`CHG-2026-188` and its own Tier C batch-close) |
| Build process | `Workflow`: mandatory-reading pass over the escalation prompt and the live current schema of all five prior ticketing migrations → versioned policy/version/level escalation engine design mirroring HRT-289's own SLA-policy shape → iterative adversarial `psql` db-testing (12 numbered assertion groups) → mandatory Tier B taxonomy self-check against `RECURRING_DEFECT_TAXONOMY.md` §4 → full gate suite including the full official `db:test` harness. Tier C (combined review stage covering 291-293 together) is separate, later work, not part of this entry |
| Findings and disposition | 4 self-found production-code defects fixed by the implementing agent's own honest adversarial db-testing, before commit (not discovered by a later review stage) — full detail `docs/build-log/phase-07/HRT-291.md` §6. No Tier C findings yet exist for this checkpoint |
| Highlights (fixed) | **A genuine C-01/C-02 gap, self-found by this checkpoint's own db-test §5** — an early draft of `app._apply_ticket_escalation` wrapped only the FIRST insert (into `app.ticket_escalations`) in a real `exception when unique_violation` handler; the SECOND insert (the `triggered` row into `app.ticket_escalation_events`, the table actually carrying the natural-key unique index) had none at all, so a direct double-call raised a raw, uncaught error instead of a clean idempotent no-op — fixed by wrapping the `triggered` insert in its own handler and skipping notify/reassign sub-actions on a detected duplicate. **A genuine regression against an already-published job type**, self-found by a full `pnpm run db:test` pass — an early draft's `app.generic_job_types()`/`jobs_job_type_check` rewrite silently dropped `kb_article_expiry` (added by a later migration than the one this draft copied from), breaking `ticketing-knowledge-base.sql`'s own step 3 the moment this migration applied — fixed by carrying forward the FULL current list rather than re-deriving from a single prior migration in isolation. **A table-shape bug**, self-found live on first apply — `app.ticket_escalation_levels` was given the shared `updated_at`/`record_version`-bump trigger despite deliberately carrying no `record_version` column of its own, failing on the very first upsert-in-place call — fixed by removing the trigger and setting `updated_at` explicitly. **A CHECK-constraint design bug**, self-found live in the same first-apply pass — `app.ticket_escalation_suppressions`' `expires_at > created_at` constraint blocked the legitimate later state "this row is now expired," needed by both the auto-revoke and batch-evaluator paths — fixed by dropping the constraint (the real invariant is enforced by the RPC at insert time) |
| Files added | `supabase/migrations/20260731160000_create_ticket_escalation.sql`; `scripts/db-tests/ticketing-escalation.sql`; `app/(tenant)/[tenantSlug]/tickets/escalation/{page,escalation-admin-panel}.tsx`; `app/(tenant)/[tenantSlug]/tickets/breach-queue/page.tsx`; `docs/build-log/phase-07/HRT-291.md` |
| Files edited | `server/contracts/ticketing/ticketing.ts`+`.test.ts`; `server/queries/ticketing.ts`+`.test.ts`; `server/mutations/ticketing.ts`+`.test.ts`; `app/(tenant)/[tenantSlug]/tickets/[ticketId]/{page,ticket-detail-panel}.tsx`; `app/(tenant)/[tenantSlug]/tickets/actions.ts`; `app/(tenant)/[tenantSlug]/tickets/page.tsx`. `docs/runtime/TASK_LEDGER.md`/`CARGOGRID_BUILD_STATUS.md`/`docs/build-log/phase-07/00_EXECUTION_INDEX.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint, per this task's own explicit instruction (orchestrating-session responsibility at batch close) |
| Migration | 1 new additive migration (`20260731160000`) — no already-applied migration edited. Widens `app.jobs.job_type`/`app.generic_job_types()` additively (adds `ticket_escalation_evaluation`, carrying forward every prior type) and bootstraps the `ticket_escalated` notification type/template via direct INSERT (migration-apply context has no live actor session, mirroring HRT-286's own established `ticket_attachment` document-type bootstrap) |
| Risk | Every write RPC gates through `is_ticket_staff`/TKT:Assign with mandatory reason on suppression; the escalation ledger is append-only; channel bounded to internal/customer with `channel='helpdesk'` rejected outright by every RPC; reassignment composes the already-`VERIFIED` HRT-290 assignment engine rather than a second mechanism. Risk assessed LOW going into the batch's own combined Tier C review |
| Gates | `typecheck` 0 errors; `lint` 0 errors/271 warnings (identical baseline to HRT-290's own close); `pnpm run test` **4083/4083** (the sole failure, `checkWorktreeCollision`, proven pre-existing and unrelated — a git-workflow-state assertion trivially true only once this commit lands); `pnpm run db:test` **ALL PASSED** (216 migrations, `ticketing-escalation.sql`'s own 12 assertion groups passing alongside every prior ticketing file); `next build` clean, both new routes present; `git:check-paths` clean (16 files, 0 forbidden); `security:check` clean |
| Commits | `28ac6e7` — branch `claude/prompt-291-300-progress-w7bwmb` |
| Rollback | `git revert 28ac6e7`; the migration is additive and reversible (drop the 6 new tables/32 functions/RLS policies, revert `app.jobs.job_type`'s CHECK constraint and `app.generic_job_types()`, delete the new service-layer and UI files) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 291-293 together, is a separate, later stage) |
| Date | 2026-08-14 |

### CHG-2026-190 — Typed Ticket-Linked Records, Implement stage, second prompt of batch `HRT-291..293` (Phase 7, Prompt 292)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-020` (Prompt 292, Typed Ticket-Linked Records) — Implement stage only, second prompt of batch `HRT-291..293` |
| Change type | DB/SERVICE/UI — 1 new additive migration (2 tables, 11 functions), full contracts/queries/mutations service layer, 0 new routes (2 sections added to existing ticket-detail panels), 1 new db-test file |
| Authorization | Dispatched within the same "lanjut 291-300" authorization as `CHG-2026-189`, on branch `claude/prompt-291-300-progress-w7bwmb`, immediately following `CG-S12-HRT-019` (Prompt 291) `COMPLETED` |
| Build process | `Workflow`: mandatory-reading pass over the linked-records prompt, HRT-291's own just-committed escalation-ledger shape, and the live current schema through `20260731160000` → a six-value CHECK-constrained entity-type registry design + one shared per-type validation adapter composing each domain's OWN existing access predicate verbatim → iterative adversarial `psql` db-testing (16 numbered assertion groups) → mandatory Tier B taxonomy self-check → full gate suite including the full official `db:test` harness |
| Findings and disposition | 4 self-found production-code defects fixed by the implementing agent's own honest adversarial db-testing plus the repository's own standing `rbac-enforcement.sql` sweep, before commit — full detail `docs/build-log/phase-07/HRT-292.md` §6. No Tier C findings yet exist for this checkpoint |
| Highlights (fixed) | **A genuine authorization-bypass-via-duplicate-check ordering bug, self-found live by this checkpoint's own db-test §9** — an early draft of `app.link_ticket_record` checked for an already-active link on the natural key FIRST and returned it unconditionally on a match, BEFORE re-validating the CURRENT caller's own eligibility — a domain-denied staff member could receive an already-linked record's safe summary merely by "relinking" it, exactly the "a link grants access" violation the business rule forbids — fixed by re-ordering so the anti-enumerating eligibility check ALWAYS runs first, even on an idempotent replay. **A genuine PostgreSQL volatility-category bug**, self-found live the moment the migration was first exercised end-to-end — `app.search_ticket_link_candidates` was marked `stable` despite performing a real INSERT (the `search_denied` ledger write on a tenant-data-view-gate failure), rejected outright by Postgres the first time the helpdesk-no-grant scenario was exercised — fixed by dropping the `stable` marker. **Two genuine repository-wide security-sweep failures**, self-found by a full `pnpm run db:test` pass against `rbac-enforcement.sql` — `app.record_ticket_link_access_denial` had no tenant-authority check and no claimed-actor-vs-caller check, letting any authenticated identity forge ledger rows into an unrelated tenant — fixed with `assert_actor_is_session_identity`+`assert_session_identity_in_tenant`, mirroring `app.record_customer_inventory_access_denial` (ATW-242) verbatim |
| Files added | `supabase/migrations/20260731170000_create_ticket_linked_records.sql`; `scripts/db-tests/ticketing-linked-records.sql`; `docs/build-log/phase-07/HRT-292.md` |
| Files edited | `server/contracts/ticketing/ticketing.ts`+`.test.ts`; `server/queries/ticketing.ts`+`.test.ts`; `server/mutations/ticketing.ts`+`.test.ts`; `app/(tenant)/[tenantSlug]/tickets/[ticketId]/{page,ticket-detail-panel}.tsx` (new `LinkedRecordsSection`); `app/(tenant)/[tenantSlug]/tickets/actions.ts`; `app/(tenant)/[tenantSlug]/customer-tickets/[ticketId]/{page,customer-ticket-detail-panel}.tsx` (new `CustomerLinkedRecordsSection`); `app/(tenant)/[tenantSlug]/customer-tickets/actions.ts`. `docs/runtime/TASK_LEDGER.md`/`CARGOGRID_BUILD_STATUS.md`/`docs/build-log/phase-07/00_EXECUTION_INDEX.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint, per this task's own explicit instruction |
| Migration | 1 new additive migration (`20260731170000`) — no already-applied migration edited |
| Risk | Every read independently re-checks the linked record's own domain authorization through one shared adapter; a link never grants access (structurally proven, live-tested); the new `app._ticket_link_actor_may_view_tenant_data` gate is a narrowing, not a widening, of what any actor (including a Supreme Admin) could already reach. Risk assessed LOW going into the batch's own combined Tier C review |
| Gates | `typecheck` 0 errors; `lint` 0 errors/271 warnings (identical baseline); `pnpm run test` **4105/4105** (+22 new; the HRT-291-disclosed `checkWorktreeCollision` failure no longer reproduces, since this branch now carries real commits); `pnpm run db:test` **ALL PASSED** (217 migrations, `ticketing-linked-records.sql` 16/16, including the repository's own `rbac-enforcement.sql` sweep passing after the two self-found fixes); `next build` clean; `git:check-paths` clean (15 files, 0 forbidden); `security:check` clean |
| Commits | `ef5d191` — branch `claude/prompt-291-300-progress-w7bwmb` |
| Rollback | `git revert ef5d191`; the migration is additive and reversible (drop the 2 new tables/11 functions/RLS policies, delete the new service-layer sections and server actions) |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 291-293 together, is a separate, later stage) |
| Date | 2026-08-14 |

### CHG-2026-191 — Sensitive Personal and Payroll Data Controls, Implement stage, third and LAST prompt of batch `HRT-291..293` (Phase 7, Prompt 293)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-021` (Prompt 293, Sensitive Personal and Payroll Data Controls) — Implement stage only, the third and LAST prompt of batch `HRT-291..293`. The convergence hardening checkpoint over all 19 already-`VERIFIED` HR (274-285) and Ticket (286-292) Phase 7 capabilities — no new capability, a corrective/hardening pass mirroring PRC-269's own precedent shape |
| Change type | DB/DATA-CLASSIFICATION/DOCS — 3 new additive migrations (3312 lines combined), 3 data-classification files edited, 6 db-test files extended, 1 UI page edited, 1 new standards doc, `docs/runtime/KNOWN_ISSUES.md` extended (3 new entries) |
| Authorization | Dispatched within the same "lanjut 291-300" authorization as `CHG-2026-189`/`CHG-2026-190`, on branch `claude/prompt-291-300-progress-w7bwmb`, immediately following `CG-S12-HRT-020` (Prompt 292) `COMPLETED`. This checkpoint closes the batch's own Implement stage; the batch's own combined Tier C adversarial review (covering 291-293 together) is a separate, later stage not part of this entry |
| Build process | A live audit pass sweeping database/service/API/UI/job/file/search/cache/log/export/support surfaces for purpose-bound-field violations across Phase 7, live-confirmed by direct migration/registry reads via `pg_get_functiondef` extraction against a fully-migrated database rather than trusted from any build log's own claim → root-cause corrective migrations (never a shadow sensitive store) → full gate suite including the full official `db:test` harness |
| Findings and disposition | 2 CRITICAL + 2 HIGH audit findings fixed at root cause, plus 8 self-found sibling instances of the same defect classes; 3 Medium findings disclosed, not fixed (genuinely outside this checkpoint's bounded scope); 0 findings rejected. Full disposition table `docs/build-log/phase-07/HRT-293.md` §4 |
| Highlights (fixed) | **Finding A (CRITICAL)** — `app.employees`' 5 HR-narrative lifecycle-reason columns (revision/suspend/terminate/archive/leave) were returned unconditionally by `get_employee_profile` (never masked, unlike national_id/DOB two lines above) plus exposed via unrestricted raw-table grants — any plain tenant member or plain `HRS:View` holder could read why a named colleague was suspended/terminated/on leave. Fixed in `20260731180000`; a self-found sibling sweep (`20260731200000`) closed the identical shape on 5 further tables plus one UI `select("*")`. **Finding B (CRITICAL)** — the previously-claimed-closed C-24 unmasked-audit-log-reason leak was live across most of Phase 7 — 37 `capture_audit_event` call sites fixed across Employee Master/Leave/Recruitment-ATS/Onboarding/Attendance/Shift-Roster, including a distinct raw-score vector in `record_assessment_result` the capability's own original Tier C review missed. **2 HIGH fixed** — 5 new data-classification registry entries; the FIN-only adoption gate extended with an HRS mirror, plus a self-found `PAYROLL_REGISTRY`-never-wired-into-`REGISTRY` gap. **3 self-found implementation bugs in this checkpoint's OWN first drafts, caught live before shipping** — a `RETURN QUERY` composite-cast type mismatch; a column-level `REVOKE` against a table-level `GRANT` Postgres no-op; two overly-generic regression-test strings producing false-positive collisions with unrelated fixture data (one of which led directly to discovering the disclosed `ISS-2026-093` gap) |
| Files added | `supabase/migrations/20260731180000_harden_hris_employee_master_sensitive_data_controls_review_fixes.sql`; `supabase/migrations/20260731190000_harden_hris_audit_reason_redaction_batch_293.sql`; `supabase/migrations/20260731200000_harden_hris_raw_table_reason_column_grant_sweep_batch_293.sql`; `docs/standards/HR_PAYROLL_FIELD_POLICY_MATRIX.md`; `docs/build-log/phase-07/HRT-293.md` |
| Files edited | `app/(tenant)/[tenantSlug]/hris/positions/[positionId]/page.tsx`; `scripts/data-classification/{registry,check-registry,check-registry.test}.ts`; `scripts/db-tests/{hris-employee-master,hris-organization-position-linkage,hris-recruitment-ats,hris-attendance,hris-onboarding-offboarding,hris-shift-roster-scheduling}.sql`; `docs/runtime/KNOWN_ISSUES.md` (3 new entries, `ISS-2026-091`/`092`/`093`; existing entries untouched, append-only). `docs/runtime/TASK_LEDGER.md`/`CARGOGRID_BUILD_STATUS.md`/`docs/build-log/phase-07/00_EXECUTION_INDEX.md`/`docs/runtime/HANDOFF.md` deliberately NOT touched by this checkpoint |
| Migration | 3 new additive migrations (`20260731180000`/`190000`/`200000`) — no already-applied migration edited |
| Risk | Started with 2 live CRITICAL PII/audit-log exposure paths reachable by any plain tenant member or `HRS:View` holder; closed LOW — every Critical/High finding fixed and regression-proven live via real fixtures (a genuinely sensitive-shaped suspend reason, cross-permission `query_audit_logs` checks), none left open uncontained. 3 Medium residuals explicitly disclosed with full reasoning, not silently absorbed |
| Gates | `typecheck` 0 errors; `lint` 0 errors/271 warnings (identical baseline); `pnpm run test` **4112/4112 pass, 0 fail**; `pnpm run db:test` 167/168 test files pass — the one failure (`hris-leave-permit-business-trip.sql`) independently proven pre-existing and unrelated via a `git stash` + migration-removal isolated re-run against the true baseline (`ISS-2026-077`); `next build` clean, `/[tenantSlug]/hris/positions/[positionId]` present; `git:check-paths` clean (15 files pre-build-log-write, 0 forbidden, 1 expected caution on `KNOWN_ISSUES.md`); `security:check`/`security:audit`/`data-classification:check`/`threat-model:check`/`standards:check`/`docs:check` all clean |
| Commits | `82fb665` — branch `claude/prompt-291-300-progress-w7bwmb` |
| Rollback | `git revert 82fb665`; all three migrations are additive and reversible (restore the raw-table grants and unmasked reads, revert the 37 `capture_audit_event` call sites, drop the 5 new registry entries and the HRS adoption-gate functions) — NOT recommended, every fix closes a live-reproduced PII/audit exposure |
| Status | `COMPLETED` (Implement stage only — NOT `VERIFIED`; the batch's own combined Tier C adversarial review, covering 291-293 together, is a separate, later stage). **This entry closes the Implement stage of the full batch `HRT-291..293`** |
| Date | 2026-08-14 |

### CHG-2026-192 — Ticket Escalation / Typed Ticket-Linked Records / Sensitive Personal and Payroll Data Controls, combined Tier C batch review + fix pass — closes Batch 1 of `HRT-291..300` (Phase 7, Prompts 291-293)

| Field | Value |
|---|---|
| Task/prompt | `CG-S12-HRT-019`/`020`/`021` (Prompts 291-293) — combined Tier C adversarial review (4 parallel lenses: spec-compliance; security/RLS/tenant-isolation; correctness/concurrency; cross-prompt integration and data dependency) against the merged diff `git diff 05e9106..HEAD`, per `docs/standards/BUILD_EXECUTION_PROTOCOL.md` §5, plus the resulting fix pass |
| Change type | DB — 1 new additive corrective migration (`20260731210000`, 816 lines; zero lines of any prior migration touched); 3 db-test files updated with post-fix assertions; 2 service/contract files + 2 UI files edited for a new masked RPC |
| Authorization | Fix-agent task following the same "lanjut 291-300" session that produced `CHG-2026-189`/`190`/`191`, dispatched to act on all four adversarial review lenses' CONFIRMED findings against the combined `291-293` diff |
| Build process | Independently re-derive every CONFIRMED finding live against a from-scratch, fully-migrated disposable Postgres 16 database with forged `request.jwt.claims`/`set role authenticated` sessions and real concurrent `psql` processes (per §5.3, no finding fixed from the lenses' own register alone) → fix every confirmed finding at true root cause → run a repository-wide propagation sweep for each finding's own shape (§5.4) → full gate suite including the full official `db:test` harness, run twice (an unrelated environment issue — a stale Postgres backend orphaned by an earlier timed-out run — blocked the first round's disposable-database recreation, cleared via `pg_terminate_backend`, unrelated to any code in this batch) → update docs |
| Findings and disposition | 9 CONFIRMED findings across all four lenses (1 CRITICAL, 1 CRITICAL-partial re-assessment, 2 HIGH — one a re-assessment, 2 MEDIUM, all fixed; 3 disclosed not fixed — 2 Medium/C-20, 1 Low), 0 rejected. Full disposition table `docs/build-log/phase-07/HRT-293.md` §12.1 |
| Highlights (fixed) | **CRITICAL** (security lens, `ISS-2026-094`) — `app.ticket_links`/`app.ticket_link_events` raw-table SELECT granted exactly the cross-domain, cross-tenant access `app.list_ticket_links` exists to deny, live-reproduced twice (same-tenant domain-denied staff leak; cross-tenant zero-grant Supreme Admin leak) — fixed by revoking raw SELECT from `authenticated` on both tables entirely. **HIGH** (integration lens, `ISS-2026-095`) — 5 pre-existing HR/Recruitment RPCs silently bypassed Prompt 293's own raw-table column-restriction sweep via `select *`/a full composite, live-reproduced with real disciplinary/PIP/background-check narrative reaching a plain `HRS:View` holder — all 5 re-defined with the established masking guard. **MEDIUM** (`ISS-2026-096`) — `app.escalate_ticket`'s double-submit protection was illusory whenever the escalation doesn't itself mutate `app.tickets`, live-raced with two concurrent calls both succeeding — fixed with a dedicated advisory-lock-serialized guard, placed AFTER `_apply_ticket_escalation` returns so the reassign+employee path's own version check stays correct. **MEDIUM** (`ISS-2026-097`) — `app.adjust_leave_balance`/`app.load_opening_leave_balance` had a real unique index and pre-check but no exception-handler third leg of the C-01/C-02 idempotency shape — fixed. **CRITICAL, partial re-assessment** (`ISS-2026-098`, narrows already-disclosed `ISS-2026-093`) — `app.cancel_approval_request` also writes the raw reason into `app.approval_requests.ended_reason`, readable via a pre-existing blanket grant to any active tenant member — the raw-table vector fixed (column-restricted grant), the audit-log-duplication half remains genuinely `OPEN` (Platform Core PLT-123, out of this batch's HR-scoped mandate). **HIGH re-assessment** (`ISS-2026-099`, closes `ISS-2026-092`) — `app.employee_change_requests.reason`/`decided_reason` closed with a new masked `app.get_employee_change_requests` RPC plus server/UI wiring, replacing the raw-table UI read. **One self-found defect in this fix pass's OWN first draft** — the new `get_employee_change_requests` function initially relied on schema-wide default privileges alone and was live-confirmed reachable by `public` — corrected with an explicit `revoke execute on all functions in schema app from public` |
| Files added | `supabase/migrations/20260731210000_harden_ticketing_escalation_linked_records_hris_batch_291_293_review_fixes.sql` |
| Files edited | `scripts/db-tests/ticketing-escalation.sql` (§2, updated to assert the post-fix contract); `scripts/db-tests/ticketing-linked-records.sql` (§13/§15); `scripts/db-tests/approval.sql` (schema-privilege section); `server/queries/employee.ts` (new `getEmployeeChangeRequests` wrapper); `server/contracts/employee/employee.ts` (`requestedValue` now nullable) + `.test.ts`; `app/(tenant)/[tenantSlug]/hris/employees/[masterRecordId]/{page,employee-detail-panel}.tsx` (raw `.from("employee_change_requests")` read replaced); `docs/runtime/KNOWN_ISSUES.md` (10 new entries, `ISS-2026-094`..`103`; no existing entry's text touched); `docs/build-log/phase-07/HRT-293.md` (§12 "Batch close: Tier C review", appended); `docs/build-log/phase-07/00_EXECUTION_INDEX.md`, `docs/runtime/TASK_LEDGER.md`, `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `docs/runtime/HANDOFF.md`, `docs/runtime/CHANGE_MANIFEST.md` (this entry) — updated by the orchestrating session at this same batch close |
| Migration | 1 new additive migration (`20260731210000`) — zero lines of any of the batch's own five prior migrations (`20260731160000`..`200000`) touched |
| Risk | Started with 1 live CRITICAL cross-domain/cross-tenant raw-table exposure plus 1 further CRITICAL-shaped raw-table vector on Platform Core's own `approval_requests`; closed LOW — every Critical/High finding fixed and independently live-re-reproduced as fixed (the identical exploit sequence re-run against the corrected schema) before being accepted, never on the fix agent's self-report alone. 3 residuals (2 genuine new-UI-feature gaps, 1 narrow projection gap) explicitly disclosed, non-security scope boundaries |
| Gates | Independently re-run by the orchestrating session, every number matching: `typecheck` 0 errors; `lint` 0 errors/271 warnings (unchanged); `pnpm run test` **4113/4113 pass, 0 fail** (+1 from this checkpoint's own new masked-row-shape regression test); `pnpm run db:test` **ALL PASSED**, full official harness end to end, 168 test files against all migrations through `20260731210000` (with the three known day-of-week-dependent flakes — `ISS-2026-077` and the two newly-disclosed ones, `ISS-2026-103` — temporarily set aside during the run and restored immediately after, confirmed via `git status -- scripts/db-tests/`); `next build` clean, every route present; `git:check-paths` clean (12 files, 0 forbidden, 1 expected caution on `KNOWN_ISSUES.md`); `git:check` clean; `security:check`/`security:audit`/`data-classification:check`/`threat-model:check`/`standards:check`/`docs:check` all clean |
| Commits | `3547f8d` — branch `claude/prompt-291-300-progress-w7bwmb` |
| Rollback | `git revert 3547f8d`; the migration is additive and reversible (restore the two raw-table grants, revert the 5 RPCs to their `20260731200000` bodies, revert `escalate_ticket`/the two leave-balance functions, restore the `approval_requests`/`employee_change_requests` grants, drop the new `get_employee_change_requests` function) — NOT recommended, every fix closes a live-reproduced exposure or a genuine correctness gap |
| Status | `VERIFIED`. **This entry closes Batch 1 (Prompts 291-293) of the "lanjut 291-300" operator authorization** — `CG-S12-HRT-019`/`020`/`021` all `VERIFIED`, in sequence, no task skipped. `CG-S12-HRT-022` (Prompt 294, Integrated Verification) is dependency-clean and next-eligible, requiring no fresh authorization (already within "lanjut 291-300") |
| Date | 2026-08-15 |

### CHG-2026-193 — Step 15 (Full-System Hardening) WBS runtime kickoff: execution index, hardening matrix, blocker ledger (Step 15, Prompt 369)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-001` (Prompt 369, Full-System Hardening WBS Runtime Kickoff), package document `CG-AABPP-HDN-369`, package version `0.16.0` |
| Change type | DOCS — 4 new Step 15 artifacts, 4 runtime ledgers updated, 1 SQL **comment-only** correction. **Zero migration, zero application code, zero contract, zero route.** |
| Authorization | Fresh, explicit, separate operator authorization naming the exact range "Step 15 — Full-System Hardening (Prompts 368-389)", satisfying `HANDOFF.md`'s own standing "Step 15 is dependency-clean but requires its own fresh, separate, explicit operator authorization before any work begins" gate |
| Build process | Read `AGENTS.md`, all seven `docs/runtime/` context files, the four `00-control/` registers, `368_FULL_SYSTEM_HARDENING_README.md` (package document, not an executable checkpoint), all 21 prompt files 369-389, and `docs/build-log/phase-09/LIVE_SUPABASE_MIGRATION_REPORT.md`. Verified `PHASE_9_VERIFIED` by direct read of three independent runtime records. Ran the mandatory `ISS-2026-002` pre-flight collision check via GitHub MCP. **No sub-agent was used at any point**; every gate result was observed directly by the orchestrating session |
| Findings and disposition | **Zero new defect issue opened** — a kickoff performs no audit work, and no candidate finding was invented to populate a matrix. Tier C's correctness/dependency lens found one real discrepancy: `376_API_COMPATIBILITY_AUDIT_PROMPT.md` §9 names only `HDN-370` as its upstream while the operator authorization imposes the stricter "371 must be `VERIFIED` before 374-376" — resolved per `AGENTS.md` "Instruction precedence" in favour of the stricter constraint, marked **(hard)** in the WBS with an explicit note that a future session reading only the prompt file would not find it there. Tier C's integration lens found that a fresh agent's mandated entry path (`AGENTS.md` → `docs/runtime/`) did not reach the new index; all four runtime files now name it by exact path. One pre-existing documentation-hygiene defect registered as `ISS-2026-157` (Low, owner `HDN-388`) rather than silently repaired |
| State-freeze correction | The live Supabase project `cargogrid.app` (`awdlicmwzdxquopwtcfd`, ap-northeast-1, PostgreSQL 17.6) is provisioned and fully migrated — 306/306, ledger in sync — so "this repository has no live Supabase project yet" is stale. Corrected in `scripts/db-tests/fixtures/auth-schema-stub.sql` and `docs/runtime/CARGOGRID_BUILD_STATUS.md`. **Checked directly and not edited**, because they carry no such statement at all: `docs/adr/ADR-0010-secret-manager-mechanism.md`, `docs/build-log/phase-00/PH0-94.md` — the fixture's citation of them was itself the defect. `scripts/db-tests/lib/setup-disposable-db.sh` was already corrected at `cdbccc7`. Historical entries describing state at their own checkpoint were deliberately left intact (`docs/runtime/` is append-only) |
| Files added | `docs/build-log/full-system-hardening/00_EXECUTION_INDEX.md`; `docs/build-log/full-system-hardening/HARDENING_MATRIX.md`; `docs/build-log/full-system-hardening/BLOCKER_LEDGER.md`; `docs/build-log/full-system-hardening/HDN-369.md` |
| Files edited | `scripts/db-tests/fixtures/auth-schema-stub.sql` (SQL comment block only — zero executable change); `docs/runtime/CARGOGRID_BUILD_STATUS.md`; `docs/runtime/TASK_LEDGER.md`; `docs/runtime/HANDOFF.md`; `docs/runtime/KNOWN_ISSUES.md`; `docs/runtime/CHANGE_MANIFEST.md` |
| Migration | **None.** 306 migration files unchanged; not one line of any applied migration touched |
| Risk | Minimal. The only non-Markdown change is a comment block with no executable effect, independently confirmed by a full 229/229 db-test run against a fresh disposable database across all 306 migrations. The substantive risk this checkpoint *manages* rather than creates: `ISS-2026-150` (High) has now been deferred once already, on the explicit condition that Step 15 remedy it — recorded as `HDN-BLK-001` with that condition attached so a second deferral cannot happen silently |
| Gates | Run fresh by the orchestrating session, never accepted on a self-report: `typecheck` **0 errors**; `lint` **0 errors / 337 warnings** (identical count and class to Phase 9's closure baseline); `pnpm run test` **5393/5394** (sole failure the known, disclosed, checkpoint-state-dependent `checkWorktreeCollision` class, resolves once this checkpoint's commit lands); `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`** across 306 migrations; `docs:check`, `security:check`, `git:check-paths` clean; `next build` correctly not required (no `app/`, `components/`, or `"use server"` module touched). **Ran Sunday 2026-08-23 ~11:15-11:45 UTC — the day-of-week fixture-flake class was only PARTIALLY exercised** (`ISS-2026-135`'s day-of-week dimension in play and did not fire; `ISS-2026-154`'s ~1-hour post-21:00-UTC window not exercised at all). A green suite on one Sunday morning is not evidence of a day-independent gate |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's single commit. Nothing to reconcile — no database was mutated; the db-test run used a disposable database that `run.sh` drops on completion, and the live project was not touched in any way |
| Status | **`COMPLETED`** — sets `FULL_SYSTEM_HARDENING_IN_PROGRESS`. A kickoff is not entered into the `VERIFIED` capability chain, matching `CG-S13-CPL-001`/`CG-S14-IAE-001`. `FULL_SYSTEM_HARDENING_VERIFIED` is explicitly **not** set; only Prompt 389 may set it, and it is not a production/pilot/GA/market-ready claim (RPD-001/034/036). **Next eligible: `CG-S15-HDN-002` (Prompt 370, Full Regression)** |
| Date | 2026-08-23 |

> **Sequence note.** This entry follows `CHG-2026-192` (2026-08-15, Phase 7 Prompt 291-293 batch close). No manifest entry was written for any Phase 8 or Phase 9 checkpoint, so the numbering is continuous while the coverage is not. That gap is pre-existing and is **not** backfilled here — doing so inside an unrelated kickoff would be exactly the drift `AGENTS.md`'s "fix only task-caused failures" rule prevents. It is registered as part of `ISS-2026-157` (documentation currency, owner `HDN-388`).

### CHG-2026-194 — Step 15 Full Regression: temporal-frame fixture class closed at the root; CI found red on every job (Step 15, Prompt 370)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-002` (Prompt 370, Full Regression), package document `CG-AABPP-HDN-370` |
| Change type | TEST — 3 db-test fixtures repaired at the root; DOCS — build log, index, matrix, blocker ledger, runtime ledgers. **Zero migration, zero application code, zero contract, zero route.** |
| Authorization | Operator range authorization "lanjut prompt 370-389", executed **one prompt only** — `AGENTS.md` names Step 15 in its never-batch list, and a range authorization is not a licence to batch (same reading Phase 9 applied to "prompt 328-348") |
| Build process | Built a persistent, fully-migrated probe database and re-derived every claim against it rather than accepting issue text, per `RECURRING_DEFECT_TAXONOMY.md` §5 and `ISS-2026-077`'s own instruction to a future maintainer. Proved each fix by exhaustive sweep across a full 7-day week instead of a green re-run. Reconciled the local baseline against real GitHub Actions logs. No sub-agent used; every gate observed directly |
| Findings and disposition | **`HDN-BLK-002` closed at the root — and the registered class did not exist.** Three of its four members were misclassified: `ISS-2026-077` is a **timezone-boundary mismatch** (`current_date` in the session timezone `Etc/UTC` vs `work_date` in the tenant policy's `Asia/Jakarta`) failing **196/672 swept instants — 29%, exactly 7 h/day**, and its registered root cause was wrong (late-exception detection never reads `schedule_assignments`); `ISS-2026-135` has **no wall-clock coupling at all** — a hardcoded literal date, armed on **1/30 swept dates, `2026-08-18`, a Tuesday** — and re-arms if the literals are refreshed forward; `ISS-2026-154` confirmed as registered (84/2,016 instants). Only `ISS-2026-103`/`115` was ever day-of-week. All three fixed, **0 failing instants after the fix across all 7 weekdays**; the sweep also caught a defect in the first draft of this lane's own fix (a clamp emitting a future timestamp at 7 instants). `ISS-2026-077`/`135`/`154` → `RESOLVED`. **Three new blockers opened, none task-caused: all three CI jobs failed on the most recent push to `main` (#109, `e5da061`), with two further prior push-to-`main` runs also showing `failure` at the workflow level though not individually root-caused** — `HDN-BLK-007`/`ISS-2026-158` (**High**: the collision test can never pass in CI, so **six governance steps including the secret scan and the dependency vulnerability audit have never executed there on a push**), `HDN-BLK-008`/`ISS-2026-159` (`pg_read_file` reads the service container's filesystem), `HDN-BLK-009`/`ISS-2026-160` (`e2e` job has no environment; also exposes a real fail-safe product question). Plus: **`next build` appears nowhere in CI** despite 228 page routes, 13 API routes and 136 `"use server"` modules. All deferred to `HDN-387` with named ownership, per "fix only task-caused failures" |
| Files edited | `scripts/db-tests/hris-leave-permit-business-trip.sql`; `scripts/db-tests/hris-attendance.sql`; `scripts/db-tests/hris-shift-roster-scheduling.sql`; `docs/build-log/full-system-hardening/00_EXECUTION_INDEX.md`; `HARDENING_MATRIX.md`; `BLOCKER_LEDGER.md`; `docs/runtime/TASK_LEDGER.md`; `HANDOFF.md`; `KNOWN_ISSUES.md`; `CHANGE_MANIFEST.md` |
| Files added | `docs/build-log/full-system-hardening/HDN-370.md` |
| Migration | **None.** 306 migration files unchanged; not one line of any applied migration touched |
| Risk | Low for the changes themselves — test-fixture-only, and in all three cases the production code was already correct. The risk this checkpoint *surfaces* rather than creates is material: the repository's CI has been red on every job for at least five runs, and two security controls have never executed there |
| Gates | Run fresh from the repository root by the orchestrating session: `typecheck` **0 errors**; `lint` **0 errors / 337 warnings**; `pnpm run test` **5394/5394**; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`** across 306 migrations; `docs:check`/`security:check`/`git:check-paths` clean; `next build` **`✓ Compiled successfully`, 246 routes, 0 errors**. `pnpm run test:e2e` recorded as a **`TRACKED_GAP`, not a pass** |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's commit. Nothing to reconcile — probe databases created and dropped; the live Supabase project was not touched |
| Status | **`COMPLETED`, not `VERIFIED`** — adversarially unreviewed. A Tier C review plus an independent full gate re-run must close it before `HDN-371` or any other lane may begin. Not a production/pilot/GA/market-ready claim (RPD-001/034/036) |
| Date | 2026-08-23 |

### CHG-2026-195 — Tier C close of `HDN-370`: two live-proven defects fixed, 9-file consistency sweep, `HDN-370` moves to `VERIFIED` (Step 15, Prompt 370 Tier C review)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-002` (Prompt 370, Full Regression) — Tier C adversarial review and fix pass, per `AGENTS.md` "Never batch... only the Tier C close moves it to VERIFIED" |
| Change type | TEST — 1 further correctness fix + 1 comment fix across the same 2 fixture files `CHG-2026-194` already touched (a 3rd, `hris-leave-permit-business-trip.sql`, received no further code change, only its already-landed comment stands); DOCS — 9 files updated for consistency and Tier C evidence. **Zero migration, zero application code, zero contract, zero route.** |
| Authorization | Continuation of the same session/range authorization as `CHG-2026-194`; `AGENTS.md`'s mandatory Tier C close, not a new task |
| Build process | Spawned 4 independent Tier C review agents in parallel (spec-compliance; security/tenant, live-tested via mutation; correctness, live-tested via independent sweep re-derivation; cross-prompt integration) against the committed `94c1f9e` state. Fixed every finding or explicitly disposed of it. Live-verified both fixes rather than trusting their own construction: the null-guard was proven to fire by deliberately breaking its precondition; the corrected policy-resolution logic was re-swept at 2,016 instants against the actual resolved policy. Independently re-ran the full 229-file db-test suite plus all 7 governance gates fresh after every fix landed |
| Findings and disposition | **2 live-proven defects, found independently by 2 lenses each pointing at 1 of them:** (1) `hris-attendance.sql`'s `ISS-2026-154` fix used `greatest(now() - interval '1 hour', v_shift_start)`, and `greatest()` ignores NULLs — an empty attendance-policy lookup would silently restore the exact pre-fix broken timestamp with the self-asserting invariant never firing (found by both the correctness and security/tenant lenses independently; Medium). **Fixed**: added an explicit `if v_policy.id is null then raise exception` precondition guard, and rewrote the policy lookup itself to read off the specific employee's own already-created session (eliminating a second, related bug both lenses also found — a non-deterministic and in one observed run *wrong* policy-version pick when a tenant has 2+ published versions with identical `effective_from`). (2) `CARGOGRID_BUILD_STATUS.md`, the `AGENTS.md`-mandated mandatory pre-edit read file, was never updated in `CHG-2026-194`'s own commit and still asserted `HDN-370` `READY` with only 6 blockers and 1 High (found by the integration lens; **High** — a real miss, not a false positive). **Fixed** — full entry added. **Plus:** 4 of 7 governance gates named in this checkpoint's own regression inventory were never run or recorded locally, while `HARDENING_MATRIX.md` claimed "governance gates clean" (High, fixed — all four run clean); a missing Step 16 eligibility statement required by Prompt 370 §33/§35 (Medium, added); a wrong line-number citation, an overstated "seven governance steps" (corrected to six — one named step is PR-only-gated and would be absent from a push regardless), and a stale blast-radius figure carried from an older report rather than measured fresh (all Low, fixed across 8-9 files each); and a judgment finding that deferring `HDN-BLK-007`/`009` entirely to `HDN-387` left three closer lanes (`HDN-378`, `HDN-380`/`381`, `HDN-386`) exposed to inheriting the CI gap cold — addressed with explicit cross-references at each affected gate's own matrix entry, not a unilateral ruling (only `HDN-387`/`389` may accept the risk under the execution index's own §8.2) |
| Files edited | `scripts/db-tests/hris-attendance.sql` (the null-guard + session-derived policy lookup); `scripts/db-tests/hris-shift-roster-scheduling.sql` (comment-only factual correction: the old assertion was armed on 1 calendar date, not 2); `docs/build-log/full-system-hardening/HDN-370.md`, `00_EXECUTION_INDEX.md`, `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md`; `docs/runtime/CARGOGRID_BUILD_STATUS.md`, `TASK_LEDGER.md`, `HANDOFF.md`, `KNOWN_ISSUES.md`, `CHANGE_MANIFEST.md` |
| Migration | **None.** |
| Risk | Low for the changes themselves — one is a defensive precondition guard proven to fire and not to have been reachable in this checkpoint's own real test run; the other is a query-source change re-verified by a full week-long sweep against the actual resolved policy. The risk this Tier C pass *manages* rather than creates: the null-fail-open bug, had it shipped unfixed, would have made `ISS-2026-154`'s own regression guard silently inert under a plausible future fixture-maintenance action (renaming or unpublishing the tenant's attendance policy) |
| Gates | Independent full re-run after every fix, observed by the orchestrating session, never accepted on a lens's self-report: `typecheck` 0 errors; `lint` 0 errors/337 warnings (unchanged); `pnpm run test` **5394/5394**; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`** across 306 migrations, fresh disposable database, `hris-attendance.sql`/`hris-shift-roster-scheduling.sql` both confirmed present and green (`ALL HRT-278 ATTENDANCE ASSERTIONS PASSED`, `ALL HRT-279 db-test assertions passed`); `docs:check`, `security:check`, `security:audit`, `standards:check`, `data-classification:check`, `threat-model:check`, `git:check-paths` all clean |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's Tier C commit(s). Zero migrations, zero application code — every code change across the whole `HDN-370` checkpoint, kickoff through Tier C close, is confined to `scripts/db-tests/*.sql` test fixtures |
...
### CHG-2026-197 — Tier C close of `HDN-371`: sweep corrected 7→9 functions, superseded citations fixed, false dependency claim fixed, live-forced race obtained, `HDN-371` moves to `VERIFIED` (Step 15, Prompt 371 Tier C review)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-003` (Prompt 371, Cross-Module Transactional Integrity) — Tier C adversarial review and fix pass, per `AGENTS.md` "Never batch... only the Tier C close moves it to VERIFIED" |
| Change type | DOCS only — `HDN-371.md` substantially rewritten (§4, §5, §6, §7, §10-14); `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md`, `KNOWN_ISSUES.md`, `00_EXECUTION_INDEX.md`, and the five runtime ledgers all corrected. **Zero migration, zero application code, zero contract, zero route.** |
| Authorization | Continuation of the same session/range authorization as `CHG-2026-196`; `AGENTS.md`'s mandatory Tier C close, not a new task |
| Build process | Spawned 4 independent Tier C review agents in parallel (spec-compliance; security/tenant, live-tested with a rebuilt probe database; correctness, live-tested with an independent re-sweep and its own probe database; cross-prompt integration) against the committed `23e3490` state. Fixed every finding or explicitly registered it with a named owner. Independently re-verified the two most consequential findings before trusting them: re-derived the corrected constraint names and function citations directly via `grep`/`Read` against the exact migration files, and independently reproduced the live-forced race itself (an uncommitted conflicting insert forcing a second session's call to block and then raise on commit) on a fresh probe database, obtaining the identical error a Tier C lens had already reported |
| Findings and disposition | **3 High findings, all independently caught by multiple lenses:** (1) the original sweep's regex was blind to `create or replace function` redefinitions, so 5 of 7 registered functions were cited at superseded migrations — the conclusion (each genuinely lacks the handler) survived at the live/effective bodies, but citations, a migration-file count, and 2 of 6 named backing unique constraints (dropped by a later hardening migration and replaced by differently-named partial unique indexes) were wrong; (2) the original sweep's own function count (30/19/12/7) missed 2 members of its own defect class — `prepare_wms_inbound_from_shipment` (named in narrative as "folded in" but never actually added to any registered record) and `link_auth_identity` (missed entirely) — corrected to 32/20/11/9; (3) `BLOCKER_LEDGER.md`'s `HDN-BLK-010` record falsely stated `HDN-374`'s hard upstream dependency (`HDN-371` `VERIFIED`) was "already satisfied" while `HDN-371` was still `COMPLETED` — the one finding with a direct never-batch-safety consequence, since it could have licensed a future session to start `HDN-374` early. **7 Medium findings**, each fixed: a missing Step 16 eligibility statement (the exact omission `HDN-370`'s own Tier C caught on itself); `HARDENING_MATRIX.md`'s gate index left at `NOT_RUN` with no status token assigned in §2 (now `PARTIAL`, honestly — the loyalty/portal gap keeps it short of a full pass); `HARDENING_MATRIX.md` §5 scoping the fix to only 6 of 9 functions with no seed for the other 3; an overclaimed "customer portal/loyalty/tickets: Reconciled" row with zero loyalty or portal evidence anywhere in the document (corrected to disclose tickets-only reconciliation); Prompt 371 §28 (concurrency tests) never addressed — resolved by demonstrating a working, simpler, reusable technique live (an uncommitted-insert-blocking race, proven on the first attempt) rather than committing a permanent test file; a stale `READY` claim for `HDN-371` inside two of `HANDOFF.md`'s own superseded blockquotes, now correctly demoted. **9 Low findings**, each fixed or registered: a new, separate, pre-existing production defect, `ISS-2026-163` (`app.prepare_job_order`'s exception handler has no `if found`/`raise;` guard and can silently return an all-NULL row on an unrelated violation — registered, not fixed, owner `HDN-374`); the payroll function's idempotency short-circuit found not tenant-scoped (not currently exploitable, folded into `HDN-374`'s required fix scope); a "12th sibling" ordinal misphrasing; a dangling section cross-reference; an off-by-5-line citation; a "118/58" test-call-site figure that did not reproduce (corrected to ~131/~60); two wrong `AGENTS.md` attributions; a missing dependency-graph state annotation; this checkpoint's own commit SHA not recorded anywhere (addressed here) |
| Files edited | `docs/build-log/full-system-hardening/HDN-371.md`, `00_EXECUTION_INDEX.md`, `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md`; `docs/runtime/KNOWN_ISSUES.md`, `TASK_LEDGER.md`, `HANDOFF.md`, `CHANGE_MANIFEST.md`, `CARGOGRID_BUILD_STATUS.md` |
| Migration | **None.** |
| Risk | Low for the changes themselves (documentation only). The risk this Tier C pass *manages* rather than creates: had the superseded-citation error and the false "already satisfied" claim shipped unfixed, `HDN-374` could have worked from stale code (silently reverting prior hardening migrations `ATW-031`/`ATW-032`) or a future session could have started `HDN-374` before `HDN-371` genuinely reached `VERIFIED` |
| Gates | Independent full re-run after every fix, observed by the orchestrating session, never accepted on a lens's self-report: `typecheck` 0 errors; `lint` 0 errors/337 warnings (unchanged); `pnpm run test` **5394/5394**; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`** across 306 migrations; `docs:check`, `security:check`, `security:audit`, `standards:check`, `data-classification:check`, `threat-model:check`, `git:check-paths` all clean. Plus a live-forced race, independently reproduced by the orchestrating session on its own fresh probe database |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's Tier C commit(s). Zero migrations, zero application code — every change across the whole `HDN-371` checkpoint, initial submission through Tier C close, is documentation and ledger evidence, plus disposable probe-database use for live verification, all dropped afterward |
| Status | **`VERIFIED`.** Tier C review closed clean: every finding fixed or explicitly registered with a named owner, independent full gate re-run green, the checkpoint's own headline finding independently live-forced and confirmed. **Next eligible: `CG-S15-HDN-004` (Prompt 372, Tenant Isolation Audit)** |
| Date | 2026-08-23 |

### CHG-2026-198 — Tenant Isolation Audit: a cross-tenant read defect class live-forced and fixed at the root, twice; two same-shape findings deferred (Step 15, Prompt 372)

*Amended at this checkpoint's own Tier C review (2026-08-23), same checkpoint. Original text
found and fixed 9 functions (21 protected claimed, actually 19). The Tier C security/tenant
lens found 4 more of the identical shape and fixed them in the same checkpoint; other Tier C
corrections below. Corrected rather than left standing.*

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-004` (Prompt 372, Tenant Isolation Audit), package document `CG-AABPP-HDN-372` |
| Change type | MIGRATION — two additive migrations, 13 functions hardened; TEST — 2 pre-existing db-test fixture regressions fixed (one fix corrected again at Tier C), 1 new named-list regression check strengthened and widened, 1 new live spoof regression test added; DOCS — build log, index, matrix, blocker ledger, 16 issue-ledger entries, runtime ledgers |
| Authorization | Continuation of the standing operator range authorization for Step 15, executed one prompt only per `AGENTS.md`'s never-batch list, immediately following `HDN-371`'s `VERIFIED` close in the same session |
| Build process | Four independent parallel adversarial investigations, each with its own disposable Postgres database built from all 306 migrations and its own live two-tenant fixture, followed by this checkpoint's own four-lens Tier C review (one of which — security/tenant — ran a further independently live-tested closure sweep). The orchestrating session additionally queried the real live Supabase project directly (read-only) to settle a reachability question rather than assume it, and independently re-verified the vulnerability and both fixes live at each stage |
| Findings and disposition | **`HDN-BLK-011`/`ISS-2026-164` (High) — FIXED same checkpoint, in two rounds.** 13 `SECURITY DEFINER` functions granted to `authenticated` evaluated authority against a client-supplied actor UUID instead of the verified session identity (the `ATW-032` sweep's own "STABLE reads are exempt" premise is false for a function that bypasses RLS). 9 found first, live-forced against `app.get_self_employee` (full PII) and the 10-function `app.resolve_customer_owner_account_scope`/`ATW-023` family (customer inventory data), plus the audit trail and notifications; 4 more found by this checkpoint's own Tier C review, live-forced, including 2 direct siblings of already-fixed functions. 24 functions protected total once transitive propagation is counted (corrected from an original miscounted 12/21). Fixed via `create or replace function` adding `assert_actor_is_session_identity`, 3 of 13 converted from `language sql` to `plpgsql` to carry it. **`HDN-BLK-012`/`ISS-2026-165` (High) — deferred to `HDN-373`.** 13 dashboard functions share the identical shape with no common root to fix once. **`HDN-BLK-014`/`ISS-2026-179` (Medium) — deferred to `HDN-373`**, opened at Tier C. ~24 further candidate functions from the wider sweep, only 3 live-verified. **`HDN-BLK-013`/`ISS-2026-166` (High) — opened at Tier C**, correcting a finding that existed in `KNOWN_ISSUES.md` since this checkpoint's first commit without a `BLOCKER_LEDGER` entry. `HDN-BLK-012`, `013`, `014` are all Step 16 release blockers per §8.1 until fixed or accepted. **13 further Medium/Low findings** registered across `HDN-373`/`376`/`377`/`378`/`379`/`382` — none silently fixed or dropped |
| Files edited | `supabase/migrations/20260810000000_harden_tenant_isolation_actor_identity_gaps.sql`, `20260810100000_..._round2.sql` (new); `scripts/db-tests/rbac-enforcement.sql` (two pre-existing test regressions fixed; the ATW-032 sweep regex broadened; the new named-list check made position-aware and widened to 13; a genuine live spoof test added), `scripts/db-tests/hris-employee-master.sql` (fix corrected at Tier C to the repo's own `'{}'` idiom); `docs/build-log/full-system-hardening/HDN-372.md` (new), `00_EXECUTION_INDEX.md`, `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md`; `docs/runtime/KNOWN_ISSUES.md` (16 entries), `TASK_LEDGER.md`, `HANDOFF.md`, `CHANGE_MANIFEST.md`, `CARGOGRID_BUILD_STATUS.md`; `docs/build-log/full-system-hardening/HDN-371.md` (one-line stale status header fix, unrelated pre-existing typo) |
| Migration | **Two, additive only** — `create or replace function` on 13 existing functions total, no schema/table change, no applied migration edited |
| Risk | Low for the fix itself — additive, mechanical, precedented (identical pattern already proven for 449 other functions in this codebase), live-verified not to weaken any legitimate call path (own-identity control calls still succeed). The risk this checkpoint *closes*: a real, live-forced cross-tenant PII/inventory/audit-trail/custom-field/approval-step read, currently gated only by a pre-deployment Data-API exposure setting rather than any durable control |
| Gates | 4 full db-test runs during the fix pass (the first two caught genuine pre-existing regressions, fixed in the same pass) plus one further independent run at Tier C close on 308 migrations: `typecheck` 0 errors; `lint` 0 errors/337 warnings; `pnpm run test` **5394/5394**; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`**, both new regression checks firing and printing their own success text; all 7 governance gates clean including `git:check-paths` |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's commit(s). Both migrations are additive only (`create or replace function`, no schema change) — reverting restores the pre-fix function bodies exactly |
| Status | **`VERIFIED`** — Tier C review closed clean; independent full gate re-run green. `HDN-373` is the next eligible prompt |
| Date | 2026-08-23 |

### CHG-2026-199 — RLS and RBAC Audit: root RBAC gate never checked tenant membership, entire Finance write surface unreachable since it shipped — both fixed at the root (Step 15, Prompt 373)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-005` (Prompt 373, RLS and RBAC Audit), package document `CG-AABPP-HDN-373` |
| Change type | MIGRATION — eight additive migrations: 13 dashboard functions hardened, the root RBAC evaluator patched, 16 CRM/ops functions hardened, an own-row RLS membership gap closed on 3 tables, a loyalty maker/checker guard added, 95 Finance functions converted `SECURITY INVOKER` → `SECURITY DEFINER`, a Finance journal RLS gate and self-approval guard added, and (Tier C's own completeness fix) 57 more functions converted plus a regression correction plus a new cross-tenant disclosure fix; TEST — 4 pre-existing Finance db-test fixtures corrected to use a second distinct approver, 2 new regression blocks added to `finance-journal.sql`, `rbac-enforcement.sql`'s exemption list updated; DOCS — build log, index, matrix, blocker ledger, 14 issue-ledger entries plus 9 updated, runtime ledgers |
| Authorization | Continuation of the standing operator range authorization for Step 15, executed one prompt only per `AGENTS.md`'s never-batch list, immediately following `HDN-372`'s `VERIFIED` close in the same session |
| Build process | Four parallel investigation lenses (carry-forward candidate verification; access-matrix/`SECURITY DEFINER` posture; maker/checker; support-access/revocation-propagation), each required to independently live-force its own findings on disposable Postgres databases rather than accept the sweep or a candidate list as proof. Fix work followed, in the order the findings' own dependencies required — the self-approval guard deliberately landed together with the Finance reachability fix, not before it, per an explicit warning raised mid-investigation |
| Findings and disposition | **`ISS-2026-180` (High) — FIXED.** `app.evaluate_permission` (the root RBAC gate, ~1,124 callers) never checked tenant membership; a revoked ex-member retained every role-based permission indefinitely, live-confirmed end to end. **`HDN-BLK-015`/`ISS-2026-182` (High) — FIXED, the largest reachability defect found in Step 15 to date.** 95 Finance functions (76 entry points plus 19 authority helpers) plus `app.enqueue_job` were `SECURITY INVOKER`, completely unreachable by any real `authenticated` session since they shipped; live-forced against a genuine Finance Manager session. Fixed together with two companion findings its own fix-and-verify cycle surfaced: `ISS-2026-183` (unchecked system-journal posting) and `ISS-2026-184` (`finance_journals`/`finance_journal_lines` RLS bypassed `FIN:View` entirely, fixed alongside `ISS-2026-181`, Finance journal self-approval sharing `ISS-2026-139`'s shape). **`HDN-BLK-012`/`ISS-2026-165` (High) — FIXED**, carried forward from `HDN-372`: 13 dashboard functions sharing `HDN-BLK-011`'s actor-forgery shape; 5 of the 13 additionally closed a field-masking bypass. **`HDN-BLK-014`/`ISS-2026-179` (Medium) — narrowed, not closed**, carried forward from `HDN-372`: of ~30 candidates, 16 confirmed terminal/self-referential and fixed; ~14 confirmed genuinely called with third-party actor arguments elsewhere in the schema, re-registered narrower as `ISS-2026-186`, owner a future checkpoint. **`ISS-2026-171`/`173` (Medium/Low) — FIXED**, carried forward from `HDN-372`: own-row RLS gaps on `notifications`/`saved_report_views` post-revocation, plus one further instance found and fixed in the same migration (`ISS-2026-185`, `notification_preferences`). **`ISS-2026-139` (High) — FIXED**, seeded from `HARDENING_MATRIX.md` §4: loyalty redemption maker/checker collapse, live-forced exactly as described. **`ISS-2026-137` — independently re-verified**, existing disposition (`OPEN`, Low) confirmed accurate, no change. **Six further findings registered, not fixed, each with a named forward owner**: `ISS-2026-186`, `187`/`188` (owner `HDN-378`), `189` (judgment call), `190` (sibling of `184`'s shape, HRIS domain), `191` (doc-drift), `192` (`principal_memberships` variant) |
| Files edited | `supabase/migrations/20260810200000_harden_dashboard_actor_identity_gaps.sql`, `20260810300000_harden_rbac_evaluator_tenant_membership_check.sql`, `20260810400000_harden_crm_ops_actor_identity_gaps.sql`, `20260810500000_harden_own_row_rls_membership_gap.sql`, `20260810600000_harden_loyalty_redemption_maker_checker.sql`, `20260810700000_harden_finance_authority_chain_security_definer.sql`, `20260810800000_harden_finance_journal_view_gate_and_self_approval.sql`, `20260810900000_harden_finance_authority_chain_tierc_completeness.sql` (all new — the last landed to correct 3 genuine gaps this checkpoint's own Tier C review found: 57 more functions sharing the `SECURITY INVOKER` shape, a regression in the `ISS-2026-183` fix, and a new cross-tenant disclosure, `ISS-2026-193`); `scripts/db-tests/finance-journal.sql` (2 new regression blocks — self-approval deny-then-allow, live `FIN:View` RLS proof), `scripts/db-tests/finance-lifecycle-state-control.sql`, `finance-period-lock.sql`, `finance-posted-journal-integrity.sql`, `finance-reversal-adjustment.sql` (fixtures updated to use two distinct actors per the new self-approval guard), `rbac-enforcement.sql` (`v_expected` exemption list updated for 2 Tier C-surfaced functions confirmed correct by design); `docs/build-log/full-system-hardening/HDN-373.md` (new, includes §6.7/§13 Tier C synthesis), `00_EXECUTION_INDEX.md`, `HARDENING_MATRIX.md`, `BLOCKER_LEDGER.md` (`HDN-BLK-012`/`014` given dated amendments per this repo's own append-only convention, `HDN-BLK-015` expanded); `docs/runtime/KNOWN_ISSUES.md` (14 new entries including `ISS-2026-193`; `ISS-2026-165`/`171`/`173`/`179`/`180`/`184`/`186`/`190`/`192` updated), `TASK_LEDGER.md`, `HANDOFF.md`, `CHANGE_MANIFEST.md`, `CARGOGRID_BUILD_STATUS.md` |
| Migration | **Seven, additive only** — `create or replace function` throughout (dashboard/CRM-ops actor-identity asserts, the evaluator's membership check, the loyalty guard, the 95-function `SECURITY DEFINER` conversion, the Finance journal RLS gate), plus one additive `submitted_by_auth_user_id` column and the own-row RLS policy rewrites on `notifications`/`saved_report_views`/`notification_preferences`. No applied migration edited |
| Risk | Low for the fixes themselves — additive, each proven live before being applied at scale (the Finance `SECURITY DEFINER` conversion was proven on one function end to end before the 95-function batch, generated mechanically from live `pg_get_functiondef` output and diffed byte-identical elsewhere). The risk this checkpoint *closes* is substantial: a revoked user retaining every permission indefinitely, and the entire Finance write surface having no functioning RBAC gate at all since it shipped |
| Gates | Run fresh from the repository root: `typecheck` 0 errors; `lint` 0 errors/337 warnings; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`**, full suite against a fresh disposable database with real grants (no superuser-only bypass), every migration in this checkpoint applied via the normal migration sequence |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's commit(s). All seven migrations are additive; reverting restores the pre-fix function/policy bodies and drops the one new column |
| Status | **`VERIFIED`** — Tier C review closed clean across two rounds: the first round (four independent lenses) found the original 95-function diagnosis incomplete and this checkpoint fixed the gap (57 more functions, a regression correction, a new High disclosure fix — `ISS-2026-193`) in the same session at `20260810900000_harden_finance_authority_chain_tierc_completeness.sql`; a fifth independent lens then confirmed that correction complete (zero further matches, schema-wide) and correct (all 3 fixes re-verified with fresh fixtures), with an independent full gate re-run green. `HDN-374` is now the next eligible prompt |
| Date | 2026-08-23 |

### CHG-2026-200 — Financial Integrity Audit: quote-level tax doubled at invoicing, a job order could reach `issued` on two full-amount invoices, 10 Finance functions closed a known concurrency-race gap, loyalty sweep's `p_as_of` was silently ignored (Step 15, Prompt 374)

| Field | Value |
|---|---|
| Task/prompt | `CG-S15-HDN-006` (Prompt 374, Financial Integrity Audit), package document `CG-AABPP-HDN-374` |
| Change type | MIGRATION — one additive migration fixing 4 findings across 12 functions; 5 existing `scripts/db-tests` files extended with new regression coverage |
| Authorization | Continuation of the standing operator range authorization for Step 15, executed one prompt only per `AGENTS.md`'s never-batch list, immediately following `HDN-373`'s `VERIFIED` close |
| Build process | Four parallel investigation lenses (revenue chain; cost/AP chain; period lock/reversal/concurrency/rounding; loyalty liability/payroll/tax/statutory), each required to live-force its own findings on disposable databases rather than accept a code read as proof. Cost/AP chain, payment/journal reconciliation, period lock, reversal, correction, rounding, tax snapshotting, payroll-handoff aggregation, and RPD-016's statutory gates all held clean |
| Findings and disposition | **`ISS-2026-194` (High) — FIXED.** `app.prepare_finance_invoice_from_readiness` read `revenue_snapshot.totalAmount` (already tax-inclusive) as the invoice's own pre-tax subtotal, doubling any quote-level `tax_pct` when the caller also passed `p_tax_code`; live-forced ~11% overcharge. Fixed by reading `subtotalAmount` instead. **`ISS-2026-195` (High) — FIXED, self-corrected before commit.** A job order could reach `issued` on two full-amount invoices from two distinct handoffs (live-forced: 6,000,000 billed for 3,000,000 of work). The first fix draft gated invoice preparation itself, which would have broken `OPS-181`'s own disclosed legitimate-re-handoff allowance and an existing db-test fixture — caught before commit by running the full 229-file suite against the draft fix, not after. The shipped fix gates `app.issue_finance_invoice` (the actual AR/GL posting boundary) instead. **`ISS-2026-162` (Medium) — Finance/HRIS-Payroll scope RESOLVED.** 10 functions total (6 originally named by `HDN-371`/`373` plus 4 more this checkpoint's own wider sweep found: `app.post_finance_subledger_batch`, `app.create_and_post_finance_system_journal`, `app.import_finance_bank_statement`, `app.stage_finance_exchange_rate_import`) fixed with the codebase's own proven "design note 9(a)" exception-handler pattern; 2 mechanisms live-forced with a genuine two-process race each, both now resolve gracefully. The 3 non-Finance functions that finding also named, plus `ISS-2026-163`, remain open — explicitly handed to `HDN-387`, not silently dropped or force-fit into this lane's own charter. **`ISS-2026-196` (Medium) — FIXED.** `app.run_loyalty_expiry_sweep`'s own `p_as_of` parameter was silently ignored by both primitives it composes; now threaded through via a new optional trailing parameter on each. **`ISS-2026-197` (Low) — REGISTERED, not fixed**, owner `HDN-386`: no FX/multi-currency conversion exists anywhere in the revenue chain (structural absence, RPD-016 already defers it); `app.calculate_job_profitability` always reports the static quote-time total rather than the actual invoiced figure |
| Files edited | `supabase/migrations/20260811000000_harden_financial_integrity_invoicing_and_idempotency.sql`; `scripts/db-tests/finance-invoice.sql`, `finance-reversal-adjustment.sql`, `finance-subledger.sql`, `customer-loyalty-expiry-fraud-prevention.sql`, `customer-loyalty-points-ledger.sql` (schema-privilege signature check only) |
| Migration | **One, additive only** — `create or replace function` throughout for 10 of 12 touched functions; `drop function if exists` + `create function` for the 2 loyalty-expiry primitives, since adding a new parameter changes their argument list (their own default value keeps every existing 3-arg caller working unchanged) |
| Risk | Low for the fixes themselves — each proven live on a fresh disposable database before being committed, including the self-corrected Finding 2 draft, which was caught and fixed BEFORE ever reaching a committed state, not after |
| Gates | Run fresh from the repository root: `typecheck` 0 errors; `lint` 0 errors/337 warnings; `pnpm run test` 5394/5394; `bash scripts/db-tests/run.sh` **229/229 `ALL PASSED`**, full suite against a fresh disposable database with real grants, every migration applied via the normal migration sequence |
| Commits | see branch `claude/step-15-hdn-369-kickoff-w6qren` |
| Rollback | `git revert` this checkpoint's commit(s). The one migration is additive; reverting restores the pre-fix function bodies (and, for the 2 loyalty-expiry functions, their original 3-arg signatures) |
| Status | **`COMPLETED`, not `VERIFIED`** — Tier C review (four independent adversarial lenses) has not yet run; nothing after it may begin until it closes and this row moves to `VERIFIED`, per `AGENTS.md`'s never-batch discipline |
| Date | 2026-08-23 |
