...
| Field | Value |
|---|---|
| Task ID/name | `CG-S5-PH0-017` — Initial Threat Model |
| Prompt | `05-phase-00-discovery-foundation/96_INITIAL_THREAT_MODEL_PROMPT.md` |
| Status | `READY` — upstream `PH0-083..095` all `VERIFIED`; `ERR-2026-003` `RECOVERED`; `ISS-2026-002` `RESOLVED` |
| Reason | Next sequential Phase 0 capability prompt per `docs/build-log/phase-00/00_PHASE0_EXECUTION_INDEX.md` row `017`. Read `96_INITIAL_THREAT_MODEL_PROMPT.md` in full before resolving its exact allowed-file scope. |
| Upstream | `CG-S5-PH0-016` (`VERIFIED` — `docs/build-log/phase-00/PH0-95.md`; `docs/standards/DATA_CLASSIFICATION_STANDARDS.md` + `scripts/data-classification/registry.ts` establish the data-classification foundation) |
...
- **Phase 0 Prompt 95** (`CG-S5-PH0-016`, this checkpoint): Data Classification Foundation — `docs/standards/DATA_CLASSIFICATION_STANDARDS.md` (two-axis sensitivity-level/category taxonomy, constructed and disclosed from `02_CANONICAL_DATA_FLOW_MAP.md` §10's prose defaults; 8-dimension handling matrix; RPD-025 retention mapping), `scripts/data-classification/registry.ts`/`check-registry.ts` (real, CI-enforced adoption gate confirming this repository's one real secret-classified env var is registered). Full detail: `docs/build-log/phase-00/PH0-95.md`.
...
3. `ERR-2026-003` is `RECOVERED` (§1 decision recorded, `ERROR_LEDGER.md` §3 recovery + verification). `docs/architecture/14..16_*.md` are single coherent Lineage A documents; the 607-item baseline is authoritative. Prompts 83–95 (`CG-S5-PH0-004..016`) are all `VERIFIED` — see `docs/build-log/phase-00/PH0-83.md`–`PH0-95.md`.
4. Resume Phase 0 at `CG-S5-PH0-017` (Prompt 96, `96_INITIAL_THREAT_MODEL_PROMPT.md`) and continue looping through subsequent Phase 0 capability prompts (97–102) in strict sequential order as usage/context allow — completing one prompt is not a stop condition. Write each Phase 0 build log to the singular path `docs/build-log/phase-00/PH0-NN.md`.

**First safe action for anyone picking this up: confirm the worktree is clean, read `docs/build-log/phase-00/00_PHASE0_EXECUTION_INDEX.md` (row `017`) for the Phase 0 dependency graph, then execute `CG-S5-PH0-017` (Prompt 96). Prompts 83–95 and the `ERR-2026-003` blocker are resolved.**
