...
| Field | Value |
|---|---|
| Task ID/name | `CG-S5-PH0-021` — Phase 0 Hardening |
| Prompt | `05-phase-00-discovery-foundation/100_PHASE0_HARDENING_PROMPT.md` |
| Status | `READY` — `CG-S5-PH0-020` (Prompt 99, Phase 0 Integrated Verification) `VERIFIED` this checkpoint |
| Reason | Second of Phase 0's four closing/verification prompts (`99`–`102`). Per `docs/build-log/phase-00/PH0-99.md` §13, this task's remediation scope is named by that checkpoint's own failure matrix (§5): candidates are `ISS-2026-008` (cross-module secret-pattern coverage gap — needs an explicit scope decision, widen vs. keep intentionally narrower) and potentially `ISS-2026-005` (the `CHANGE_MANIFEST.md` historical-entry backfill, now also carrying a disclosed ID-collision detail). Read `100_PHASE0_HARDENING_PROMPT.md` in full before resolving its exact scope — do not assume either candidate is automatically in scope until the prompt's own preconditions are checked. |
| Upstream | `CG-S5-PH0-020` (`VERIFIED` — `docs/build-log/phase-00/PH0-99.md`) and all 18 prior capability tasks |
...
3. `ERR-2026-003` is `RECOVERED` (§1 decision recorded, `ERROR_LEDGER.md` §3 recovery + verification). `docs/architecture/14..16_*.md` are single coherent Lineage A documents; the 607-item baseline is authoritative. Prompts 83–99 (`CG-S5-PH0-004..020`) are all `VERIFIED` — see `docs/build-log/phase-00/PH0-83.md`–`PH0-99.md`. **All 18 Phase 0 capability tasks plus the integrated-verification prompt are complete.**
4. Resume Phase 0 at `CG-S5-PH0-021` (Prompt 100, `100_PHASE0_HARDENING_PROMPT.md`) — the second of 4 closing prompts (99–102); per `PH0-99.md` §13/§5 it consumes that checkpoint's own failure matrix as named remediation scope — and continue through 101–102 in strict sequential order as usage/context allow. Write each Phase 0 build log to the singular path `docs/build-log/phase-00/PH0-NN.md`.

**First safe action for anyone picking this up: confirm the worktree is clean, read `docs/build-log/phase-00/00_PHASE0_EXECUTION_INDEX.md` (row `021`) for the Phase 0 dependency graph, then read `100_PHASE0_HARDENING_PROMPT.md` in full before executing `CG-S5-PH0-021` (Prompt 100). Prompts 83–99 and the `ERR-2026-003` blocker are resolved.**
