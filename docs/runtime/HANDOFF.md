...
| Dirty worktree | This checkpoint's changes only (`supabase/migrations/20260716075355_create_tenants.sql`, `scripts/db-tests/**`, `server/contracts/tenant/**`, `server/mutations/tenant.ts`(+test), `package.json`, `.github/workflows/ci.yml` — see `docs/build-log/phase-01/PLT-105.md`) |
...
| Field | Value |
|---|---|
| Task ID/name | `CG-S6-PLT-003` — Subscription/Module/Feature Entitlement |
| Prompt | `06-phase-01-platform-core/106_SUBSCRIPTION_MODULE_FEATURE_ENTITLEMENT_PROMPT.md` |
| Status | `READY` — `CG-S6-PLT-002` (Prompt 105, Tenant Provisioning and Lifecycle) `VERIFIED` this checkpoint. |
| Reason | Second of 32 Platform Core capability prompts. `docs/build-log/phase-01/00_PLATFORM_CORE_EXECUTION_INDEX.md` row `003` confirms `PLT-105` is `VERIFIED`. This capability adds versioned entitlement/package/tenant-assignment/limit records on top of the real `app.tenants` table `PLT-105` just created. |
| Upstream | `CG-S6-PLT-002` (`VERIFIED` — `docs/build-log/phase-01/PLT-105.md`) and all 24 prior tasks |
...
3. `ERR-2026-003` is `RECOVERED` (§1 decision recorded, `ERROR_LEDGER.md` §3 recovery + verification). `docs/architecture/14..16_*.md` are single coherent Lineage A documents; the 607-item baseline is authoritative. Prompts 83–105 (`CG-S5-PH0-004..023`, `CG-S6-PLT-001..002`) are all `VERIFIED` — see `docs/build-log/phase-00/PH0-83.md`–`PHASE0_CLOSURE_REPORT.md` plus `docs/build-log/phase-01/00_PLATFORM_CORE_EXECUTION_INDEX.md`/`00_PLATFORM_CORE_WBS.md`/`PLT-105.md`. **All of Phase 0 is complete; Phase 1's kickoff and first capability are both `VERIFIED`.**
4. Resume at `CG-S6-PLT-003` (Prompt 106, `06-phase-01-platform-core/106_SUBSCRIPTION_MODULE_FEATURE_ENTITLEMENT_PROMPT.md`) — the second of 32 Platform Core capability prompts, building on the real `app.tenants` table `PLT-105` created. Read `docs/build-log/phase-01/00_PLATFORM_CORE_WBS.md` §3 (row 2) and `PLT-105.md` first — the entitlement schema will reference `app.tenants` by FK.

**First safe action for anyone picking this up: confirm the worktree is clean, run `pnpm run db:test` to confirm the current migration state is real and green (requires a reachable Postgres — see `scripts/db-tests/run.sh`'s own header comment for the connection-string convention), then read `106_SUBSCRIPTION_MODULE_FEATURE_ENTITLEMENT_PROMPT.md` in full before executing `CG-S6-PLT-003` (Prompt 106). All of Phase 0, Phase 1's kickoff and first capability, and the `ERR-2026-003` blocker are resolved.**
