...
**Updated:** 2026-08-03 (`ATW-228` — Advanced Milestone and Exception with Multi-Source Telemetry — `VERIFIED`. Second task within the explicit range authorization "lanjut prompt 227-230", executed in ascending order. Extends rather than re-implements `ATW-226G`'s own already-staged milestone-candidate/exception-signal design: real telemetry-derived confidence/freshness provenance now populates `app.milestone_events`/`app.operational_exceptions` (additive columns) at confirmation time; a new `tracking_health_no_signal` signal type reuses `226G`'s own staged-review table via a new scan-based detector; a real, pre-existing defect in `ATW-225`'s own `app.evaluate_leg_no_signal_escalation` is fixed (it measured session age rather than actual telemetry recency); a new dependency-aware live ETA projection (`app.get_shipment_leg_eta_projection`) and an explicit `app.rebaseline_shipment_leg_schedule` for unstarted legs; the public tracking projection widened a fourth time with `live_eta_status`/`live_eta_at`. Seven real defects found and fixed during authoring (full detail `docs/build-log/phase-05/ATW-228.md` §4.1, including a `CREATE OR REPLACE`-vs-`DROP`+`CREATE` overload-ambiguity mistake and a literal `'success'`/`'ok'` copy-paste bug caught by a pre-existing db-test re-failing). `docs/build-log/phase-05/ADVANCED_TMS_WMS_EXECUTION_INDEX.md` corrected: row `228` moves `READY` -> `VERIFIED`; `CG-S10-ATW-010` (229) is the next task. Root `node:test` 2440/2440 (15 net new), `db:test` PASS across 111 migrations/113 db-test files (1 new), `next build` PASS (88 routes, unchanged). See `docs/build-log/phase-05/ATW-228.md` and `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` §1.3 for full detail. This session's own explicit range authorization continues; proceeding directly to `229` next. Supersedes the `ATW-227` checkpoint note below, retained in history.)
...
| Current phase/workstream | **Phase 1-4 are CLOSED** (37/37, 24/24, 22/22, 29/29 `VERIFIED` respectively). **Phase 5 -- Advanced TMS/WMS is `PHASE_5_IN_PROGRESS`**: `CG-S10-ATW-001..006` all `VERIFIED`; row `226`'s own nine-child decomposition is fully `VERIFIED` (row `226`/`CG-S10-ATW-007` itself `VERIFIED`); **`CG-S10-ATW-008`/`009` (Prompts 227/228) are now also `VERIFIED`**. See `docs/build-log/phase-05/00_ADVANCED_TMS_WMS_WBS.md`/`ADVANCED_TMS_WMS_EXECUTION_INDEX.md` for the full 37-task hierarchy. `CG-S10-ATW-010` (Prompt 229) is dependency-clean, marked `READY`, next within this session's own explicit range authorization ("lanjut prompt 227-230"). |
| Active task | `ATW-228` (Advanced Milestone and Exception with Multi-Source Telemetry) -- `VERIFIED` this checkpoint. |
| Active task status | Implemented and verified. `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` row `228` corrected `READY` -> `VERIFIED`. Full detail: `docs/build-log/phase-05/ATW-228.md`, `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` §1.3. |
| Branch | `claude/prompt-227-230-itvres`, tracked to `origin/claude/prompt-227-230-itvres` |
| HEAD | this checkpoint's commit |
| Last known good commit (both lineages agree, pre-divergence) | `origin/main`@`27389a4` (PR #8, Prompt 45) — historical; current last-known-good is `ATW-228`'s own commit |
| Schema/migration head | 111 migrations applied (1 new this checkpoint: `20260730130000_create_advanced_tms_milestone_exception_telemetry.sql`). |
...
| Last full green gate | `ATW-228` -- `typecheck`/`lint` (0 errors, 85 warnings, unchanged) + root `node:test` 2440/2440 (15 net new) + `db:test` PASS across 111 migrations/113 db-test files (1 new) + a real `next build` (88 routes, unchanged) -- all re-run fresh this checkpoint. |
...
| 5 | Advanced TMS/WMS | `PHASE_5_IN_PROGRESS` (`docs/build-log/phase-05/ADVANCED_TMS_WMS_EXECUTION_INDEX.md`) -- kickoff through Prompt 225 all `VERIFIED`; row `226` (all nine `226A`-`226I` children) `VERIFIED`; Prompt 227 Capacity/Utilization/Tracking Coverage and Prompt 228 Advanced Milestone/Exception with Multi-Source Telemetry both `VERIFIED` | 46% (17/37 WBS rows) | `CG-S10-ATW-010` (Prompt 229, Warehouse and Zone) -- dependency-`READY` (independent Warehouse lane), next within this session's own explicit range authorization ("lanjut prompt 227-230") |
