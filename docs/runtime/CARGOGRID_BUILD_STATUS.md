...
**Updated:** 2026-07-24 (`CG-S7-COM-016`, Prompt 157, Credit and Commercial Control `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial `PHASE_2_IN_PROGRESS`** (kickoff + Lead Management + Prospect Lifecycle + Contact and Activity Management + CRM Sales Plan and Pipeline + Opportunity Management + RFQ and Costing Request + Rate and Cost Lookup + Margin Calculation + Quotation Builder + Quotation Versioning + Quotation Approval + Customer Acceptance + Customer and Account Conversion + Contract and Customer Pricing + Credit and Commercial Control) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), Quotation Approval (`153`), Customer Acceptance (`154`), Customer and Account Conversion (`155`), Contract and Customer Pricing (`156`), and Credit and Commercial Control (`157`) all `VERIFIED`; 4 remaining capability prompts (`158`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-016` (Prompt 157, Credit and Commercial Control) `VERIFIED` this checkpoint. `CG-S7-COM-017` (Prompt 158, Commercial Dashboard) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `158` proceeds. |
| Active task status | `CG-S7-COM-016` (Prompt 157, Credit and Commercial Control) `VERIFIED` this checkpoint. Commercial credit profile and transaction eligibility controls, deferring AR/GL posting to Phase 4 (no exposure/balance figure is ever modeled or invented). New `app.credit_profiles`/`app.credit_profile_overrides`/`app.credit_check_snapshots`. Per Prompt 157's own literal instruction, governance routing reuses the Platform Approval Engine (`PLT-121`/`123`) unchanged, unconditionally (never conditionally skipped, unlike `COM-153`'s own threshold-based quotation routing) — the same generic, tenant-wide approval config object `COM-153` already uses, distinguished only by `entity_type='credit_profile'`. "MFA for privileged approvers" reuses `PLT-115`'s own reauth-freshness mechanism (no live MFA/IdP challenge provider exists anywhere in this repository). `app.check_customer_credit` is the deterministic pre-conversion check — lazily flips an expired profile, evaluates hold/not-active/currency-mismatch/limit against the effective limit (a valid override if any, else the approved limit), always persists an immutable snapshot, and masks its own returned row per-caller directly (the same technique `app.get_effective_customer_price`, `COM-156`, already established) so an ordinary viewer still sees the real allow/hold outcome without the raw figures. New `CreditPanel` on the Account Detail page plus a `commercial/credit-approvals/` inbox route. `node:test` 1292/1292, `db:test` PASS across 47 migrations/47 files, all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151..157 build logs §5). This repository defines no `build` script. |
...
| Schema/migration head | 47 migrations applied — 46 as of `COM-156` plus `20260724310000_create_commercial_credit_commercial_control.sql` (`COM-157`, adds `app.credit_profiles`/`app.credit_profile_overrides`/`app.credit_check_snapshots` + 6 functions + 3 masked views, edits zero prior migration file). `COM-158` (Commercial Dashboard) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-157`) — `node:test` 1292/1292, `db:test` PASS across 47 migrations/47 db-test files, all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-017` (Prompt 158) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-017` (Prompt 158, Commercial Dashboard), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `017` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `157` only). |
