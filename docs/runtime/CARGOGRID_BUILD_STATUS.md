...
**Updated:** 2026-07-23 (`CG-S7-COM-006`, Prompt 147, Opportunity Management `VERIFIED` — covered by `CG-S7-COM-001`'s own open-ended "lanjut")
...
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), and Opportunity Management (`147`) all `VERIFIED`; 14 remaining capability prompts (`148`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-007` (Prompt 148, RFQ and Costing Request) is the next eligible task — dependency-`READY` (`147`, `VERIFIED`). Still covered by this session's open-ended "lanjut" authorization — no further per-task authorization request needed, subject to every other standing stop-and-escalate condition in `AGENTS.md`. |
| Active task status | `CG-S7-COM-006` (Prompt 147, Opportunity Management) `VERIFIED` this checkpoint. `app.opportunities` references `app.prospects` only (Account entity doesn't exist until `COM-155`, disclosed); stage machine is domain-native; requirements are a bounded jsonb snapshot (no Operations service-catalogue master yet). **First Commercial consumer of the seeded, protected `COM:View selling price` permission** — new `app.has_view_selling_price` gate plus `app.opportunities_directory` field-masked view (value_amount/value_currency/probability nulled without the permission; base-table column grant structurally excludes those columns, proven via a real Postgres permission-denied `db:test` assertion). Extended `app.resolve_commercial_record_ref` (`COM-145`) and the contact_links/activities CHECK constraints to accept `'opportunity'`. **Two real defects found and fixed**: (1) an invoker-mode masked view cannot read a column-REVOKEd field at all, even inside a masking CASE — fixed by adopting PLT-114's proven security_invoker=false technique plus this view's own added row filter; (2) extending RelatedType surfaced a hardcoded lead/prospect route-segment ternary in `_shared/activity-actions.ts` that would have silently under-revalidated the Opportunity Detail page — fixed with an exhaustive switch. Full detail `docs/build-log/phase-02/COM-147.md` §3.3/§3.7/§8. New route `app/(tenant)/[tenantSlug]/commercial/opportunities/` (list + detail with stage history, costing readiness, requirements/value/stage/clone actions). `node:test` 1049/1049, `db:test` PASS across 37 migrations/37 files, `next build` PASS (17 routes), all other gates green. |
...
| Schema/migration head | 37 migrations applied — 32 Platform Core plus `20260723090000_create_commercial_lead_management.sql` (`COM-143`), `20260723120000_create_commercial_prospect_lifecycle.sql` (`COM-144`), `20260723150000_create_commercial_contact_activity_management.sql` (`COM-145`), `20260723180000_create_commercial_sales_pipeline.sql` (`COM-146`, also carries a corrective `CREATE OR REPLACE` of `app.can_access_record`, `PLT-114`), and `20260723210000_create_commercial_opportunity_management.sql` (`COM-147`, also extends `app.resolve_commercial_record_ref` and two `COM-145` CHECK constraints). `COM-149` (Rate and Cost Lookup) will be the next Commercial migration to extend `PLT-120`'s master-data registry. |
...
| Last full green gate | This checkpoint (`COM-147`) — `node:test` 1049/1049, `db:test` PASS across 37 migrations/37 db-test files, a real `next build` producing 17 routes (up from 15), all applicable gates green. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. The next runtime agent proceeds directly to `CG-S7-COM-007` (Prompt 148) under this session's own open-ended authorization — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-007` (Prompt 148, RFQ and Costing Request), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `007` — dependency-`READY`, authorized (open-ended "lanjut," this session). |
