...
**Updated:** 2026-07-24 (`CG-S7-COM-011`, Prompt 152, Quotation Versioning `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
...
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), and Quotation Versioning (`152`) all `VERIFIED`; 9 remaining capability prompts (`153`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-011` (Prompt 152, Quotation Versioning) `VERIFIED` this checkpoint. `CG-S7-COM-012` (Prompt 153, Quotation Approval) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `153` proceeds. |
| Active task status | `CG-S7-COM-011` (Prompt 152, Quotation Versioning) `VERIFIED` this checkpoint. Widens `COM-151`'s `app.quotations` with a real root/version identity (`root_quotation_id`, monotonic `version_number`, `is_current`, `superseded_by_id`, `revision_reason`), database-enforced via `unique(root_quotation_id, version_number)` and a partial-unique `is_current` index. `app.create_quotation_revision` serves both the "revise the latest" main flow and the "restore an older version as new latest" alternative flow identically (source may be current or historical), gated by `COM:Edit` + record access + a mandatory reason, using real row-level locking (`SELECT ... FOR UPDATE`) on the root's current version for concurrency safety. Six `COM-151` functions/one view widened via `CREATE OR REPLACE` (never editing `COM-151`'s own migration file) to additionally require `is_current=true` before any edit/submit — closing a real gap where a superseded-but-still-`draft`-status version was directly editable. Five disclosed scope boundaries (no "issued"/"accepted" status values invented ahead of `COM-153`/`154`; "supersede" is an internal side-effect, not a separate action; "compare" is computed in TypeScript, not a third SQL function; RPD-022 disclosed not newly implemented; legacy migration not applicable) — full detail `docs/build-log/phase-02/COM-152.md` §3.1. New `server/contracts/quotation/quotation-diff.ts` (pure, unit-tested version comparison). Quotation Detail page gained version history, revise/restore, and compare UI. `node:test` 1172/1172, `db:test` PASS across 42 migrations/42 files, `next build` PASS (23 routes, unchanged count), all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151/152 build logs §5). |
...
| Schema/migration head | 42 migrations applied — 41 as of `COM-151` plus `20260724240000_create_commercial_quotation_versioning.sql` (`COM-152`, widens `app.quotations` and six `COM-151` functions/one view via `CREATE OR REPLACE`, adds `app.create_quotation_revision`, edits zero prior migration file). `COM-153` (Quotation Approval) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-152`) — `node:test` 1172/1172, `db:test` PASS across 42 migrations/42 db-test files, a real `next build` producing 23 routes (unchanged), all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-012` (Prompt 153) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-012` (Prompt 153, Quotation Approval), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `012` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `152` only). |
...
| 2 | Commercial | **`IN_PROGRESS`** (`PHASE_2_IN_PROGRESS` set at `CG-S7-COM-001`, Prompt 142, `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md`) — 11/24 tasks `VERIFIED` (kickoff, Lead Management, Prospect Lifecycle, Contact and Activity Management, CRM Sales Plan and Pipeline, Opportunity Management, RFQ and Costing Request, Rate and Cost Lookup, Margin Calculation, Quotation Builder, Quotation Versioning) | 46% (11/24 tasks) | `CG-S7-COM-012` — Quotation Approval (Prompt 153), pending fresh authorization |
