...
**Updated:** 2026-07-24 (`CG-S7-COM-008`, Prompt 149, Rate and Cost Lookup `VERIFIED` — covered by `CG-S7-COM-001`'s own open-ended "lanjut")
...
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), and Rate and Cost Lookup (`149`) all `VERIFIED`; 12 remaining capability prompts (`150`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-009` (Prompt 150, Margin Calculation) is the next eligible task — dependency-`READY` (`149`, `VERIFIED`). Still covered by this session's open-ended "lanjut" authorization — no further per-task authorization request needed, subject to every other standing stop-and-escalate condition in `AGENTS.md`. |
| Active task status | `CG-S7-COM-008` (Prompt 149, Rate and Cost Lookup) `VERIFIED` this checkpoint. `ADR-0015` is the controlling document: exactly one new child table (`app.vendor_rate_versions`, keyed by `master_record_id references app.master_records(id)`) plus `app.rate_selections` (a point-in-time snapshot, never a live reference) on top of `PLT-120`'s master-data foundation — never a second, independently-writable rate master. Write authority (`app.create_rate_version`/`approve`/`reject`/`withdraw`) reuses `app.is_support_grant_authority` unchanged, per `ADR-0015`. `app.v_active_vendor_rates` (seeded empty at `PLT-120`) is dropped and recreated to be validity/approval-aware, composed on a new `app.vendor_rate_versions_directory` view reusing `COM-148`'s `app.has_view_cost` for cost masking — tenant-wide visibility (not owner-scoped, unlike every prior Commercial table). `app.search_vendor_rates` (bounded lookup, `COM:View`-gated). `app.select_vendor_rate` (dual `COM:Edit`+`COM:View cost` gate, snapshots the selected/ad-hoc rate, mandatory override reason for ad-hoc/non-approved selections). **Three real defects found and fixed**: (1) mutation RPC results (base-table shape) were wrongly parsed through the read-path's directory-shaped schema — fixed with a distinct `RateVersionRecordSchema`; (2) `scripts/db-tests/master-data.sql`'s own pre-existing view assertion broke against this checkpoint's real data — fixed, proactively tenant-scoped; (3) `app.search_vendor_rates` silently returned zero rows when composed on the auth.uid()-keyed view despite its own explicit-actor gate passing — fixed by querying the base tables directly with explicit-actor masking. Full detail `docs/build-log/phase-02/COM-149.md` §3.3/§8. New route `app/(tenant)/[tenantSlug]/commercial/rates/` plus a Rate Selections section on the Costing Request Detail page. `node:test` 1094/1094, `db:test` PASS across 39 migrations/39 files, `next build` PASS (20 routes), all other gates green. |
...
| Schema/migration head | 39 migrations applied — 32 Platform Core plus `20260723090000_create_commercial_lead_management.sql` (`COM-143`), `20260723120000_create_commercial_prospect_lifecycle.sql` (`COM-144`), `20260723150000_create_commercial_contact_activity_management.sql` (`COM-145`), `20260723180000_create_commercial_sales_pipeline.sql` (`COM-146`, also carries a corrective `CREATE OR REPLACE` of `app.can_access_record`, `PLT-114`), `20260723210000_create_commercial_opportunity_management.sql` (`COM-147`, also extends `app.resolve_commercial_record_ref` and two `COM-145` CHECK constraints), `20260724090000_create_commercial_costing_request.sql` (`COM-148`, also seeds a new `('View cost', 'COM', ...)` permission row), and `20260724150000_create_commercial_rate_cost_lookup.sql` (`COM-149`, adds `app.vendor_rate_versions`/`app.rate_selections` on `PLT-120`'s master-data foundation and drops+recreates `app.v_active_vendor_rates`). `COM-150` (Margin Calculation) will be the next Commercial migration. |
...
| Last full green gate | This checkpoint (`COM-149`) — `node:test` 1094/1094, `db:test` PASS across 39 migrations/39 db-test files, a real `next build` producing 20 routes (up from 18), all applicable gates green. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. The next runtime agent proceeds directly to `CG-S7-COM-009` (Prompt 150) under this session's own open-ended authorization — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-009` (Prompt 150, Margin Calculation), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `009` — dependency-`READY`, authorized (open-ended "lanjut," this session). |
