...
**Updated:** 2026-07-26 (`CG-S7-COM-018`, Prompt 159, Commercial Reports `VERIFIED` — authorized by the user message "lanjut sd prompt 160," covering this task and Prompt 160 in sequence)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial `PHASE_2_IN_PROGRESS`** (kickoff + Lead Management + Prospect Lifecycle + Contact and Activity Management + CRM Sales Plan and Pipeline + Opportunity Management + RFQ and Costing Request + Rate and Cost Lookup + Margin Calculation + Quotation Builder + Quotation Versioning + Quotation Approval + Customer Acceptance + Customer and Account Conversion + Contract and Customer Pricing + Credit and Commercial Control + Commercial Dashboard + Commercial Reports) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), Quotation Approval (`153`), Customer Acceptance (`154`), Customer and Account Conversion (`155`), Contract and Customer Pricing (`156`), Credit and Commercial Control (`157`), Commercial Dashboard (`158`), and Commercial Reports (`159`) all `VERIFIED`; 2 remaining capability prompts (`160`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-018` (Prompt 159, Commercial Reports) `VERIFIED` this checkpoint. `CG-S7-COM-019` (Prompt 160, Full Lineage into Job Order) is the next dependency-`READY` task, **already authorized** by this checkpoint's own "lanjut sd prompt 160" — proceeding directly, no further prompt needed. |
| Active task status | `CG-S7-COM-018` (Prompt 159, Commercial Reports) `VERIFIED` this checkpoint. Governed Commercial reports reusing COM-158's own dashboard queries verbatim (zero new aggregation SQL). New `app.report_types` (Supreme-registered, code-shipped catalogue, 7 seeded rows) and `app.report_runs` (preview synchronous; export asynchronous via `PLT-132`'s real `app.enqueue_job('report_generation', ...)`, no live worker anywhere in this repository so an export reaches `status=queued` and stops there, disclosed). Export gated on the real, previously-unused `COM:Export` permission. New unit-tested CSV/formula-injection guard (`server/policies/csv-export-sanitize.ts`). New `commercial/reports/` catalogue + detail-page UI. `node:test` 1348/1348 (32 net new), `db:test` PASS across 49 migrations/49 files, `npx next build` PASS, all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151..159 build logs §5). This repository defines no `build` script. |
...
| Schema/migration head | 49 migrations applied — 48 as of `COM-158` plus `20260724330000_create_commercial_reports.sql` (`COM-159`, adds 2 tables/4 functions, edits zero prior migration file). `COM-160` (Full Lineage into Job Order) will be the next Commercial migration. |
...
| Last full green gate | This checkpoint (`COM-159`) — `node:test` 1348/1348, `db:test` PASS across 49 migrations/49 db-test files, all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. |
| Next eligible task | `CG-S7-COM-019` (Prompt 160, Full Lineage into Job Order), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `019` — dependency-`READY`, **already authorized** ("lanjut sd prompt 160" covers this task). |
