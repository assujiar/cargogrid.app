...
**Updated:** 2026-07-24 (`CG-S7-COM-012`, Prompt 153, Quotation Approval `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
...
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), and Quotation Approval (`153`) all `VERIFIED`; 8 remaining capability prompts (`154`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-012` (Prompt 153, Quotation Approval) `VERIFIED` this checkpoint. `CG-S7-COM-013` (Prompt 154, Customer Acceptance) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `154` proceeds. |
| Active task status | `CG-S7-COM-012` (Prompt 153, Quotation Approval) `VERIFIED` this checkpoint. Instantiates two already-`VERIFIED` Platform primitives (`PLT-121` Configuration Engine, `PLT-123` Approval Engine) rather than forking them. New `app.quotation_approval_rules` (versioned tenant-wide margin/discount/value threshold policy, mirrors `app.margin_rule_versions`). Widens `app.quotations` with `approval_status`/`approval_request_id`/`approval_rule_version_id`/`approval_required_reasons` — a second axis independent of submission `status`. `CREATE OR REPLACE FUNCTION app.submit_quotation` (widened, identical signature) resolves routing inside the same transaction as draft->submitted: no threshold crossed -> auto-approved; a threshold crossed -> opens a real routed request via the unmodified `app.request_approval`, failing closed (`approval_definition_not_configured`) if no routing definition is published. New `app.decide_quotation_approval_step` is the one domain-specific sync wrapper over the engine — delegates the real decision to the unmodified `app.decide_approval_step`, syncs `approval_status` only once the request reaches a final state. Delegation/escalation call the engine's own functions directly, unwrapped. "Request revision" is not a fourth verb — reject with a reason, then `app.create_quotation_revision` (`COM-152`, unmodified) starts a fresh path. Legacy submitted quotations never retroactively approved. New `commercial/approval-rules/` and `commercial/approvals/` routes; Quotation Detail page gained an `ApprovalPanel`. `node:test` 1196/1196, `db:test` PASS across 43 migrations/43 files, `next build` PASS (25 routes, up from 23), all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151/152/153 build logs §5). |
...
| Schema/migration head | 43 migrations applied — 42 as of `COM-152` plus `20260724270000_create_commercial_quotation_approval.sql` (`COM-153`, adds `app.quotation_approval_rules` + 4 functions, widens `app.quotations`/`app.submit_quotation`/`app.quotations_directory` via `ALTER TABLE`/`CREATE OR REPLACE`, edits zero prior migration file). `COM-154` (Customer Acceptance) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-153`) — `node:test` 1196/1196, `db:test` PASS across 43 migrations/43 db-test files, a real `next build` producing 25 routes (up from 23), all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-013` (Prompt 154) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-013` (Prompt 154, Customer Acceptance), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `013` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `153` only). |
...
| 2 | Commercial | **`IN_PROGRESS`** (`PHASE_2_IN_PROGRESS` set at `CG-S7-COM-001`, Prompt 142, `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md`) — 12/24 tasks `VERIFIED` (kickoff, Lead Management, Prospect Lifecycle, Contact and Activity Management, CRM Sales Plan and Pipeline, Opportunity Management, RFQ and Costing Request, Rate and Cost Lookup, Margin Calculation, Quotation Builder, Quotation Versioning, Quotation Approval) | 50% (12/24 tasks) | `CG-S7-COM-013` — Customer Acceptance (Prompt 154), pending fresh authorization |
