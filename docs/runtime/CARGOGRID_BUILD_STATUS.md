...
**Updated:** 2026-07-26 (`CG-S7-COM-017`, Prompt 158, Commercial Dashboard `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
**Updated by:** Claude Code (autonomous build agent)
**Last verified commit:** this checkpoint's own commit, branch `claude/lanjut-f27dxr`
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial `PHASE_2_IN_PROGRESS`** (kickoff + Lead Management + Prospect Lifecycle + Contact and Activity Management + CRM Sales Plan and Pipeline + Opportunity Management + RFQ and Costing Request + Rate and Cost Lookup + Margin Calculation + Quotation Builder + Quotation Versioning + Quotation Approval + Customer Acceptance + Customer and Account Conversion + Contract and Customer Pricing + Credit and Commercial Control + Commercial Dashboard) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), Quotation Approval (`153`), Customer Acceptance (`154`), Customer and Account Conversion (`155`), Contract and Customer Pricing (`156`), Credit and Commercial Control (`157`), and Commercial Dashboard (`158`) all `VERIFIED`; 3 remaining capability prompts (`159`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-017` (Prompt 158, Commercial Dashboard) `VERIFIED` this checkpoint. `CG-S7-COM-018` (Prompt 159, Commercial Reports) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `159` proceeds. |
| Active task status | `CG-S7-COM-017` (Prompt 158, Commercial Dashboard) `VERIFIED` this checkpoint. Role-specific live Commercial dashboard — lead aging, activity queue, pipeline, quote SLA, margin, win/loss and forecast, permission-safe drill-down, zero new tables (Prompt 158 §13's own "no duplicate analytics store"). Eight new read-only functions, every one `security definer` with its own explicit, re-stated `app.can_access_record(...)` row filter — an invoker-mode first draft failed with a real "permission denied for table opportunities," the exact lesson `COM-147`'s own build log already documented for `app.opportunities_directory`, now reconfirmed for plain functions too. Field masking reuses `app.has_view_selling_price`/`app.has_view_cost` unchanged — no new permission action. Mixed-currency ambiguity resolved structurally (grouped by currency, never summed). New `commercial/dashboard/` route plus a real query-budget timeout (`DashboardQueryTimeoutError`) in `server/queries/dashboard.ts`, RPD-014's own "timeouts" control, the first such mechanism in this repository's service layer. `node:test` 1315/1316 (one pre-existing, environment-dependent, unrelated failure), `db:test` PASS across 48 migrations/48 files, `npx next build` PASS, all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151..158 build logs §5). This repository defines no `build` script. |
| Branch | `claude/lanjut-f27dxr` (this session's harness-assigned branch) |
...
| Schema/migration head | 48 migrations applied — 47 as of `COM-157` plus `20260724320000_create_commercial_dashboard.sql` (`COM-158`, adds 8 functions, zero new tables, edits zero prior migration file). `COM-159` (Commercial Reports) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-158`) — `node:test` 1315/1316, `db:test` PASS across 48 migrations/48 db-test files, all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-018` (Prompt 159) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-018` (Prompt 159, Commercial Reports), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `018` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `158` only). |
