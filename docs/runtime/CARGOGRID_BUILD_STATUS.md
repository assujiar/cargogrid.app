...
**Updated:** 2026-07-28 (`CG-S8-OPS-014`, Prompt 180, Basic Public Customer Tracking `VERIFIED` — authorized by the explicit user message "LANJUT PROMP 176 SD PROM 183"; this is the fifth task in that authorized range)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial CLOSED (`PHASE_2_VERIFIED`)**; **Phase 3 — Operations `PHASE_3_IN_PROGRESS`** (kickoff + Job Order + Shipment Order + Shipment Lifecycle + Land/Air/Sea Baseline + Resource Assignment + Milestone Management + Exception and Escalation + Basic Dispatch + Document Requirement + ePOD Capture and Review + Actual Cost + Basic Job Profitability + Basic Public Customer Tracking `VERIFIED`) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`). **Phase 2 — Commercial is CLOSED** (`PHASE_2_VERIFIED`, 24/24 `VERIFIED`). **Phase 3 — Operations is `IN_PROGRESS`**: kickoff (`167`) through Basic Job Profitability (`179`) `VERIFIED` (prior checkpoints this session), and now Basic Public Customer Tracking (`180`) `VERIFIED` this checkpoint — **the fifth task in this session's "LANJUT PROMP 176 SD PROM 183" authorized range**. `181`–`183` remain authorized and dependency-`READY`; `184` onward requires fresh explicit user authorization. See `docs/build-log/phase-03/00_OPERATIONS_WBS.md`/`OPERATIONS_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S8-OPS-014` (Prompt 180, Basic Public Customer Tracking) `VERIFIED` this checkpoint — fifth task in this session's "LANJUT PROMP 176 SD PROM 183" authorized range. |
| Active task status | `CG-S8-OPS-014` (Prompt 180) `VERIFIED` this checkpoint. Unguessable, revocable, expiring per-shipment tracking token resolving to a minimal, sanitized status/timeline/ePOD-availability view -- rate-limited and durably anti-enumeration-logged on every outcome (a redesign away from `raise exception` was required so the attempt log survives a failed lookup; see the build log). Full detail: `docs/build-log/phase-03/OPS-180.md`. |
...
| Schema/migration head | 65 migrations applied — `OPS-180` added the thirteenth Operations migration (`20260728130000_create_operations_public_tracking.sql`, additive: 2 new tables, 3 new functions; the one deliberate `EXECUTE` grant to `anon` in the schema). |
...
| Last full green gate | `OPS-180` — `node:test` 1697/1697 (15 net new), `db:test` PASS across 65 migrations/66 db-test files (1 net new, zero regression), a real `next build` (53 routes, new: `/tracking/[token]`). |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S8-OPS-015` (Prompt 181) is dependency-`READY` and within this session's authorized range. |
| Next eligible task | `CG-S8-OPS-015` (Prompt 181, Billing Readiness), per `docs/build-log/phase-03/OPERATIONS_EXECUTION_INDEX.md` row `015` — dependency-`READY` and authorized this session. |
