...
**Updated:** 2026-08-03 (`ATW-012` — WMS Inbound — `VERIFIED`. First task in the operator's own explicit sequence "231, 234, 232, 233, 235... hingga 238" (named to resolve `ISS-2026-011`'s own 232/233-vs-234 sequencing risk by building the Inventory Ledger right after Inbound, before Receiving/Putaway). Two new tables (`app.wms_inbound_orders`/`app.wms_inbound_order_lines`), a numbering counter, one composite type, 14 new RPCs. `owner_account_id` inherited from `app.shipment_orders.shipper_account_id`; `source_type` closed enum excluding `purchase_order` (Prompt 231 §24); real partial-unique-index duplicate-source prevention; line control-flag snapshots from `app.item_masters` (`ATW-011A`); lifecycle bounded draft->scheduled->confirmed/cancelled, `ATW-232` owns receiving-progress cancellation guards (disclosed deferred). Root `node:test` 2546/2546 (24 net new), `db:test` PASS across 116 migrations/117 db-test files (1 new), `next build` PASS (90 routes, unchanged). See `docs/build-log/phase-05/ATW-012.md`. Proceeding directly to `ATW-234` (Inventory Ledger) next, per the operator's own named sequence. Supersedes the `ATW-011B` checkpoint note below, retained in history.)
...
| Current phase/workstream | **Phase 1-4 are CLOSED** (37/37, 24/24, 22/22, 29/29 `VERIFIED` respectively). **Phase 5 -- Advanced TMS/WMS is `PHASE_5_IN_PROGRESS`**: `CG-S10-ATW-001..006` all `VERIFIED`; row `226`'s own nine-child decomposition is fully `VERIFIED`; `CG-S10-ATW-008`/`009`/`010`/`011`/`011A` `VERIFIED`; **`CG-S10-ATW-012` (Prompt 231, WMS Inbound) is now also `VERIFIED`**. See `docs/build-log/phase-05/00_ADVANCED_TMS_WMS_WBS.md`/`ADVANCED_TMS_WMS_EXECUTION_INDEX.md` for the full 38-task hierarchy. |
| Active task | `ATW-012` (WMS Inbound) -- `VERIFIED` this checkpoint. |
| Active task status | Implemented and verified. `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` row `231` corrected `READY` -> `VERIFIED`. Full detail: `docs/build-log/phase-05/ATW-012.md`. |
| Branch | `claude/prompt-231-238-e2wb54`, tracked to `origin/claude/prompt-231-238-e2wb54` |
| HEAD | this checkpoint's commit |
| Last known good commit (both lineages agree, pre-divergence) | `origin/main`@`27389a4` (PR #8, Prompt 45) — historical; current last-known-good is `ATW-012`'s own commit |
| Schema/migration head | 116 migrations applied (1 new this checkpoint: `20260730180000_create_advanced_tms_wms_inbound.sql`). |
...
| Last full green gate | `ATW-012` -- `typecheck`/`lint` (0 errors, 85 warnings, unchanged) + root `node:test` 2546/2546 (24 net new) + `db:test` PASS across 116 migrations/117 db-test files (1 new) + a real `next build` (90 routes, unchanged) -- all re-run fresh this checkpoint. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. |
| Next eligible task | `CG-S10-ATW-015` (Prompt 234, Inventory Ledger) — next in the operator's own explicit sequence ("231, 234, 232, 233, 235... hingga 238"), authorized, proceeding directly without a further pause. |
...
| 5 | Advanced TMS/WMS | `PHASE_5_IN_PROGRESS` (`docs/build-log/phase-05/ADVANCED_TMS_WMS_EXECUTION_INDEX.md`) -- kickoff through Prompt 230 all `VERIFIED`; inserted row `CG-S10-ATW-011A` (Item/SKU and UOM Master, `ADR-0019`) `VERIFIED`; **Prompt 231 (WMS Inbound) `VERIFIED`** | 55% (21/38 WBS rows) | `CG-S10-ATW-015` (Prompt 234, Inventory Ledger) -- next in the operator's own explicit sequence, authorized |
