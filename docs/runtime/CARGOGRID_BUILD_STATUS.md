...
**Updated:** 2026-08-03 (`ATW-227` — Capacity, Utilization and Tracking Coverage — `VERIFIED`. First task within a fresh explicit range authorization "lanjut prompt 227-230", executed in ascending order. A real capacity-reservation ledger (`app.vehicle_capacity_reservations`, row-lock-then-sum overbooking prevention against `app.vehicle_operational_profiles`, reserve/consume/release) plus tracking coverage/utilization read projections (`app.get_tenant_tracking_coverage`/`app.get_tenant_tracking_utilization_summary`), reusing rather than re-deriving `226A`'s entitlement, `223`'s eligibility/device data, `226F`'s live position/health, and `225`'s tracking-required-leg detection. New route `app/(tenant)/[tenantSlug]/operations/capacity/` (read-only dashboard). Four real defects found and fixed during authoring (missing `OPS:Assign` grant on the fixture rep role, a cargo-allocation total-reconciliation check, a non-existent `vehicle_operational_profiles.code` column reference, a narrow idempotency/partial-unique-index interaction in the reserve function's own exception handler) — full detail `docs/build-log/phase-05/ATW-227.md` §4.1. `docs/build-log/phase-05/ADVANCED_TMS_WMS_EXECUTION_INDEX.md` corrected: row `227` moves `READY` -> `VERIFIED`; `CG-S10-ATW-009` (228) is now dependency-clean, marked `READY`. Root `node:test` 2425/2425 (28 net new), `db:test` PASS across 110 migrations/112 db-test files (1 new), `next build` PASS (88 routes, 1 new). See `docs/build-log/phase-05/ATW-227.md` and `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` §1.3 for full detail. This session's own explicit range authorization continues; proceeding directly to `228` next. Supersedes the `ATW-226I` checkpoint note below, retained in history.)
...
| Current phase/workstream | **Phase 1-4 are CLOSED** (37/37, 24/24, 22/22, 29/29 `VERIFIED` respectively). **Phase 5 -- Advanced TMS/WMS is `PHASE_5_IN_PROGRESS`**: `CG-S10-ATW-001..006` all `VERIFIED`; row `226`'s own nine-child decomposition is fully `VERIFIED` (row `226`/`CG-S10-ATW-007` itself `VERIFIED`); **`CG-S10-ATW-008` (Prompt 227) is now also `VERIFIED`**. See `docs/build-log/phase-05/00_ADVANCED_TMS_WMS_WBS.md`/`ADVANCED_TMS_WMS_EXECUTION_INDEX.md` for the full 37-task hierarchy. `CG-S10-ATW-009` (Prompt 228) is now dependency-clean, marked `READY`, next within this session's own explicit range authorization ("lanjut prompt 227-230"). |
| Active task | `ATW-227` (Capacity, Utilization and Tracking Coverage) -- `VERIFIED` this checkpoint. |
| Active task status | Implemented and verified. `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` row `227` corrected `READY` -> `VERIFIED`; row `228` corrected `NOT_STARTED` -> `READY`. Full detail: `docs/build-log/phase-05/ATW-227.md`, `ADVANCED_TMS_WMS_EXECUTION_INDEX.md` §1.3. |
| Branch | `claude/prompt-227-230-itvres`, tracked to `origin/claude/prompt-227-230-itvres` |
| HEAD | this checkpoint's commit |
| Last known good commit (both lineages agree, pre-divergence) | `origin/main`@`27389a4` (PR #8, Prompt 45) — historical; current last-known-good is `ATW-227`'s own commit |
| Schema/migration head | 110 migrations applied (1 new this checkpoint: `20260730120000_create_advanced_tms_capacity_utilization.sql`). |
...
| Last full green gate | `ATW-227` -- `typecheck`/`lint` (0 errors, 85 warnings, unchanged) + root `node:test` 2425/2425 (28 net new) + `db:test` PASS across 110 migrations/112 db-test files (1 new) + a real `next build` (88 routes, 1 new) -- all re-run fresh this checkpoint. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. |
| Next eligible task | `CG-S10-ATW-009` (Prompt 228, Advanced Milestone and Exception with Multi-Source Telemetry) is dependency-clean (`READY`), authorized to start next within this session's own explicit range authorization "lanjut prompt 227-230". |
...
| 5 | Advanced TMS/WMS | `PHASE_5_IN_PROGRESS` (`docs/build-log/phase-05/ADVANCED_TMS_WMS_EXECUTION_INDEX.md`) -- kickoff through Prompt 225 all `VERIFIED`; row `226` (all nine `226A`-`226I` children) `VERIFIED`; Prompt 227 Capacity/Utilization/Tracking Coverage `VERIFIED` | 43% (16/37 WBS rows) | `CG-S10-ATW-009` (Prompt 228, Advanced Milestone and Exception with Multi-Source Telemetry) -- dependency-`READY`, next within this session's own explicit range authorization ("lanjut prompt 227-230"); `CG-S10-ATW-010` (Prompt 229) also `READY` (independent Warehouse lane), scheduled after `228` per ascending execution order |
