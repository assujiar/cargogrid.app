...
**Updated:** 2026-07-24 (`CG-S7-COM-014`, Prompt 155, Customer and Account Conversion `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial `PHASE_2_IN_PROGRESS`** (kickoff + Lead Management + Prospect Lifecycle + Contact and Activity Management + CRM Sales Plan and Pipeline + Opportunity Management + RFQ and Costing Request + Rate and Cost Lookup + Margin Calculation + Quotation Builder + Quotation Versioning + Quotation Approval + Customer Acceptance + Customer and Account Conversion) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), Quotation Approval (`153`), Customer Acceptance (`154`), and Customer and Account Conversion (`155`) all `VERIFIED`; 6 remaining capability prompts (`156`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-014` (Prompt 155, Customer and Account Conversion) `VERIFIED` this checkpoint. `CG-S7-COM-015` (Prompt 156, Contract and Customer Pricing) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `156` proceeds. |
| Active task status | `CG-S7-COM-014` (Prompt 155, Customer and Account Conversion) `VERIFIED` this checkpoint. Resolves `ADR-CAND-ARCH-012` via newly ratified `ADR-0018`: a flat-column `app.accounts` (not the Master Data Engine, not two separate accounts/customers tables), `customer_status` resolving the account/customer distinction as one entity. New `app.accounts` (legal/trade name, tax ID, normalized fingerprint columns reused directly from `COM-144`'s own normalization functions, `billing_address` jsonb, database-enforced anti-duplication via a partial unique fingerprint index) and `app.account_conversions` (`unique(quotation_id)` — one final outcome per quotation, ever). `app.find_duplicate_accounts`/`app.get_account_conversion_readiness` are read-only previews; `app.convert_quotation_to_account` is the one atomic, `COM:Approve`-gated, idempotent create-or-link write (early-returns on retry, gracefully resolves a concurrent-race `unique_violation` as "link to the winner," re-links existing contacts, backfills `app.opportunities.account_ref` closing `COM-147`'s own disclosed forward-reference). RLS is deliberately tenant-wide (not record-scoped), the same precedent `app.margin_rule_versions`/`app.quotation_approval_rules` set. **Disclosed boundary**: legal/tax/billing field restriction is enforced via the `COM:Approve` conversion gate itself, not per-field masking — `app.permissions_action_check`'s fixed 19-action enum has no billing-shaped action to add one with; `app.accounts` carries no `*_directory` view at all (mirrors `app.quotation_approval_rules`'s "no view when nothing needs masking" precedent). New `commercial/accounts/` list+detail routes plus an `AccountConversionPanel`/`ConvertAccountForm` on the Quotation Detail page. `node:test` 1238/1238, `db:test` PASS across 45 migrations/45 files, all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151/152/153/154/155 build logs §5). This repository defines no `build` script. |
...
| Schema/migration head | 45 migrations applied — 44 as of `COM-154` plus `20260724290000_create_commercial_customer_account_conversion.sql` (`COM-155`, adds `app.accounts`/`app.account_conversions` + 3 functions, widens `app.contact_links`'s CHECK constraint and `app.resolve_commercial_record_ref`, edits zero prior migration file). `COM-156` (Contract and Customer Pricing) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-155`) — `node:test` 1238/1238, `db:test` PASS across 45 migrations/45 db-test files, all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-015` (Prompt 156) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-015` (Prompt 156, Contract and Customer Pricing), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `015` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `155` only). |
