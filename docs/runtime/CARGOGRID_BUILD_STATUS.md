...
**Updated:** 2026-07-24 (`CG-S7-COM-015`, Prompt 156, Contract and Customer Pricing `VERIFIED` — authorized by a single, unscoped user "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent)
**Updated by:** Claude Code (autonomous build agent)
**Last verified commit:** this checkpoint's own commit, branch `claude/cargogrid-design-system-expansion-ljbzkm`
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 — Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 — Commercial `PHASE_2_IN_PROGRESS`** (kickoff + Lead Management + Prospect Lifecycle + Contact and Activity Management + CRM Sales Plan and Pipeline + Opportunity Management + RFQ and Costing Request + Rate and Cost Lookup + Margin Calculation + Quotation Builder + Quotation Versioning + Quotation Approval + Customer Acceptance + Customer and Account Conversion + Contract and Customer Pricing) |
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), RFQ and Costing Request (`148`), Rate and Cost Lookup (`149`), Margin Calculation (`150`), Quotation Builder (`151`), Quotation Versioning (`152`), Quotation Approval (`153`), Customer Acceptance (`154`), Customer and Account Conversion (`155`), and Contract and Customer Pricing (`156`) all `VERIFIED`; 5 remaining capability prompts (`157`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-015` (Prompt 156, Contract and Customer Pricing) `VERIFIED` this checkpoint. `CG-S7-COM-016` (Prompt 157, Credit and Commercial Control) is the next dependency-`READY` task, but is **not authorized to start automatically** — this checkpoint's own authorization was a single, unscoped "lanjut," read as authorizing exactly this one task per `docs/runtime/HANDOFF.md`'s own standing precedent. A fresh explicit authorization is required before `157` proceeds. |
| Active task status | `CG-S7-COM-015` (Prompt 156, Contract and Customer Pricing) `VERIFIED` this checkpoint. Versioned customer contracts and pricing derived from accepted, converted quotations, with effective dates, service/lane price components, and a deterministic effective-price lookup. New `app.customer_contracts` (root/version identity reusing `app.quotations`' own self-referencing-insert technique; unlike a quotation's single current row, multiple versions may be simultaneously published as long as their effective windows do not overlap, enforced via row-level locking at publish time) and `app.customer_contract_price_components` (service/lane price rows, a unique index guaranteeing at most one deterministic match). `app.create_customer_contract_draft` serves both the main flow (from an accepted+converted quote) and the alternative flow (renewal/amendment, copying components into a new draft) identically. `app.publish_customer_contract`/`app.retire_customer_contract` are direct `COM:Approve` governance gates (not a second Approval Engine routing — a deliberately smaller-scope choice than `COM-153`'s own quotation approval). `app.get_effective_customer_price` is the deterministic, reproducible lookup, raising `no_effective_price` explicitly on zero rows. **`COM:View margin` newly seeded for `COM`** (reusing the already-real enum value, the same bounded mechanism `COM-148` used for `COM:View cost`) — masks price components via the already-established `COM:View selling price` instead, since no separate margin figure is stored here. New `commercial/contracts/` list+detail routes plus a `ContractCreationPanel`/`CreateContractForm` on the Quotation Detail page. `node:test` 1265/1265, `db:test` PASS across 46 migrations/46 files, all other gates green except one disclosed pre-existing `git:check-paths` tool false positive (COM-151..156 build logs §5). This repository defines no `build` script. |
| Branch | `claude/cargogrid-design-system-expansion-ljbzkm` (this session's harness-assigned/designated branch — a design-system-expansion checkpoint, then `COM-151`, precede this one on the same branch, per the harness's one-session-one-branch model, not separate numbered Commercial prompts on separate branches) |
...
| Schema/migration head | 46 migrations applied — 45 as of `COM-155` plus `20260724300000_create_commercial_customer_contract_pricing.sql` (`COM-156`, adds `app.customer_contracts`/`app.customer_contract_price_components` + 1 new `COM:View margin` permission row + 6 functions + 1 masked view, edits zero prior migration file). `COM-157` (Credit and Commercial Control) will be the next Commercial migration, once authorized. |
...
| Last full green gate | This checkpoint (`COM-156`) — `node:test` 1265/1265, `db:test` PASS across 46 migrations/46 db-test files, all applicable gates green except the one disclosed pre-existing `git:check-paths` false positive. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. `CG-S7-COM-016` (Prompt 157) is dependency-`READY` but requires a fresh explicit user authorization before any runtime agent begins it (see "Active task" row) — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-016` (Prompt 157, Credit and Commercial Control), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `016` — dependency-`READY`, **not yet authorized** (this checkpoint's own authorization was a single, unscoped "lanjut," read as scoped to `156` only). |
...
| 2 | Commercial | **`IN_PROGRESS`** (`PHASE_2_IN_PROGRESS` set at `CG-S7-COM-001`, Prompt 142, `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md`) — 13/24 tasks `VERIFIED` (kickoff, Lead Management, Prospect Lifecycle, Contact and Activity Management, CRM Sales Plan and Pipeline, Opportunity Management, RFQ and Costing Request, Rate and Cost Lookup, Margin Calculation, Quotation Builder, Quotation Versioning, Quotation Approval, Customer Acceptance) | 54% (13/24 tasks) | `CG-S7-COM-014` — Customer and Account Conversion (Prompt 155), pending fresh authorization |
