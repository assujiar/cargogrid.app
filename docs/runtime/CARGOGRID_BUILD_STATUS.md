...
**Updated:** 2026-07-19 (`CG-S6-PLT-026`, Prompt 129, API Key and Webhook Primitives `VERIFIED` — authorized by a third, separate, unscoped "lanjut" after `PLT-128` closed)
...
| Current phase/workstream | Phase 1 — Platform Core. Kickoff + 25 capabilities `VERIFIED` (Tenant Provisioning/Lifecycle; Subscription/Module/Feature Entitlement; Supabase Auth Integration; Four-Layer Identity/Access Context; Organization Hierarchy; User Lifecycle; Role and Permission Builder; RBAC Enforcement; RLS Tenant Policy Foundation; Field-Level and Record-Level Access; Support Access and Impersonation; Audit Trail Foundation; White-Label Foundation; Custom Domain; Localization; Master Data Foundation; Configuration Engine; Workflow Engine; Approval Engine; Status Engine; Numbering Engine; Form and Custom-Field Builder; Notification Engine; Document and File Engine; API Key and Webhook Primitives). |
| Active task | **NONE — explicit authorization required.** `CG-S6-PLT-027` (Prompt 130) is the next execution-index row; no runtime agent may execute it without a fresh, explicit user authorization message. |
| Active task status | `CG-S6-PLT-026` (Prompt 129, API Key and Webhook Primitives) `VERIFIED` this checkpoint — authorized by a third, separate, unscoped "lanjut" after `PLT-128` closed. `app.api_keys`/`app.webhook_event_types`/`app.webhook_endpoints`/`app.webhook_subscriptions`/`app.webhook_deliveries`/`app.webhook_delivery_attempts` implement tenant-scoped hashed API key lifecycle plus webhook endpoint/signed-delivery primitives. API key scopes are validated against the actor's own currently-granted RBAC permissions (can only narrow, never widen) via `app.evaluate_permission()`, proven with a real role/permission/assignment chain. Raw keys/secrets are never stored — keys as a one-way sha256 hash, webhook secrets access-controlled to `service_role` only (HMAC verification is symmetric and needs the raw value) — both return exactly once. **`ADR-0011` newly ratified**, resolving `ADR-CAND-ARCH-018`'s signature/timestamp/auto-disable sub-questions: HMAC-SHA256, 5-minute tolerance, 10-consecutive-failure auto-disable, all real and provable without any live HTTP call. A bounded, disclosed-partial SSRF check rejects literal localhost/private/link-local hosts at registration. One real bug (the exact `RETURNS TABLE` column-shadowing class `PLT-117` found) caught via an actual failing `db:test` run and fixed. `node:test` 601/601, `db:test` 317 total scenario groups, 10/11 gates green (`test:e2e` `NOT_RUN` — sandbox Playwright browser-binary revision skew, disclosed). |
...
| Schema/migration head | 26 migrations applied (`app.tenants` through `app.api_keys`/`app.webhook_event_types`/`app.webhook_endpoints`/`app.webhook_subscriptions`/`app.webhook_deliveries`/`app.webhook_delivery_attempts`, including the `ERR-2026-004` corrective migration) |
| Latest environment verified | local sandbox (read-only); no deployed environment exists yet (`preflight` correctly fails closed) |
| Last full green gate | This checkpoint — `node:test` 601/601, `db:test` 317 total scenario groups, 10/11 gates green (`test:e2e` sandbox-`NOT_RUN`, disclosed) |
| **Active blockers** | **Explicit-authorization gate ACTIVE.** Zero `OPEN` error, zero Critical/High-severity issue — `ERR-2026-001..004` all `RECOVERED`/`SUPERSEDED` — but this checkpoint was authorized by a single, unscoped "lanjut" (one task only, not a range). The next runtime agent/session must **stop and obtain fresh explicit user authorization** before proceeding to `CG-S6-PLT-027` (Prompt 130) or beyond. This is not a technical blocker; it is this build's own standing process discipline. |
| Next eligible task | `CG-S6-PLT-027` (Prompt 130), per `docs/build-log/phase-01/00_PLATFORM_CORE_EXECUTION_INDEX.md` row `027` — **contingent on fresh user authorization**, not yet authorized. |
