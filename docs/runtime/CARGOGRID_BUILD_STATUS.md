...
**Updated:** 2026-07-29 (`CG-S9-FIN-017`, Prompt 206, Reversal and Adjustment `VERIFIED` -- additive migration adding governed journal reversal/adjustment (`app.finance_journal_corrections`), closing FIN-203's own "reverse posted journal via FIN-206" and FIN-202's own reserved `finance_subledger_batches.status = 'reversed'` disclosed forward references; new route `/[tenantSlug]/finance/corrections`. First task in this session's fresh explicit Prompts 206-210 authorization. `PHASE_4_IN_PROGRESS` continues; `CG-S9-FIN-018` (Prompt 207, Period Lock and Governed Reopen) is dependency-eligible and authorized this session -- next. Supersedes the `CG-S9-FIN-016` checkpoint note below, retained in history.)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 -- Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 -- Commercial CLOSED (`PHASE_2_VERIFIED`)**; **Phase 3 -- Operations CLOSED (`PHASE_3_VERIFIED`)**; **Phase 4 -- Finance `PHASE_4_IN_PROGRESS`** (kickoff through Reversal and Adjustment `VERIFIED` -- first task of this session's authorized Prompts 206-210 range) |
| Current phase/workstream | **Phase 1 -- Platform Core is CLOSED** (37/37 `VERIFIED`). **Phase 2 -- Commercial is CLOSED** (`PHASE_2_VERIFIED`, 24/24 `VERIFIED`). **Phase 3 -- Operations is CLOSED** (`PHASE_3_VERIFIED`, 22/22 `VERIFIED`). **Phase 4 -- Finance is `PHASE_4_IN_PROGRESS`** -- kickoff through Draft-versus-Posted State (`190`-`205`) `VERIFIED`; Reversal and Adjustment (`206`) `VERIFIED` this checkpoint, first of this session's freshly authorized Prompts 206-210 range. See `docs/build-log/phase-04/00_FINANCE_WBS.md`/`FINANCE_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S9-FIN-017` (Prompt 206, Reversal and Adjustment) `VERIFIED` this checkpoint. Next: `CG-S9-FIN-018` (Prompt 207, Period Lock and Governed Reopen) is dependency-eligible and authorized this session. |
| Active task status | `CG-S9-FIN-017` (Prompt 206) `VERIFIED` this checkpoint. Additive migration (1 new table, 6 new functions, 2 widened `CHECK` constraints, 1 function-signature extension with its own prior overload explicitly dropped); zero prior migration file edited. Governed reversal (system-derived exact opposite lines) and adjustment (preparer-supplied, pre-validated balanced lines) against a posted journal, posting through FIN-203's own shared system-journal path; a subledger-sourced reversal also marks its source batch `reversed`, closing FIN-202's own disclosed reference. Full detail: `docs/build-log/phase-04/FIN-206.md`. |
...
| Schema/migration head | 87 migrations applied -- `20260729200000_create_finance_reversal_adjustment.sql` is the sixteenth Finance migration, additive only. |
...
| Last full green gate | `CG-S9-FIN-017` -- `typecheck`/`lint` (0 errors), `node:test` 2066/2066 (20 net new), `db:test` PASS across 87 migrations/88 db-test files (1 net new, zero regression), a real `next build` (73 routes, 1 new). |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. This session's explicit authorized range is Prompts 206-210; `207`-`210` remain within that same authorization. |
| Next eligible task | `CG-S9-FIN-018` (Prompt 207, Period Lock and Governed Reopen) -- dependency-eligible per `FINANCE_EXECUTION_INDEX.md` and **authorized this session**. |
