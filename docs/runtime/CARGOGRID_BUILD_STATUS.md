...
**Updated:** 2026-07-24 (`CG-S7-COM-007`, Prompt 148, RFQ and Costing Request `VERIFIED` — covered by `CG-S7-COM-001`'s own open-ended "lanjut")
...
| Current phase/workstream | **Phase 1 — Platform Core is CLOSED** (37/37 `VERIFIED`, unchanged). **Phase 2 — Commercial is `IN_PROGRESS`**: kickoff (`142`), Lead Management (`143`), Prospect Lifecycle (`144`), Contact and Activity Management (`145`), CRM Sales Plan and Pipeline (`146`), Opportunity Management (`147`), and RFQ and Costing Request (`148`) all `VERIFIED`; 13 remaining capability prompts (`149`–`161`) plus verification/hardening/documentation/closure (`162`–`165`) remain `NOT_STARTED`. See `docs/build-log/phase-02/00_COMMERCIAL_WBS.md`/`COMMERCIAL_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S7-COM-008` (Prompt 149, Rate and Cost Lookup) is the next eligible task — dependency-`READY` (`148`, `VERIFIED`; `ADR-0015` already `ACCEPTED`). Still covered by this session's open-ended "lanjut" authorization — no further per-task authorization request needed, subject to every other standing stop-and-escalate condition in `AGENTS.md`. |
| Active task status | `CG-S7-COM-007` (Prompt 148, RFQ and Costing Request) `VERIFIED` this checkpoint. Bounded Commercial cost-request shell over one opportunity, explicitly not full Procurement RFQ. `app.costing_requests` (idempotent on tenant/opportunity/version, point-in-time requirements snapshot), `app.costing_request_components`, `app.costing_responses` (server-computed total from components), `app.costing_response_components`. `app.request_costing` reuses `app.get_opportunity_costing_readiness` (`COM-147`) directly. **New Commercial-scoped permission**: `('View cost', 'COM', 'sensitive', true)` — the existing catalogue row only covered `FIN`/`PRC`. `app.costing_responses_directory` applies `COM-147`'s security_invoker=false lesson from the start (no rediscovery needed). **Two real defects found and fixed**: (1) the module-scoped `'View cost'` gap, found via a genuine test failure, fixed by seeding a new catalogue row; (2) this capability's own legitimate fixture calls broke two sibling db-test files' unscoped audit-count assertions purely due to alphabetical run order — fixed durably by scoping both to their own tenant_id. Full detail `docs/build-log/phase-02/COM-148.md` §3.3/§3.7/§8. New route `app/(tenant)/[tenantSlug]/commercial/costing-requests/[requestId]/` plus a Costing Requests section on the Opportunity Detail page. `node:test` 1068/1068, `db:test` PASS across 38 migrations/38 files, `next build` PASS (18 routes), all other gates green. |
...
| Schema/migration head | 38 migrations applied — 32 Platform Core plus `20260723090000_create_commercial_lead_management.sql` (`COM-143`), `20260723120000_create_commercial_prospect_lifecycle.sql` (`COM-144`), `20260723150000_create_commercial_contact_activity_management.sql` (`COM-145`), `20260723180000_create_commercial_sales_pipeline.sql` (`COM-146`, also carries a corrective `CREATE OR REPLACE` of `app.can_access_record`, `PLT-114`), `20260723210000_create_commercial_opportunity_management.sql` (`COM-147`, also extends `app.resolve_commercial_record_ref` and two `COM-145` CHECK constraints), and `20260724090000_create_commercial_costing_request.sql` (`COM-148`, also seeds a new `('View cost', 'COM', ...)` permission row). `COM-149` (Rate and Cost Lookup) will be the next Commercial migration to extend `PLT-120`'s master-data registry, per `ADR-0015`. |
...
| Last full green gate | This checkpoint (`COM-148`) — `node:test` 1068/1068, `db:test` PASS across 38 migrations/38 db-test files, a real `next build` producing 18 routes (up from 17), all applicable gates green. |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. The next runtime agent proceeds directly to `CG-S7-COM-008` (Prompt 149) under this session's own open-ended authorization — the same standing stop-and-escalate discipline in `AGENTS.md` still applies to any genuine blocker encountered there. |
| Next eligible task | `CG-S7-COM-008` (Prompt 149, Rate and Cost Lookup), per `docs/build-log/phase-02/COMMERCIAL_EXECUTION_INDEX.md` row `008` — dependency-`READY`, authorized (open-ended "lanjut," this session). |
