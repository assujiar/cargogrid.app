...
**Updated:** 2026-07-29 (`CG-S9-FIN-023`, Prompt 212, Job, Customer and Service Profitability `VERIFIED` -- additive migration adding a versioned Finance-grade profitability fact per Job Order (billed revenue from FIN-197's own issued invoices minus approved actual cost from OPS-178, never OPS-179's own Commercial-estimate revenue_snapshot), a named blocked_reason rather than a fabricated figure when revenue/cost is missing or currencies mismatch, and customer/branch/service grouped aggregation, deny outright without FIN:View margin; new route `/[tenantSlug]/finance/profitability`. Second task in a fresh explicit Prompts 211-213 authorization ("lanjut sd promp 213"), following the fully-complete Prompts 206-210 range. `PHASE_4_IN_PROGRESS` continues; `CG-S9-FIN-024` (Prompt 213, Finance Dashboard and Reports) is dependency-eligible and authorized this session -- next, the final task in this session's range. Supersedes the `CG-S9-FIN-022` checkpoint note below, retained in history.)
...
| Field | Value |
|---|---|
| Package/repository version | Package `0.18.0-step17` (`FINAL_PACKAGE_VALIDATED`); runtime Step 2 **closed** (`RUNTIME_DISCOVERY_VERIFIED`); Step 3 **closed and reconciled** (`RUNTIME_ARCHITECTURE_VERIFIED`, Lineage A authoritative); **Phase 0 closed (`PHASE_0_VERIFIED`)**; **Phase 1 -- Platform Core closed (`PHASE_1_VERIFIED`)**; **Phase 2 -- Commercial CLOSED (`PHASE_2_VERIFIED`)**; **Phase 3 -- Operations CLOSED (`PHASE_3_VERIFIED`)**; **Phase 4 -- Finance `PHASE_4_IN_PROGRESS`** (kickoff through Job, Customer and Service Profitability `VERIFIED` -- second task of a fresh authorized Prompts 211-213 range) |
| Current phase/workstream | **Phase 1 -- Platform Core is CLOSED** (37/37 `VERIFIED`). **Phase 2 -- Commercial is CLOSED** (`PHASE_2_VERIFIED`, 24/24 `VERIFIED`). **Phase 3 -- Operations is CLOSED** (`PHASE_3_VERIFIED`, 22/22 `VERIFIED`). **Phase 4 -- Finance is `PHASE_4_IN_PROGRESS`** -- kickoff through Cash and Bank Baseline (`190`-`211`) `VERIFIED`; Job, Customer and Service Profitability (`212`) `VERIFIED` this checkpoint, second of a fresh authorized Prompts 211-213 range. See `docs/build-log/phase-04/00_FINANCE_WBS.md`/`FINANCE_EXECUTION_INDEX.md` for the full hierarchy. |
| Active task | `CG-S9-FIN-023` (Prompt 212, Job, Customer and Service Profitability) `VERIFIED` this checkpoint. Next: `CG-S9-FIN-024` (Prompt 213, Finance Dashboard and Reports) is dependency-eligible and authorized this session -- the final task in this session's range. |
| Active task status | `CG-S9-FIN-023` (Prompt 212) `VERIFIED` this checkpoint. Additive migration (1 new table, 5 new functions); zero prior migration file edited. Billed revenue (issued invoices) minus approved actual cost (Operations' own governed evidence), never a fabricated figure; deny outright without FIN:View margin; customer/branch/service grouped aggregation. Full detail: `docs/build-log/phase-04/FIN-212.md`. |
...
| Schema/migration head | 93 migrations applied -- `20260729260000_create_finance_job_profitability.sql` is the twenty-second Finance migration, additive only. |
...
| Last full green gate | `CG-S9-FIN-023` -- `typecheck`/`lint` (0 errors), `node:test` 2144/2144 (12 net new), `db:test` PASS across 93 migrations/94 db-test files (1 net new, zero regression), a real `next build` (76 routes, 1 new). |
| **Active blockers** | **None.** Zero `OPEN` error, zero Critical/High-severity issue. This session's explicit authorized range is Prompts 211-213; `213` remains within that same authorization. |
| Next eligible task | `CG-S9-FIN-024` (Prompt 213, Finance Dashboard and Reports) -- dependency-eligible per `FINANCE_EXECUTION_INDEX.md` and **authorized this session**. |
