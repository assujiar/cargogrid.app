...
CPL-300 (Customer User Scope) ── the mandatory scope primitive; gates every capability below
   ...
   │    (302→303 real: 303's linked_quote_request_id is a    │          │   dashboard itself)
   │     live FK into 302's own table. 303⇢304 is DASHED:    │          │
   │     Tier C review confirmed 304's own migration never   │          │
   │     references 303's table/columns at all -- see note   │          │
   │     below, same "named, not schema" shape as 304⇢305/   │          │
   │     307/308 already has.)                                │          │
...
**Tier C cross-prompt-integration review correction (post-implementation, added when Batch 1's own code was actually read):** the 302→303 edge in the diagram above is a REAL schema dependency, confirmed by direct code read — `app.customer_portal_booking_requests.linked_quote_request_id` is a live FK into `app.customer_portal_quote_requests`, and `app.create_customer_booking_request_draft` structurally depends on 302's own `status='converted'` state machine. The 303→304 edge, by contrast, is NOT a real schema/data dependency, despite being drawn as a solid arrow above and despite `CG-S13-CPL-006`'s own predecessor row (§7 below) and the `304_SHIPMENT_ORDER_PROMPT.md` source prompt package both naming `CPL-005`/CPL-303 as a stated prerequisite: `supabase/migrations/20260801050000_create_customer_portal_shipment_order_access.sql` was grep-confirmed, live, to contain zero references to `app.customer_portal_booking_requests`, `linked_job_order_id`, or `linked_shipment_order_id` anywhere in its body — every one of 304's 5 RPCs reads only the pre-existing, already-`VERIFIED` `app.shipment_orders` (Phase 5/OPS-169) plus CPL-300's own `resolve_customer_account_scope`, and `scripts/db-tests/customer-shipment-orders.sql` builds its own independent job-order/shipment-order fixture and passes with zero `customer_portal_booking_requests` rows present. The relationship is therefore the SAME "named upstream in prose, not a hard schema dependency" shape the diagram already calls out for 304→305/307/308 (this paragraph's own prior text, above) — it simply was not extended to 303→304 at kickoff time, one batch earlier, because the code did not exist yet to check it against. `CPL-303`/`CPL-304`'s own `CG-S13-CPL-005`/`006` predecessor rows (§7 below) are left as-is: predecessor rows in that table encode *release/execution ORDER* within a strictly-numbered batch (a real, correctly-enforced constraint — 304 was not started before 303 closed), not a schema-dependency claim, and changing them risks conflating the two. This correction is scoped to the diagram/prose's own schema-dependency framing only.
...
| `CG-S13-CPL-002` | 300 | Customer User Scope | `CPL-001` `COMPLETED` | **`VERIFIED`** 2026-08-16 — cut-short single-prompt Tier C review (RLS/auth primitive), 2 Critical findings fixed at root cause, 1 Low disclosed (`ISS-2026-116`). `docs/build-log/phase-08/CPL-300.md` |
| `CG-S13-CPL-003` | 301 | Customer Portal Dashboard | `CPL-002` `VERIFIED` | **`VERIFIED`** 2026-08-16 — Batch 1 Tier C close. `docs/build-log/phase-08/CPL-301.md` |
| `CG-S13-CPL-004` | 302 | Request Quotation | `CPL-002` | **`VERIFIED`** 2026-08-16 — Batch 1 Tier C close (1 Critical IDOR + 1 High race + 1 High fabricated-citation/scan-gate finding fixed). `docs/build-log/phase-08/CPL-302.md` |
| `CG-S13-CPL-005` | 303 | Booking | `CPL-002`, `CPL-004` | **`VERIFIED`** 2026-08-16 — Batch 1 Tier C close (shares the Critical IDOR/race fix class with 302/304). `docs/build-log/phase-08/CPL-303.md` |
| `CG-S13-CPL-006` | 304 | Shipment Order | `CPL-002`, `CPL-005` | **`VERIFIED`** 2026-08-16 — Batch 1 Tier C close (shares the Critical IDOR/race fix class with 302/303). `docs/build-log/phase-08/CPL-304.md` |
| `CG-S13-CPL-007` | 305 | Tracking | `CPL-002` | **`VERIFIED`** 2026-08-17 — Batch 2 Tier C close. `docs/build-log/phase-08/CPL-305.md` |
| `CG-S13-CPL-008` | 306 | Shipment Monitoring | `CPL-002`, `CPL-007` | **`VERIFIED`** 2026-08-17 — Batch 2 Tier C close (1 Low cross-tenant test-coverage gap fixed). `docs/build-log/phase-08/CPL-306.md` |
| `CG-S13-CPL-009` | 307 | ePOD Access | `CPL-002` | **`VERIFIED`** 2026-08-17 — Batch 2 Tier C close (1 Medium audit `access_type` mislabel fixed). `docs/build-log/phase-08/CPL-307.md` |
| `CG-S13-CPL-010` | 308 | Document Center | `CPL-002`, `CPL-004`(booking refs), `CPL-006`(shipment refs), `CPL-009` | **`VERIFIED`** 2026-08-17 — Batch 2 Tier C close (shares the Medium `access_type` mislabel fix class with 307). `docs/build-log/phase-08/CPL-308.md` |
| `CG-S13-CPL-011` | 309 | Warehouse Inventory Visibility | `CPL-002` | **`VERIFIED`** 2026-08-17 — Batch 2 Tier C close; closes `ISS-2026-117` (ATW-023/CPL-300 scope-model reconciliation), live regression-proven. `docs/build-log/phase-08/CPL-309.md` |
| `CG-S13-CPL-012` | 310 | Warehouse Order/Fulfillment Visibility | `CPL-002`, `CPL-011` | **`READY`** — dependency-clean, next eligible, first prompt of Batch 3 |
...
**`CG-S13-CPL-012` (Prompt 310, Warehouse Order/Fulfillment Visibility).** Dependency-clean — `CPL-002` `VERIFIED` (§15), `CPL-011` `VERIFIED` (§17), Batch 2 (`CPL-007..011`, Prompts 305-309) closed `VERIFIED` (§17). First prompt of Batch 3 (`CPL-012..016`, Prompts 310-314).

## 15. Update — CPL-300 (Prompt 300, Customer User Scope) `VERIFIED`

Implemented the mandatory Layer-4 scope primitive: `app.customer_portal_account_memberships` (new many-to-many account/site grant table, `app.accounts`/ADR-0018 stays the sole customer/account/site master, no new site table — resolves §3 item 5's open question), a widened `app.resolve_customer_account_scope` composing (never editing) ATW-242's own resolver, and a self-contained Layer-4 `account_admin` authority chain for invite/accept/suspend/revoke (ADR-0024 Part B). RLS: `authenticated` holds zero direct table grant on either new table — all 8 new RPCs are the only access path (ADR-0024 Part A). New portal-entry guard `lib/portal/customer-portal-guard.ts` (general-purpose, mirrors HRT-287's `customer-ticket-guard.ts`, which stays untouched) and a scope-preview page under the existing `app/(tenant)/[tenantSlug]/` route group.

Cut short as its own single-prompt Tier C review ahead of the rest of Batch 1, per `AGENTS.md`'s RLS/auth-primitive rule. Two adversarial lenses (security/RLS, correctness/spec-compliance), each independently live-testing against a fresh disposable database, found two Critical, live-reproduced defects: (1) an IDOR on three read RPCs missing an actor-identity session cross-check (any authenticated principal could pass an arbitrary target identity and read that identity's cross-account scope and membership roster); (2) revocation not propagating to the legacy `app.principal_memberships` marker this migration itself writes, leaving a suspended/revoked member with live WMS (ATW-023), ticketing customer-channel, and portal-entry access. Both fixed at root cause (identity cross-check added to all four affected functions; the legacy grant moved from invite-time to accept-time and explicitly revoked on suspend/revoke) and independently re-verified by the orchestrating session against live code, not accepted from either review lens's or the fix work's own report — the fix agent's own final message was truncated mid-task, so every claimed fix was confirmed directly by reading the resulting migration file before being trusted. One Low finding (a distinguishable not-found-vs-forbidden error on two membership RPCs) disclosed, not fixed, registered as `ISS-2026-116`.

Independent gate re-run by the orchestrating session: `typecheck` 0 errors; `lint` 0 errors/271 warnings; `pnpm run test` **4179/4179 pass**; `next build` clean, new route present; `db:test` **169/169 files `ALL PASSED`** (the one pre-existing, unrelated `hris-overtime-timesheet.sql` date-dependent flake, `ISS-2026-115`, excluded as before); `docs:check`/`security:check`/`git:check-paths` all clean. **`CG-S13-CPL-002` is `VERIFIED`.** Full detail: `docs/build-log/phase-08/CPL-300.md`.

## 16. Update — Batch 1 (CPL-301..304, Prompts 301-304: Dashboard, Request Quotation, Booking, Shipment Order) `VERIFIED` — Batch 1 (`CPL-002..006`) now fully closed

Implemented the remaining four prompts of Batch 1. **CPL-301** (Dashboard): a live-aggregating `app.get_customer_portal_dashboard_summary` composing only real, already-existing customer-facing sources (CPL-300 scope, ATW-023 warehouse inventory, HRT-287 tickets) — six other cards honestly return `available=false` until their own owning prompt lands, never fake data. **CPL-302** (Request Quotation): `app.customer_portal_quote_requests`, a portal-owned intake table handing off to Commercial's `app.quotations` via one staff-gated linking RPC; `app.quotations` itself untouched. **CPL-303** (Booking): `app.customer_portal_booking_requests`, referencing an accepted quote request, handing off to Operations job/shipment orders via one staff-gated linking RPC; never calls a staff-gated canonical mutation directly. **CPL-304** (Shipment Order): a customer-safe read projection over the existing `app.shipment_orders` (21 of 29 columns) plus a portal-owned change-request table; `app.confirm_shipment_order`/`cancel_shipment_order` never called.

Batch 1's own Tier C review ran with all four lenses AGENTS.md names (spec-compliance; security/RLS; correctness/concurrency; cross-prompt integration), each independently live-testing against a fresh disposable database. Found and fixed at root cause, independently re-verified against live code by the orchestrating session (not accepted from either the review lenses' or the fix agent's own report): a **Critical** cross-account IDOR via idempotency-key collision on all three "create" RPCs (302/303/304) — the idempotent short-circuit returned a matching row before verifying it belonged to the caller's own account, live-reproduced as exploitable in ordinary double-submit use, not just adversarial guessing; a **High** missing `unique_violation` guard on the same three RPCs, surfacing a raw Postgres error under a genuine race instead of an idempotent no-op; a **High** fabricated "source prompt" citation in CPL-302 that had been used to justify skipping a spec-mandated malware-scan gate on submit (real gate added); a **Medium** (×3) C-05 disclosure-ordering leak on all three staff-acknowledgement RPCs, echoing a row's real `tenant_id` before the RBAC check; a **Medium** client-side idempotency-key instability (regenerated per Server Action invocation across four call sites, defeating double-submit protection); and a **High/integrity** finding — `scripts/db-tests/hris-overtime-timesheet.sql` had been silently deleted mid-batch by an earlier pass with no replacement, restored via `git checkout` and confirmed byte-identical to the pre-batch committed version. One Low finding disclosed, not fixed: the dashboard's stub cards for bookings/shipments aren't yet wired to 303/304's own new data, registered as `ISS-2026-118` (a genuine design decision on which statuses count as "open," left to a future checkpoint).

Independent gate re-run by the orchestrating session: `typecheck` 0 errors; `lint` 0 errors/289 warnings; `pnpm run test` **4296/4296 pass**; `next build` clean, 7 new routes present; `db:test` **172/172 files `ALL PASSED`** (the two pre-existing, unrelated Sunday-date-dependent HRIS flakes, `ISS-2026-115` and `ISS-2026-077`, excluded); `docs:check`/`security:check`/`git:check-paths` all clean. **`CG-S13-CPL-003`, `-004`, `-005`, `-006` are all `VERIFIED`. Batch 1 (`CPL-002..006`) is fully closed.** Full detail: `docs/build-log/phase-08/CPL-301.md` through `CPL-304.md`.

## 17. Update — Batch 2 (CPL-007..011, Prompts 305-309: Tracking, Shipment Monitoring, ePOD Access, Document Center, Warehouse Inventory Visibility) `VERIFIED` — Batch 2 (`CPL-007..011`) now fully closed

This checkpoint's own container was restarted mid-batch by the host environment (not a task failure) after Prompts 305-308 had already finished writing to disk but before 309 had started; the orchestrating session independently audited 305-308's on-disk output for restart-induced truncation/corruption before trusting it (none found — the restart landed cleanly between 308 finishing and 309 starting), then resumed with 309's implementation followed by the batch's Tier C review across all five.

Implemented the remaining five prompts of Batch 2. **CPL-305** (Tracking): a single `app.get_customer_shipment_tracking` read RPC, no new table. **CPL-306** (Shipment Monitoring): a new `app.customer_shipment_alert_subscriptions` table (RLS enabled, zero `authenticated` grant, matching CPL-300's own convention) plus 4 RPCs (subscribe/unsubscribe/list subscriptions/list alerts); subscribe/unsubscribe use `INSERT ... ON CONFLICT (natural key) DO UPDATE` rather than a client idempotency key — structurally immune to Batch 1's own idempotency-race defect class, live-confirmed race-safe under a real 20-process concurrent test. **CPL-307** (ePOD Access): a single `app.get_customer_epod` read RPC, no new table. **CPL-308** (Document Center): `app.list_customer_documents`/`app.get_customer_document`, no new table. **CPL-309** (Warehouse Inventory Visibility): closes `ISS-2026-117` (the ATW-023/CPL-300 scope-model gap disclosed at Batch 1 close) via a new additive migration — `app.evaluate_customer_portal_inventory_access` composes CPL-300's widened `app.resolve_customer_account_scope` with ATW-023's unmodified `app.customer_warehouse_eligibility_active`, and three new wrapper RPCs (`get_customer_portal_inventory_balance`, `list_customer_portal_inventory_balances`, `list_customer_portal_warehouse_eligibility`) mirror ATW-023's own RPC surface column-for-column without ever editing the applied ATW-023 or CPL-300 migrations. Movement-summary/lot/serial/export drill-down deliberately not mirrored onto the new resolver (zero live caller today), disclosed as `ISS-2026-119`.

Batch 2's own Tier C review ran with all four lenses (spec-compliance; security/RLS; correctness/concurrency; cross-prompt integration), each independently live-testing against a fresh disposable database. Found and fixed at root cause, independently re-verified against live code by the orchestrating session (not accepted from either the review lenses' or the fix agent's own report): a **High** test-infrastructure defect — CPL-309's own new db-test reused ATW-023's sibling test's hardcoded fake-UUID fixture literal in two `app.audit_logs` precondition/postcondition count queries without scoping by `tenant_id`, so it collided with a real leftover fixture row from `scripts/db-tests/advanced-tms-customer-inventory-access.sql` (which runs earlier, alphabetically, against the same shared disposable database every real `pnpm run db:test` invocation uses) and aborted the entire suite before ever reaching `rbac-enforcement.sql` and ~20 later files — fixed by scoping both queries by `tenant_id` (the fix agent found and fixed a second, related unscoped query none of the three lenses that flagged this individually had caught); two **Medium** findings — `app.get_customer_epod` (CPL-307) and `app.get_customer_document` (CPL-308) both wrote the wrong `app.file_access_logs.access_type` literal (`'signed_url_issued'` instead of the schema-correct `'metadata_view'`, since no signed URL is ever actually fabricated in either capability) — fixed in both migrations plus their db-test assertions; one **Low** finding — CPL-306's db-test provisioned a full second tenant "for cross-tenant isolation" but never actually exercised it — fixed by adding a real cross-tenant assertion block. CPL-309's own core deliverable (a `customer-multi` actor granted a second account, Gamma, *only* through CPL-300's new `app.customer_portal_account_memberships` grant table — zero row in the legacy `app.principal_memberships` marker — provably seeing that account's inventory through the new RPCs while ATW-023's original, unmodified RPCs see nothing for it) was independently live-reproduced by all four review lenses, by the fix agent, and a second time by the orchestrating session itself, including against the real, unmodified `bash scripts/db-tests/run.sh` entry point.

The orchestrating session additionally corrected one documentation-accuracy defect the fix pass itself introduced: `docs/build-log/phase-08/CPL-309.md` initially claimed `scripts/db-tests/rbac-enforcement.sql` ran clean *within* the same continuous 159-of-179-file `db:test` run that halted at `procurement-vendor-performance.sql` — false, since `rbac-enforcement.sql` sorts alphabetically at position 162, three files past that halt point, and so could not have executed before it. The underlying claim (the ATW-032 authority-surface sweep passes clean against Batch 2's merged schema) is true and was independently re-confirmed by the orchestrating session via a standalone run against the halted run's own leftover, fully-migrated disposable database (`ALL PLT-112 db-test assertions passed`, exit 0) — `docs/build-log/phase-08/CPL-309.md` §14.3 corrected to describe the standalone re-run accurately rather than misstating it as part of the continuous sequence.

Independent gate re-run by the orchestrating session: `typecheck` 0 errors; `lint` 0 errors/289 warnings; `pnpm run test` **4393/4393 pass**; `db:test` (`bash scripts/db-tests/run.sh`) ran clean through all 158 files preceding, and including, `customer-warehouse-inventory-visibility.sql` (with the `KEY REGRESSION TEST PASSED` notice, live, with ATW-023's own colliding fixture row genuinely present), halting only at the pre-existing, already-registered, wall-clock-dependent `ISS-2026-059` flake (`procurement-vendor-performance.sql`, Phase 6/PRC-264 scope, fails deterministically ~00:00–03:59 UTC — this run's own timestamp was inside that window); `rbac-enforcement.sql` independently confirmed passing via a standalone re-run against the same schema state (`ALL PLT-112 db-test assertions passed`); `docs:check`/`security:check`/`git:check-paths` all clean. **`CG-S13-CPL-007`, `-008`, `-009`, `-010`, `-011` are all `VERIFIED`. Batch 2 (`CPL-007..011`) is fully closed.** Full detail: `docs/build-log/phase-08/CPL-305.md` through `CPL-309.md`.
