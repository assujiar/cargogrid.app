...
| `004` | `170` Shipment lifecycle | Shipment Execution / Canonical Shipment State | `READY` | `OPS-168..169` (`VERIFIED`) | — | `docs/build-log/phase-03/OPS-170.md` | Runtime build agent | `CG-S8-OPS-005` |
...
**Tally:** of the 22 rows in this index (`167`–`188`), **3 are `VERIFIED`** (kickoff, Job Order, Shipment Order), **1 is `READY`** (`170`, Shipment Lifecycle), **5 are `BLOCKED`** on their own not-yet-started predecessor within this session's authorized range (`171`–`175`), and **13 are `NOT_STARTED`** (`176`–`188`, out of the "lanjut sd prompt 175" authorized range).
