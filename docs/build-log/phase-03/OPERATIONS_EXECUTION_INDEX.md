...
**Status:** `PHASE_3_IN_PROGRESS` — updated at `OPS-172` (Resource Assignment, `VERIFIED`); this document's own row/tally table is updated at every checkpoint, most recently to reflect `172`'s completion and `173`'s dependency-`READY` state
...
| `007` | `173` Milestone management | Control Tower / Shipment Visibility | `READY` | `OPS-169..172` (`VERIFIED`) | — | `docs/build-log/phase-03/OPS-173.md` | Runtime build agent | `CG-S8-OPS-008` |
| `008` | `174` Exception and escalation | Control Tower / Operational Exception | `BLOCKED` | `OPS-173` | — | `docs/build-log/phase-03/OPS-174.md` | Runtime build agent | `CG-S8-OPS-009` |
...
**Tally:** of the 22 rows in this index (`167`–`188`), **6 are `VERIFIED`** (kickoff, Job Order, Shipment Order, Shipment Lifecycle, Land/Air/Sea Baseline, Resource Assignment), **1 is `READY`** (`173`, Milestone Management), **2 are `BLOCKED`** on their own not-yet-started predecessor within this session's authorized range (`174`–`175`), and **13 are `NOT_STARTED`** (`176`–`188`, out of the "lanjut sd prompt 175" authorized range).
