...
| `006` | `172` Resource/vendor assignment | Shipment Execution / Execution Responsibility | `READY` | `OPS-168..171` (`VERIFIED`) | — | `docs/build-log/phase-03/OPS-172.md` | Runtime build agent | `CG-S8-OPS-007` |
| `007` | `173` Milestone management | Control Tower / Shipment Visibility | `BLOCKED` | `OPS-169..172` | — | `docs/build-log/phase-03/OPS-173.md` | Runtime build agent | `CG-S8-OPS-008` |
...
**Tally:** of the 22 rows in this index (`167`–`188`), **5 are `VERIFIED`** (kickoff, Job Order, Shipment Order, Shipment Lifecycle, Land/Air/Sea Baseline), **1 is `READY`** (`172`, Resource Assignment), **3 are `BLOCKED`** on their own not-yet-started predecessor within this session's authorized range (`173`–`175`), and **13 are `NOT_STARTED`** (`176`–`188`, out of the "lanjut sd prompt 175" authorized range).
