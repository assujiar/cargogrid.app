...
**Status:** `PHASE_5_IN_PROGRESS`. Rows `220`–`225` (`CG-S10-ATW-001..006`) are `VERIFIED`. Row `226`'s own nine-child decomposition is **fully `VERIFIED`** (`ATW-226A` through `ATW-226I`), and row `226` itself (`CG-S10-ATW-007`) is `VERIFIED`. **Rows `227`, `228`, and `229` (`CG-S10-ATW-008`/`009`/`010`) are now also `VERIFIED`** (`docs/build-log/phase-05/ATW-227.md`, `ATW-228.md`, `ATW-229.md`) — a real capacity-reservation ledger plus tracking coverage/utilization reads (`227`); telemetry-derived milestone/exception provenance, a new tracking-health signal detector, dependency-aware live ETA, and an explicit schedule-rebaseline operation (`228`); and tenant/company warehouse and zone masters with versioned topology, explicit customer eligibility, and dependency-checked deactivation (`229`). This session's own explicit range authorization ("lanjut prompt 227-230") is in progress; `CG-S10-ATW-011` (Prompt 230) is now the next task, next in ascending order within the authorized range. Every other row remains `NOT_STARTED`. Only Prompt 248 may set `PHASE_5_VERIFIED`.
...
| `CG-S10-ATW-011` | 230 — Bin and Racking | Warehouse Foundation / Location Topology | `ATW-229` | `ATW-231..240` | **`READY`** | Dependency-clean; authorized to start next within this session's own explicit range |
