...
| **Disposition** | **`PARTIALLY_RESOLVED` at `HDN-373`** — 16 fixed, ~14 re-registered narrower as `ISS-2026-186` (owner `HDN-387`), not blindly fixed. See amendment note above **`RESOLVED` 2026-08-31.** Both linked entries are now closed: `ISS-2026-179` (the whole ~24-candidate sweep, every member individually dispositioned with evidence) and `ISS-2026-186` (the final 6, at `20260831010000`). The last 6 were closed by guarding their `public.*` wrappers rather than the `app.*` functions — the only PostgREST-reachable layer — after live counts established that guarding `app.*` would have broken ~918 RLS policy evaluations (304/276/266/72 policies reference these four) and would have turned `app.evaluate_permission`'s clean `not_active_tenant_member` denial into an exception. Proven by a four-property live regression in `scripts/db-tests/rbac-enforcement.sql`, including a guard-the-guard assertion that fails if the assert is ever pushed down into `app.*`. |
