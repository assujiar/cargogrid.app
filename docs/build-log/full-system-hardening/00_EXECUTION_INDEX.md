...
| 17 | `HDN-385` | `CG-S15-HDN-017` | 385 | Data Migration Rehearsal | Migration Assurance | `HDN-371` | `COMPLETED` — first round, Tier C pending. Generic import framework + `employee_import` adapter live-rehearsed; a real duplicate-swallowing defect fixed. 10 findings registered (`ISS-2026-269..278`). See `HDN-385.md` |
...
**Tally: 21 rows — 2 `COMPLETED` (kickoff, `HDN-385`), 4 `BLOCKED`, 0 `READY`,
...
> ### `HDN-385` (Data Migration Rehearsal, `CG-S15-HDN-017`) Tier C review is next
>
> - **`HDN-385` first round is `COMPLETED`.** Three investigation lenses
>   (migration/import framework inventory and feasibility; a real, live
>   migration rehearsal against a disposable Postgres; financial
>   reconciliation and business-rule compliance review). Corrected a
>   seeded-state error: the Import/Export Job Framework (`PLT-131`) is
>   Phase 1 (Platform Core), not Phase 5 as originally seeded. **The
>   generic framework's full pipeline live-tested end-to-end** via
>   `employee_import` (mapping → preview → validation → commit, a real
>   7-row batch, all correctly classified). **A real, live-reproduced
>   defect found and fixed**: a genuine `employee_number` collision was
>   silently swallowed instead of surfaced — the identical defect class an
>   earlier commit already found and fixed in a sibling adapter
>   (`vendor_rate_import`, PRC-255), simply never applied here. Fixed this
>   checkpoint, mirroring the proven pattern exactly. **10 findings
>   registered, not fixed**: `ISS-2026-269`/`HDN-BLK-037` (High — no
>   duplicate detection at all for un-keyed rows on a fresh re-import, a
>   more severe, live-reproduced duplicate-person defect distinct from the
>   fixed one); `ISS-2026-273`/`HDN-BLK-038` (High — no bulk financial
>   opening-balance import path exists anywhere, self-disclosed to never
>   reach the GL journal, confirmed live); `ISS-2026-270`/`272`/`274`/`275`
...
>   root cause as `HDN-384`'s own `ISS-2026-265`); `ISS-2026-271`/`276`/
>   `277`/`278` (Low). No Critical finding anywhere. Authored `docs/
>   runbooks/data-migration-rehearsal.md` (new). Full disposition:
>   `HDN-385.md`.
> - **Tier C review of `HDN-385` (4 independent adversarial lenses) is
>   next, against this committed first-round state, before `CG-S15-HDN-018`
>   (`HDN-386`, Integrated Verification) may begin.**
