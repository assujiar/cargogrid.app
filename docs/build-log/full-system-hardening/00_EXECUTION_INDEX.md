...
**Tally: 21 rows — 1 `COMPLETED` (kickoff), 14 `BLOCKED`, 5 `VERIFIED` (`HDN-370`,
`HDN-371`, `HDN-372`, `HDN-373`, `HDN-374`), 1 `READY` (`HDN-377`).**
...
> ### **`HDN-375` (Data Lineage Audit, `CG-S15-HDN-007`) is the single next eligible prompt**
>
> - **`HDN-374` is `VERIFIED`** — Tier C closed clean (4 independent adversarial lenses
>   found 5 more real, live-forced defects beyond the first round's own 4 — 3 fixed same
>   checkpoint, 2 registered with a named forward owner), independent full gate re-run
>   green. Full disposition: `HDN-374.md` §13.
> - `HDN-375`'s own dependency (`HDN-371` **`VERIFIED`**, hard) has been satisfied since
>   before `HDN-373` began; it must run in its own separate session — it may not share a
>   session with `HDN-374`.
...
>   This range's established cadence runs the numbered prompts in sequence; `HDN-375` is
>   the next one in that sequence and the one this section names as next.
> - **Carry-forward for `HDN-375`, so it does not discover these cold:**
>   1. **`HDN-BLK-016`/`ISS-2026-199`** (High, new, owner `HDN-386`) —
>      `app.request_finance_settlement_reversal` posts no reversing GL journal at all,
>      permanently desyncing the GL from the AP subledger on every settlement reversal.
>      Not this lane's charter to fix; noted only in case `HDN-375`'s own data-lineage
>      audit touches settlement-reversal lineage.
>   2. **`ISS-2026-186`** (Medium, owner `HDN-387`) — untouched by `HDN-374`'s own
>      investigation; still open, still `HDN-387`'s to resolve.
>   3. **`ISS-2026-197`** (Low, owner `HDN-386`) — no FX/multi-currency conversion
>      anywhere in the revenue chain; `app.calculate_job_profitability`'s own
>      planned-vs-actual figure split.
>   4. **`HDN-BLK-010`'s residual 3 non-Finance functions plus `ISS-2026-163`** (owner
>      `HDN-387`) — the Finance/HRIS-Payroll portion (10 functions) is `RESOLVED` at
>      `HDN-374`; the remaining 3 (Commercial, WMS, Platform/Auth) plus `app.prepare_job_
>      order`'s own defective exception handler are explicitly not this lane's charter.
