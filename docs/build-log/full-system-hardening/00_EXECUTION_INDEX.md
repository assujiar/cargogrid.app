...
| 10 | `HDN-378` | `CG-S15-HDN-010` | 378 | Security Hardening | Security Assurance | `HDN-372` **(hard)** `VERIFIED`, `HDN-373..377` **all `VERIFIED`** | `COMPLETED` — first round only, Tier C pending. `ISS-2026-150` (High, "must not be deferred again") `RESOLVED` across all 4 named functions; a self-caught `CREATE OR REPLACE` overload defect corrected before commit; `pg_trgm`/`btree_gist` relocated (`postgis` found non-relocatable, registered separately as `ISS-2026-234`); OWASP sweep fixed 1 finding (`ISS-2026-233`); `ISS-2026-168`/`169`/`232` fixed; `ISS-2026-151`/`149`/`146` reconfirmed and deferred again; 4 runbooks authored. See `HDN-378.md` |
...
**Tally: 21 rows — 1 `COMPLETED` (kickoff), 11 `BLOCKED`,
8 `VERIFIED` (`HDN-370`, `HDN-371`, `HDN-372`, `HDN-373`, `HDN-374`, `HDN-375`, `HDN-376`, `HDN-377`), 1 `COMPLETED` (`HDN-378`, first round only, Tier C pending)
(`HDN-378`).**
...
>   `HDN-BLK-020`/`ISS-2026-229`, `HDN-BLK-021`/`ISS-2026-230`, `ISS-2026-231`) — each
>   remains registered at its already-named owner. **Still squarely `HDN-378`'s own
>   charter, not yet resolved**: `HDN-BLK-022`/`ISS-2026-225` (corrected High, ~35-table
>   Procurement/HR RLS-bypass sweep-and-fix — genuinely too large for a bounded-repair
>   checkpoint), `ISS-2026-223` (repository-wide `is_support_grant_authority` convention
>   question, a policy decision this checkpoint has no mandate to decide alone),
>   `ISS-2026-151` (re-scoped, deliberately deferred again — see above),
>   `ISS-2026-234` (new — postgis relocation, out of this checkpoint's own bounded
>   scope).
...
**Nothing after `HDN-378` may begin until `HDN-378` is `VERIFIED`.**
