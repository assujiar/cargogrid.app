...
| 17 | `HDN-385` | `CG-S15-HDN-017` | 385 | Data Migration Rehearsal | Migration Assurance | `HDN-371` | `READY` |
...
**Tally: 21 rows — 1 `COMPLETED` (kickoff), 4 `BLOCKED`, 1 `READY` (`HDN-385`),
15 `VERIFIED` (`HDN-370`, `HDN-371`, `HDN-372`, `HDN-373`, `HDN-374`, `HDN-375`, `HDN-376`, `HDN-377`, `HDN-378`, `HDN-379`, `HDN-380`, `HDN-381`, `HDN-382`, `HDN-383`, `HDN-384`).**
...
> ### `HDN-385` (Data Migration Rehearsal, `CG-S15-HDN-017`) is next
>
> - **`HDN-384` (Disaster Recovery Rehearsal) is `VERIFIED`.** First round:
>   3 investigation lenses covered scenario definition, a real live DR
>   drill, and communication/enterprise-DR-controls review. Data corruption
>   and security incident scenarios live-rehearsed (a real defect fixed in
>   `database-restore.md`'s own composed in-place restore procedure — 80
>   silent errors, PK collisions + FK race); major outage/provider failure
>   tabletop-only (no Docker in this sandbox, no infrastructure failover in
>   CargoGrid's own architecture). Authored `docs/runbooks/
>   disaster-recovery.md` (new). **Tier C found 7 more real gaps, all
>   corrected directly in the runbooks**: the `TRUNCATE` step itself
>   silently bypasses 9 security/integrity triggers with zero audit trail
>   (`ISS-2026-265`/`HDN-BLK-034`, High); session revocation confirmed
>   inert — never enforced anywhere, the real lockout in the drill traced
>   entirely to role/IP-allowlist revocation (`ISS-2026-264`/`HDN-BLK-035`,
>   High); no mutual-exclusion for concurrent restore attempts, live
>   -reproduced to race (`ISS-2026-267`/`HDN-BLK-036`, High); materialized
>   views never restored by `pg_restore --data-only` (`ISS-2026-266`,
>   Medium, new required refresh step); `ISS-2026-263` root-caused and
>   re-scoped from an unreproduced anomaly to a confirmed, deterministic
>   defect in `app.transition_user_status`; 2 nuance corrections
>   (`ISS-2026-259` narrowed for a 2-table exception, `ISS-2026-261`
>   sharpened — Supabase Read Replicas exist but are plan-gated and
>   Auth-excluded); RTO corrected from a single figure to a measured range.
>   No Critical finding anywhere. Independent full gate re-run green after
>   the fix pass. Full disposition: `HDN-384.md` §13.
> - **`HDN-385`'s own charter, seeded per `HARDENING_MATRIX.md` §16**: real
>   precedent (306/306 migrations applied cleanly to the real hosted
>   project); non-idempotency per §14 item 2; exercise the Import/Export
>   Job Framework's own mapping/preview/validation/duplicate-handling;
>   commit + reconcile counts/totals/files/lineage in a rehearsal
>   environment only; never fabricate historical data. **`HDN-384`'s own
>   durable findings now forward-linked**: the Docker-absent sandbox
>   constraint will likely recur; the TRUNCATE-before-restore/migration
>   -seeded-PK-collision defect class is directly relevant to this
>   checkpoint's own "duplicate handling" dimension — test for it
>   explicitly rather than assuming import tooling is immune; never trust
>   `pg_restore`'s exit code alone as a pass signal. **Upstream `HDN-371`
>   and `HDN-384` are both satisfied — `HDN-385` may begin.**
...
**`HDN-384` is `VERIFIED`. Nothing after `HDN-385` may begin until `HDN-385` is `VERIFIED`.**
