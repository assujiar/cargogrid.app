...
| 7 | `HDN-375` | `CG-S15-HDN-007` | 375 | Data Lineage Audit | Integrity Assurance | `HDN-371` **(hard)** | **`COMPLETED`** — Tier C review pending. 4 lenses, 2 real defects fixed (`ISS-2026-201` High, `app.transaction_lineage_edges` freely mutable; `ISS-2026-202` Medium, orphan `source_id` on loyalty/finance journals), 1 registered (`HDN-BLK-017`/`ISS-2026-200`, High, hash-chain not genuine, owner `HDN-386`); see `HDN-375.md` |
...
**Tally: 21 rows — 2 `COMPLETED` (kickoff; `HDN-375` Tier C pending), 13 `BLOCKED`,
5 `VERIFIED` (`HDN-370`, `HDN-371`, `HDN-372`, `HDN-373`, `HDN-374`), 1 `READY`
(`HDN-377`).**
...
> - **Carry-forward already named for `HDN-375`, still open, not this checkpoint's own to
>   resolve** (`HDN-BLK-016`/`ISS-2026-199`, `ISS-2026-186`, `ISS-2026-197`, `HDN-BLK-010`'s
>   residual 3 non-Finance functions plus `ISS-2026-163`) — none were touched by this
>   checkpoint's own investigation; each remains registered at its already-named owner
>   (`HDN-386` or `HDN-387`).
...
**Nothing after `HDN-375` may begin until `HDN-375` is `VERIFIED`.**
