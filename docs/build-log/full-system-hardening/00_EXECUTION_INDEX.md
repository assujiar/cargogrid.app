...
| 13 | `HDN-381` | `CG-S15-HDN-013` | 381 | Browser and Device Compatibility | UX Assurance | `HDN-380` | `COMPLETED` — first round, Tier C pending. Zero real device-compat defects found live (16/16 route×device combos); 5 static gaps fixed (touch-target sizing, toast viewport clipping, 4 unwrapped tables, mobile/tablet e2e coverage added — 30/30 passing); `ISS-2026-244..247` registered (Safari/Firefox untestable, PWA scoping, unused primitives, 19 remaining unwrapped tables). See `HDN-381.md` |
...
**Tally: 21 rows — 2 `COMPLETED` (kickoff, `HDN-381`), 8 `BLOCKED`, 0 `READY`,
...
> ### `HDN-381` (Browser and Device Compatibility, `CG-S15-HDN-013`) Tier C review is next
>
> - **`HDN-381` first round is `COMPLETED`.** Three investigation lenses
>   (environment feasibility; source-level static evidence sweep; live
>   mobile/tablet-viewport e2e testing). **Zero real device-compatibility
>   defects found live** — 16/16 route×device combinations (login/vendor-intake/
>   2 guard routes × iPhone 13/Pixel 5/iPad Pro 11/375×667) zero horizontal
>   overflow, zero obscured controls; mobile tap-to-submit resolved in 50ms,
>   confirming `HDN-380`'s dev-mode-hang fix holds under touch too. 5 real
>   static gaps found and fixed: touch-target sizing on
>   `Button`/`IconButton`/`Checkbox` (was ~36px/~20px, under the 44px
>   guidance); `ToastProvider`'s viewport clipping below 336px wide; 4 files
>   with the worst unwrapped tables (of 23 found) wrapped in
>   `overflow-x-auto`; zero permanent mobile/tablet e2e coverage existed —
>   `playwright.config.ts` extended with `mobile-chrome`/`tablet-chrome`
>   projects + new `e2e/browser-device-compat.spec.ts`, 30/30 passing (up
>   from 18/18), catching a real config bug (`devices["iPad Pro 11"]`'s own
>   `defaultBrowserType` is `webkit`, absent in this sandbox) before Tier C.
>   4 findings registered, not fixed: `ISS-2026-244` (Low, `TRACKED_GAP` —
>   Safari/Firefox untestable, no binary present), `ISS-2026-245` (Low — no
>   PWA manifest/service worker, RPD-004 language needs scoping),
>   `ISS-2026-246` (Low — 6 shared UI primitives unused in any real screen;
>   **a prior investigation claim that `DataTable` was also unused did not
...
>   unit tests, `next build` clean, `test:e2e` 30/30, 229/229 db-tests (328
>   migrations, unchanged — no schema change). Full disposition:
>   `HDN-381.md`.
> - **Tier C review of `HDN-381` (4 independent adversarial lenses) is next,
>   against this committed first-round state, before `CG-S15-HDN-014`
>   (`HDN-382`, Observability) may begin.**
