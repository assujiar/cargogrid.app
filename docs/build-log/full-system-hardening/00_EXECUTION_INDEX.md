...
| 10 | `HDN-378` | `CG-S15-HDN-010` | 378 | Security Hardening | Security Assurance | `HDN-372` **(hard)** `VERIFIED`, `HDN-373..377` **all `VERIFIED`** | `READY` |
...
8 `VERIFIED` (`HDN-370`, `HDN-371`, `HDN-372`, `HDN-373`, `HDN-374`, `HDN-375`, `HDN-376`, `HDN-377`), 1 `READY`
(`HDN-378`).**
...
> ### `HDN-378` (Security Hardening, `CG-S15-HDN-010`) is next eligible
>
> - **`HDN-377` (Storage and Signed URL Audit, `CG-S15-HDN-009`) is `VERIFIED`.** Tier C
>   review (4 independent adversarial lenses) closed: all 6 first-round fixes confirmed
>   solid; 1 Critical self-inflicted bypass found and fixed in the first round's own new
>   trigger (`ISS-2026-226`); 2 more same-domain gaps fixed (`ISS-2026-227` High,
>   `ISS-2026-228` Medium-High); 1 finding drafted then self-corrected before commit
>   (`ISS-2026-231`); 2 new out-of-charter findings registered (`ISS-2026-229`/
>   `HDN-BLK-020` Critical, `ISS-2026-230`/`HDN-BLK-021` High, both owner `HDN-386`); 1
>   first-round disposition corrected from Low to High (`ISS-2026-225`/`HDN-BLK-022`); 1
>   more out-of-charter finding registered (`ISS-2026-232`, Medium, owner `HDN-378`);
>   several documentation miscounts corrected across 9 ledger files. Independent full
>   gate re-run green: 5440/5440 unit tests, 228/229 db-tests (324 migrations, same
>   pre-existing `ISS-2026-204` flake). Full disposition: `HDN-377.md` §13.
> - **`HDN-378` (Security Hardening) is dependency-satisfied** (`HDN-372` `VERIFIED`,
>   `HDN-373..377` all `VERIFIED`) and is this range's next prompt in sequence.
...
>   remains registered at its already-named owner. **Squarely `HDN-378`'s own charter**
>   (Security Hardening, its own named owner): `HDN-BLK-022`/`ISS-2026-225` (corrected
>   High, ~35-table Procurement/HR RLS-bypass sweep-and-fix), `ISS-2026-223` (repository-
>   wide `is_support_grant_authority` convention question), `ISS-2026-232` (3 more
>   `token_hash` blanket-grant exposures).
