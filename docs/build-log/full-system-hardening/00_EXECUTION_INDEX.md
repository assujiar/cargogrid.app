...
**Tally: 21 rows — 1 `COMPLETED` (kickoff), 4 `BLOCKED`, 0 `READY`,
...
> ### `HDN-385` (Data Migration Rehearsal, `CG-S15-HDN-017`) is `VERIFIED` — `HDN-386` is next
>
> - **`HDN-385` is `VERIFIED`** — first round + Tier C review both closed
>   at commit `c524bf0` (Tier C's own ledger fix is a documentation-only
>   change, no further code commit required). First round: three
>   investigation lenses (migration/import framework inventory and
>   feasibility; a real, live migration rehearsal against a disposable
>   Postgres; financial reconciliation and business-rule compliance
>   review). Corrected a seeded-state error: the Import/Export Job
>   Framework (`PLT-131`) is Phase 1 (Platform Core), not Phase 5 as
>   originally seeded. **The generic framework's full pipeline live-tested
>   end-to-end** via `employee_import` (mapping → preview → validation →
>   commit, a real 7-row batch, all correctly classified). **A real,
>   live-reproduced defect found and fixed**: a genuine `employee_number`
>   collision was silently swallowed instead of surfaced — the identical
>   defect class an earlier commit already found and fixed in a sibling
>   adapter (`vendor_rate_import`, PRC-255), simply never applied here.
>   Fixed this checkpoint, mirroring the proven pattern exactly. Tier C's 4
>   adversarial lenses (correctness re-derivation; schema-wide completeness
>   sweep; ledger/documentation consistency; attack-surface adversarial
>   testing) confirmed the fix itself defect-free — no Critical or new High
>   finding — but live-reproduced one genuinely new gap and fixed one
>   real, pre-existing ledger defect predating this checkpoint by 7
>   checkpoints (a duplicate `ISS-2026-235` ID, from `HDN-378`'s own Tier
>   C, undetected by every intervening checkpoint's own Tier C sweep,
>   renumbered to `ISS-2026-236` to match `BLOCKER_LEDGER.md`'s own
>   already-correct cross-reference). **11 findings registered, not
>   fixed**: `ISS-2026-269`/`HDN-BLK-037` (High — no duplicate detection at
>   all for un-keyed rows on a fresh re-import, a more severe,
>   live-reproduced duplicate-person defect distinct from the fixed one);
>   `ISS-2026-273`/`HDN-BLK-038` (High — no bulk financial opening-balance
>   import path exists anywhere, self-disclosed to never reach the GL
>   journal, confirmed live); `ISS-2026-270`/`272`/`274`/`275`/`279`
>   (Medium — no safe reference-table import path; no migration-rehearsal
>   tracking mechanism; no master-data/tenant-setup import; the
>   posted-journal-immutability trigger never fires on INSERT, the same
>   root cause as `HDN-384`'s own `ISS-2026-265`; case/whitespace-
>   insensitive `employee_number` uniqueness lets trivial variants of an
>   already-blocked duplicate through, found live at Tier C);
>   `ISS-2026-271`/`276`/`277`(corrected at Tier C)/`278` (Low). No
>   Critical finding anywhere. Authored `docs/runbooks/
>   data-migration-rehearsal.md` (new). Full disposition: `HDN-385.md`.
> - **`CG-S15-HDN-018` (`HDN-386`, Prompt 386, Integrated Verification) is
