...
| 6 | `HDN-374` | `CG-S15-HDN-006` | 374 | Financial Integrity Audit | Financial Assurance | `HDN-371` **(hard)** | `COMPLETED` — Tier C review pending; see `HDN-374.md` |
...
**Tally: 21 rows — 1 `COMPLETED` (kickoff) + 1 `COMPLETED` (`HDN-374`, Tier C pending),
14 `BLOCKED`, 4 `VERIFIED` (`HDN-370`, `HDN-371`, `HDN-372`, `HDN-373`), 1 `READY`
(`HDN-377`).**
...
| 2026-08-23 | `HDN-374` | `CG-S15-HDN-006` | `claude/step-15-hdn-369-kickoff-w6qren` | see `HDN-374.md` | **`COMPLETED`** — Tier C review pending. Four independent parallel investigation lenses (revenue chain; cost/AP chain; period lock/reversal/concurrency/rounding; loyalty liability/payroll/tax/statutory), each required to live-force its own findings on disposable databases. Cost/AP chain, payment/journal reconciliation, period lock, reversal, correction, rounding, tax snapshotting, payroll-handoff aggregation, and RPD-016's statutory gates all held clean. **4 real, live-forced defects fixed** at `20260811000000_harden_financial_integrity_invoicing_and_idempotency.sql`: `ISS-2026-194` (High, quote-level tax silently double-applied at invoicing — `revenue_snapshot.totalAmount`, already tax-inclusive, was read as the invoice's own pre-tax subtotal); `ISS-2026-195` (High, a job order could reach `issued` on two full-amount invoices from two distinct handoffs, doubling the bill — **the first fix draft was self-corrected before commit**: it gated invoice preparation itself, which would have broken `OPS-181`'s own disclosed legitimate-re-handoff allowance and an existing db-test fixture; the shipped fix gates `app.issue_finance_invoice`, the actual AR/GL posting boundary, instead); closes `ISS-2026-162`'s Finance/HRIS-Payroll scope (Medium, 10 functions total — the 6 named plus 4 more this checkpoint's own wider sweep found sharing the identical unguarded check-then-insert race — fixed with the codebase's own proven "design note 9(a)" pattern, 2 mechanisms live-forced with a genuine two-process race each, both now resolve gracefully); `ISS-2026-196` (Medium, `app.run_loyalty_expiry_sweep`'s own `p_as_of` parameter was silently ignored by both primitives it composes, now threaded through). **2 findings registered, not fixed** (`ISS-2026-197`, Low, owner `HDN-386`): no FX/multi-currency conversion exists anywhere in the revenue chain (structural absence, RPD-016 already defers it); `app.calculate_job_profitability` (Operations) always reports the static quote-time total rather than the actual invoiced figure (a planned-vs-actual split, not a formula defect). **`ISS-2026-162`'s residual 3 non-Finance functions plus `ISS-2026-163`** (same function family) explicitly handed to `HDN-387`, not silently dropped. Fresh full gate run green: `typecheck` 0, `lint` 0 errors/337 warnings, 5394/5394 unit tests, 229/229 db-tests (317 migrations), fresh disposable database with real grants. One additive migration, 5 existing db-test files extended with new regression coverage, zero application code, zero contract, zero route. Full disposition: `HDN-374.md` §6/§12 |

---

## 16. Next eligible prompt

> ### **No prompt is eligible to begin. `HDN-374` (Financial Integrity Audit, `CG-S15-HDN-006`) is `COMPLETED`, not yet `VERIFIED`.**
>
> Per `AGENTS.md`'s never-batch discipline: "Only the Tier C close moves a row to
> VERIFIED. No later prompt may begin until the current one is VERIFIED." `HDN-374`'s own
> investigation (4 lenses), fix pass (4 findings fixed, 1 self-corrected before commit, 1
> registered), regression tests, and a fresh full Tier A gate run are all done and
> committed — full disposition `HDN-374.md` §1/§6/§12. **Tier C review (§13) has not yet
> run.** This section will name the actual next-eligible prompt once `HDN-374` reaches
> `VERIFIED`.
>
> - **`HDN-373` is `VERIFIED`** — Tier C closed clean across two rounds, independent full
>   gate re-run green. Full disposition: `HDN-373.md` §13/§13.1.
> - **Carry-forward `HDN-374` resolved, so `HDN-375` does not discover these cold:**
>   1. **`HDN-BLK-010`/`ISS-2026-162`** — the Finance/HRIS-Payroll portion (10 functions:
>      the 6 originally named plus 4 more `HDN-374`'s own wider sweep found) is now
>      **`RESOLVED`**. The 3 non-Finance functions (Commercial, WMS, Platform/Auth) plus
>      `ISS-2026-163` (`app.prepare_job_order`'s defective exception handler, same
>      function family) were explicitly **not** fixed here (outside this lane's own
>      Financial Integrity charter) and are handed to **`HDN-387`**, not silently
>      dropped — see both issues' own `HDN-374` amendment for the full reasoning.
>   2. **`ISS-2026-186`** (Medium, owner `HDN-387`) — untouched by `HDN-374`'s own
>      investigation; still open, still `HDN-387`'s to resolve.
>   3. **`ISS-2026-197`** (Low, new, owner `HDN-386`) — no FX/multi-currency conversion
>      anywhere in the revenue chain; `app.calculate_job_profitability`'s own
>      planned-vs-actual figure split. Both structural/scope questions, not bounded
>      repairs — registered for `HDN-386`'s own cross-cutting scope ruling before Step 16
>      go/no-go.
