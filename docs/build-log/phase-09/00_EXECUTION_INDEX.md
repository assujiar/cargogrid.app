...
IAE-001 (kickoff)
   ...
   │
   ├──► API/Webhook Ecosystem track (needs 336's catalog + PLT-129 primitives)
   │    IAE-008 ──► IAE-009 (337 Public API Platform) ──► IAE-010 (338 Customer API)
   │                                                   └─► IAE-011 (339 Vendor API)
   │    IAE-008 ──► IAE-012 (340 Webhook Management) ──► IAE-013 (341 n8n Integration)
   │
...
| `CG-S14-IAE-009` | 337 | Public API Platform | `IAE-008` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green, db-test suite green including a self-caught `rbac-enforcement.sql` actor-identity fix); first prompt of Batch 3, awaits Batch 3's own Tier C review to reach `VERIFIED`. `docs/build-log/phase-09/IAE-337.md` |
| `CG-S14-IAE-010` | 338 | Customer API | `IAE-009` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green including a new `customer-api.test.ts`, db-test suite green including a self-caught `SECURITY DEFINER` gap on the widened `revoke_api_key`/`rotate_api_key` grant); second prompt of Batch 3, awaits Batch 3's own Tier C review to reach `VERIFIED`. `docs/build-log/phase-09/IAE-338.md` |
| `CG-S14-IAE-011` | 339 | Vendor API | `IAE-009` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green including a new `vendor-api.test.ts`, db-test suite green); third prompt of Batch 3, awaits Batch 3's own Tier C review to reach `VERIFIED`. `docs/build-log/phase-09/IAE-339.md` |
| `CG-S14-IAE-012` | 340 | Webhook Management | `IAE-008` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green including new tests against a real local HTTP server, db-test suite green after fixing a self-caught cross-file regression in `background-job.sql`); fourth prompt of Batch 3, awaits Batch 3's own Tier C review to reach `VERIFIED`. `docs/build-log/phase-09/IAE-340.md` |
| `CG-S14-IAE-013` | 341 | n8n Integration | `IAE-012` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green including a new `n8n-integration.test.ts`, db-test suite green including a self-caught test-authoring fix); fifth and FINAL prompt of Batch 3 — Batch 3 is now fully `COMPLETED` and awaits its own mandatory Tier C review before Batch 4 may begin. `docs/build-log/phase-09/IAE-341.md` |
