...
| `CG-S14-IAE-008` | 336 | Integration Hub | `IAE-001` | **`COMPLETED`** 2026-08-21 — Tier A/B clean (0 typecheck/lint errors, full test suite green, db-test suite green including a self-caught rbac-enforcement.sql fix); last prompt of Batch 2 -- Batch 2's own Tier C review is now due before `IAE-007`/`IAE-008` reach `VERIFIED`. `docs/build-log/phase-09/IAE-336.md` |
...
**`CG-S14-IAE-007`** (Prompt 335, Automation Rule Engine, Batch 2) was the first eligible prompt, released by Batch 1's own `VERIFIED` status per `AGENTS.md`'s cross-batch rule; both it and `CG-S14-IAE-008` (Prompt 336, Integration Hub) reached `COMPLETED` 2026-08-21, closing out Batch 2. Batch 2's own Tier C review (4 parallel adversarial lenses + fix pass + full gate re-run) is now due before either capability may be promoted to `VERIFIED`. Only after that close does `CG-S14-IAE-009` (Prompt 337, Public API Platform, Batch 3) become eligible.
