...
| `CG-S14-IAE-004` | 332 | Saved View and Configurable Report | `IAE-002`, `IAE-003` | **`READY`** — next eligible prompt, Batch 1 (released on `IAE-003` `COMPLETED` within the same batch, per `AGENTS.md`'s cross-batch rule) |
...
**`CG-S14-IAE-004` (Prompt 332, Saved View and Configurable Report).** `IAE-003` (Prompt 331, Dashboard Builder) reached `COMPLETED` 2026-08-21 (Tier A/B clean; full detail `docs/build-log/phase-09/IAE-331.md`) — released within Batch 1 per `AGENTS.md`'s cross-batch rule (a downstream prompt may begin on its upstream's `COMPLETED` status within the same batch). Batch 1 (`IAE-002..006`) reaches `VERIFIED` only at its own Tier C review close, after `IAE-006` (Prompt 334) also reaches `COMPLETED`.
