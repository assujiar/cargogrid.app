...
| Schema state | `app` is **not** exposed through the Data API, so no `app` table is reachable over PostgREST regardless of its RLS state (carried forward from Step 15 §2; not independently re-verified this checkpoint — `RGL-402` owns re-proving it) |
