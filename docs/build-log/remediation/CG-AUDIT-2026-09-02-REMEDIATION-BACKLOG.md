...
| B4 | Multi-currency postings summed as raw numbers, no FX/base-amount columns | `CODE-BIG` | **PARTIAL** | AR/AP exposure-summary cross-currency blend bug closed with honest per-currency + base-currency figures; `finance_journal_lines` retroactive FX persistence, `currency_restriction` enforcement at posting time, and true consolidated multi-currency financial statements remain deferred. A dedicated audit-prose re-verification (2026-09-26) found one more sub-claim from B4's own paragraph, present in its prose but never mentioned in this row or its execution log: "cross-currency receipt allocation is hard-rejected" -- confirmed still live and unconditional in `app.allocate_finance_receipt`'s own current body (`finance_receipt_currency_mismatch`, re-asserted verbatim across every redefinition through its latest, `20260902100000_harden_tenant_id_disclosure_finance.sql`). Correctly stays `DEFERRED_LARGE`, disclosed rather than silently dropped: relaxing the check without more would violate AGENTS.md's "balanced ... reconcilable" posting rule -- it needs a real accounting decision (which FX rate and when it locks, a realized-FX-gain/loss GL account that does not exist anywhere in the current chart of accounts, and whether cross-currency allocation should even be allowed vs. requiring an upstream conversion step), the same class of decision already deferred for B4's own sibling sub-claims |
...
| F4 | `has_active_tenant_membership` costs ~138µs/row, unindexable, no caching layer anywhere | `CODE-BIG` (research) | DEFERRED_LARGE | needs a load-bearing-function redesign, not a quick patch. A dedicated audit-prose re-verification (2026-09-26) found F4's own compounding-cost pattern recurs one join-level deeper than F4's row itself discusses: `app.shipment_leg_stops_select_scoped`/`shipment_leg_cargo_allocations_select_scoped`/`shipment_leg_custody_events_select_scoped` (`20260729290000_create_advanced_tms_multi_leg_shipment.sql`) each run a correlated 2-table join (`shipment_legs` JOIN `shipment_orders`) inside their own RLS `USING` clause, and `app.can_access_record` (confirmed via its own current body) itself calls `app.has_active_tenant_membership` plus up to 3 more EXISTS sub-selects -- so a read of these 3 tables can trigger `shipment_legs`' own RLS (itself another `can_access_record` call), `shipment_orders`' own RLS, and the outer table's own check, all per row. A same-shape, smaller-scope fix to F4's own (a `SECURITY DEFINER` helper collapsing the cascade into one non-RLS-triggering check, mirroring `can_access_record`'s own established wrapping pattern) is plausible, but deliberately NOT implemented in this pass: it is a genuine RLS-primitive change over 4 tables' own SELECT authorization, and this session's own standing discipline requires exactly the same "measure before optimizing" rigor AGENTS.md holds F4 itself to (no committed evidence exists yet, under real RLS enforcement at a realistic row count, that this cascade is an actual bottleneck rather than a theoretical one) plus full live-tested cross-tenant/sibling-org-unit/customer-layer regression coverage before touching a live SELECT policy on any of these 4 tables -- exactly the scrutiny AGENTS.md itself requires for "RLS primitives," never to be rushed. Recorded here as its own disclosed sub-finding rather than silently folded into F4's own already-deferred scope. A companion, separately dropped audit sub-claim -- "the only committed performance evidence for this schema was captured with RLS bypassed and at volumes far below production" -- is the actual prerequisite measurement this fix (and F4's own eventual fix) needs before either is attempted; also deliberately not run in this pass, for the same reason: it is real, scoped, bounded work (extend `scripts/load-tests/`'s own existing `pagination-explain.sh` precedent to a realistic seeded row count under a genuine `authenticated`+RLS session, per `procurement-scale-seed.sql`'s own scaling pattern), but is its own dedicated task, not a same-session tail addition after a long work session |
...
| UNTRACKED-F-tenant-index | Section F's own closing parenthetical separately states: "99 of 550 tenant-scoped tables (including `vehicle_current_positions`, `ticket_events`, the route-deviation/geofence tables, and the WMS order-line tables) lack any index leading on `tenant_id`" -- "the exact index coverage that would otherwise absorb some of F4's own RLS-predicate cost." Never mentioned anywhere in this backlog under any row | `CODE` (mechanical, additive) | **DONE** (batch 1+2, 2026-09-26) | Personally re-verified against the CURRENT live schema (not the audit's own 2026-09-02 snapshot): applied every migration to a fresh disposable database and queried `pg_class`/`pg_attribute`/`pg_index` directly. Result: 551 tenant-scoped tables today (ordinary growth since 2026-09-02), missing-index count STILL exactly 99 -- the gap is real and current, not stale. This is a mechanical, purely additive schema fix (the same pattern F5's own migration already established: `CREATE INDEX ... (tenant_id, ...)`, zero RLS/policy change), not a product decision. Batch 1 closed the exact 4 table groups the audit's own sentence named by name -- `vehicle_current_positions`, `ticket_events`, `shipment_leg_route_deviation_states`/`shipment_leg_stop_geofence_states` (the route-deviation/geofence tables), and `wms_inbound_order_lines`/`wms_outbound_order_lines`/`wms_package_lines`/`wms_shipment_issue_lines` (the WMS order-line tables) -- 8 tables. Batch 2 (2026-09-26) closed the remaining 91, re-querying the live schema to confirm the exact same list batch 1 had disclosed, generating every `CREATE INDEX` statement programmatically (not hand-typed, to eliminate transcription risk across 91 repeated statements) from that captured list. Plain single-column `(tenant_id)` indexes throughout (not a composite) since the audit's own framing is a coverage gap, not a specific slow query with a known secondary column -- a tuned composite would be speculative optimization ahead of a measured need. The schema-wide gap this item tracks is now genuinely zero, asserted directly by db-test (99 = 550 (now 551) tenant-scoped tables all carry a leading-`tenant_id` index). See execution log |
...
| E4 | Whole cost/document domains absent (fixed assets, maintenance, customs, BOM, …) | `PRODUCT` | NEEDS_PRODUCT_DECISION | A dedicated audit-prose re-verification (2026-09-26) found E4's own paragraph carries a second, distinct sentence never separately tracked: "no chargeable or volumetric weight anywhere." Confirmed no such capability exists uncredited anywhere in the schema -- `app.shipment_orders`' own `basis_weight_kg`/`basis_volume_cbm` columns exist purely for job-order capacity allocation (OPS-169), not rating; neither `app.customer_contract_price_components` nor `app.quotation_lines` has any weight/volume/dimension input to a pricing formula, and no table anywhere captures per-piece cargo dimensions. Correctly stays `NEEDS_PRODUCT_DECISION`: which modes get which volumetric-weight convention (air freight's own 6000cm³/kg divisor vs. sea/LCL/trucking's different or absent ones), whether/how per-piece L/W/H gets captured at all, and how a weight-based rating basis composes with the existing flat-lane/manual-line pricing model are all genuine pricing-policy decisions, the same "decide what CargoGrid is" class as E1's own already-ratified disposition |
| E5 | Telematics: device can never reach `installed` (blocked by A6); ETA is straight-line/40kmh; driver licence/vehicle serviceability unchecked at dispatch; overdue-arrival detection unenqueueable | `CODE` (installation-evidence upload wiring) + `CODE` (ETA speed half, closed 2026-09-26) / `DEFERRED_LARGE` (ETA distance half) + `CODE` (dispatch driver/vehicle checks, closed 2026-09-23) + untracked (scheduler wiring, does not survive re-verification -- see execution log) | **PARTIAL** | Split: the "device can never reach `installed`" half is now fixed -- `app.record_gps_device_installation` (ATW-226B) and its own db-test already fully built and exercised the evidenced-installation RPC; the real blocker was that no real migration ever registered the `gps_device_installation` document type (only six different db-test fixtures' own throwaway registrations did), so every real tenant's first upload would have failed `document_type_not_configured` before ever reaching its own per-tenant publish step, and no Server Action/UI ever called the upload+store+scan sequence at all. Both fixed: a new catalogue-registration migration plus a real "record installation" upload form in `fleet-panel.tsx`. "ETA is straight-line/40kmh" was carried as one undivided `DEFERRED_LARGE` item; a dedicated re-verification pass (2026-09-26) found it conflated two genuinely different gaps. The DISTANCE half (straight-line, no road-network/traffic awareness) really does need a mapping-API/routing-engine integration and stays `DEFERRED_LARGE`. The SPEED half -- a single hardcoded 40km/h constant (`app.route_planning_default_speed_kmh`, shared by exactly 2 consumers, not the "3+ capabilities" originally claimed) -- had a real, bounded, zero-external-API fix hiding in it: this codebase already ingests and stores real per-vehicle telemetry (`app.canonical_telemetry_events.speed_kmh`, ATW-226F, live-fed) that the flat constant never used. Closed by a new internal helper (`app._route_planning_effective_speed_kmh`) computing each vehicle's own recent observed average speed (>=5 qualifying samples in 30 days, implausible/idle readings excluded), falling back to the unchanged constant for an untracked vehicle -- purely additive, never worse than before. Two more sub-claims from the audit's own E5 paragraph, present in its prose but dropped from this table's original condensed summary: "driver licence expiry and vehicle serviceability are checked at neither assignment nor dispatch" is now closed (`app.evaluate_dispatch_readiness` gained two data-backed blocker checks); "overdue-arrival detection is not merely unscheduled but unenqueueable" does NOT survive independent re-verification at the scope originally proposed -- see execution log for the full disposition. A further audit-prose re-verification (2026-09-26) found a THIRD dropped E5 sub-claim, missed even by the row's own earlier dropped-sub-claim reconciliation: '"Multi-provider" third-party GPS accepts exactly one proprietary payload shape and CargoGrid''s own HMAC scheme.' Confirmed exactly accurate and unchanged in current code -- `app.ingest_third_party_provider_webhook_event` (`20260729380000_create_advanced_tms_third_party_provider_adapter.sql`) hardcodes one JSON contract regardless of `provider_code`, and that same migration's own header (design note 3) and the WBS row (`ATW-226E`, `external_evidence_status: CONDITIONALLY_SKIPPED_PROVIDER_UNAVAILABLE`) already candidly disclose this as a deliberate reference-contract choice, never a claim any named real vendor is live -- "multi-provider"/"Multi-Source" is internal spec/task-naming, confirmed absent from any customer-facing copy. Correctly stays undispositioned as a code fix and is now at least recorded: building genuine multi-vendor payload support needs picking specific real vendors and obtaining their live contracts (the codebase's own external-evidence policy already forbids a fake lowest-common-denominator parser as a substitute) -- a product/business decision, not a coding task |
...
- 2026-09-19 — B3 (credit notes half) closed, partial/milestone billing
  stays DEFERRED_LARGE (this commit). A dedicated research pass (recorded
  in the migration's own header) found B3 bundles two genuinely separable
  claims. "One issued invoice per job order, hard-capped" is confirmed
  accurate at the job-order level -- `finance_invoices_job_order_issued_
  unique` (a real unique partial index) plus an application-level
  pre-check in `app.issue_finance_invoice` both enforce it, and every
  issued invoice bills the job's full `revenue_snapshot` total with no
  partial-portion parameter anywhere. Genuine partial/milestone billing
  needs a real billing-model product decision this session cannot make
  unilaterally -- stays `DEFERRED_LARGE`, untouched by this migration.
  "No credit notes" turned out WORSE than the audit's own "all-or-nothing
  void" framing: there is no way to correct an issued invoice AT ALL
  today, not even a full void -- `app.discard_finance_invoice_draft` only
  accepts `status in ('draft','submitted')`; no `app.void_finance_invoice`
  exists anywhere (confirmed by grep); `app.finance_ar_open_items`
  structurally forbade a negative/credit row. This half is structurally
  independent of the job-order/partial-billing question -- a credit note
  is a new row/table referencing an already-issued invoice, never
  mutating `finance_invoices.job_order_id`/`billing_readiness_handoff_id`
  cardinality -- and there is already a working precedent for AR-
  collectible diverging from an invoice's own face value (B5's own
  withholding-tax-net-of-AR fix).
  Closed: new migration `20260919020000_b3_finance_credit_note.sql` adds
  `app.finance_credit_notes` (a new, append-only header/lineage table --
  id, tenant, invoice, customer account, currency, amount, mandatory
  reason, idempotency key, issued-by/at) and `app.issue_finance_credit_
  note` (FIN:Edit-gated, idempotent on `(tenant_id, idempotency_key)`,
  mandatory reason, a caller-supplied credit date mirroring `app.issue_
  finance_invoice`'s own `p_issue_date`). It validates the target
  invoice is genuinely `issued`, caps cumulative credits against the
  invoice's own original AR amount (never letting repeat credits exceed
  what was ever billed), then posts a real, negative AR open item via
  the ALREADY-EXISTING, already-tested `app.post_finance_ar_open_item`
  (`CREATE OR REPLACE`, same signature -- widened for a new `credit_note`
  source type with its own negative-amount rule) -- the exact "reuse the
  shared primitive, don't invent a second posting path" pattern this same
  session's own E3 fix established for `app.post_inventory_movement`.
  `app.finance_ar_open_items`'s own CHECK constraints widened in step:
  `source_document_type` admits `credit_note`; `original_amount`'s sign
  rule splits by type (negative only for `credit_note`); `allocated_
  amount` is pinned to exactly 0 for a `credit_note` row (never allocated
  against in this bounded core). `app.search_finance_ar_candidates_for_
  receipt` (FIN-198, `CREATE OR REPLACE`, same signature) narrowed with
  `and original_amount > 0` -- a standing credit balance is never a
  sensible receipt-allocation target.
  ...
  scoping precedent -- `app.finance_ar_open_items`'s own header states
  "Carries no GL journal line -- FIN-202/203's own scope" and `app.
  finance_receipts` carries the identical disclosure. The credit note's
  own AR-side effect (reducing the customer's net collectible balance) is
  real and closes the audit's own literal complaint, already correctly
  reflected in B4's own generalized per-currency exposure-summary
  aggregation with zero further change needed there. Also out of scope:
  any "apply this credit to a future invoice" allocation flow -- the
  credit stands as an independent reduction to the customer's aggregate
  AR exposure.
  The TS/UI blast radius: new `server/contracts/finance-credit-note/
  finance-credit-note.ts` and `server/mutations/finance-credit-note.ts`;
  `finance/invoices/actions.ts` gains `issueFinanceCreditNoteAction`
  (validates amount/reason/credit-date), `invoice-forms.tsx` gains
  `IssueFinanceCreditNoteForm`, wired into `finance/invoices/page.tsx`'s
  Actions column for every `issued`-status row -- a fresh idempotency key
  generated once per page render via `randomUUID()` and bound into the
  action, the same precedent `customer-portal-users/page.tsx` already
  established (a double-click/network retry of the same rendered form
  never double-posts; a fresh page load gets a fresh key).
  Three real, independently-caught bugs during this pass's own db-test
  verification, all fixed before ever reaching the full suite: (1) the
  first draft never widened `app.validate_finance_open_item_source`
  (ISS-2026-319) for the new `credit_note` source type -- that guard's
  own deliberate else-branch (present specifically so a CHECK-constraint
  widening without a matching lineage rule fails loudly instead of
  silently reopening a fabricated-source-id gap) raised on the very
  ...
  credit_note` originally posted with `current_date` instead of a
  caller-supplied credit date -- caught the moment the db-test ran
  against a fiscal calendar that does not cover today's real wall-clock
  date; fixed by adding an explicit `p_credit_date` parameter, mirroring
  `app.issue_finance_invoice`'s own `p_issue_date` exactly (both the
  `app.*` function and its `public.*` wrapper updated together). (3) this
  migration's own `CREATE OR REPLACE` of `app.search_finance_ar_
  candidates_for_receipt` initially omitted the pre-existing `SECURITY
  DEFINER`/`search_path`/`has_active_tenant_membership` check the live
  function already carried (`20260902100000`) -- `CREATE OR REPLACE`
  does not preserve those unless restated, so the omission silently
  downgraded it to `SECURITY INVOKER`, caught by `public-api-wrapper-
  regression.sql`'s own definer/invoker-mismatch assertion before this
  ever shipped.
  New db-test coverage: `scripts/db-tests/finance-accounts-receivable.
  sql` gained a dedicated block proving `app.issue_finance_credit_note`
  posts a real negative AR item, caps cumulative credits, rejects a
  not-yet-issued invoice/empty reason/null credit date/no-FIN-grant
  caller, and is idempotent on its own key; `scripts/db-tests/finance-
  receipt-allocation.sql` gained a block proving a `credit_note`-type AR
  item is excluded from `app.search_finance_ar_candidates_for_receipt`'s
  own results while a genuinely separate positive invoice item posted in
  the same block correctly still appears. New unit test coverage: `server/
  contracts/finance-credit-note/finance-credit-note.test.ts` and `server/
  mutations/finance-credit-note.test.ts`.
  Still deferred, correctly out of this bounded core's own scope:
  partial/milestone billing (needs a real billing-model product
  decision) and GL journal reversal for a credit note's own revenue/tax
  (a genuinely separate, larger accounting-correctness undertaking, the
  same class of decision B6b's own internal-cost-to-GL gap was correctly
  left open rather than solved with a governance-bypassing shortcut). B3
  is PARTIAL, not DONE.
  Full Tier A gates verified clean: `typecheck`, full `lint` (0 errors,
  ...
  full `pnpm run db:test` (`ALL PASSED`, 560 migrations / 279 db-test
  files -- critical here since two SHARED primitives were widened, `app.
  post_finance_ar_open_item` and `app.search_finance_ar_candidates_for_
  receipt`, and every existing caller of both stayed unaffected),
  `git:check-paths`, `security:check`, `release:check-freeze` (HUNDRED-
...
- 2026-09-26 — UNTRACKED-F-tenant-index (batch 1 of a CODE-BIG
  ...
  trigger and not run.
- 2026-09-26 — Remaining 5 candidates from the same audit-prose sweep
  disclosed (documentation-only, no code changes). The sweep (see the
  UNTRACKED-A1/UNTRACKED-A2-dedup/UNTRACKED-C-PPN/UNTRACKED-F-tenant-
  index entries above) surfaced 9 candidates total; 4 were implemented
  (E1, E5-ETA, UNTRACKED-A1, UNTRACKED-A2-dedup) plus the tenant-index
  and PPN disclosures. The remaining 5 were each independently
  adversarially re-verified and confirmed real and genuinely dropped
  from tracking, but every one needs a real product/accounting/legal
  decision or a level of RLS-primitive care this session's own standing
  discipline will not rush at the tail of an already-long work session
  -- recorded as disclosed disposition updates on their existing rows
  rather than silently left out or forced through:
  - B4 (Money): "cross-currency receipt allocation is hard-rejected" --
    confirmed still live and unconditional in `app.allocate_finance_
    receipt`'s own current body across every redefinition. Needs a real
    accounting decision (FX-rate timing, a realized-FX-gain/loss GL
    account that does not exist yet, whether cross-currency allocation
    should be allowed at all) -- the same class already deferred for
    B4's own sibling sub-claims. Stays `DEFERRED_LARGE`.
  - F4 (its own row): the exact same RLS-predicate compounding pattern
    F4 already names recurs one join-level deeper on
    `shipment_leg_stops`/`shipment_leg_cargo_allocations`/
    `shipment_leg_custody_events`'s own SELECT policies -- confirmed by
    reading `app.can_access_record`'s current body (it itself calls
    `app.has_active_tenant_membership`) and all 4 policies'
    `20260729290000_create_advanced_tms_multi_leg_shipment.sql` source.
    A same-shape `SECURITY DEFINER`-wrapping fix is plausible but
    deliberately NOT implemented: no measured evidence yet exists that
    this cascade is a real bottleneck under realistic, RLS-enforced
    volume (the same "measure before optimizing" bar F4 itself is held
    to), and any live SELECT-policy change on 4 tables needs full
    live-tested cross-tenant/sibling-org-unit/customer-layer regression
    coverage this session will not rush. A companion dropped sub-claim
    -- the only committed performance evidence for this schema was
    captured with RLS bypassed and at unrealistic volume -- is the
    actual prerequisite measurement, itself real, bounded work
    (extending `scripts/load-tests/pagination-explain.sh`'s own
    precedent under a genuine RLS-enforced session), also deliberately
    left for a dedicated future task rather than a rushed tail-of-
    session addition.
  - E4 (Domain modeling): "no chargeable or volumetric weight anywhere"
    -- confirmed no such capability exists uncredited anywhere (the
    existing `basis_weight_kg`/`basis_volume_cbm` columns are for
    job-order capacity allocation, not rating). Needs real pricing-
    policy decisions (per-mode volumetric conventions, per-piece
    dimension capture, how it composes with the existing flat/manual
    pricing model) -- the same "decide what CargoGrid is" class as E1's
    own already-ratified disposition. Stays `NEEDS_PRODUCT_DECISION`.
  - E5 (its own row): '"Multi-provider" third-party GPS accepts exactly
    one proprietary payload shape and CargoGrid''s own HMAC scheme' --
    confirmed accurate and unchanged; the ingestion migration's own
    header and the WBS row (`ATW-226E`) already candidly disclose this
    as a deliberate single-reference-contract choice, "multi-provider"
    being internal spec-naming, not customer-facing copy. Needs a real
    business decision (which named real vendors to integrate, requiring
    their live contracts) before any adapter code can be written --
    this repository's own external-evidence policy already forbids a
    fake lowest-common-denominator parser as a substitute.
  No code, migration, or test changed by this entry -- documentation-
  only disposition updates on 3 existing rows (B4, F4, E4) plus one
  sub-clause on E5's own row. `git:check-paths`/`security:check` re-run
  clean regardless.
- 2026-09-26 — UNTRACKED-F-tenant-index CLOSED (batch 2 of 2). Batch 1
  (see the entry above) closed the 8 tables the audit's own sentence
  named explicitly and disclosed the remaining 91 as a separate,
  deliberate follow-up. This entry closes that follow-up in full.
  Re-verified the live schema again before writing anything: applied
  every migration (now including batch 1 itself) to a fresh disposable
  database via the same `lib/setup-disposable-db.sh` harness and
  re-ran the identical `pg_class`/`pg_attribute`/`pg_index` query used
  for batch 1. Got back exactly the same 91-table list batch 1 had
  already disclosed by name in its own row -- confirming no other
  migration touched any of these tables' indexes in the interim, and
  that the list going into this migration was genuinely re-derived
  from the live query, not copied from memory or hand-compiled.
  Every one of the 91 `CREATE INDEX ... (tenant_id)` statements, and
  the corresponding 91-entry db-test fixture array, was generated
  programmatically by a Python script from that captured table list --
  never hand-typed -- specifically to eliminate transcription risk
  across that many repeated, easy-to-mistype statements (a table name
  typo would silently create an index on the WRONG table, or the
  script would simply fail loudly against a real disposable database,
  either of which is a real risk at this volume that this session's
  own established "generate, don't hand-type, for repetitive
  mechanical bodies" discipline exists to prevent). Also verified
  programmatically that none of the 91 generated index identifiers
  (`<table>_tenant_idx`) exceed Postgres' 63-character limit (the
  longest, `onboarding_checklist_template_task_dependencies_tenant_idx`,
  is 58 characters).
  New db-test coverage: a new `scripts/db-tests/untracked-f-tenant-
  index-coverage-batch2.sql` asserts all 91 named tables now carry a
  leading-`tenant_id` index, AND separately asserts the schema-wide
  remaining-gap count is exactly 0 -- the authoritative "this item is
  now genuinely closed" proof, not merely "these specific 91 got
  indexed."
  The first `pnpm run db:test` run after writing this migration caught
  that batch 1's OWN db-test had gone stale: its "exactly 91 still
  missing" assertion was written before batch 2 existed, and because
  db-tests always run against the fully-migrated schema (every
  migration -- including this one -- applies before any test file
  runs), that assertion would now see the count already at 0 and fail.
  Fixed by narrowing batch 1's own check from an exact-91 equality to
  an at-most-91 regression guard (batch 1's own 8 additions must never
  have been silently reverted or skipped) -- an honest correction of
  an assertion that baked in a since-falsified assumption (that no
  later, independent batch would ever land), not a weakening to hide a
  real regression; the underlying 8-table coverage batch 1 itself
  proves is untouched and still checked exactly as before.
  Full Tier A gates verified clean: `typecheck` (0 errors), full `lint`
  (0 errors, only pre-existing warnings -- no app/ file touched by
  this pass), the full unit test suite (6182/6182, including the
  release-freeze self-test after this digest update -- the first run
  caught the digest mismatch from the two new files, fixed before this
  pass was ever trusted), a full `pnpm run db:test` (`ALL PASSED`, 570
  migrations / 282 db-test files -- the first run caught batch 1's own
  stale assertion, fixed before this pass was ever trusted),
  `git:check-paths`, `security:check`, `release:check-freeze`
  (HUNDRED-AND-SEVENTY-SECOND PASS, both digests updated -- one new
  migration file, one new db-test file plus a correction to batch 1's
  own). No app/, components/, or "use server" file touched by this
  pass -- `next build` not required by this file's own Tier A trigger
  and not run.
  This closes the entire UNTRACKED-F-tenant-index finding: every one
  of the 99 tenant-scoped tables the audit's own section-F closing
  parenthetical named as lacking a leading-`tenant_id` index now has
  one, verified live and asserted by a real, executable db-test that
  the schema-wide gap is genuinely zero.
