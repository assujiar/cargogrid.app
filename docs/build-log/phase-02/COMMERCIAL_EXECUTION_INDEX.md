...
**Status:** `PHASE_2_IN_PROGRESS` — updated at `COM-149` (Rate and Cost Lookup, `VERIFIED`); this document itself remains index/planning only and performs no Commercial-domain runtime source/schema change (each capability's own migration/code lives in its own commit, cited per row)
...
| `009` | `150` Margin calculation | Costing and Pricing / Commercial Pricing | `READY` | `COM-149` (`VERIFIED`) | — | `docs/build-log/phase-02/COM-150.md` | Runtime build agent | `CG-S7-COM-010` |
...
**Tally (updated at `COM-149`, row `008` — Rate and Cost Lookup `VERIFIED`):** of the 24 rows in this index (`142`–`165`), **8 are `VERIFIED`** (kickoff, Lead Management, Prospect Lifecycle, Contact and Activity Management, CRM Sales Plan and Pipeline, Opportunity Management, RFQ and Costing Request, Rate and Cost Lookup) and **1 is `READY`** (`150`, Margin Calculation — its dependency, `149`, now `VERIFIED`). The remaining 15 rows are `BLOCKED` on their own not-yet-started predecessor. [Historical: at `COM-148`'s own checkpoint, the tally was 7 `VERIFIED`, 1 `READY`, 16 `BLOCKED` — retained here only as a superseded data point, not current state.]
...
| Migrations | `supabase/migrations/` listing | 39 migrations as of `COM-149` (32 Platform Core-scoped plus `20260723090000_create_commercial_lead_management.sql` (`COM-143`), `20260723120000_create_commercial_prospect_lifecycle.sql` (`COM-144`), `20260723150000_create_commercial_contact_activity_management.sql` (`COM-145`), `20260723180000_create_commercial_sales_pipeline.sql` (`COM-146`, also carries a corrective `CREATE OR REPLACE` of `app.can_access_record`, `PLT-114`), `20260723210000_create_commercial_opportunity_management.sql` (`COM-147`, extends `app.resolve_commercial_record_ref` and the `contact_links`/`activities` polymorphic CHECK constraints to accept `'opportunity'`), `20260724090000_create_commercial_costing_request.sql` (`COM-148`, also seeds a new `('View cost', 'COM', ...)` permission catalogue row), and `20260724150000_create_commercial_rate_cost_lookup.sql` (`COM-149`, adds `app.vendor_rate_versions`/`app.rate_selections` on top of `PLT-120`'s master-data foundation per `ADR-0015`, and drops+recreates `app.v_active_vendor_rates` to be validity/approval-aware — disclosed in `COM-149.md` §3.3/§6, its one pre-existing consumer updated to match). `150` (Margin Calculation) will be the next Commercial migration. |
| Schema | `05_DATABASE_SCHEMA_WORKSTREAM.md` §3/§4 | Single `app` schema, unchanged — no Quotation table exists yet. No collision possible at this checkpoint. |
| Application code | `git ls-files app/ lib/ server/ components/ \| grep -iE "customer\|lead\|prospect\|contact\|activit\|pipeline\|opportunity\|costing\|quote\|rate"` | As of `COM-149`: real, expected matches under `app/(tenant)/[tenantSlug]/commercial/{leads,prospects,contacts,pipeline,opportunities,costing-requests,rates}/`, `server/{contracts,queries,mutations}/{lead,prospect,contact,pipeline,opportunity,costing,rate}*` — the scope `143`–`149`'s own prompts authorize. Zero match for `customer`/`quote` — those remain out of scope until their own capability prompts (`151`, `155`) run. |
...
**Result: zero file/schema/environment collision found.** `150` (Margin Calculation) is dependency-`READY` and may proceed under this session's own open-ended authorization.
