...
| `010` | `151` Quotation builder | Quotation Lifecycle / Customer Offer | `READY` | `COM-147..150` (all `VERIFIED`) | — | `docs/build-log/phase-02/COM-151.md` | Runtime build agent | `CG-S7-COM-011` |
...
**Tally (updated at `COM-150`, row `009` — Margin Calculation `VERIFIED`):** of the 24 rows in this index (`142`–`165`), **9 are `VERIFIED`** (kickoff, Lead Management, Prospect Lifecycle, Contact and Activity Management, CRM Sales Plan and Pipeline, Opportunity Management, RFQ and Costing Request, Rate and Cost Lookup, Margin Calculation) and **1 is `READY`** (`151`, Quotation Builder — its dependencies `147..150` all now `VERIFIED`). The remaining 14 rows are `BLOCKED` on their own not-yet-started predecessor. [Historical: at `COM-149`'s own checkpoint, the tally was 8 `VERIFIED`, 1 `READY`, 15 `BLOCKED` — retained here only as a superseded data point, not current state.]
...
| Migrations | `supabase/migrations/` listing | 40 migrations as of `COM-150` (32 Platform Core-scoped plus `20260723090000_create_commercial_lead_management.sql` (`COM-143`), `20260723120000_create_commercial_prospect_lifecycle.sql` (`COM-144`), `20260723150000_create_commercial_contact_activity_management.sql` (`COM-145`), `20260723180000_create_commercial_sales_pipeline.sql` (`COM-146`, also carries a corrective `CREATE OR REPLACE` of `app.can_access_record`, `PLT-114`), `20260723210000_create_commercial_opportunity_management.sql` (`COM-147`, extends `app.resolve_commercial_record_ref` and the `contact_links`/`activities` polymorphic CHECK constraints to accept `'opportunity'`), `20260724090000_create_commercial_costing_request.sql` (`COM-148`, also seeds a new `('View cost', 'COM', ...)` permission catalogue row), `20260724150000_create_commercial_rate_cost_lookup.sql` (`COM-149`, adds `app.vendor_rate_versions`/`app.rate_selections` on top of `PLT-120`'s master-data foundation per `ADR-0015`, and drops+recreates `app.v_active_vendor_rates` to be validity/approval-aware), and `20260724180000_create_commercial_margin_calculation.sql` (`COM-150`, adds `app.margin_rule_versions`/`app.margin_calculations` reusing `app.rate_selections`/`app.has_view_cost`/`app.has_view_selling_price` directly). `151` (Quotation Builder) will be the next Commercial migration. |
...
| Application code | `git ls-files app/ lib/ server/ components/ \| grep -iE "customer\|lead\|prospect\|contact\|activit\|pipeline\|opportunity\|costing\|quote\|rate\|margin"` | As of `COM-150`: real, expected matches under `app/(tenant)/[tenantSlug]/commercial/{leads,prospects,contacts,pipeline,opportunities,costing-requests,rates,margin-rules}/`, `server/{contracts,queries,mutations}/{lead,prospect,contact,pipeline,opportunity,costing,rate,margin}*` — the scope `143`–`150`'s own prompts authorize. Zero match for `customer`/`quote` — those remain out of scope until their own capability prompts (`151`, `155`) run. |
...
**Result: zero file/schema/environment collision found.** `151` (Quotation Builder) is dependency-`READY` and may proceed under this session's own open-ended authorization.
