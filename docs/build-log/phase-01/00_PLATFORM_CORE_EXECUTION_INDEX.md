...
**Tally (updated at `PLT-135`, row `032`):** of the 37 rows in this index (`104`–`140`), **32 are `VERIFIED`** (`104`–`135`, kickoff plus 31 capabilities) and **5 are `BLOCKED`** (`136`–`140`). `136` (Supreme Admin portal) is the other place `00_PLATFORM_CORE_WBS.md` §6 Lane G names a "required subset" (`105..135`) rather than an exact ID list — deliberately not resolved to `READY`/`VERIFIED` by this tally update; `136`'s own kickoff must perform the same reconciliation `135`'s own kickoff performed (`docs/build-log/phase-01/PLT-135.md` §2). `137`–`140` remain `BLOCKED` transitively behind `136`. No task is `BLOCKED` for a missing-evidence or unresolved-ADR reason. [Historical: at this index's own authoring checkpoint (`PLT-104`), the tally was 1 `READY` (`PLT-105`) and 35 `BLOCKED` — retained here only as a superseded data point, not current state; the one still-open Platform Core-scoped item from that checkpoint (`server/contracts/` folder timing) was resolved as a WBS convention there (`00_PLATFORM_CORE_WBS.md` §3).]
