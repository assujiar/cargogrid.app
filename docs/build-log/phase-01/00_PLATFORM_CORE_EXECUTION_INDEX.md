...
**Tally (updated at `PLT-134`, row `031`):** of the 37 rows in this index (`104`–`140`), **31 are `VERIFIED`** (`104`–`134`, kickoff plus 30 capabilities) and **6 are `BLOCKED`** (`135`–`140`). `135`/`136` (Tenant Admin/Supreme Admin portals) are the one place `00_PLATFORM_CORE_WBS.md` §6 Lane G names a "required subset" rather than an exact ID list — deliberately not resolved to `READY` by this checkpoint's own tally update, since `135`'s own kickoff is the task responsible for reconciling exactly which subset of `106`–`134` it needs, per that document's own disclosed convention, not a mechanical count this index can shortcut. `137`–`140` remain `BLOCKED` transitively behind `135`/`136`. No task is `BLOCKED` for a missing-evidence or unresolved-ADR reason. [Historical: at this index's own authoring checkpoint (`PLT-104`), the tally was 1 `READY` (`PLT-105`) and 35 `BLOCKED` — retained here only as a superseded data point, not current state; the one still-open Platform Core-scoped item from that checkpoint (`server/contracts/` folder timing) was resolved as a WBS convention there (`00_PLATFORM_CORE_WBS.md` §3).]
