...
## Update (Checkpoint 7, 2026-07-27, out-of-band)

Closed two more `migration-map.ts` items: every route-level `loading.tsx`'s hand-rolled `animate-pulse` bars, and every page's inline `role="alert"` load-failure block. Full detail: `07_GAP_ANALYSIS_AND_ROADMAP.md` §13, `docs/runtime/CHANGE_MANIFEST.md` `CHG-2026-098`. 28 pages now render `ErrorState` on load failure; 27 `loading.tsx` files now render `SkeletonTable` (the 13 whose route is a single real `DataTable`, with that page's own real column count) or `SkeletonText` (the other 14 — detail pages and mixed/non-tabular list pages). No new component was built. `migration-map.ts` now has only `EmptyState`, the 39-file form-field-primitive migration, and the two-form `ApprovalDecisionPanel` migration still open, each with its own disclosed reason.

## Update (Checkpoint 8, 2026-07-27, out-of-band)

A scoped-down slice of the form-field-primitive migration item: every raw `<input>`/`<select>`/`<textarea>` in a Commercial form using the unmodified standard className now renders through `Input`/`Select`/`Textarea` (26 files, ~40 elements). Full detail: `07_GAP_ANALYSIS_AND_ROADMAP.md` §14, `docs/runtime/CHANGE_MANIFEST.md` `CHG-2026-099`. Four sub-categories deliberately deferred and named: width-customized inputs (no `tailwind-merge`-style utility exists yet to safely override `Input`'s hardcoded `w-full`), checkbox/radio inputs (`Checkbox`/`Radio` change DOM structure, not a drop-in swap), `FormField` (every current form has one whole-form error, not per-field data to bind `FormField.error` to), and the two `app/(public)/` files (outside this item's own Commercial-module scope). A real regex bug (naive `[^>]*` attribute matching breaking on `onChange={(e) => ...}`'s embedded `>`) was found and fixed mid-migration, then every diff was manually re-read to confirm no other instance survived.
