...
> **Correction (2026-09-01, `ISS-2026-138`):** the `RPD-023` row's WBS ID(s) cell names only `PLT-107..108` (Phase 1). That covers RPD-023's own always-on current-auth-identity half (`app.assert_actor_is_session_identity`), but not its step-up/mandatory-MFA half, which was built later and is not named here: `IAE-027` (`20260807100000_create_intelligence_enterprise_mfa_session_controls.sql`, the MFA policy/challenge primitives), the `evaluate_permission` step-up enforcement branch (`20260830110000`, `ISS-2026-236`), and the `LYL:Configure` platform-default classification that closes the last gap in it (`20260831330000`, `ISS-2026-138`). No cell is rewritten; this note names the real implementing artefacts alongside the original.
