...
| PH-002 (resolved) | `docs/runtime/*.md` (7 files) | `CONFIRMED_DEAD` → **remediated this checkpoint** | High | Was reachable and could be mistaken for canonical (AGENTS.md pointed to it) | Governance/orientation accuracy for future agents | Medium (now mitigated) | Diff evidence in `docs/discovery/01_REPOSITORY_INVENTORY.md` §8 item 4; `KNOWN_ISSUES.md` `KI-004` | Runtime build agent | Superseded banners added to all 7 files and `AGENTS.md` repointed to root this checkpoint (see `CHANGE_MANIFEST.md`); full removal remains a follow-up cleanup task since deletion of governance history is a distinct, larger decision |
