...
**Prompt ID:** `CG-S13-CPL-005`  
**Package document:** `CG-AABPP-CPL-303`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-303.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-005` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Commercial Self-Service; Epic: Customer Booking; Capability: Booking; Feature slice: Customer booking creation from accepted quote or allowed service with schedule, pickup/delivery and documents; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement customer booking self-service that creates canonical shipment/job handoff records only through approved Commercial/Operations contracts.

## 5. Business value

Let customers initiate shipments faster while keeping operational feasibility and approval controls intact.

## 6. Source requirement

CPT-QBK-001..004; Brief Booking shipment; UX Booking; Delivery Phase 8.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300, CPL-302 and verified Commercial quote/job plus Operations shipment intake contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-304..308 tracking, ePOD and document center; Finance billing readiness and tickets.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Store portal booking draft/submission metadata, idempotency, customer references, linked accepted quote/job/shipment and audit without duplicating shipment truth. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Booking draft/submit/reschedule/cancel/status APIs with shared validation, service availability, idempotency, file scan and handoff semantics. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Booking form with quote selection, cargo, pickup/delivery, schedule, documents, contact, instructions, status and conflict/denied states. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define accepted-quote and direct-booking eligibility rules.
- Implement booking submit with canonical job/shipment handoff and idempotent retry.
- Build responsive booking UX with reusable address/site/contact/document components.
- Add reschedule/cancel request workflow when operational work already exists.
- Test duplicate booking, out-of-scope quote, invalid site, file scan and handoff audit.

## 21. Main flow

Customer selects an accepted quote or eligible service, enters pickup/delivery details, submits booking and receives a canonical booking/shipment reference.

## 22. Alternative flow

Customer requests reschedule/cancellation; internal Operations approves or denies with visible reason and preserved history.
...
- Booking cannot bypass Commercial approval, service eligibility or Operations acceptance controls.
- Customer-provided account/site/contact must match resolved membership scope.
- A portal booking becomes operational truth only after canonical handoff acceptance.
- Changes after operational assignment become requests, not direct edits to dispatch/WMS/Finance.
- No offline booking queue is claimed; online-first responsive PWA only.
...
- Booking unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Booking.
...
- Booking works with canonical source-domain ownership and no duplicate truth.
