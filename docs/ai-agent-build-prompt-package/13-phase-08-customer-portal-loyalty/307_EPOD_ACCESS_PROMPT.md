# Prompt 307 — ePOD Access

**Prompt ID:** `CG-S13-CPL-009`  
**Package document:** `CG-AABPP-CPL-307`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-307.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-009` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Documents; Epic: Proof of Delivery; Capability: ePOD Access; Feature slice: Customer access to permitted ePOD files and delivery confirmation metadata; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Expose ePOD to customers through private scanned files and scope-bound signed URLs while preserving Operations document authority.

## 5. Business value

Let customers retrieve delivery evidence quickly without insecure document sharing.

## 6. Source requirement

CPT-TRK-001..004; Brief Akses ePOD; UX Document & ePOD; Delivery signed file gate.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300, Operations ePOD contracts and Platform file scanning/signed URL contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-308, CPL-311 and tickets/claims.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Store only customer-visible ePOD projection metadata, file reference, scan status, access audit and source version; source ePOD remains Operations-owned. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

ePOD list/detail/download/request-copy APIs enforcing file scan, scope, short-lived URL and audit parity in REST/GraphQL. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

ePOD tab/list with delivery status, signer metadata allowed by policy, download action, unavailable/quarantine/denied states and audit-safe previews. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Classify ePOD metadata and file visibility by customer scope.
- Implement signed download and preview APIs with file-scan gate.
- Build responsive ePOD document UX and unavailable/quarantine states.
- Add download/access audit and legal retention references.
- Test unscanned/quarantined file, revoked access, forged document ID and replayed signed URL.

## 21. Main flow

Customer opens a delivered shipment and downloads a scanned, permitted ePOD through a short-lived scope-bound signed URL.

## 22. Alternative flow

If ePOD is not yet available or quarantined, show exact status and allow ticket/request without leaking internal notes.
...
- Every ePOD download requires current scope and file scan pass.
- Signed URLs are short-lived and cannot be reused after revocation or expiry.
- Customer-visible signer/location fields must follow policy and minimize personal data.
- Portal cannot replace, delete or falsify Operations ePOD evidence.
- Document access is audited with actor, scope, source and outcome.
...
- ePOD Access unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to ePOD Access.
...
- ePOD Access works with canonical source-domain ownership and no duplicate truth.
