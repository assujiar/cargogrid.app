# Prompt 300 — Customer User Scope

**Prompt ID:** `CG-S13-CPL-002`  
**Package document:** `CG-AABPP-CPL-300`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-300.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-002` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Identity and Scope; Epic: Customer Access Boundary; Capability: Customer User and Company/Account/Site Scope; Feature slice: Layer 4 customer membership, account/site grants, delegated permission and portal session scope; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement the Customer User access boundary so every portal read/action is resolved from tenant-owned company/account/site membership rather than route parameters or request payloads.

## 5. Business value

Prevent cross-customer leakage while enabling external customer teams to self-serve safely.

## 6. Source requirement

CPT-QBK-001..004, CPT-TRK-001..004, CPT-WHS-001..004, CPT-BIL-001..004, CPT-CX-001..004; Brief §3.4; UX Layer 4; Delivery Phase 8.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-299; verified Platform identity/access/config/files/jobs, Commercial customer/quote, Operations shipment/ePOD, WMS customer-owner projections, Finance invoice/payment and Ticketing customer-channel contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-301..327 as the mandatory scope primitive for all portal and loyalty flows.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Create/extend customer portal membership, account/site assignment, delegated permissions, invite state, scope snapshot and audit tables without duplicating Platform users or customer master truth. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Expose scoped membership/session/scope-preview APIs and shared authorization adapters used by every Customer Portal REST/GraphQL endpoint. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Customer admin scope preview, user scope badge, denied-state explanations and account/site switcher with no data visible before server scope confirmation. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Model customer portal membership and scope resolution from canonical customer/company/account/site records.
- Implement database/service policy that joins every portal entity to effective membership scope.
- Add scope preview, session context, account/site selector and denied-state UX.
- Test forged route IDs, copied tokens, revoked membership and account/site boundary cases.
- Publish scope contract for all downstream portal prompts.

## 21. Main flow

A customer user signs in, server resolves active tenant/company/account/site membership, returns a scoped portal context and every downstream list/detail/action uses that context.

## 22. Alternative flow

Customer admin invites a user, limits them to selected sites/accounts, previews effective access, then revokes or changes scope with immediate downstream effect.
...
- Customer scope is never taken from browser state, URL, filter or payload alone.
- A portal user may belong to multiple customer accounts only through explicit active membership grants.
- Customer admin can manage portal users only inside their customer company/account/site scope.
- Revocation invalidates sessions, saved views, exports, signed URLs and cached summaries.
- Internal users do not become Layer 4 users unless separate customer membership exists.
...
- Customer User and Company/Account/Site Scope unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Customer User and Company/Account/Site Scope.
...
- Customer User and Company/Account/Site Scope works with canonical source-domain ownership and no duplicate truth.
