# Prompt 302 — Request Quotation

**Prompt ID:** `CG-S13-CPL-004`  
**Package document:** `CG-AABPP-CPL-302`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-302.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-004` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Commercial Self-Service; Epic: Quote Request Intake; Capability: Request Quotation; Feature slice: Customer quote request with cargo, route, service, attachment, status and internal Commercial handoff; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Allow scoped customer users to submit and monitor quote requests that hand off to the canonical Commercial quotation pipeline without creating duplicate quote truth.

## 5. Business value

Reduce manual sales intake while preserving pricing approval and Commercial ownership.

## 6. Source requirement

CPT-QBK-001..004; Brief Request quotation; UX Request Quote; Charter Phase 8.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300 and Commercial quote/customer/service/location/file contracts from Step 7.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-303, CPL-304, Commercial quotation approval, ticket/document flows and dashboard status.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Add portal quote request intake/status tables or extend Commercial intake with source=customer_portal, idempotency key, files, customer scope, config version and handoff references. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Create/update/submit/cancel/status/comment/file APIs sharing Commercial validation, file scan and idempotency semantics. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Quote request form with cargo detail, pickup/delivery route, service, dates, notes, attachments, status timeline and safe resubmit/cancel. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Map portal quote intake to canonical Commercial quote/request contracts.
- Implement customer-scoped create/save/submit/cancel/status APIs with idempotency.
- Build accessible responsive quote request forms and status timeline.
- Add malware-scanned attachment support and safe internal handoff evidence.
- Test validation, duplicate submit, revoked scope, denied service and handoff reconciliation.

## 21. Main flow

Customer creates a quote request, uploads required documents, submits once with idempotency and receives Commercial-owned status updates.

## 22. Alternative flow

Customer saves a draft, edits before submit, cancels before internal acceptance or converts an accepted quote to booking through approved handoff.
...
- Portal quote request is not an approved quotation or price commitment until Commercial approval exists.
- Customer cannot override customer, account, site, tariff, margin, tax or approval fields.
- Files remain private, scanned and scope-bound before any download or handoff.
- Accepted quote conversion uses Commercial source references, never re-enters data silently.
- Pricing/AI recommendation remains outside Step 13 unless already approved by Commercial contracts; AI depth stays Step 14.
...
- Request Quotation unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Request Quotation.
...
- Request Quotation works with canonical source-domain ownership and no duplicate truth.
