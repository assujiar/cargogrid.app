# Prompt 318 — Points Ledger

**Prompt ID:** `CG-S13-CPL-020`  
**Package document:** `CG-AABPP-CPL-318`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-318.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-020` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Value Ledger; Epic: Points; Capability: Points; Feature slice: Point balance, earning, reversal, adjustment, expiry lot and customer ledger visibility; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement exact point ledger accounting with idempotent earning/reversal/expiry/redemption events and customer-visible balance history.

## 5. Business value

Prevent loyalty liability errors and support auditable customer trust.

## 6. Source requirement

LYL-PNT-001..004, LYL-RDM-001..004; Brief Points/Point expiration; UX Loyalty Account.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-316, CPL-317 and Finance payment eligibility.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-320..323 and liability reconciliation.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Point ledger event table with event type, amount, source, lot/expiry, balance snapshot, reversal reference, fraud hold, config version and audit. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Point balance/history/earn/reverse/expire/adjust-if-authorized APIs with strict idempotency, concurrency and parity. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Customer point balance, ledger history, expiry schedule and tenant-admin adjustment/reversal workflow with reason/approval. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Design point ledger event model, lots and balance derivation.
- Implement idempotent earning, reversal, adjustment and expiry logic.
- Build customer point ledger/expiry UI and admin governed adjustment UI.
- Add concurrency locks, reconciliation and liability hooks.
- Test duplicate events, negative balance prevention, expiry lot ordering and Supreme Admin residual-risk disclosure.

## 21. Main flow

Eligible paid transaction writes point earning event; balance derives from ledger and customer sees earning history plus expiry.

## 22. Alternative flow

A refund/dispute writes reversal event linked to original earning without deleting history.
...
- Normal roles cannot edit point_balance directly; balance is derived from ledger.
- Every point event has source/config version, idempotency key and audit.
- Adjustments require reason, approval, policy and residual-risk handling for Supreme Admin.
- Expiry consumes eligible lots by configured method and records events.
- Ledger history is customer-scoped and cannot leak other customers or internal investigation notes.
...
- Points unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Points.
...
- Points works with canonical source-domain ownership and no duplicate truth.
