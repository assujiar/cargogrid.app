# Prompt 312 — Payment Visibility

**Prompt ID:** `CG-S13-CPL-014`  
**Package document:** `CG-AABPP-CPL-312`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-312.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-014` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Finance Visibility; Epic: Customer Payment Status; Capability: Payment Visibility; Feature slice: Customer payment status, receipts, allocation summary and safe reconciliation visibility; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Expose payment and receipt status to authorized customer finance users while Finance retains bank/cash/reconciliation ownership.

## 5. Business value

Reduce AR status questions and support accurate customer self-service.

## 6. Source requirement

CPT-BIL-001..004; Brief Payment monitoring; UX Invoice, Billing & Payment; Finance Step 9.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300, CPL-311 and Finance payment/allocation/reconciliation contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-318 loyalty earning eligibility and CPL-323 liability/reconciliation.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Customer payment projection with payment/receipt references, invoice allocation, status, reconciliation state, source version and customer scope. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Payment status/receipt/download APIs with customer finance scope, signed files, field policy and idempotent refresh. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Payment tab showing paid/partial/unpaid, receipts, allocation to invoices, pending reconciliation and clear disputed/blocked states. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Map Finance payment statuses into customer-safe portal statuses.
- Implement scoped payment projection and receipt access APIs.
- Build responsive payment visibility UI integrated with invoices.
- Add reconciliation/freshness indicators and Finance source links.
- Test unauthorized finance access, partial payment, stale bank reconciliation and receipt file access.

## 21. Main flow

Customer finance user opens an invoice and sees payment allocation and receipt status derived from Finance.

## 22. Alternative flow

If payment is pending reconciliation, show pending state and allow customer to submit proof through document/ticket workflow.
...
- Portal cannot post, allocate, reverse, reconcile or delete payments.
- Payment visibility is limited to customer finance scope and linked invoices/accounts.
- Bank account internal details and reconciliation workpapers are hidden.
- Loyalty earning can use paid eligibility only after Finance-approved status.
- Payment documents follow RPD-032 scan and signed URL rules.
...
- Payment Visibility unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Payment Visibility.
...
- Payment Visibility works with canonical source-domain ownership and no duplicate truth.
