# Prompt 325 — Customer Portal and Loyalty Privacy Integrity Hardening

**Prompt ID:** `CG-S13-CPL-027`  
**Package document:** `CG-AABPP-CPL-325`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-325.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-027` and exactly one approved WBS/task-ledger item.
...
Workstream: Integrated Verification, Hardening, Documentation and Closure; Epic: Phase 8 Evidence Closure; Capability: Customer Portal and Loyalty Privacy Integrity Hardening; Feature slice: repository-specific bounded slice; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Harden customer-scope isolation, file privacy, portal inference resistance, loyalty ledger exactness, fraud/approval controls and liability reconciliation after integrated verification.

## 5. Business value

Reduce the most dangerous Phase 8 risks before closure: portal data leakage, unsafe signed files, loyalty double-spend and liability mismatch.
...
CPL-324 VERIFIED. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-326..327. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.

## 11. Allowed files/folders

Use only exact Phase 8 hardening, security, test, evidence paths authorized by WBS. Resolve repository paths; preserve unrelated user-owned changes.
...
- Attack customer scope through route parameters, filters, IDs, saved views, cache, exports, search, realtime and signed URLs.
- Stress loyalty ledger idempotency, concurrency, reversal, expiry, redemption, fraud hold and liability reconciliation.
- Verify files are private, scanned, short-lived, scope-bound and audited across ePOD, documents, tickets, invoices and rewards.
- Harden privileged actions with MFA/current-auth, approvals, separation and RPD-022 residual-risk warnings.
- Re-run targeted regression and document residual risks with owner/resume.

## 21. Main flow

Hardening attacks and repairs the highest-risk portal and loyalty paths, then reruns targeted evidence before handoff.
...
Integrated evidence index, traceability matrix, critical-flow results, customer scope matrix, loyalty ledger/liability reconciliation, file/access/security report, performance profile, migration/recovery report, defect register and CPL-325 build log. Update persistent ledgers/traceability and exact resume instructions.
...
Only the execution index may release CPL-326 after this task is `VERIFIED`. Do not set the final Phase 8 closure flag; only Prompt 327 may do so.
