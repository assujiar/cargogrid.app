# Prompt 311 — Invoice and Billing Visibility

**Prompt ID:** `CG-S13-CPL-013`  
**Package document:** `CG-AABPP-CPL-311`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-311.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-013` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Finance Visibility; Epic: Customer Billing; Capability: Invoice and Billing Visibility; Feature slice: Customer-scoped invoice, billing, aging, support document and dispute visibility; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Expose customer billing/invoice status from Finance without allowing portal edits to posted/finalized Finance records.

## 5. Business value

Improve billing transparency and reduce manual AR follow-up.

## 6. Source requirement

CPT-BIL-001..004; Brief Billing/Invoice control; UX Invoice, Billing & Payment; FIN-AR handoff.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300, Finance AR/invoice/payment contracts from Step 9 and source shipment/ePOD billing readiness.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-312, CPL-318, CPL-323 and customer disputes/tickets.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Customer invoice projection with invoice ID/number, amount, tax if permitted, status, aging, due date, linked shipments/docs, dispute state, source version and scope. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Invoice list/detail/download/dispute APIs with finance field policy, signed files, idempotency and REST/GraphQL parity. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Invoice and billing screens with aging, payment status, download package, linked shipments/docs and dispute/ticket action. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define customer-visible invoice/billing fields and masking rules.
- Implement scoped invoice projections and download/dispute APIs.
- Build responsive invoice list/detail and support-document UX.
- Add Finance reconciliation and no-edit safeguards for posted records.
- Test forged invoice IDs, finance scope, document access, dispute idempotency and aging accuracy.

## 21. Main flow

Customer finance user views scoped invoices, downloads permitted documents and opens a dispute/ticket without mutating Finance ledger records.

## 22. Alternative flow

If invoice data is under review or period-locked, display status and route questions through ticket/dispute workflow.
...
- Finance remains owner of invoice, AR, posting, payment allocation and period lock.
- Portal dispute is a workflow/ticket, not direct invoice edit or reversal.
- Invoice visibility requires customer finance scope, not generic shipment access alone.
- Amounts/taxes/payment details must follow field policy and source version.
- No journal/GL/vendor cost/internal margin data is exposed.
...
- Invoice and Billing Visibility unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Invoice and Billing Visibility.
...
- Invoice and Billing Visibility works with canonical source-domain ownership and no duplicate truth.
