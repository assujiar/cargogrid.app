# Prompt 323 — Liability Reconciliation Analytics

**Prompt ID:** `CG-S13-CPL-025`  
**Package document:** `CG-AABPP-CPL-323`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-323.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-025` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Finance and Analytics; Epic: Liability Control; Capability: Liability and Reconciliation; Feature slice: Loyalty liability, ledger reconciliation, engagement metrics and customer/admin analytics; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement loyalty liability and reconciliation evidence over point/benefit/reward ledgers with Finance-safe handoffs and scoped analytics.

## 5. Business value

Keep loyalty financially controlled and auditable before enterprise analytics expands in Step 14.

## 6. Source requirement

LYL-ANL-001..004; Brief Loyalty liability and analytics; UX Campaign Analytics; Delivery liability risk.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-316..322 and Finance ledger/payment contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-324..327 and Step 14 analytics/AI expansion.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Liability snapshot/reconciliation tables referencing loyalty ledger events, Finance handoff/control account if configured, customer/program/tier dimensions and source versions. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Liability summary/reconcile/export/admin analytics and customer-safe loyalty summary APIs with scope, field policy and audit. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Admin liability/reconciliation dashboard and customer loyalty summary/history with clear freshness and exclusions. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define liability model for points, cashback, vouchers and rewards with Finance handoff boundaries.
- Implement reconciliation from loyalty ledger to liability snapshots and Finance references.
- Build admin reconciliation dashboard and customer-safe analytics widgets.
- Add engagement metrics without exposing cross-customer or internal profitability data.
- Test ledger totals, reversals, expiry, unredeemed liability, export and source mismatch blockers.

## 21. Main flow

Admin runs reconciliation for a period; system compares loyalty ledger events to liability snapshots/Finance references and reports differences with owners.

## 22. Alternative flow

Customer views own loyalty summary and history; aggregate/admin analytics remain tenant-internal and scoped.
...
- Finance owns official GL/journal posting; Loyalty provides source-linked liability evidence/handoff only unless Finance contract explicitly posts.
- Liability calculations must be reproducible from ledger events and config versions.
- Analytics cannot infer or expose other customers, margins or internal profitability.
- Reconciliation mismatches block closure until owned or repaired.
- Step 14 may add advanced analytics; Step 13 must still deliver deterministic liability evidence.
...
- Liability and Reconciliation unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Liability and Reconciliation.
...
- Liability and Reconciliation works with canonical source-domain ownership and no duplicate truth.
