# Prompt 308 — Document Center

**Prompt ID:** `CG-S13-CPL-010`  
**Package document:** `CG-AABPP-CPL-308`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-308.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-010` and exactly one approved WBS/task-ledger item.
...
Workstream: Portal Documents; Epic: Customer Document Access; Capability: Document Center; Feature slice: Shipment, invoice-support, booking and ticket document center with scoped upload/download; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Create a unified customer document center over Platform files and source-domain document references without duplicating file storage or metadata truth.

## 5. Business value

Make approved customer documents easy to find while enforcing privacy, malware scanning and source ownership.

## 6. Source requirement

CPT-TRK-001..004, CPT-BIL-001..004, CPT-CX-001..004; Brief shipment document; UX Document & ePOD.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-300, Platform files, Commercial booking, Operations shipment/ePOD, Finance invoice and Ticketing file contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-311, CPL-313 and documentation handoff.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Customer document index/projection with source module, source entity, document type, scan status, retention/legal hold, scope and access log; file bytes remain Platform-owned. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Document search/list/upload/download/link APIs with source-domain authorization, scan gate, signed URL, idempotency and audit. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Document center with filters by shipment/invoice/ticket/type/date, upload/request states, safe previews and denied/quarantine/expired states. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Build source-domain document registry for customer-safe documents.
- Implement scoped document search/list/upload/download and access audit.
- Build responsive document center with filters and safe preview/download.
- Add retention/legal-hold metadata display where customer-safe.
- Test file-scan, signed URL, cross-source authorization, export and inference cases.

## 21. Main flow

Customer searches scoped documents, downloads permitted files and uploads requested documents which become available only after scanning.

## 22. Alternative flow

A document linked to a ticket or invoice requires both document scope and source-record scope before preview/download.
...
- A document link grants no access to its source record or other linked documents.
- Unscanned/quarantined files are never downloadable, previewed, indexed or emailed.
- Customer upload cannot overwrite canonical source-domain evidence without approved workflow.
- Search snippets/counts cannot reveal out-of-scope document existence.
- Retention and legal hold follow RPD-025 and source-domain rules.
...
- Document Center unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Document Center.
...
- Document Center works with canonical source-domain ownership and no duplicate truth.
