# Prompt 321 — Redemption Approval and Fulfillment

**Prompt ID:** `CG-S13-CPL-023`  
**Package document:** `CG-AABPP-CPL-321`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-321.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-023` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Rewards; Epic: Redemption; Capability: Redemption and Approval; Feature slice: Reward redemption, approval, stock reservation, point/benefit consumption and fulfillment; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement redemption with transactional eligibility, approval, fraud review, fulfillment and ledger consumption/reversal.

## 5. Business value

Enable customer rewards while protecting against fraud, double-spend and untracked liability.

## 6. Source requirement

LYL-RDM-001..004; Brief Redemption/Redemption approval; UX loyalty flow.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-318, CPL-319, CPL-320 and approval/job/file/notification contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-322 fraud/expiry and CPL-323 liability reconciliation.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Redemption request, approval, stock reservation, fulfillment, point/benefit consumption ledger, reversal and audit records. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Redeem/approve/reject/fulfill/cancel/status APIs with idempotency, concurrency, fraud hold and REST/GraphQL parity. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Customer redemption checkout/status/history and tenant approval/fulfillment workbench with reasoned decisions. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Implement transactional redemption eligibility and stock reservation.
- Write point/benefit consumption ledger and approval workflow.
- Build customer redemption flow and admin approval/fulfillment UI.
- Add notification, reversal, retry and DLQ handling.
- Test double-click/double-spend, stock race, approval denial, cancellation and fulfillment failure.

## 21. Main flow

Customer submits redemption; system locks eligibility/stock, records pending redemption, routes approval if required, consumes ledger on approval and tracks fulfillment.

## 22. Alternative flow

Fraud hold or insufficient balance pauses/rejects redemption with customer-safe reason and preserved audit.
...
- No reward is fulfilled without server-side revalidation at the current checkpoint.
- Point/benefit consumption is ledgered and reversible; history is never deleted.
- Approval rules, approver, reason and config version are mandatory where required.
- Concurrent redemptions cannot overspend points or stock.
- Customer-facing denial reasons must not expose fraud signals or internal investigation.
...
- Redemption and Approval unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Redemption and Approval.
...
- Redemption and Approval works with canonical source-domain ownership and no duplicate truth.
