# Prompt 317 — Membership Tier

**Prompt ID:** `CG-S13-CPL-019`  
**Package document:** `CG-AABPP-CPL-317`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-317.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-019` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Program; Epic: Tier Management; Capability: Membership Tier; Feature slice: Tier rules, thresholds, upgrade/downgrade, effective dates and customer tier projection; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement membership tier configuration and calculation with effective-dated thresholds and auditable tier movement.

## 5. Business value

Support tiered customer engagement while preserving deterministic, explainable eligibility.

## 6. Source requirement

LYL-PRG-001..004; Brief Membership tier/Tier upgrade/Tier downgrade; UX Loyalty.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-316 and loyalty/customer/transaction source contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-318..323 and portal loyalty display.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Tier configuration/version, customer tier state, tier movement ledger and threshold evidence linked to source transaction summaries. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Tier config, recalculation, status/history APIs with effective version, idempotency and audit. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Tenant tier setup/history and customer tier card with current tier, progress, benefits and expiry/review dates. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define tier threshold dimensions, effective dates and downgrade rules.
- Implement deterministic tier calculation and movement ledger.
- Build admin tier config and customer tier/progress UX.
- Add review/recalculation jobs with idempotency and evidence.
- Test threshold edges, config changes, reversals, fraud hold and customer scope.

## 21. Main flow

Customer transaction history reaches a configured threshold and a tier movement ledger records upgrade with applied config version.

## 22. Alternative flow

Downgrade occurs at review period end or after reversal according to configured policy, with customer-safe explanation.
...
- Tier status is derived from source transactions and tier movement ledger, not free-text profile flags.
- Tier benefits must be versioned and auditable.
- Downgrades/reviews cannot silently change historical earning/redemption entries.
- Fraud hold or unresolved dispute can suspend tier benefits according to policy.
- Customer sees their own tier/progress only; aggregate tier analytics stay scoped/admin.
...
- Membership Tier unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Membership Tier.
...
- Membership Tier works with canonical source-domain ownership and no duplicate truth.
