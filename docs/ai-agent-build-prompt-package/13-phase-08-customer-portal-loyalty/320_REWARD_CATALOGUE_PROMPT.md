# Prompt 320 — Reward Catalogue

**Prompt ID:** `CG-S13-CPL-022`  
**Package document:** `CG-AABPP-CPL-320`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-320.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-022` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Rewards; Epic: Reward Management; Capability: Reward; Feature slice: Reward catalogue, stock/eligibility, terms, customer display and admin management; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Build a configurable reward catalogue with eligibility, stock, effective dates and customer-safe presentation.

## 5. Business value

Let tenants offer rewards while controlling entitlement, availability and liability.

## 6. Source requirement

LYL-RDM-001..004, CPT-CX-001..004; Brief Reward; UX Loyalty & Reward.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-316..319 and Platform config/files.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-321 redemption and CPL-323 liability/analytics.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Reward catalogue/config/version, stock/availability, eligibility criteria, media/file references, terms and audit tables. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Reward catalogue/list/detail/config APIs with customer eligibility projection, admin approval and field policy. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Customer reward catalogue with eligible/locked/out-of-stock states and tenant admin reward setup with media/terms/stock controls. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define reward types, eligibility criteria, stock and terms models.
- Implement customer-scoped catalogue APIs and admin config workflow.
- Build responsive reward catalogue/detail UX with locked/out-of-stock states.
- Add media/file scanning and approval lifecycle.
- Test eligibility, stock races, expired reward, hidden internal terms and media access.

## 21. Main flow

Customer views a scoped reward catalogue and sees which rewards they can redeem based on points/tier/eligibility.

## 22. Alternative flow

Admin schedules a reward for future effective date or pauses it; customers see safe unavailable state without stale redemption paths.
...
- Reward visibility does not guarantee redemption until server revalidates at submission.
- Reward media/documents follow private scanned file rules if not public assets.
- Stock and eligibility are checked transactionally during redemption.
- Terms/effective dates are versioned and auditable.
- Internal cost/vendor fulfillment data is hidden from customers.
...
- Reward unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Reward.
...
- Reward works with canonical source-domain ownership and no duplicate truth.
