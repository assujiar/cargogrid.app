# Prompt 319 — Cashback Discount Voucher

**Prompt ID:** `CG-S13-CPL-021`  
**Package document:** `CG-AABPP-CPL-319`  
**Version:** `0.14.0`  
**Runtime build log:** `docs/build-log/phase-08/CPL-319.md`
...
`{{TASK_ID}}` mapped to `CG-S13-CPL-021` and exactly one approved WBS/task-ledger item.
...
Workstream: Loyalty Value Ledger; Epic: Non-Point Benefits; Capability: Cashback, Discount and Voucher; Feature slice: Cashback, discount, voucher/coupon issuance, eligibility, hold, use and reversal; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement cashback/discount/voucher benefits as auditable ledger or entitlement records linked to eligible transactions and redemption/Finance controls.

## 5. Business value

Enable flexible promotions without uncontrolled financial exposure.

## 6. Source requirement

LYL-PNT-001..004, LYL-RDM-001..004; Brief Cashback/Discount/Voucher/Coupon/Promotion.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
CPL-316, CPL-318 and Finance invoice/payment/customer contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

CPL-320..323 and Phase 9 analytics boundaries.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, Customer Portal, Tenant Internal, Commercial, Operations, WMS, Finance, Ticketing, Loyalty, tests, docs and compatibility contracts.
...
Benefit entitlement/ledger tables for cashback, discount, voucher, coupon with issued/redeemed/reversed/expired states, value cap, liability, source and config version. Preserve canonical source-domain ownership and include tenant_id, customer_id/account_id/site_id where applicable, config/source versions, idempotency, audit and rollback-safe additive migrations.

## 14. API impact

Issue/preview/apply/redeem/reverse/expire APIs with eligibility, fraud hold, approval and Finance handoff where needed. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit and version semantics.

## 15. UI/UX impact

Customer benefit wallet/catalogue and admin benefit configuration/issuance/reversal screens with clear status and terms. Include keyboard/focus/labels, mobile-friendly responsive online-first behavior, loading/empty/error/denied/stale/conflict/degraded states, unsaved-change protection and no dead action.
...
- Define benefit types, value units, caps, validity and accounting treatment.
- Implement entitlement issuance, use, reversal, expiry and fraud hold.
- Build customer wallet and admin configuration UI.
- Add Finance/liability and invoice-discount handoff evidence where applicable.
- Test duplicate issuance, capped value, expired voucher, disputed invoice and fraud hold.

## 21. Main flow

A configured eligible transaction issues a cashback/discount/voucher entitlement recorded with value, source and expiry, visible in customer wallet.

## 22. Alternative flow

A voucher is redeemed against an allowed future transaction or reversed after source transaction cancellation according to policy.
...
- Benefit values are governed records, not free-form customer balance edits.
- Cashback with financial liability must reconcile to Finance liability/control account design.
- Voucher/coupon codes are unique, non-guessable, scoped and auditable.
- Fraud hold blocks usage and customer display follows safe reason policy.
- Expiry/reversal preserves history and source references.
...
- Cashback, Discount and Voucher unit/service/database/API/contract tests.
...
Update Phase 8 build log, customer portal scope contract, source-domain ownership notes, API/schema/field policy, UX states, runbook, test evidence, traceability, dependency and rollback/handoff artifacts relevant to Cashback, Discount and Voucher.
...
- Cashback, Discount and Voucher works with canonical source-domain ownership and no duplicate truth.
