# Prompt 415 - Requirement Coverage Audit

**Prompt ID:** `CG-S17-FPV-002`  
**Package document:** `CG-AABPP-FPV-415`  
**Version:** `0.18.0`  
**Runtime build log:** `docs/build-log/final-package-validation/FPV-415.md`
...
`{{TASK_ID}}` mapped to `CG-S17-FPV-002` and exactly one approved final-validation WBS/task-ledger item.
...
Workstream: Coverage Audit; Epic: Source Coverage; Capability: Requirement Coverage Audit; Feature slice: Final package audit for Requirement Coverage Audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Audit whether all source requirements, RPD decisions, NFRs, delivery gates, test gates and go-live rules are represented in the package.

## 5. Business value

Prevent final handoff with uncovered source requirements or hidden gaps.

## 6. Source requirement

Master Prompt Step 17: Apakah seluruh requirement tercakup; source requirements; completion evidence. Cite exact package files, source controls, manifest rows and validation evidence.
...
FPV-416, final package closure, future agent execution, runtime safety and package handoff. Identify affected controls, manifests, START_HERE, validation reports, runbooks, task ledgers and final gap/risk register.
...
- Audit checklist for Requirement Coverage Audit is complete.
...
If this task is `VERIFIED`, continue only to `FPV-416`. If blocked, resume this audit or update the final gap/risk register before continuing.
