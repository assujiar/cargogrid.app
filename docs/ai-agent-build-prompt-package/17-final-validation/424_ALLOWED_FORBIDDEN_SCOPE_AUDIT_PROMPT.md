# Prompt 424 - Allowed Forbidden Scope Audit

**Prompt ID:** `CG-S17-FPV-011`  
**Package document:** `CG-AABPP-FPV-424`  
**Version:** `0.18.0`  
**Runtime build log:** `docs/build-log/final-package-validation/FPV-424.md`
...
`{{TASK_ID}}` mapped to `CG-S17-FPV-011` and exactly one approved final-validation WBS/task-ledger item.
...
Workstream: Safety Audit; Epic: Scope Safety; Capability: Allowed Forbidden Scope Audit; Feature slice: Final package audit for Allowed Forbidden Scope Audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Audit whether every prompt clearly defines allowed scope, forbidden scope, safe existing-code behavior and safe tenant-data behavior.

## 5. Business value

Prevent unsafe implementation, tenant data exposure and scope creep.

## 6. Source requirement

Master Prompt Step 17: safe existing code, safe tenant data, allowed/forbidden scope. Cite exact package files, source controls, manifest rows and validation evidence.
...
FPV-425, final package closure, future agent execution, runtime safety and package handoff. Identify affected controls, manifests, START_HERE, validation reports, runbooks, task ledgers and final gap/risk register.
...
- Audit checklist for Allowed Forbidden Scope Audit is complete.
...
If this task is `VERIFIED`, continue only to `FPV-425`. If blocked, resume this audit or update the final gap/risk register before continuing.
