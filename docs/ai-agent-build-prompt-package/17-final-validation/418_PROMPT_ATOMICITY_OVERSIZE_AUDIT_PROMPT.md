# Prompt 418 - Prompt Atomicity Oversize Audit

**Prompt ID:** `CG-S17-FPV-005`  
**Package document:** `CG-AABPP-FPV-418`  
**Version:** `0.18.0`  
**Runtime build log:** `docs/build-log/final-package-validation/FPV-418.md`
...
`{{TASK_ID}}` mapped to `CG-S17-FPV-005` and exactly one approved final-validation WBS/task-ledger item.
...
Workstream: Atomicity Audit; Epic: Prompt Size Control; Capability: Prompt Atomicity Oversize Audit; Feature slice: Final package audit for Prompt Atomicity Oversize Audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Audit whether prompts are bounded, atomic, testable and not too large for reliable execution.

## 5. Business value

Reduce failed big-bang tasks, regression risk and resume complexity.

## 6. Source requirement

Master Prompt Step 17: Apakah ada prompt yang terlalu besar; do not tell agent to do everything at once. Cite exact package files, source controls, manifest rows and validation evidence.
...
FPV-419, final package closure, future agent execution, runtime safety and package handoff. Identify affected controls, manifests, START_HERE, validation reports, runbooks, task ledgers and final gap/risk register.
...
- Audit checklist for Prompt Atomicity Oversize Audit is complete.
...
If this task is `VERIFIED`, continue only to `FPV-419`. If blocked, resume this audit or update the final gap/risk register before continuing.
