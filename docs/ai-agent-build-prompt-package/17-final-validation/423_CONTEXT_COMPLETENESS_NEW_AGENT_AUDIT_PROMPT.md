# Prompt 423 - Context Completeness New Agent Audit

**Prompt ID:** `CG-S17-FPV-010`  
**Package document:** `CG-AABPP-FPV-423`  
**Version:** `0.18.0`  
**Runtime build log:** `docs/build-log/final-package-validation/FPV-423.md`
...
`{{TASK_ID}}` mapped to `CG-S17-FPV-010` and exactly one approved final-validation WBS/task-ledger item.
...
Workstream: Context Audit; Epic: New Agent Usability; Capability: Context Completeness New Agent Audit; Feature slice: Final package audit for Context Completeness New Agent Audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Audit whether a new agent can understand and use the package without prior conversation context.

## 5. Business value

Make the package independent, durable and usable by future agents.

## 6. Source requirement

Master Prompt Step 17: package dapat digunakan oleh agent baru tanpa konteks percakapan sebelumnya; context-complete. Cite exact package files, source controls, manifest rows and validation evidence.
...
FPV-424, final package closure, future agent execution, runtime safety and package handoff. Identify affected controls, manifests, START_HERE, validation reports, runbooks, task ledgers and final gap/risk register.
...
- Audit checklist for Context Completeness New Agent Audit is complete.
...
If this task is `VERIFIED`, continue only to `FPV-424`. If blocked, resume this audit or update the final gap/risk register before continuing.
