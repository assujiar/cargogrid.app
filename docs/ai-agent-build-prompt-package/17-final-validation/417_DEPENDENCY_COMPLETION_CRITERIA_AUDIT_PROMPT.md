# Prompt 417 - Dependency Completion Criteria Audit

**Prompt ID:** `CG-S17-FPV-004`  
**Package document:** `CG-AABPP-FPV-417`  
**Version:** `0.18.0`  
**Runtime build log:** `docs/build-log/final-package-validation/FPV-417.md`
...
`{{TASK_ID}}` mapped to `CG-S17-FPV-004` and exactly one approved final-validation WBS/task-ledger item.
...
Workstream: Dependency Audit; Epic: Dependency and Completion; Capability: Dependency Completion Criteria Audit; Feature slice: Final package audit for Dependency Completion Criteria Audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Audit whether every prompt has explicit dependency, entry gate, completion criteria, next prompt and closure evidence.

## 5. Business value

Ensure future agents can execute in order without guessing readiness.

## 6. Source requirement

Master Prompt Step 17: dependency, completion criteria, clear completion evidence and next step. Cite exact package files, source controls, manifest rows and validation evidence.
...
FPV-418, final package closure, future agent execution, runtime safety and package handoff. Identify affected controls, manifests, START_HERE, validation reports, runbooks, task ledgers and final gap/risk register.
...
- Audit checklist for Dependency Completion Criteria Audit is complete.
...
If this task is `VERIFIED`, continue only to `FPV-418`. If blocked, resume this audit or update the final gap/risk register before continuing.
