# Template 53 — New Feature Slice

**Prompt ID:** `CG-S4-REUSE-001`  
**Package document:** `CG-AABPP-REUSE-053`  
**Version:** `0.5.0`  
**Intended use:** Implement one end-to-end business capability slice inside one module boundary.
...
Implement {{FEATURE_SLICE}} from persistence/business service through authorized interface and UX, within the approved atomic boundary.

## 5. Business value

Deliver a usable business outcome without duplicating canonical data or destabilizing adjacent modules.

## 6. Source requirement

{{REQUIREMENT_IDS}}, {{BUSINESS_RULE_IDS}}, {{WBS_TASK_ID}}. Cite exact CPD/RPD/requirement/business-rule/ADR/runtime evidence; do not rely on chat.
...
Create only the approved additive schema changes; tenant keys, constraints, RLS, indexes, and migration tests are mandatory when persistence changes.

## 14. API impact

Preserve REST/GraphQL parity over one domain service; document compatibility, validation, pagination, errors, and idempotency.

## 15. UI/UX impact

Implement complete portal/actor route and loading/empty/error/success/denied/degraded states with WCAG 2.2 AA.

## 16. Security impact

Enforce tenant, role/scope, field, record, export/search/report/file controls at server and database boundaries.

## 17. Performance impact

Use server pagination and bounded queries; compare baseline and post-change route/query/build evidence.

## 18. Audit impact

Record create/update/status/approval/privileged actions with actor, tenant, reason, before/after or event context.

## 19. Data migration impact

Use additive/expand-contract migration only when approved; preserve existing records and provide upgrade/rebuild evidence.

## 20. Detailed implementation tasks

1. Trace the slice across data, service, REST/GraphQL, UI, jobs/files and audit boundaries.
2. Implement main, alternative and exception paths without placeholder actions.
3. Add compatibility-safe contracts, tenant enforcement, observability and documentation.
4. Compare baseline/post-change evidence and update persistent records; do not expand scope to adjacent defects.

## 21. Main flow

Authorized actor completes {{FEATURE_SLICE}} and sees the persisted canonical result.

## 22. Alternative flow

Supported alternate actor/configuration path completes without bypass or duplicate entry.

## 23. Exception flow

Validation, authorization, conflict, dependency, timeout and partial-service failures are explicit and recoverable.

## 24. Business rules

- Canonical business state and rules come from cited requirements/configuration snapshots.
- No hard-coded tenant, role, approval, numbering, status, branding or workflow.
- Preserve canonical entities/statuses, configuration version/effective date and cited approval rules.

## 25. Validation rules

- Required fields, state transitions, concurrency and idempotency are server-enforced.
- No unresolved variable or placeholder behavior may ship.
- Validate on trusted server/database boundaries; never rely on client-only checks.

## 26. Access rules

- Map portal, actor, role, scope, field and record access including negative cases.
- Enforce least privilege, strict tenant isolation, RLS, RBAC plus scope, field/record access and server-only service role.

## 27. Test data requirement

At least two tenants, multiple roles/scopes, valid/invalid records, conflicts and alternate configurations. Use synthetic/redacted data only; never commit real tenant, credential, payroll, tax, bank or personal data.

## 28. Tests to create/update

- Unit/business-rule, database/constraint/RLS, API contract, component and E2E main/alternative/exception tests.
- Cross-tenant, field/record, audit and performance tests where applicable.
- Include main, alternative, exception, audit and negative security evidence proportional to risk.

## 29. Regression tests

- Adjacent module flows, public contracts, migration upgrade/rebuild and existing critical E2E paths.
...
- One real business capability works end to end with no placeholder.
- All access, data, contract, UX, audit and test evidence passes.
