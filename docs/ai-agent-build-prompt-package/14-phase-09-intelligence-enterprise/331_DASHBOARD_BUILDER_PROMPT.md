# Prompt 331 — Dashboard Builder

**Prompt ID:** `CG-S14-IAE-003`  
**Package document:** `CG-AABPP-IAE-331`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-331.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-003` and exactly one approved WBS/task-ledger item.
...
Workstream: Intelligence Reporting; Epic: Configurable Dashboards; Capability: Dashboard Builder; Feature slice: Tenant-configurable dashboards, widgets, layout, filters and scoped metrics; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Build a governed dashboard builder that lets authorized users configure widgets and layouts using approved metrics and report sources.

## 5. Business value

Let tenants adapt dashboards without backend coding while keeping metric definitions consistent and safe.

## 6. Source requirement

Phase 9 Dashboard builder; Charter configurable dashboard via UI; UX navigation/reports; BPR dashboard pre-aggregation.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-330 and Platform configuration/entitlement/role engine.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-332..335 analytics and scheduled reporting.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Dashboard, widget, layout, filter, metric binding, version, publish and access records linked to report/metric catalog. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Dashboard CRUD/publish/rollback/render APIs with metric allowlist, scope checks, cache policy and versioned definitions. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Dashboard builder canvas, widget picker, filter editor, preview/publish/rollback, responsive layout and no-data/error states. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define dashboard/widget schema and approved metric/report bindings.
- Implement draft/publish/rollback workflow and permission policy.
- Build responsive dashboard builder and runtime viewer.
- Add cache invalidation and safe widget execution budgets.
- Test tenant isolation, field leakage, widget query load and mobile layout.

## 21. Main flow

Authorized admin creates a dashboard draft, adds approved widgets, previews with scoped data, publishes and users view it by role.

## 22. Alternative flow

Admin rolls back to a prior dashboard version after a metric or layout issue.
...
- Dashboard builder uses approved datasets/metrics only; it is not raw SQL for tenants.
- Published dashboard versions are immutable snapshots except RPD-022 residual-risk path.
- Widget counts/snippets cannot reveal out-of-scope data.
- Dashboard cache keys include tenant, role/scope, filters and config version.
- Responsive layouts must preserve access policy across screen sizes.
...
- Dashboard Builder unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Dashboard Builder.
...
- Dashboard Builder works with canonical source-domain ownership and no duplicate truth.
