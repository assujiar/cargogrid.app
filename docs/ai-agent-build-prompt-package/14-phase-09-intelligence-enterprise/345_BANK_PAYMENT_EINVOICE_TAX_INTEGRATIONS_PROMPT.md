# Prompt 345 — Bank Payment E-Invoice Tax Integrations

**Prompt ID:** `CG-S14-IAE-017`  
**Package document:** `CG-AABPP-IAE-345`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-345.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-017` and exactly one approved WBS/task-ledger item.
...
Workstream: Finance Integrations; Epic: Financial Provider Integrations; Capability: Bank, Payment Gateway, E-Invoice and Tax; Feature slice: Bank, payment gateway, e-invoice and tax provider adapters with Finance governance; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Integrate financial providers through Finance-owned contracts, exact reconciliation and strict human approval for posting/legal effects.

## 5. Business value

Automate finance data exchange while preventing unauthorized money, tax or ledger mutation.

## 6. Source requirement

Phase 9 Bank, payment gateway, e-invoice, tax; RPD-016/021/038; Finance Step 9; NFR-API.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Finance invoice/payment/tax/reconciliation, Platform secrets/jobs, IAE-336 hub.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

AI/OCR financial extraction, reconciliation reports and release hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Provider connection, bank statement/payment/e-invoice/tax payload, match proposal, reconciliation, approval, retry and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Provider sync/webhook/upload/match/propose/status APIs with Finance authorization and idempotency. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Finance integration setup, statement/import queue, match proposals, approval and reconciliation evidence. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define Finance-owned provider adapter and approval boundaries.
- Implement idempotent financial payload ingestion and match proposals.
- Add dated SME/provider validation gate for statutory/tax behavior.
- Build setup/reconciliation/proposal UI.
- Test duplicate bank webhook, tax payload errors, unauthorized posting and rollback.

## 21. Main flow

Provider sync imports payment/tax data, proposes matches and Finance user approves posting/reconciliation through canonical Finance workflow.

## 22. Alternative flow

Provider payload changes; adapter blocks ingestion, records compatibility issue and awaits mapping update.
...
- Provider data cannot autonomously post journals, payments, tax or legal status.
- RPD-016 dated SME/provider evidence is required before statutory activation.
- Finance owns all posting, settlement and reconciliation truth.
- Secrets/payloads are protected and logs are redacted.
- Duplicate provider events are idempotent and reconciled.
...
- Bank, Payment Gateway, E-Invoice and Tax unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Bank, Payment Gateway, E-Invoice and Tax.
...
- Bank, Payment Gateway, E-Invoice and Tax works with canonical source-domain ownership and no duplicate truth.
