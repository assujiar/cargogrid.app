# Prompt 346 — External Accounting HR Integrations

**Prompt ID:** `CG-S14-IAE-018`  
**Package document:** `CG-AABPP-IAE-346`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-346.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-018` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Integrations; Epic: Back-Office Integrations; Capability: External Accounting and HR; Feature slice: External accounting/HR import/export/sync adapters and reconciliation; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Support controlled sync with external accounting and HR systems through explicit mappings, ownership and reconciliation.

## 5. Business value

Let enterprise tenants connect legacy systems during phased adoption without corrupting CargoGrid source truth.

## 6. Source requirement

Phase 9 External accounting and HR; RPD-038; Finance/HRIS boundaries.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Finance Step 9, HRIS Step 12, Platform import/export/jobs, IAE-336.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Enterprise onboarding, migration and support.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
External system, mapping, sync job, source-of-truth rule, conflict, reconciliation, version and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Import/export/sync/test APIs with mapping validation, dry-run, conflict review and rollback evidence. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Integration setup, mapping table, dry-run preview, conflict queue, reconciliation and sync history. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define ownership direction per entity/field and tenant contract.
- Implement dry-run mapping and validation for accounting/HR sync.
- Add conflict review, reconciliation and rollback support.
- Build setup/mapping/conflict UX.
- Test source conflicts, PII/payroll/finance field masking and no duplicate truth.

## 21. Main flow

Enterprise configures external HR sync for limited fields, runs dry-run, reviews conflicts and executes approved sync with reconciliation.

## 22. Alternative flow

Accounting system remains source for a subset during transition; CargoGrid records read-only references and exceptions.
...
- CargoGrid source-domain ownership must be explicit before sync.
- External sync cannot silently overwrite posted Finance or finalized HR/payroll records.
- Sensitive personal/finance fields follow field policy and masking.
- Every mapping version and sync run is auditable.
- No generic connector claim without adapter proof.
...
- External Accounting and HR unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to External Accounting and HR.
...
- External Accounting and HR works with canonical source-domain ownership and no duplicate truth.
