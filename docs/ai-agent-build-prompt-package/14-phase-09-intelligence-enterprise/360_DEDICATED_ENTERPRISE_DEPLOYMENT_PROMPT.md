# Prompt 360 — Dedicated Enterprise Deployment

**Prompt ID:** `CG-S14-IAE-032`  
**Package document:** `CG-AABPP-IAE-360`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-360.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-032` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Deployment; Epic: Dedicated Instance; Capability: Optional Dedicated Enterprise Deployment; Feature slice: Dedicated instance architecture option, qualification, provisioning, isolation and runbooks; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Define and implement optional dedicated Enterprise deployment path without changing the default shared-schema/RLS product.

## 5. Business value

Serve high-requirement enterprise customers while preserving SaaS maintainability.

## 6. Source requirement

Phase 9 Optional dedicated enterprise deployment; RPD-011; RPD-013; BPR OD-004.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform tenancy, CI/CD, observability, secrets, backup and enterprise contract qualification.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Data residency, scale-up, DR and release go-live.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Deployment topology, tenant deployment mapping, provisioning status, environment config reference, contract qualifier and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Provisioning/status/admin APIs for internal platform operations with strict privileged access. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Internal enterprise deployment qualification/provisioning checklist and environment status, not tenant self-service. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Document shared default versus dedicated exception criteria.
- Implement deployment mapping/provisioning evidence model.
- Create provisioning/runbook/checklist artifacts and status surface.
- Add observability/backup/secrets separation checks.
- Test tenant mapping, data isolation, environment drift and rollback.

## 21. Main flow

Enterprise deal qualifies for dedicated deployment; internal team provisions tracked environment and maps tenant with evidence/runbook.

## 22. Alternative flow

Request does not meet criteria; tenant remains shared-schema SaaS with contractual explanation.
...
- Dedicated deployment is paid Enterprise option, not default architecture.
- No tenant code fork; source code and tests remain shared.
- Provisioning requires contract/security/CTO approval.
- Environment secrets and data are isolated by deployment.
- Claims about RPO/RTO/region follow actual contract and evidence only.
...
- Optional Dedicated Enterprise Deployment unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Optional Dedicated Enterprise Deployment.
...
- Optional Dedicated Enterprise Deployment works with canonical source-domain ownership and no duplicate truth.
