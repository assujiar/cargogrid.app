# Prompt 330 — Reporting Engine

**Prompt ID:** `CG-S14-IAE-002`  
**Package document:** `CG-AABPP-IAE-330`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-330.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-002` and exactly one approved WBS/task-ledger item.
...
Workstream: Intelligence Reporting; Epic: Report Runtime; Capability: Reporting Engine; Feature slice: Permission-aware report execution, query catalog, export jobs and evidence; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement a governed reporting engine that runs tenant-, role-, field- and record-scoped reports without hitting transactional systems unsafely.

## 5. Business value

Give management usable operational, finance, customer, warehouse, HR, ticket and loyalty reports while preserving performance and access controls.

## 6. Source requirement

Phase 9 Reporting engine; Product Brief performance principles; BPR NFR-PERF/NFR-SEC/NFR-AUD; Delivery Phase 9.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-329; verified Platform config/access/jobs/files, all domain schemas through Phase 8 and reporting handoff contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-331..367 reporting, dashboard, analytics, scheduled report, API and enterprise evidence.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Define report catalog, report definition/version, parameter, permission, execution, export, artifact and audit tables with tenant/source/config versions. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Report catalog/run/status/cancel/export APIs with server-side parameters, field policy, async jobs, idempotency and REST/GraphQL parity. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Report library, parameter forms, run history, export status, denied/empty/large-report states and source/freshness notes. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Inventory report categories and domain source contracts from all prior phases.
- Implement report catalog/versioning and permission-aware execution.
- Implement async export artifacts with signed URLs and expiry.
- Build report library and run-history UX.
- Test field masking, export scope, heavy report budgets and source reconciliation.

## 21. Main flow

Authorized user selects a report, fills parameters, runs it as a job, views paginated results and downloads a scope-safe export.

## 22. Alternative flow

Large report runs asynchronously with progress, cancel/resume and signed download when complete.
...
- A report cannot bypass source-domain field/record policy.
- Large exports are async, scoped, expiring and audited.
- Report definitions are versioned and changes do not rewrite prior evidence.
- No report uses SELECT * or client-loaded full dataset.
- Reports cannot expose cross-tenant, margin, payroll, personal or loyalty data outside policy.
...
- Reporting Engine unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Reporting Engine.
...
- Reporting Engine works with canonical source-domain ownership and no duplicate truth.
