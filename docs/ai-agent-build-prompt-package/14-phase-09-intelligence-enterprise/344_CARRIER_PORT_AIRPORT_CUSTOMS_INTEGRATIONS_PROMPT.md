# Prompt 344 — Carrier Port Airport Customs Integrations

**Prompt ID:** `CG-S14-IAE-016`  
**Package document:** `CG-AABPP-IAE-344`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-344.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-016` and exactly one approved WBS/task-ledger item.
...
Workstream: External Logistics Integrations; Epic: Logistics Ecosystem; Capability: Carrier, Port, Airport and Customs; Feature slice: Case-specific adapters for carrier, port, airport and customs data exchange; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement governed integration patterns for logistics ecosystem providers without claiming generic universal connectors.

## 5. Business value

Reduce manual status/document exchange while keeping adapter-specific risk visible.

## 6. Source requirement

Phase 9 Carrier, port, airport, customs; RPD-038; Product Brief logistics ecosystem.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Commercial/Operations/Document/Finance contracts and IAE-336 hub.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Tracking, documents, billing readiness, analytics and support flows.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Provider adapter, mapping, external reference, event/document ingestion, validation, retry, DLQ and reconciliation records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Provider-specific sync/receive/poll/test APIs behind shared adapter boundary with signature/idempotency. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Adapter setup, mapping, status, reconciliation and error queue screens. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define one shared adapter contract and per-provider mapping rules.
- Implement external reference registry and idempotent event/document ingestion.
- Add validation/reconciliation against shipment/document records.
- Build adapter setup and error review UI.
- Test provider variance, duplicate events, invalid documents and no-tenant-fork compliance.

## 21. Main flow

Approved provider sends shipment/document status; adapter validates and maps it to canonical Operations/Document record with audit.

## 22. Alternative flow

Provider payload is incomplete; integration queues error for owner review without updating source truth.
...
- Do not promise universal provider support; each provider requires explicit adapter evidence.
- External events cannot bypass source-domain lifecycle rules.
- Customs/legal statuses require human or domain approval where configured.
- Provider payloads are redacted in logs/evidence.
- Adapter changes require compatibility tests and runbook.
...
- Carrier, Port, Airport and Customs unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Carrier, Port, Airport and Customs.
...
- Carrier, Port, Airport and Customs works with canonical source-domain ownership and no duplicate truth.
