# Prompt 335 — Automation Rule Engine

**Prompt ID:** `CG-S14-IAE-007`  
**Package document:** `CG-AABPP-IAE-335`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-335.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-007` and exactly one approved WBS/task-ledger item.
...
Workstream: Automation; Epic: Governed Automation; Capability: Automation Rule Engine; Feature slice: Condition/action automation with versioning, approvals, dry run, replay and guardrails; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement a rules-based automation engine for notifications, assignments, reminders, workflow steps and safe system actions with governance.

## 5. Business value

Reduce manual follow-up while preventing automation from silently mutating critical business records.

## 6. Source requirement

Phase 9 Automation rule engine; BPR workflow/approval/queue patterns; Delivery Phase 9 automation governance.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform config/workflow/jobs/audit and all domain events through Phase 8.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-336..346 integrations and IAE-347 AI governance.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Automation rule, trigger, condition, action, version, dry-run, execution, idempotency, suppression, DLQ and audit tables. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Rule CRUD/draft/publish/rollback/test/execution APIs with action allowlist, approval, rate limits and idempotency. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Automation builder, trigger/action picker, dry-run preview, publish approval, execution log and failure handling. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define automation triggers/actions and critical-action deny/approval list.
- Implement versioned rules, dry-run and publish workflow.
- Implement execution engine with idempotency, suppression and DLQ.
- Build automation builder and execution log UX.
- Test loops, duplicate triggers, unauthorized action and rollback.

## 21. Main flow

Admin creates an automation rule, dry-runs it on sample events, publishes with approval and executions are logged/idempotent.

## 22. Alternative flow

A rule misfires; admin pauses it, reviews executions and rolls back to prior version.
...
- Automation cannot post ledgers, payments, legal status or irreversible changes without human approval and source-domain contract.
- Rules are versioned and execution references the applied version.
- Loop/storm prevention, cooldown and suppression are mandatory.
- Execution grants no extra record access beyond trigger/action policies.
- AI suggestions may draft rules but cannot publish them autonomously.
...
- Automation Rule Engine unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Automation Rule Engine.
...
- Automation Rule Engine works with canonical source-domain ownership and no duplicate truth.
