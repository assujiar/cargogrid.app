# Prompt 364 — Intelligence, Automation and Enterprise Integrated Verification

**Prompt ID:** `CG-S14-IAE-036`  
**Package document:** `CG-AABPP-IAE-364`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-364.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-036` and exactly one approved WBS/task-ledger item.
...
Workstream: Integrated Verification, Hardening, Documentation and Closure; Epic: Phase 9 Evidence Closure; Capability: Intelligence, Automation and Enterprise Integrated Verification; Feature slice: repository-specific bounded slice; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Independently verify the 34 Phase 9 capabilities and cross-system intelligence, automation, API, AI and enterprise flows at one repository/schema/environment checkpoint before hardening.

## 5. Business value

Detect cross-system AI, API, integration, analytics, automation and enterprise-control failures that isolated tasks cannot prove.
...
IAE-330..363 all VERIFIED. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-365..367. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, reports/dashboards, API/webhooks, AI providers, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.

## 11. Allowed files/folders

Use only exact Phase 9 verification, repair, test, evidence paths authorized by WBS. Resolve repository paths; preserve unrelated user-owned changes.
...
- Freeze one checkpoint and reconcile every task, capability, file, migration, contract, spec, test and evidence link.
- Execute reporting/dashboard/analytics/scheduled-report and automation critical flows.
- Execute public/customer/vendor API, webhook, n8n and provider integration compatibility/retry/DLQ flows.
- Execute AI governance, quotation/OCR/ETA/optimization/risk/forecasting review flows with human approval evidence.
- Execute enterprise IAM/MFA/IP/audit/monitoring/retention/dedicated/region/scale/DR/onboarding evidence matrices.

## 21. Main flow

Verification freezes the checkpoint, runs source-to-runtime traceability and critical Phase 9 flows, reconciles outputs/side effects and produces a signed evidence matrix.
...
Integrated evidence index, traceability matrix, API specs, webhook docs, AI governance/evaluation records, integration runbooks, enterprise IAM/monitoring/retention/deployment/DR/support reports, performance profile, migration/recovery report, defect register and IAE-364 build log.
...
Only the execution index may release IAE-365 after this task is `VERIFIED`. Do not set the final Phase 9 closure flag; only Prompt 367 may do so.
