# Prompt 362 — Scale-Up Architecture

**Prompt ID:** `CG-S14-IAE-034`  
**Package document:** `CG-AABPP-IAE-362`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-362.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-034` and exactly one approved WBS/task-ledger item.
...
Workstream: Reliability; Epic: Scale-Up; Capability: Scale-Up Architecture; Feature slice: Workload isolation, queues, read models, partitioning, backpressure and capacity planning; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement scale-up controls for high-volume tenants and enterprise workloads without weakening source-domain correctness.

## 5. Business value

Keep CargoGrid reliable as tenants, data and integrations grow.

## 6. Source requirement

Phase 9 Scale-up architecture; Product Brief performance principles; Delivery Phase 9 analytics/AI jobs isolated from OLTP.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-333 analytics, IAE-358 monitoring and all job/integration/API workloads.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

DR, hardening and release readiness.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Capacity profile, workload classification, queue/backpressure policy, partition/archive recommendation, performance budget and scaling evidence records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Capacity/status/backpressure/admin APIs where needed plus health evidence for jobs/API/report/AI workloads. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Internal scale dashboard, workload profile, queue/backpressure status, performance budget and remediation backlog. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Classify OLTP, analytics, reports, AI, webhooks, imports/exports and notifications workloads.
- Implement queue/backpressure and workload budget evidence.
- Add read-model/materialized-view/partitioning recommendations where needed.
- Build scale monitoring and remediation surface.
- Test high-volume reports, API bursts, webhook storms, AI jobs and import/export queues.

## 21. Main flow

High-volume workload approaches budget; system applies backpressure/queueing, alerts owner and protects OLTP.

## 22. Alternative flow

A tenant requires higher capacity; capacity profile recommends dedicated deployment or architecture upgrade.
...
- Scale-up cannot bypass correctness, idempotency or access policy.
- OLTP remains protected from reports/AI/integration spikes.
- Performance budgets are measured and tied to environment/profile.
- Backpressure is explicit to users/jobs, not silent data loss.
- No global realtime or browser-loaded full datasets.
...
- Scale-Up Architecture unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Scale-Up Architecture.
...
- Scale-Up Architecture works with canonical source-domain ownership and no duplicate truth.
