# Prompt 336 — Integration Hub

**Prompt ID:** `CG-S14-IAE-008`  
**Package document:** `CG-AABPP-IAE-336`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-336.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-008` and exactly one approved WBS/task-ledger item.
...
Workstream: Integrations; Epic: Integration Governance; Capability: Integration Hub; Feature slice: Integration catalog, credentials, adapter registry, health and ownership; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Create a governed integration hub for API, webhook, file, n8n and case-specific third-party adapters without tenant forks.

## 5. Business value

Make integrations operationally manageable while preserving security, idempotency and support visibility.

## 6. Source requirement

Phase 9 Integration hub; Source matrix Integration; RPD-038; BPR integration checklist.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform API key/webhook primitives, secrets, jobs and audit.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-337..346 all integration prompts.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Integration catalog, connection, adapter, credential reference, environment, owner, health, config version and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Integration catalog/connect/config/test/disable/health APIs with secret isolation and adapter capability metadata. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Integration hub marketplace/catalog, setup wizard, connection health, logs and owner/runbook links. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define adapter registry and integration lifecycle states.
- Implement secure connection/config storage using server-only secret references.
- Build health checks, logs and owner/runbook linkage.
- Build integration hub UI with setup/test/disable flows.
- Test tenant isolation, secret leakage, adapter failure and no-tenant-fork rule.

## 21. Main flow

Tenant admin configures an approved integration adapter, tests connectivity and monitors health without seeing secrets.

## 22. Alternative flow

Adapter fails health checks; owner receives alert and traffic can be paused safely.
...
- No tenant-specific code fork; case-specific adapters stay in shared codebase with tests/runbooks.
- Secrets never reach browser, logs, exports or AI prompts.
- Integration config is versioned and auditable.
- Disabling an integration must stop new jobs while preserving evidence/history.
- Unsupported providers require explicit ADR and implementation task.
...
- Integration Hub unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Integration Hub.
...
- Integration Hub works with canonical source-domain ownership and no duplicate truth.
