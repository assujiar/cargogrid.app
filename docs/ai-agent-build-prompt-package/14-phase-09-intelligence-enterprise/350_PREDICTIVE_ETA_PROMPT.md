# Prompt 350 — Predictive ETA

**Prompt ID:** `CG-S14-IAE-022`  
**Package document:** `CG-AABPP-IAE-350`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-350.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-022` and exactly one approved WBS/task-ledger item.
...
Workstream: AI and Analytics; Epic: Predictive Operations; Capability: Predictive ETA; Feature slice: ETA prediction, confidence bands, source features and customer-safe display; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement predictive ETA as an auditable advisory signal over canonical shipment/tracking data.

## 5. Business value

Improve customer and ops visibility while preventing false certainty or autonomous operational decisions.

## 6. Source requirement

Product Brief Predictive ETA; Delivery Phase 9; Operations tracking contracts.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-347, IAE-333, Operations tracking/milestone, maps/telematics if available.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Customer portal tracking, dashboards and monitoring.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
ETA model/version, feature snapshot, prediction, confidence, evaluation, override, display policy and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

ETA prediction/read/evaluate APIs with freshness, confidence, scope and source version. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Ops/customer ETA display with confidence/caveats, freshness, source status, override and feedback capture. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define ETA eligible shipments, features and customer-safe fields.
- Implement prediction request/storage and model/version evidence.
- Integrate ETA display without replacing canonical milestone status.
- Add evaluation, feedback and drift/accuracy tracking.
- Test stale data, low confidence, route changes and customer disclosure.

## 21. Main flow

System generates ETA prediction from allowed shipment/location data and displays it as advisory with confidence and freshness.

## 22. Alternative flow

Prediction is unavailable or low confidence; UI falls back to canonical milestone/status with degraded note.
...
- Predictive ETA is advisory and cannot overwrite shipment milestones.
- Customer display must distinguish predicted versus confirmed status.
- Model/version, features and freshness are auditable.
- Sensitive driver/vendor/internal exception data is not exposed.
- Poor accuracy or drift can disable prediction per tenant/module.
...
- Predictive ETA unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Predictive ETA.
...
- Predictive ETA works with canonical source-domain ownership and no duplicate truth.
