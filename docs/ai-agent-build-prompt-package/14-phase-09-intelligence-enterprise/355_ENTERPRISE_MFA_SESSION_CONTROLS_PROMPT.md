# Prompt 355 — Enterprise MFA Session Controls

**Prompt ID:** `CG-S14-IAE-027`  
**Package document:** `CG-AABPP-IAE-355`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-355.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-027` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Security; Epic: Strong Authentication; Capability: MFA and Session Control; Feature slice: MFA policy, step-up auth, session timeout, device/IP/session review and high-risk action control; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Extend MFA/session controls for enterprise and high-risk actions across the whole platform.

## 5. Business value

Reduce account takeover and privileged-action risk for enterprise deployments.

## 6. Source requirement

Phase 9 MFA; RPD-023; Product Brief security; Delivery authentication tests.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform auth/session/role and IAE-354 IAM.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-356 IP restriction, IAE-357 advanced audit and Step 15 hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
MFA policy, factor enrollment, step-up challenge, session, device, risk event, exception and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

MFA policy/enroll/verify/step-up/session-revoke APIs with role/action conditions and audit. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

MFA policy admin, enrollment UX, high-risk step-up prompts, session/device management and exception review. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define high-risk actions and step-up requirements across modules.
- Implement tenant/role/action MFA policies and session controls.
- Build enrollment, challenge and session/device UX.
- Add exception/break-glass with approval and audit.
- Test revoked sessions, factor reset, privileged exports and SSO interaction.

## 21. Main flow

User attempts high-risk export/API key/payment/AI approval action and must complete current authorization/MFA before proceeding.

## 22. Alternative flow

User loses MFA factor; admin follows approved recovery process with identity proof and audit.
...
- High-risk actions require current authorization; stale sessions are denied.
- MFA exceptions are time-bound, reasoned and audited.
- SSO does not automatically weaken CargoGrid step-up policy.
- Session revocation propagates to API keys/tokens where applicable.
- RPD-022 residual risk is disclosed for Supreme Admin overrides.
...
- MFA and Session Control unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to MFA and Session Control.
...
- MFA and Session Control works with canonical source-domain ownership and no duplicate truth.
