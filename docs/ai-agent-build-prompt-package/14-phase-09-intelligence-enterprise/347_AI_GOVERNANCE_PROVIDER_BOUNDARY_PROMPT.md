# Prompt 347 — AI Governance Provider Boundary

**Prompt ID:** `CG-S14-IAE-019`  
**Package document:** `CG-AABPP-IAE-347`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-347.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-019` and exactly one approved WBS/task-ledger item.
...
Workstream: AI Governance; Epic: Human-Governed AI; Capability: AI-Assisted Capability with Human Governance; Feature slice: AI provider boundary, prompt/evidence logging, approval, evaluation and metering; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Establish the AI/OCR governance foundation before any AI-assisted CargoGrid feature executes in production.

## 5. Business value

Allow useful AI assistance while preventing autonomous critical decisions and uncontrolled provider cost.

## 6. Source requirement

Phase 9 AI-assisted capability with human governance; RPD-021; RPD-028; Delivery Phase 9 model governance.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform audit/jobs/secrets, all domain approval contracts, IAE-336 hub.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-348..353 AI feature prompts and Step 15 hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
AI provider config, model/version, prompt template/version, request, evidence, output, human decision, evaluation, cost meter and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

AI assist request/review/approve/reject/status APIs with redaction, policy, idempotency, timeout and cost metering. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

AI governance console, output review, evidence links, confidence/caveat display, approvals and usage/cost dashboard. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define AI policy matrix for allowed assistance and forbidden autonomous actions.
- Implement provider boundary, prompt/output/evidence logging and redaction.
- Add human review/approval workflow and evaluation dataset hooks.
- Implement usage/cost metering and tenant entitlement controls.
- Test hallucination handling, prompt injection, sensitive data minimization and critical-action denial.

## 21. Main flow

User requests AI assistance; system gathers allowed context, calls provider, presents output with evidence/caveats and waits for human approval where needed.

## 22. Alternative flow

AI provider fails or confidence is low; system returns safe degraded state and manual workflow remains available.
...
- AI cannot autonomously post ledgers/payments, approve legal/tax status, change contracts, release payroll, assign blame or finalize critical decisions.
- AI output is suggestion/evidence, not source truth.
- Prompts and outputs are redacted, scoped, logged and retention-governed.
- Provider costs are metered and visible where billable.
- Prompt injection and untrusted document content are treated as hostile.
...
- AI-Assisted Capability with Human Governance unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to AI-Assisted Capability with Human Governance.
...
- AI-Assisted Capability with Human Governance works with canonical source-domain ownership and no duplicate truth.
