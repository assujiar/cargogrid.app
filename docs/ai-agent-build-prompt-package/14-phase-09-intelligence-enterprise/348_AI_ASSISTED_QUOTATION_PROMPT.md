# Prompt 348 — AI-Assisted Quotation

**Prompt ID:** `CG-S14-IAE-020`  
**Package document:** `CG-AABPP-IAE-348`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-348.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-020` and exactly one approved WBS/task-ledger item.
...
Workstream: AI Assistance; Epic: Commercial AI; Capability: AI-Assisted Quotation; Feature slice: Human-reviewed quotation draft, service suggestion, anomaly/risk notes and evidence; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement AI quotation assistance that drafts suggestions from approved Commercial, rate and shipment context without pricing autonomy.

## 5. Business value

Speed up quotation preparation while keeping margin, approval and customer commitment under human control.

## 6. Source requirement

Product Brief AI-assisted quotation; Delivery Phase 9; RPD-021; Commercial/Procurement/Finance contracts.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-347, Commercial quotation, Procurement rate, Operations shipment profile and Finance tax/currency contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Reporting, audit and Step 15 security hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
AI quote assist request/output/evidence, selected sources, human edits/decision, cost and audit records linked to quotation version. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Quote assist/draft/review/accept-as-draft APIs with source scoping, approval and no direct send/post. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Quotation AI panel with suggested service/rate/risk notes, source citations, confidence/caveats, compare-to-human edits and approve-as-draft. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define allowed quotation context and forbidden pricing commitments.
- Implement AI draft generation through provider boundary.
- Build review UI that requires human edits/acceptance before quote workflow.
- Add evidence/cost/audit and evaluation cases.
- Test hallucinated rates, margin leakage, stale sources and unauthorized send.

## 21. Main flow

Sales user requests AI assistance on quotation; AI drafts suggestions with sources, user reviews/edits, then submits through normal quotation approval.

## 22. Alternative flow

AI flags missing rate/source evidence; user routes inquiry to Procurement instead of generating a quote.
...
- AI never sends quote, approves margin or commits price to customer autonomously.
- Rate/tax/currency values must come from canonical sources and show source version.
- Human edits and final decision are recorded separately from AI suggestion.
- Customer-specific sensitive/margin fields are protected by role policy.
- Low-confidence/no-source output blocks auto-draft acceptance.
...
- AI-Assisted Quotation unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to AI-Assisted Quotation.
...
- AI-Assisted Quotation works with canonical source-domain ownership and no duplicate truth.
