# Prompt 341 — n8n Integration

**Prompt ID:** `CG-S14-IAE-013`  
**Package document:** `CG-AABPP-IAE-341`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-341.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-013` and exactly one approved WBS/task-ledger item.
...
Workstream: Integrations; Epic: Workflow Integration; Capability: n8n Integration; Feature slice: n8n-compatible triggers/actions, credentials and workflow governance; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Provide governed n8n integration points using public API/webhook contracts and safe action allowlists.

## 5. Business value

Enable workflow automation for integration-heavy tenants without custom backend changes.

## 6. Source requirement

Phase 9 n8n integration; Product Brief future n8n; BPR automation/integration patterns.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-335, IAE-337 and IAE-340.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-342 notifications and enterprise onboarding.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
n8n connector registration, workflow reference, token scope, trigger/action allowlist, execution audit and owner records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

n8n trigger/action endpoints, OAuth/API key support where applicable, test endpoints and revocation. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

n8n integration setup guide, token/scopes, sample workflows, execution logs and risk warnings. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define n8n supported triggers/actions and forbidden critical actions.
- Implement connector endpoints over public API/webhook contracts.
- Add token scopes, revocation and execution audit.
- Publish sample workflows and setup UI/docs.
- Test unsafe action attempts, token leakage, duplicate trigger and revocation.

## 21. Main flow

Tenant configures n8n workflow that receives shipment event webhook and calls safe CargoGrid API action.

## 22. Alternative flow

Workflow error triggers retry/DLQ and owner notification without duplicating CargoGrid mutations.
...
- n8n actions cannot bypass domain approvals or human governance.
- Tokens are scoped, revocable and never logged.
- Sample workflows must show idempotency and failure handling.
- External workflow errors do not corrupt CargoGrid state.
- No tenant-specific backend code is generated for n8n.
...
- n8n Integration unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to n8n Integration.
...
- n8n Integration works with canonical source-domain ownership and no duplicate truth.
