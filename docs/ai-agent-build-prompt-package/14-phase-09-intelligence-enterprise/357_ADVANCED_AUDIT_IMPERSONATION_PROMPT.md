...
**Prompt ID:** `CG-S14-IAE-029`  
**Package document:** `CG-AABPP-IAE-357`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-357.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-029` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Security; Epic: Audit Evidence; Capability: Advanced Audit; Feature slice: Enterprise audit search, export, integrity controls, support/impersonation evidence and privileged review; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Extend audit capabilities for enterprise security review while preserving RPD-022 residual-risk honesty.

## 5. Business value

Make critical activity traceable enough for enterprise customers and internal support governance.

## 6. Source requirement

Phase 9 Advanced audit; NFR-AUD-001; Product Brief impersonation logging/support access; RPD-022.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform audit, all domain audit streams, IAE-354..356.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Monitoring, retention and Step 15 hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Audit index/projection, privileged session, impersonation reason/banner, export job, retention/legal hold and review records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Audit search/filter/export/review APIs with field policy, async export, retention and support-access controls. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Advanced audit console, privileged session review, impersonation timeline, export workflow and evidence redaction. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Consolidate audit event taxonomy and critical domains.
- Implement audit search/index/projection with scope and retention.
- Add privileged/support/impersonation session evidence.
- Build audit console and export/review UX.
- Test tenant isolation, tamper claims, export masking and Supreme Admin disclosure.

## 21. Main flow

Security user searches audit events, reviews privileged session evidence and exports a scoped audit package.

## 22. Alternative flow

Support impersonation session is opened with reason, banner and time box; all actions are linked to the session.
...
- Do not claim audit is immutable against Supreme Admin absolute CRUD.
- Tenant ordinary users cannot edit audit logs.
- Audit exports are permission-aware, async and expiring.
- Impersonation/support access requires reason, scope, banner and audit.
- Audit evidence redacts secrets and sensitive payloads.
...
- Advanced Audit unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Advanced Audit.
...
- Advanced Audit works with canonical source-domain ownership and no duplicate truth.
