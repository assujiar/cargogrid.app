# Prompt 333 — Analytics and Materialized Views

**Prompt ID:** `CG-S14-IAE-005`  
**Package document:** `CG-AABPP-IAE-333`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-333.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-005` and exactly one approved WBS/task-ledger item.
...
Workstream: Intelligence Data; Epic: Analytical Workload Isolation; Capability: Analytics and Materialized Views; Feature slice: Analytical summaries, materialized views, refresh jobs and lineage; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Build governed analytics/read-model infrastructure that isolates heavy reporting from OLTP and preserves metric lineage.

## 5. Business value

Keep CargoGrid fast at volume while supporting management analytics.

## 6. Source requirement

Phase 9 Analytics and materialized views; Product Brief §10; BPR Materialized views and separate workload; Delivery Phase 9.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-330..332, Platform jobs and all domain event/source contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-334..335, forecasting/AI, monitoring and performance gates.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Materialized view registry, metric snapshot, refresh schedule/event trigger, freshness, lineage, reconciliation and failure records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Analytics query/freshness/reconcile APIs with read-only projections, cursor pagination and explicit stale/degraded status. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Analytics admin/freshness screen, metric lineage view and dashboard/report stale indicators. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Classify analytics workloads and source facts/dimensions.
- Implement materialized view/summary registry and refresh orchestration.
- Add metric lineage and reconciliation checks.
- Expose freshness/degraded status to reports and dashboards.
- Test refresh idempotency, cross-tenant leakage, stale data and OLTP load isolation.

## 21. Main flow

A scheduled or event-driven refresh updates analytics summaries; reports and dashboards show fresh/stale status and source lineage.

## 22. Alternative flow

A refresh fails and the system preserves previous snapshot with degraded indicator, owner alert and safe retry.
...
- Analytics tables are projections, not new source truth.
- Refresh jobs are tenant-aware, idempotent, observable and backpressure-safe.
- Source/config versions and refresh time are visible to consumers.
- Analytical workload must not degrade critical OLTP paths.
- Sensitive fields are minimized or aggregated with inference controls.
...
- Analytics and Materialized Views unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Analytics and Materialized Views.
...
- Analytics and Materialized Views works with canonical source-domain ownership and no duplicate truth.
