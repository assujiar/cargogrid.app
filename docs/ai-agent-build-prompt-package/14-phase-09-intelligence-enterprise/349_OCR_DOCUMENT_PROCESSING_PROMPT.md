# Prompt 349 — OCR Document Processing

**Prompt ID:** `CG-S14-IAE-021`  
**Package document:** `CG-AABPP-IAE-349`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-349.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-021` and exactly one approved WBS/task-ledger item.
...
Workstream: AI Assistance; Epic: Document AI; Capability: OCR Document Extraction and Classification; Feature slice: OCR extraction, classification, validation queue and human review for logistics/finance/HR/ticket documents; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement OCR/document AI that extracts structured fields into review queues without autonomously posting legal, financial or HR truth.

## 5. Business value

Reduce manual document entry while keeping high-risk records accurate and auditable.

## 6. Source requirement

Product Brief OCR document processing and automated document classification; RPD-021; RPD-032.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-347, Platform files/scanning, Operations documents/ePOD, Finance invoices/tax, HRIS files and Ticketing attachments.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Finance/provider integration, document center and Step 15 hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Document AI job, file reference, extracted fields, classification, confidence, validation result, human correction, applied target and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

OCR submit/status/review/apply-if-approved APIs with file scan, redaction, provider boundary and idempotency. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Document AI review queue with side-by-side file/extraction, confidence, validation errors, correction and apply-to-draft actions. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Classify document types and allowed target fields.
- Implement OCR job pipeline after malware scan and access validation.
- Add extraction validation, review queue and apply-to-draft workflow.
- Build reviewer UX and correction/evaluation capture.
- Test prompt injection inside documents, low confidence, wrong classification and unauthorized field application.

## 21. Main flow

User uploads a scanned document; OCR extracts fields, reviewer corrects and applies them to a draft/source workflow with audit.

## 22. Alternative flow

OCR confidence is low or document is suspicious; job blocks apply and routes to manual review.
...
- OCR output is not legal/financial/HR truth until human-approved source-domain workflow accepts it.
- Unscanned/quarantined files never enter OCR.
- Untrusted document text cannot control prompts/tools/actions.
- Extracted sensitive data follows field policy, retention and redaction.
- Provider cost and job outcome are metered/audited.
...
- OCR Document Extraction and Classification unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to OCR Document Extraction and Classification.
...
- OCR Document Extraction and Classification works with canonical source-domain ownership and no duplicate truth.
