# Prompt 354 — Enterprise IAM SSO SAML OAuth SCIM

**Prompt ID:** `CG-S14-IAE-026`  
**Package document:** `CG-AABPP-IAE-354`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-354.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-026` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Security; Epic: Enterprise IAM; Capability: SSO, SAML, OAuth and SCIM; Feature slice: Enterprise IAM configuration, OIDC/SAML login, OAuth app governance and SCIM lifecycle; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement Enterprise IAM expansion following the ratified OIDC → SAML → SCIM order and Platform identity ownership.

## 5. Business value

Support enterprise customers with centralized identity while preserving CargoGrid access policy.

## 6. Source requirement

Phase 9 SSO, SAML, OAuth; RPD-017; Product Brief security; Delivery Phase 9.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform identity/auth/RBAC/MFA/session and tenant/domain contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-355 MFA/session and Step 15 security hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Enterprise IdP, OAuth app, SAML/OIDC config, SCIM mapping, domain claim, user link, provisioning event and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

IdP config/test/enable/disable, OAuth app lifecycle, SCIM provisioning and login callback APIs with safe rollback. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Enterprise IAM setup wizard, metadata exchange, domain verification, test login, mapping preview, provisioning logs and rollback. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Implement OIDC-first IdP config and domain verification.
- Add SAML support only after OIDC contract and signed need.
- Implement SCIM mapping/provisioning with dry-run and audit.
- Build IAM setup/test/provisioning UI.
- Test account takeover, wrong-domain mapping, deprovisioning and fallback login.

## 21. Main flow

Enterprise admin configures OIDC SSO, verifies domain, tests login and enables for selected tenant users.

## 22. Alternative flow

SCIM deprovisions a user; CargoGrid revokes sessions and preserves audit/history without deleting business records.
...
- Platform identity remains source for CargoGrid authorization; IdP authenticates/provisions according to mapping.
- OIDC precedes SAML, then SCIM unless signed contract changes priority.
- SSO enablement requires break-glass/admin rollback plan.
- SCIM cannot delete required audit/business history.
- MFA/session policy interaction must be explicit.
...
- SSO, SAML, OAuth and SCIM unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to SSO, SAML, OAuth and SCIM.
...
- SSO, SAML, OAuth and SCIM works with canonical source-domain ownership and no duplicate truth.
