# Prompt 359 — Data Retention Archival

**Prompt ID:** `CG-S14-IAE-031`  
**Package document:** `CG-AABPP-IAE-359`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-359.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-031` and exactly one approved WBS/task-ledger item.
...
Workstream: Governance; Epic: Retention and Archival; Capability: Data Retention and Archival; Feature slice: Retention policy, archive jobs, legal hold, retrieval and deletion governance; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement enterprise retention/archival controls consistent with ratified retention categories and legal hold.

## 5. Business value

Control data growth and compliance risk without erasing required business evidence.

## 6. Source requirement

Phase 9 Data retention and archival; RPD-025; Delivery data retention and archival.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform files/jobs/audit, Finance/HR/Ticket/Portal/Loyalty records and legal-hold policy.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Dedicated deployments, DR and release hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Retention policy/version, record classification, archive job, legal hold, deletion request, retrieval, exception and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Retention policy/dry-run/archive/retrieve/hold/release APIs with source-domain approvals and audit. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Retention admin, dry-run preview, legal hold management, archive/retrieval status and exception review. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Map records to approved retention classes and legal-hold rules.
- Implement dry-run retention classification and archive jobs.
- Add legal hold override and retrieval evidence.
- Build retention/archival admin UX.
- Test finance/audit/HR/legal hold conflicts, file retention and no silent deletion.

## 21. Main flow

Admin runs retention dry-run, reviews affected records, applies archive job and records retrieval path.

## 22. Alternative flow

Legal hold blocks deletion/archive for selected records and reports the override reason.
...
- Finance/tax, audit/security and operational retention categories follow RPD-025.
- Legal hold overrides deletion/archive.
- Archival cannot break source-domain reconciliation or audit trails.
- Deletion/minimization cannot erase required posted Finance, payroll or legal evidence.
- Retention policy versions are auditable.
...
- Data Retention and Archival unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Data Retention and Archival.
...
- Data Retention and Archival works with canonical source-domain ownership and no duplicate truth.
