# Prompt 332 — Saved View and Configurable Report

**Prompt ID:** `CG-S14-IAE-004`  
**Package document:** `CG-AABPP-IAE-332`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-332.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-004` and exactly one approved WBS/task-ledger item.
...
Workstream: Intelligence Reporting; Epic: User Views; Capability: Saved View and Configurable Report; Feature slice: Saved table/report views, columns, filters, grouping and sharing; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement saved views and configurable reports on approved datasets with scoped sharing, versioning and export controls.

## 5. Business value

Let teams reuse their most important views without unsafe ad hoc data access.

## 6. Source requirement

Phase 9 Saved view and configurable report; Product Brief configurable reports; BPR API/export/security rules.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-330 and all scoped domain list/report APIs.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-334..335 and public API/export compatibility.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Saved view, configurable report, filter, column, grouping, sort, share, favorite and version records with owner/scope and audit. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Saved view create/update/share/render/export APIs with dataset allowlist, field policy, validation and compatibility version. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

View builder for columns/filters/sorts/grouping, share dialog, personal/team views and export state. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define dataset and field allowlists per role/source domain.
- Implement saved view validation, share policy and versioning.
- Build reusable table/report view builder UX.
- Add export and scheduled-report compatibility checks.
- Test forged filters, hidden columns, shared-view leakage and stale dataset changes.

## 21. Main flow

User saves a scoped report view with chosen columns/filters and shares it with an allowed team or role.

## 22. Alternative flow

A saved view becomes invalid after source schema change; user receives migration guidance and safe fallback.
...
- Saved views never store privileged data snapshots unless explicitly exported through policy.
- Sharing a view does not grant underlying record access.
- Hidden/sensitive fields cannot be reintroduced by URL, JSON or import.
- View versions reference dataset/schema versions and migration status.
- Exports and scheduled reports reauthorize at run time.
...
- Saved View and Configurable Report unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Saved View and Configurable Report.
...
- Saved View and Configurable Report works with canonical source-domain ownership and no duplicate truth.
