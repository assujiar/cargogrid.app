# Prompt 342 — Email WhatsApp SMS Integrations

**Prompt ID:** `CG-S14-IAE-014`  
**Package document:** `CG-AABPP-IAE-342`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-342.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-014` and exactly one approved WBS/task-ledger item.
...
Workstream: Communication Integrations; Epic: Messaging Providers; Capability: Email, WhatsApp and SMS; Feature slice: Provider adapters, templates, consent, queue, delivery and cost metering; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement governed messaging integrations for notifications and scheduled/report/automation delivery with provider-specific adapters.

## 5. Business value

Support customer/internal communication while controlling consent, cost and delivery risk.

## 6. Source requirement

Phase 9 Email, WhatsApp, SMS; RPD-028 provider-cost metering; BPR notification queue patterns.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform notifications/jobs/templates, IAE-335 automation and IAE-336 integration hub.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Scheduled reports, alerts, onboarding and support workflows.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Provider connection, message template/version, consent/preference, queue, delivery, retry, cost meter and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Message template/send/status/cancel/provider-test APIs with consent, rate limit, retry and DLQ behavior. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Messaging integration setup, templates, delivery logs, consent/preference view and metered usage. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define message categories, consent/preference and template versioning.
- Implement provider adapter boundary and queued delivery.
- Add cost metering and delivery/retry/DLQ audit.
- Build setup/template/log UI.
- Test opt-out, duplicate send, provider outage, template leakage and cost meter accuracy.

## 21. Main flow

Automation triggers an approved message template; system checks consent, queues delivery, records provider response and cost.

## 22. Alternative flow

Provider outage moves messages to retry/DLQ with owner alert and no duplicate delivery after recovery.
...
- Messaging uses case-specific shared adapters; no tenant fork.
- Consent, preference, recipient scope and message category are enforced before send.
- Provider secrets and payloads are never exposed to browser or logs.
- Provider cost +20% metering must be customer-visible where billable.
- Critical legal/financial messages require stronger approval if configured.
...
- Email, WhatsApp and SMS unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Email, WhatsApp and SMS.
...
- Email, WhatsApp and SMS works with canonical source-domain ownership and no duplicate truth.
