# Prompt 356 — IP Restriction Network Access

**Prompt ID:** `CG-S14-IAE-028`  
**Package document:** `CG-AABPP-IAE-356`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-356.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-028` and exactly one approved WBS/task-ledger item.
...
Workstream: Enterprise Security; Epic: Network Policy; Capability: IP Restriction; Feature slice: Tenant/user/API IP allowlists, denied access logging and emergency bypass; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement enterprise IP and network access restrictions for UI, API, support and admin surfaces.

## 5. Business value

Give enterprise customers stronger boundary controls for sensitive deployments.

## 6. Source requirement

Phase 9 IP restriction; Delivery SSO/MFA/IP/audit gate; Product Brief security.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
Platform access/session/API key and IAE-354/355.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Advanced audit, monitoring and release hardening.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Network policy, CIDR/IP allowlist, target surface, exception, evaluation result and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Network policy CRUD/evaluate/test APIs and enforcement middleware/service hooks for UI/API/support. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Network policy setup/test, dry-run results, lockout warning, denied access log and emergency bypass workflow. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define protected surfaces and policy evaluation order.
- Implement dry-run/test before enforcement to prevent lockout.
- Enforce policy across UI/API/support/admin where applicable.
- Build policy setup and denied-log UX.
- Test IPv4/IPv6, proxy headers, lockout prevention and emergency rollback.

## 21. Main flow

Enterprise admin configures IP allowlist in dry-run, reviews denied events, then enables enforcement for admin/API surfaces.

## 22. Alternative flow

Misconfigured policy locks out users; break-glass path restores access with audit.
...
- IP restriction complements, not replaces, authentication/RBAC/RLS.
- Header trust must be deployment-aware and spoof-resistant.
- Policy changes are high-risk and require current authorization.
- Denied attempts are logged without leaking sensitive context.
- Emergency bypass is time-bound, approved and audited.
...
- IP Restriction unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to IP Restriction.
...
- IP Restriction works with canonical source-domain ownership and no duplicate truth.
