# Prompt 351 — Optimization Assistance

**Prompt ID:** `CG-S14-IAE-023`  
**Package document:** `CG-AABPP-IAE-351`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-351.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-023` and exactly one approved WBS/task-ledger item.
...
Workstream: AI and Optimization; Epic: Human-Governed Optimization; Capability: Route, Dispatch and Warehouse Optimization Assistance; Feature slice: Route/dispatch/slotting/picking recommendations with human approval and explainability; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement optimization assistance for routes, dispatch, warehouse slotting or picking recommendations without autonomous execution.

## 5. Business value

Improve operational efficiency while keeping planners and warehouse leads accountable.

## 6. Source requirement

Product Brief route optimization and warehouse slotting optimization; Delivery Phase 9 optimization; RPD-021.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-347, Operations/TMS/WMS data, maps/telematics and analytics summaries.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Performance reporting and enterprise controls.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Optimization scenario, input snapshot, recommendation, constraint set, score, human decision, applied action and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Optimization scenario/create/run/review/apply-to-draft APIs with constraints, idempotency and job controls. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Optimization workbench showing scenarios, recommendations, tradeoffs, constraints, explanation and approve-as-draft. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define optimization scopes, constraints and forbidden autonomous actions.
- Implement scenario snapshot and job-based recommendation generation.
- Build human review workbench with explanation/tradeoff display.
- Add application only to draft/request workflows after approval.
- Test infeasible constraints, stale inputs, unsafe recommendation and rollback.

## 21. Main flow

Planner runs an optimization scenario, reviews recommendations and applies selected plan to a draft workflow for normal approval.

## 22. Alternative flow

System cannot find feasible plan; it explains blocking constraints and leaves existing plan unchanged.
...
- Optimization never dispatches, reassigns, moves stock or changes route autonomously.
- Recommendations reference input snapshot and constraint version.
- Human decision and edits are recorded separately.
- Cost/margin/vendor sensitive inputs are masked by role.
- Scenario results expire when source data changes materially.
...
- Route, Dispatch and Warehouse Optimization Assistance unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Route, Dispatch and Warehouse Optimization Assistance.
...
- Route, Dispatch and Warehouse Optimization Assistance works with canonical source-domain ownership and no duplicate truth.
