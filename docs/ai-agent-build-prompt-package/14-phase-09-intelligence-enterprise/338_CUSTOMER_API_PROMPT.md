# Prompt 338 — Customer API

**Prompt ID:** `CG-S14-IAE-010`  
**Package document:** `CG-AABPP-IAE-338`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-338.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-010` and exactly one approved WBS/task-ledger item.
...
Workstream: API Ecosystem; Epic: Customer API; Capability: Customer API; Feature slice: Customer-scoped API access for quote, booking, tracking, documents, invoices, tickets and loyalty; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Expose customer-safe API surfaces that mirror Customer Portal scope and reauthorization rules.

## 5. Business value

Let customers integrate their own systems with CargoGrid without portal scraping or unsafe data sharing.

## 6. Source requirement

Phase 9 Customer API; Phase 8 CPT/LYL contracts; NFR-API-001.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-337 and Step 13 customer scope/domain contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-340 webhooks and enterprise onboarding.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Customer API client scopes, account/site grants, consent/contract references, usage, idempotency and audit. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Customer quote/booking/tracking/document/invoice/ticket/loyalty endpoints with Layer 4 membership/scope parity. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Customer API setup and docs limited to customer admin/tenant admin with scopes and usage. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Map Customer Portal actions/views to safe API endpoints.
- Implement account/site scoped API grants and usage/audit.
- Add idempotent create/update endpoints for quote/booking/ticket where allowed.
- Generate docs and examples with field policy notes.
- Test cross-customer IDs, revoked grants, signed file access and rate limits.

## 21. Main flow

Customer system calls scoped tracking or invoice endpoint using API credentials tied to account/site permissions.

## 22. Alternative flow

Customer creates booking through API; idempotent retry returns same accepted request rather than duplicate booking.
...
- Customer API uses the same Layer 4 scope as portal, not broader tenant admin scope.
- API grants no access to internal notes, vendor/cost/margin, support-only fields or other customers.
- File downloads use short-lived signed URLs and reauthorization.
- Customer API mutation remains subject to source-domain workflows/approvals.
- Saved credentials are governed by tenant/customer contract and revocation policy.
...
- Customer API unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Customer API.
...
- Customer API works with canonical source-domain ownership and no duplicate truth.
