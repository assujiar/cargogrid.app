# Prompt 339 — Vendor API

**Prompt ID:** `CG-S14-IAE-011`  
**Package document:** `CG-AABPP-IAE-339`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-339.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-011` and exactly one approved WBS/task-ledger item.
...
Workstream: API Ecosystem; Epic: Vendor API; Capability: Vendor API; Feature slice: Vendor-facing API for onboarding, rates, RFQ, capacity, assignment, proof and invoice status; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Expose optional vendor API surfaces without introducing a fifth principal layer or duplicating Procurement/Vendor truth.

## 5. Business value

Support high-volume vendor collaboration while keeping vendor/customer/internal isolation intact.

## 6. Source requirement

Phase 9 Vendor API; Step 11 optional vendor portal; NFR-API-001; RPD-038.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-337 and Step 11 Procurement/Vendor contracts.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

IAE-340 webhooks and carrier/integration adapters.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Vendor API client, vendor grant, endpoint scope, idempotency, submission, usage and audit records linked to canonical vendor records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Vendor registration/rate/RFQ/capacity/assignment/proof/invoice-status APIs with explicit vendor grants and source-domain validation. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Vendor API grant/setup/docs in integration hub with scope preview and revocation. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define vendor API resources and grants from Procurement contracts.
- Implement scoped endpoints without new principal layer or duplicate vendor truth.
- Add idempotent submissions and validation summaries.
- Build API documentation and grant management UI.
- Test vendor/customer isolation, forged PO/assignment IDs and duplicate submissions.

## 21. Main flow

Approved vendor submits capacity or RFQ response through scoped API and Procurement stores it against canonical records.

## 22. Alternative flow

Vendor API is disabled for a tenant; manual/vendor portal process continues without broken source contracts.
...
- Vendor API is optional and does not create a fifth global access layer.
- A vendor grant cannot access customer portal data or other vendors.
- Vendor submissions require source-domain validation and idempotency.
- Bank/tax/payment changes remain high-risk and require maker-checker/MFA workflow.
- No provider-specific tenant fork is allowed.
...
- Vendor API unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Vendor API.
...
- Vendor API works with canonical source-domain ownership and no duplicate truth.
