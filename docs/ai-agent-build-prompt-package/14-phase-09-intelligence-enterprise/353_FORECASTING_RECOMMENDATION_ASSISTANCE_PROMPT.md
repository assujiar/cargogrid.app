# Prompt 353 — Forecasting and Recommendation Assistance

**Prompt ID:** `CG-S14-IAE-025`  
**Package document:** `CG-AABPP-IAE-353`  
**Version:** `0.15.0`  
**Runtime build log:** `docs/build-log/phase-09/IAE-353.md`
...
`{{TASK_ID}}` mapped to `CG-S14-IAE-025` and exactly one approved WBS/task-ledger item.
...
Workstream: AI and Analytics; Epic: Predictive Business Assistance; Capability: Forecasting and Recommendation Assistance; Feature slice: Demand, revenue, churn, vendor and predictive-maintenance advisory signals; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Implement governed forecasting/recommendation assistance for planning decisions without autonomous commitments.

## 5. Business value

Help management plan demand, revenue, retention, vendor strategy and maintenance using auditable data signals.

## 6. Source requirement

Product Brief demand forecasting, revenue forecasting, customer churn prediction, vendor recommendation engine and predictive maintenance; Delivery Phase 9.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE-333, IAE-347 and all source-domain analytics summaries.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Dashboard/reporting, enterprise monitoring and management review.. Identify affected schemas, services, REST/GraphQL, jobs/integrations/files, AI provider boundary, reports/dashboards, public API/webhooks, enterprise IAM/deployment/monitoring, tests, docs and compatibility contracts.
...
Forecast/recommendation job, model/version, feature snapshot, horizon, confidence, scenario, decision/feedback and audit records. Preserve canonical domain ownership, tenant_id, source/config/model/provider versions, idempotency, audit, retention and rollback-safe additive migrations.

## 14. API impact

Forecast/run/read/compare/feedback APIs with scope, freshness, confidence and job controls. REST and GraphQL must share authentication, authorization, field policy, idempotency, audit, version and compatibility semantics.

## 15. UI/UX impact

Forecasting workbench and dashboard widgets with scenario assumptions, confidence, caveats and feedback capture. Include keyboard/focus/labels, responsive online-first behavior, loading/empty/error/denied/stale/degraded/conflict states, human-review states for AI/automation and no dead action.
...
- Define forecast/recommendation use cases and decision boundaries.
- Implement job-based forecasts using approved analytics features.
- Build workbench/widgets with assumptions and confidence disclosure.
- Add feedback/evaluation and drift monitoring.
- Test low data volume, stale features, overconfident output and unauthorized segmentation.

## 21. Main flow

Manager runs demand/revenue/churn/vendor forecast, reviews assumptions/confidence and records planning decision outside autonomous workflow.

## 22. Alternative flow

Insufficient data produces a no-forecast state with data-quality recommendations.
...
- Forecasts are advisory and cannot create commitments, budgets, vendor awards, maintenance orders or customer actions autonomously.
- Feature snapshots and model versions are auditable.
- Small-cohort and customer-sensitive inference controls apply.
- Low confidence or insufficient data must be explicit.
- Management targets/quota modules remain separate unless source-defined.
...
- Forecasting and Recommendation Assistance unit/service/database/API/contract/job/UI tests.
...
Update Phase 9 build log, source-domain ownership notes, AI/automation/API/integration/enterprise runbooks, API specs, webhook event docs, report/dashboard metric docs, security/retention/DR docs, traceability, dependency, rollback and handoff artifacts relevant to Forecasting and Recommendation Assistance.
...
- Forecasting and Recommendation Assistance works with canonical source-domain ownership and no duplicate truth.
