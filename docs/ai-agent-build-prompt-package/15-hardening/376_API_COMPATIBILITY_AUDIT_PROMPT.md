# Prompt 376 — API Compatibility Audit

**Prompt ID:** `CG-S15-HDN-008`  
**Package document:** `CG-AABPP-HDN-376`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-376.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-008` and exactly one approved WBS/task-ledger item.
...
Workstream: API Assurance; Epic: Contract Compatibility; Capability: API Compatibility Audit; Feature slice: REST, GraphQL, public API, webhooks, exports and schema compatibility; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify API, webhook, export and schema compatibility across internal and external consumers.

## 5. Business value

Prevent integrations and customer systems from breaking during release candidate preparation.

## 6. Source requirement

Master Prompt Step 15 API compatibility audit; NFR-API-001; Delivery compatibility rule.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-370 and Phase 9 API/webhook evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-378 and Step 16 release freeze.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect API version, idempotency, webhook event, export schema, migration and compatibility/deprecation records. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Run REST/GraphQL parity, public/customer/vendor API, webhook signing/retry/replay and error contract tests. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify developer/admin consoles show versions, scopes, usage, deprecation and compatibility notices. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Generate or inspect API/webhook/export contract specs.
- Run compatibility tests for supported versions and deprecation paths.
- Test idempotency, rate limit, signing, replay, error shapes and pagination.
- Verify schema/migration changes have compatibility plan.
- Block release on breaking change without approved path.

## 21. Main flow

Supported API and webhook clients run contract tests against current checkpoint and receive compatible payloads/errors.

## 22. Alternative flow

A breaking change is isolated to a new version with documented migration and old version preserved until deprecation date.
...
- Public API, webhook payload, export schema and database contract cannot break silently.
- REST/GraphQL parity must hold where promised.
- Duplicate retry cannot duplicate transactions.
- Webhook spoofing and unsigned callbacks fail.
- Compatibility evidence is required before Step 16.
...
- API Compatibility Audit automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to API Compatibility Audit.
...
- API Compatibility Audit hardening gate passes or has explicit release-blocking issue with owner and reproduction.
