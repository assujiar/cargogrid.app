# Prompt 388 — Full-System Hardening Documentation Handoff

**Prompt ID:** `CG-S15-HDN-020`  
**Package document:** `CG-AABPP-HDN-388`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-388.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-020` and exactly one approved WBS/task-ledger item.
...
Workstream: Hardening Closure; Epic: Full-System Hardening Evidence; Capability: Full-System Hardening Documentation Handoff; Feature slice: repository-specific bounded slice; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Complete hardening documentation, runbooks, traceability, blocker status and Step 16 release-candidate handoff after blocker remediation.
...
HDN-387 VERIFIED. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-389 and Step 16 release-candidate package. Identify affected tests, docs, runbooks, release blockers, rollback paths and release-candidate gates.
...
- Update full hardening report and evidence index.
- Update security, finance, API, storage, performance, accessibility, observability, backup, DR and migration runbooks.
- Update release-blocker register and residual risk disclosure.
- Prepare Step 16 handoff with go/no-go criteria and exact resume instructions.
- Verify no production, pilot, GA or market-ready claim is made.
...
- Step 16 handoff must state exactly which gates passed, failed or remain blocked.
- Runbooks must be actionable without hidden context.
- Residual risks must preserve RPD-022 and RPD-021 disclosures.
...
- Full-System Hardening Documentation Handoff output is complete, evidence-backed and checkpoint-consistent.
...
Only the execution index may release HDN-389 after this task is `VERIFIED`. Do not set `FULL_SYSTEM_HARDENING_VERIFIED`; only Prompt 389 may do so.
