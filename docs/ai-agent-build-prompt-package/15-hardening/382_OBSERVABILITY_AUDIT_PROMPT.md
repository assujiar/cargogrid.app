# Prompt 382 — Observability Audit

**Prompt ID:** `CG-S15-HDN-014`  
**Package document:** `CG-AABPP-HDN-382`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-382.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-014` and exactly one approved WBS/task-ledger item.
...
Workstream: Reliability Assurance; Epic: Monitoring and Alerting; Capability: Observability; Feature slice: Application, database, job, integration, security, audit and user activity monitoring; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify observability, alerting and runbook readiness across core app, DB, jobs, integrations, API, AI, security and tenant health.

## 5. Business value

Ensure failures are detectable, diagnosable and supportable before release.

## 6. Source requirement

Master Prompt Step 15 Observability; BPR Monitoring/Logging; Delivery enterprise monitoring.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
IAE enterprise monitoring evidence and all modules.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-383 backup/restore, HDN-384 DR and Step 16 operations readiness.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect health, incident, alert, job, integration, audit and metric records where present. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Verify health/status/metrics endpoints and alert sources do not leak tenant data. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify monitoring dashboards, incident timelines, alert ownership and support runbook links. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Map observability coverage for app, DB, queue, jobs, integrations, API, AI, storage and security events.
- Trigger or simulate representative failures and verify alerts/logs/traces.
- Verify dashboards show tenant-safe health and owners.
- Check runbook links and incident severity workflow.
- Register blind spots as blockers or backlog with severity.

## 21. Main flow

A job/webhook/API/database failure produces actionable alert, redacted logs and runbook-linked incident evidence.

## 22. Alternative flow

If external monitoring is not available locally, record instrumentation evidence and required production validation gate.
...
- Monitoring must not leak tenant data or secrets.
- Alerts require owner, severity, deduplication and escalation path.
- Security events are logged and reviewable.
- No silent DLQ/backpressure accumulation.
- Observability evidence is required for Step 16.
...
- Observability automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Observability.
...
- Observability hardening gate passes or has explicit release-blocking issue with owner and reproduction.
