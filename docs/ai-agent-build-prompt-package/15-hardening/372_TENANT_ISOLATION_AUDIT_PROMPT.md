# Prompt 372 — Tenant Isolation Audit

**Prompt ID:** `CG-S15-HDN-004`  
**Package document:** `CG-AABPP-HDN-372`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-372.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-004` and exactly one approved WBS/task-ledger item.
...
Workstream: Security Assurance; Epic: Tenant Boundary; Capability: Tenant Isolation Audit; Feature slice: Cross-tenant negative tests across database, storage, cache, queue, reports, jobs, integrations and AI; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Prove strict tenant isolation across all runtime surfaces and data paths.

## 5. Business value

Prevent the most dangerous SaaS failure: one tenant accessing another tenant's data or configuration.

## 6. Source requirement

Master Prompt Step 15 Tenant isolation audit; Security Guardrails; BPR NFR-SEC-001; Delivery security gate.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-370 and Platform/Phase 9 access evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-373 RLS/RBAC, HDN-378 security hardening and closure.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect every tenant-scoped table, view, materialized view, function, trigger, policy, storage object reference, queue job and audit projection. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Attack REST/GraphQL/public API/webhook/report/export endpoints with Tenant A/B identities, tokens, IDs, filters and cursors. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify denial/error/empty states do not reveal out-of-scope existence through messages, counts, URLs or cached snippets. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Build tenant A/B fixtures across all modules and critical records.
- Run direct database/RLS, service, API, job, report, export, cache and file negative tests.
- Test support/impersonation, API clients, AI jobs, webhooks and integrations for tenant leakage.
- Inspect logs/monitoring/audit/report artifacts for cross-tenant data exposure.
- Block release on any critical/high isolation failure.

## 21. Main flow

Tenant A attempts to access Tenant B data through every known surface and each attempt fails safely with audit evidence.

## 22. Alternative flow

If a compensating control replaces RLS for a specific object, verify the approved ADR, test evidence and monitoring.
...
- Every tenant-facing table uses RLS or documented approved compensating control.
- Storage, cache, queue, reports, exports, logs, AI context and integrations are tenant-bound.
- Errors/counts/search/cursors cannot reveal other-tenant existence.
- Service role paths are server-only and scoped by explicit checks.
- Any critical/high cross-tenant leak blocks closure.
...
- Tenant Isolation Audit automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Tenant Isolation Audit.
...
- Tenant Isolation Audit hardening gate passes or has explicit release-blocking issue with owner and reproduction.
