# Prompt 371 — Cross-Module Transactional Integrity

**Prompt ID:** `CG-S15-HDN-003`  
**Package document:** `CG-AABPP-HDN-371`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-371.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-003` and exactly one approved WBS/task-ledger item.
...
Workstream: Integrity Assurance; Epic: End-to-End Business Integrity; Capability: Cross-Module Transactional Integrity; Feature slice: Critical transaction chains and source-domain side effects; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify that cross-module transaction flows preserve source ownership, idempotency, state transitions, reconciliation and no duplicate truth.

## 5. Business value

Protect the end-to-end logistics ERP backbone from corruption across module boundaries.

## 6. Source requirement

Master Prompt Step 15 Cross-module transactional integrity; Data and Financial Integrity Guardrails critical E2E flows; BPR Traceability Matrix.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-370 plus all module closure evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-374 financial integrity, HDN-375 lineage, HDN-386 integrated verification and Step 16 UAT.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect foreign keys, unique constraints, idempotency keys, status histories, event ledgers, reconciliation tables and versioned config references across domains. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Exercise conversion/handoff APIs for lead→quote→job, shipment→billing, AP/AR/payment, WMS, portal, loyalty and tickets. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify user-visible state continuity for critical flows without duplicate entry or fake success. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Execute minimum critical E2E flows from lead-to-payment, shipment-to-invoice, WMS, portal, loyalty and ticketing.
- Verify idempotent retries and concurrent submissions at each module boundary.
- Reconcile source totals, statuses, ledgers and downstream projections.
- Test rollback/correction flows without deleting required history.
- Register every source-domain ownership conflict as blocker.

## 21. Main flow

A source entity moves through approved downstream handoffs and every side effect is reconciled to one canonical owner and audit chain.

## 22. Alternative flow

If a downstream module is unavailable, verify the handoff contract, queue state and exact resume without fabricating downstream success.
...
- A conversion/handoff cannot create duplicate customer, quote, shipment, invoice, employee, ticket, vendor, inventory or loyalty truth.
- Every retriable mutation has idempotency and replay evidence.
- Status transitions use stable canonical states and versioned rules.
- Rollback uses governed correction/reversal, not destructive deletion.
- Critical flow break blocks Step 16.
...
- Cross-Module Transactional Integrity automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Cross-Module Transactional Integrity.
...
- Cross-Module Transactional Integrity hardening gate passes or has explicit release-blocking issue with owner and reproduction.
