# Prompt 386 — Full-System Hardening Integrated Verification

**Prompt ID:** `CG-S15-HDN-018`  
**Package document:** `CG-AABPP-HDN-386`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-386.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-018` and exactly one approved WBS/task-ledger item.
...
Workstream: Hardening Closure; Epic: Full-System Hardening Evidence; Capability: Full-System Hardening Integrated Verification; Feature slice: repository-specific bounded slice; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Independently verify that all Step 15 hardening gates have current evidence, no mixed-checkpoint result and no unowned critical/high blocker before blocker remediation.
...
HDN-370..385 all VERIFIED. Every execution-index prerequisite must be `VERIFIED`.
...
- Freeze one hardening checkpoint and reconcile all Step 15 task evidence.
- Build a full hardening pass/fail matrix across 16 mandatory gates.
- Re-run targeted smoke for every repaired or high-risk area.
- Confirm runtime evidence is not replaced by package-generation claims.
- Publish blocker list, owner, severity, reproduction and Step 16 impact.
...
- All hardening evidence must refer to one compatible checkpoint.
- Any critical/high unowned blocker stops Step 16.
- Pre-existing failures require baseline proof and accepted-risk disposition.
...
- Full-System Hardening Integrated Verification output is complete, evidence-backed and checkpoint-consistent.
...
Only the execution index may release HDN-387 after this task is `VERIFIED`. Do not set `FULL_SYSTEM_HARDENING_VERIFIED`; only Prompt 389 may do so.
