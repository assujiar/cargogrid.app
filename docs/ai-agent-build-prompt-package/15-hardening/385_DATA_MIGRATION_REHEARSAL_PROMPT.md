# Prompt 385 — Data Migration Rehearsal

**Prompt ID:** `CG-S15-HDN-017`  
**Package document:** `CG-AABPP-HDN-385`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-385.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-017` and exactly one approved WBS/task-ledger item.
...
Workstream: Migration Assurance; Epic: Migration Readiness; Capability: Data Migration Rehearsal; Feature slice: Import/migration mapping, validation, duplicate handling, rollback, reconciliation and audit; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify data migration rehearsal readiness across tenant setup, master data, transactional opening balances/files and enterprise migration scenarios.

## 5. Business value

Reduce go-live failure risk caused by bad data, duplicate records or unrecoverable imports.

## 6. Source requirement

Master Prompt Step 15 Data migration rehearsal; BPR data migration checklist; Delivery enterprise migration rehearsal.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
All module import/migration contracts and HDN-371 lineage evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Step 16 cutover and go-live package.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect migration scripts/templates, staging tables, mapping, duplicate rules, validation summaries, reconciliation, rollback and audit records. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Exercise import/preview/validate/commit/rollback APIs with idempotency and partial success policy. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify migration preview, error report, reconciliation and approval UX for tenant onboarding. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Define migration rehearsal datasets and mapping scope.
- Run preview/validation/duplicate handling on representative data.
- Commit to safe rehearsal environment and reconcile counts/totals/files/lineage.
- Test rollback and rerun idempotency.
- Document blockers for data quality, missing fields or manual remediation.

## 21. Main flow

Migration rehearsal imports mapped data into safe environment, validates/reconciles it and proves rollback/rerun path.

## 22. Alternative flow

Dirty source data fails validation; migration blocks with error report, owner and remediation plan.
...
- No destructive production import without preview, backup, approval and rollback.
- Migration must preserve source references and legal/audit history.
- Duplicate handling is deterministic and reviewable.
- Financial opening balances require exact reconciliation.
- Enterprise tenants require at least two rehearsals where contracted.
...
- Data Migration Rehearsal automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Data Migration Rehearsal.
...
- Data Migration Rehearsal hardening gate passes or has explicit release-blocking issue with owner and reproduction.
