# Prompt 373 — RLS and RBAC Audit

**Prompt ID:** `CG-S15-HDN-005`  
**Package document:** `CG-AABPP-HDN-373`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-373.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-005` and exactly one approved WBS/task-ledger item.
...
Workstream: Security Assurance; Epic: Permission Enforcement; Capability: RLS and RBAC Audit; Feature slice: Role, scope, field, record, action, amount/status and support access policy verification; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify that RLS, RBAC and scoped permission enforcement match the fixed four-layer access model and module contracts.

## 5. Business value

Ensure users can do exactly what they are allowed to do—nothing more, nothing less.

## 6. Source requirement

Master Prompt Step 15 RLS and RBAC audit; Security Guardrails; BPR NFR-SEC-002.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-372 and Platform IAM evidence.. Every execution-index prerequisite must be `VERIFIED`.
...
Inspect policies, grants, functions, custom claims, role mapping, field masking, record filters, support grants and policy test fixtures. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Exercise CRUD/action/export/import/report/API-key/webhook/admin/support endpoints for allowed and denied roles. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Verify hidden/disabled UI actions are backed by server denial and complete denied states. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Build access matrix across Supreme Admin, User Admin, internal hierarchy, Customer User, support and API clients.
- Run positive and negative RLS/RBAC tests for module/action/field/record/status/amount/scope.
- Verify field masking for finance, payroll, personal, tax, bank, margin, support and AI evidence.
- Test support access, impersonation and revocation propagation.
- Document policy gaps with severity and exact owner.

## 21. Main flow

Each role attempts allowed and forbidden operations; server/database policy outcomes match the access matrix and are audited.

## 22. Alternative flow

If a permission is intentionally broad, verify decision record, warning, audit and RPD-022 residual-risk disclosure where applicable.
...
- UI-only permission is invalid.
- Managers do not inherit sensitive payroll/bank/tax/talent/customer data through hierarchy alone.
- Support grants are case-, purpose- and time-bound.
- Exports/reports/search/realtime use the same access policy as detail APIs.
- Permission weakening to pass a test is forbidden.
...
- RLS and RBAC Audit automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to RLS and RBAC Audit.
...
- RLS and RBAC Audit hardening gate passes or has explicit release-blocking issue with owner and reproduction.
