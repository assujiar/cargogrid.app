# Prompt 379 — Performance and Scalability

**Prompt ID:** `CG-S15-HDN-011`  
**Package document:** `CG-AABPP-HDN-379`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-379.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-011` and exactly one approved WBS/task-ledger item.
...
Workstream: Performance Assurance; Epic: Scale Readiness; Capability: Performance and Scalability; Feature slice: Latency, load, query plans, queues, backpressure and target-volume evidence; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify system performance and scalability budgets for core pages, APIs, reports, jobs, imports/exports, integrations and AI workloads.

## 5. Business value

Ensure the product remains usable under realistic SME-to-enterprise load.

## 6. Source requirement

Master Prompt Step 15 Performance and scalability; BPR NFR-PERF; Product Brief performance principles.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-370, IAE scale-up evidence and all high-volume modules.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-386 and Step 16 capacity go/no-go.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
Inspect indexes, query plans, materialized views, partitions/archives, slow queries, queue tables and backpressure records. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Measure representative server response, API, report/export, webhook and job performance with declared dataset/concurrency. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Measure Web Vitals or repository equivalents for core internal and customer portal journeys. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Define target-volume profiles and budgets by module/workload.
- Run load/performance tests available in repository or create scoped profiles.
- Review query plans for critical list/detail/report/import/export endpoints.
- Stress queues, webhooks, scheduled reports, AI jobs and imports/exports for backpressure.
- Record regressions, remediation and capacity recommendations.

## 21. Main flow

Performance suite runs against declared data profile and records p95, query count, payload size, job duration and bottlenecks.

## 22. Alternative flow

If environment is undersized, report scaled interpretation and required production-like retest before release.
...
- No SELECT * or client-loaded full datasets on critical paths.
- Heavy reports/imports/exports/AI jobs run async or precomputed.
- Backpressure prevents overload without silent data loss.
- Performance budgets are evidence-based and environment-labeled.
- Cross-tenant cache leakage is a security blocker, not just performance issue.
...
- Performance and Scalability automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Performance and Scalability.
...
- Performance and Scalability hardening gate passes or has explicit release-blocking issue with owner and reproduction.
