# Prompt 380 — Accessibility Audit

**Prompt ID:** `CG-S15-HDN-012`  
**Package document:** `CG-AABPP-HDN-380`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-380.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-012` and exactly one approved WBS/task-ledger item.
...
Workstream: UX Assurance; Epic: Accessible UX; Capability: Accessibility; Feature slice: WCAG-aware keyboard, focus, labels, contrast, error, responsive state checks; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify accessibility readiness across core enterprise, operations, customer, admin and support workflows.

## 5. Business value

Make the system usable and reduce enterprise adoption risk.
...
HDN-370 and all portal/UI evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

HDN-381 and Step 16 UAT.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
No planned database change except defect evidence links. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Verify accessibility fixes do not weaken server policy or API validation. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Audit keyboard navigation, focus order, labels, contrast, screen-reader semantics, error messages, forms, modals, tables, timelines, dashboards and mobile states. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Inventory critical UI flows across all four access layers.
- Run available automated accessibility checks and manual keyboard/focus review.
- Verify form validation, error summaries and denied states are understandable.
- Check color contrast and responsive readability.
- Fix bounded accessibility defects or register blockers.

## 21. Main flow

Tester navigates critical workflows without mouse and receives clear focus, labels, errors and states.

## 22. Alternative flow

A complex data grid has limited screen-reader support; document mitigation, keyboard support and backlog severity.
...
- Accessibility cannot be sacrificed for visual style.
- Hidden actions must not remain keyboard-focusable.
- Error/denied states must explain safe next step.
- Color cannot be the only signal.
- Accessibility evidence covers customer portal and internal portals.
...
- Accessibility automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Accessibility.
...
- Accessibility hardening gate passes or has explicit release-blocking issue with owner and reproduction.
