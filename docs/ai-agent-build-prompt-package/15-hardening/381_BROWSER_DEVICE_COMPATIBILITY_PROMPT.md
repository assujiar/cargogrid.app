# Prompt 381 — Browser Device Compatibility

**Prompt ID:** `CG-S15-HDN-013`  
**Package document:** `CG-AABPP-HDN-381`  
**Version:** `0.16.0`  
**Runtime build log:** `docs/build-log/full-system-hardening/HDN-381.md`
...
`{{TASK_ID}}` mapped to `CG-S15-HDN-013` and exactly one approved WBS/task-ledger item.
...
Workstream: UX Assurance; Epic: Device and Browser Readiness; Capability: Browser and Device Compatibility; Feature slice: Chrome, Edge, Safari, Firefox, tablet/mobile responsive and PWA online-first compatibility; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify supported browser and device behavior for core workflows without native/offline-sync claims.

## 5. Business value

Avoid launch friction across real customer and internal environments.

## 6. Source requirement

Master Prompt Step 15 Browser/device compatibility; RPD-004; BPR NFR-UX.. Cite exact source sections, runtime evidence, decisions/config versions and prerequisite task IDs.
...
HDN-380 and all UX evidence.. Every execution-index prerequisite must be `VERIFIED`.

## 10. Downstream impact

Step 16 UAT and release candidate.. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs and release candidate gates.
...
No planned database change except compatibility evidence. Any repair must be additive, reversible, source-domain safe and accompanied by migration/recovery evidence.

## 14. API impact

Verify browser/device differences do not break auth, CSRF, cookies, uploads, downloads, signed URLs or realtime scope. Do not break REST/GraphQL/public API/webhook/export contracts without approved versioning/deprecation and compatibility tests.

## 15. UI/UX impact

Run smoke/E2E on latest Chrome, Edge, Safari, Firefox and representative tablet/mobile breakpoints. Preserve responsive online-first behavior, accessibility, complete states and no fake success/dead action.
...
- Run core flows across internal portal and customer portal.
- Test auth/session, uploads/downloads, ePOD, dashboards, tables, modals and mobile forms.
- Verify responsive PWA online-first behavior and no offline-sync promise.
- Record incompatible issues with severity and workaround.

## 21. Main flow

Core workflows pass on the supported browser/device matrix with responsive layouts and no dead actions.

## 22. Alternative flow

An enterprise browser policy requires additional version testing; record as contract-specific Step 16/UAT condition.
...
- RPD-004 online-first responsive PWA is binding.
- Native mobile/offline sync is not claimed.
- Browser-specific failures in critical flows block release.
- Download/upload/signed URL behavior must be tested on Safari/mobile.
- Responsive mobile must stay functional, not merely visually compressed.
...
- Browser and Device Compatibility automated/manual hardening tests.
...
Update full-system hardening build log, test/evidence matrix, blocker register, issue/error ledger, change manifest, traceability, source-domain ownership, security/performance/accessibility/DR/migration runbooks and exact release-candidate handoff artifacts relevant to Browser and Device Compatibility.
...
- Browser and Device Compatibility hardening gate passes or has explicit release-blocking issue with owner and reproduction.
