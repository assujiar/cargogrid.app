# Prompt 396 — Clean Database Rebuild

**Prompt ID:** `CG-S16-RGL-006`  
**Package document:** `CG-AABPP-RGL-396`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-396.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-006` and exactly one approved WBS/task-ledger item.
...
Workstream: Database Control; Epic: Clean Rebuild; Capability: Clean Database Rebuild; Feature slice: Release/go-live gate for Clean Database Rebuild; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Validate clean database rebuild from zero using migrations, seeds/fixtures, generated types and required environment configuration.

## 5. Business value

Prove a fresh deploy or restore path can recreate the candidate without hidden manual database state.

## 6. Source requirement

Master Prompt Step 16 Clean database rebuild; Delivery Plan database migration, environment, backup/restore and release gate controls.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-397, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Clean Database Rebuild against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Clean Database Rebuild gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-397`. If blocked, resume this task or rollback according to the recorded recovery path.
