# Prompt 400 — UAT Deployment

**Prompt ID:** `CG-S16-RGL-010`  
**Package document:** `CG-AABPP-RGL-400`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-400.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-010` and exactly one approved WBS/task-ledger item.
...
Workstream: UAT Control; Epic: UAT Deployment; Capability: UAT Deployment; Feature slice: Release/go-live gate for UAT Deployment; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Deploy or refresh the tenant-isolated UAT environment and prepare representative data, user accounts, roles, scenarios and evidence capture.

## 5. Business value

Enable business acceptance against realistic end-to-end CargoGrid workflows.

## 6. Source requirement

Master Prompt Step 16 UAT deployment; Delivery Plan UAT governance, end-to-end catalogue and sign-off criteria.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-401, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute UAT Deployment against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- UAT Deployment gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-401`. If blocked, resume this task or rollback according to the recorded recovery path.
