# Prompt 410 — Release Go-Live Integrated Verification

**Prompt ID:** `CG-S16-RGL-020`  
**Package document:** `CG-AABPP-RGL-410`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-410.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-020` and exactly one approved WBS/task-ledger item.
...
Workstream: Release Verification; Epic: Integrated Verification; Capability: Release Go-Live Integrated Verification; Feature slice: Release/go-live gate for Release Go-Live Integrated Verification; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify all Step 16 release/go-live gates at one checkpoint and reconcile evidence into one release readiness matrix.

## 5. Business value

Give Step 17 final package validation a truthful, complete release/go-live evidence base.

## 6. Source requirement

Master Prompt Step 16 full gate set; Production-ready definition; Delivery Plan release, UAT, cutover, support and PIR controls.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-411, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Release Go-Live Integrated Verification against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Release Go-Live Integrated Verification gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-411`. If blocked, resume this task or rollback according to the recorded recovery path.
