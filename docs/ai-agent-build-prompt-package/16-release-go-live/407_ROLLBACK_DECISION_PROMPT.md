# Prompt 407 — Rollback Decision

**Prompt ID:** `CG-S16-RGL-017`  
**Package document:** `CG-AABPP-RGL-407`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-407.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-017` and exactly one approved WBS/task-ledger item.
...
Workstream: Recovery Control; Epic: Rollback Decision; Capability: Rollback Decision; Feature slice: Release/go-live gate for Rollback Decision; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Assess rollback triggers, decision authority, execution steps, data reconciliation, communication and retry conditions during/after deployment.

## 5. Business value

Ensure the team can stop or reverse a bad release before data/security/customer impact expands.

## 6. Source requirement

Master Prompt Step 16 Rollback decision; Delivery Plan rollback criteria, cutover rollback and RCA requirements.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-408, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Rollback Decision against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Rollback Decision gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-408`. If blocked, resume this task or rollback according to the recorded recovery path.
