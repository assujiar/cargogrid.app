# Prompt 399 — Staging Deployment

**Prompt ID:** `CG-S16-RGL-009`  
**Package document:** `CG-AABPP-RGL-399`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-399.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-009` and exactly one approved WBS/task-ledger item.
...
Workstream: Environment Control; Epic: Staging Deployment; Capability: Staging Deployment; Feature slice: Release/go-live gate for Staging Deployment; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Deploy the release candidate to staging with environment, secrets, migrations, observability, rollback and smoke-readiness controls.

## 5. Business value

Prove the release candidate can run in a production-like non-production environment before UAT/go-live.

## 6. Source requirement

Master Prompt Step 16 Staging deployment; Delivery Plan staging/UAT/production environment and release flow.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-400, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Staging Deployment against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Staging Deployment gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-400`. If blocked, resume this task or rollback according to the recorded recovery path.
