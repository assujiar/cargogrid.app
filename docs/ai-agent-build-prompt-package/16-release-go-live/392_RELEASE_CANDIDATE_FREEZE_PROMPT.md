# Prompt 392 — Release Candidate Freeze

**Prompt ID:** `CG-S16-RGL-002`  
**Package document:** `CG-AABPP-RGL-392`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-392.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-002` and exactly one approved WBS/task-ledger item.
...
Workstream: RC Freeze; Epic: Freeze Control; Capability: Release Candidate Freeze; Feature slice: Release/go-live gate for Release Candidate Freeze; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Freeze the release candidate checkpoint, dependency set, schema state, test matrix and change window so Step 16 execution has one immutable candidate baseline.

## 5. Business value

Prevent hidden scope drift and mixed-checkpoint evidence before go-live decisions.

## 6. Source requirement

Master Prompt Step 16 Release Candidate freeze; Delivery Plan phase-gated release, release board, go/no-go, no Sev-1/critical blockers.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-393, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Release Candidate Freeze against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Release Candidate Freeze gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-393`. If blocked, resume this task or rollback according to the recorded recovery path.
