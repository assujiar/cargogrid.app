# Prompt 409 — Post-Implementation Review

**Prompt ID:** `CG-S16-RGL-019`  
**Package document:** `CG-AABPP-RGL-409`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-409.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-019` and exactly one approved WBS/task-ledger item.
...
Workstream: Learning Control; Epic: Post Implementation Review; Capability: Post-Implementation Review; Feature slice: Release/go-live gate for Post-Implementation Review; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Conduct post-implementation review after hypercare covering delivery, quality, data, performance, adoption, support, incidents and improvement backlog.

## 5. Business value

Turn go-live evidence into product, delivery, onboarding and support improvements.

## 6. Source requirement

Master Prompt Step 16 Post-implementation review; Delivery Plan PIR and continuous improvement requirements.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-410, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Post-Implementation Review against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Post-Implementation Review gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-410`. If blocked, resume this task or rollback according to the recorded recovery path.
