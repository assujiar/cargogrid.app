# Prompt 393 — No-New-Feature Rule

**Prompt ID:** `CG-S16-RGL-003`  
**Package document:** `CG-AABPP-RGL-393`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-393.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-003` and exactly one approved WBS/task-ledger item.
...
Workstream: Scope Control; Epic: Feature Freeze; Capability: No-New-Feature Rule; Feature slice: Release/go-live gate for No-New-Feature Rule; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Enforce no-new-feature discipline across branches, flags, backlog, review, environment and communication paths during release candidate execution.

## 5. Business value

Keep release work focused on verified readiness, defect repair and rollout safety.

## 6. Source requirement

Master Prompt Step 16 No-new-feature rule; Product ready definitions; Delivery gate and release board governance.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-394, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute No-New-Feature Rule against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- No-New-Feature Rule gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-394`. If blocked, resume this task or rollback according to the recorded recovery path.
