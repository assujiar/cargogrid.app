# Prompt 411 — Release Go-Live Documentation Handoff

**Prompt ID:** `CG-S16-RGL-021`  
**Package document:** `CG-AABPP-RGL-411`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-411.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-021` and exactly one approved WBS/task-ledger item.
...
Workstream: Documentation Control; Epic: Documentation Handoff; Capability: Release Go-Live Documentation Handoff; Feature slice: Release/go-live gate for Release Go-Live Documentation Handoff; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Reconcile release notes, runbooks, deployment records, support docs, customer communication, known issues, rollback/resume and Step 17 handoff.

## 5. Business value

Preserve context so final package validation and future operators can verify release state without conversation history.

## 6. Source requirement

Master Prompt Step 16 documentation, go-live and support readiness; Delivery Plan runbook, support, RCA and PIR controls.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-412 closure verification, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Release Go-Live Documentation Handoff against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Release Go-Live Documentation Handoff gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-412 closure verification`. If blocked, resume this task or rollback according to the recorded recovery path.
