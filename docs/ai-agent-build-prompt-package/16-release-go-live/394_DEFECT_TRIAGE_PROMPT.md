# Prompt 394 — Defect Triage

**Prompt ID:** `CG-S16-RGL-004`  
**Package document:** `CG-AABPP-RGL-394`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-394.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-004` and exactly one approved WBS/task-ledger item.
...
Workstream: Defect Control; Epic: Release Defect Triage; Capability: Defect Triage; Feature slice: Release/go-live gate for Defect Triage; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Classify, reproduce, rank, assign and close release candidate defects with evidence-backed severity and retry paths.

## 5. Business value

Ensure Sev-1/Sev-2, tenant, security, financial, data-loss and migration blockers cannot be hidden or risk-accepted casually.

## 6. Source requirement

Master Prompt Step 16 Defect triage; Delivery Plan severity model, go/no-go no-go criteria and RCA requirement.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-395, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Defect Triage against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Defect Triage gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-395`. If blocked, resume this task or rollback according to the recorded recovery path.
