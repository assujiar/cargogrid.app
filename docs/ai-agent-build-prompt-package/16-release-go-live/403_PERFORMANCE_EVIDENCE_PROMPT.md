# Prompt 403 — Performance Evidence

**Prompt ID:** `CG-S16-RGL-013`  
**Package document:** `CG-AABPP-RGL-403`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-403.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-013` and exactly one approved WBS/task-ledger item.
...
Workstream: Performance Control; Epic: Performance Evidence; Capability: Performance Evidence; Feature slice: Release/go-live gate for Performance Evidence; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Verify performance evidence, budgets, load scenarios, p95, query plans, queue behavior and scale constraints for release scope.

## 5. Business value

Prevent go-live with known bottlenecks in critical transactional or reporting workflows.

## 6. Source requirement

Master Prompt Step 16 Performance evidence; Delivery Plan performance gates and phase budgets.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-404, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Performance Evidence against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Performance Evidence gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-404`. If blocked, resume this task or rollback according to the recorded recovery path.
