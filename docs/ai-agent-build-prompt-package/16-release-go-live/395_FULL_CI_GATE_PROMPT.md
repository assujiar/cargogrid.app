# Prompt 395 — Full CI Gate

**Prompt ID:** `CG-S16-RGL-005`  
**Package document:** `CG-AABPP-RGL-395`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-395.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-005` and exactly one approved WBS/task-ledger item.
...
Workstream: CI Control; Epic: Full CI Gate; Capability: Full CI Gate; Feature slice: Release/go-live gate for Full CI Gate; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Run and reconcile the full CI gate across lint, typecheck, tests, build, migrations, generated types/specs, audits and release checks.

## 5. Business value

Prove the release candidate is reproducible and not dependent on local/manual shortcuts.

## 6. Source requirement

Master Prompt Step 16 Full CI gate; Master Prompt quality gates; Delivery Plan automated tests and CI/CD gates.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-396, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Full CI Gate against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Full CI Gate gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-396`. If blocked, resume this task or rollback according to the recorded recovery path.
