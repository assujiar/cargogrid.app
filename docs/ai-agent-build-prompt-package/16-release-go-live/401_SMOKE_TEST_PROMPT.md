# Prompt 401 — Smoke Test

**Prompt ID:** `CG-S16-RGL-011`  
**Package document:** `CG-AABPP-RGL-401`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-401.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-011` and exactly one approved WBS/task-ledger item.
...
Workstream: Readiness Control; Epic: Smoke Test; Capability: Smoke Test; Feature slice: Release/go-live gate for Smoke Test; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Run critical smoke tests across core login, tenant isolation, lead-to-cash, shipment, finance, portal, jobs, files, reports and health checks.

## 5. Business value

Detect release-stopping failure quickly after each candidate deployment or cutover step.

## 6. Source requirement

Master Prompt Step 16 Smoke test; Delivery Plan production smoke test and cutover smoke requirements.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-402, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Smoke Test against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Smoke Test gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-402`. If blocked, resume this task or rollback according to the recorded recovery path.
