# Prompt 406 — Post-Deployment Validation

**Prompt ID:** `CG-S16-RGL-016`  
**Package document:** `CG-AABPP-RGL-406`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-406.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-016` and exactly one approved WBS/task-ledger item.
...
Workstream: Validation Control; Epic: Post Deployment Validation; Capability: Post-Deployment Validation; Feature slice: Release/go-live gate for Post-Deployment Validation; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Validate production after deployment through health, smoke, data, tenant, finance, API, file, job, observability and user-facing checks.

## 5. Business value

Confirm production is usable and safe before opening wider access or closing cutover.

## 6. Source requirement

Master Prompt Step 16 Post-deployment validation; Delivery Plan production smoke and go-live readiness.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-407, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Post-Deployment Validation against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Post-Deployment Validation gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-407`. If blocked, resume this task or rollback according to the recorded recovery path.
