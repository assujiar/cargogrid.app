# Prompt 397 — Migration Validation

**Prompt ID:** `CG-S16-RGL-007`  
**Package document:** `CG-AABPP-RGL-397`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-397.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-007` and exactly one approved WBS/task-ledger item.
...
Workstream: Migration Control; Epic: Migration Validation; Capability: Migration Validation; Feature slice: Release/go-live gate for Migration Validation; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Validate migration apply, upgrade, rollback/forward recovery, schema diff, RLS, constraints, indexes and generated type compatibility.

## 5. Business value

Prevent migration failure, data corruption or unreviewed destructive changes during cutover.

## 6. Source requirement

Master Prompt Step 16 Migration validation; Delivery Plan data migration rehearsal, cutover and rollback controls.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-398, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Migration Validation against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Migration Validation gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-398`. If blocked, resume this task or rollback according to the recorded recovery path.
