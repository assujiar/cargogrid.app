# Prompt 398 — Seed Validation

**Prompt ID:** `CG-S16-RGL-008`  
**Package document:** `CG-AABPP-RGL-398`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-398.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-008` and exactly one approved WBS/task-ledger item.
...
Workstream: Data Control; Epic: Seed Validation; Capability: Seed Validation; Feature slice: Release/go-live gate for Seed Validation; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Validate seed, fixture and reference data for release candidate, UAT, staging and production bootstrap without leaking tenant-real data.

## 5. Business value

Ensure deterministic test/onboarding data and configuration are valid, safe and reproducible.

## 6. Source requirement

Master Prompt Step 16 Seed validation; Delivery Plan UAT data, environment data policy and tenant onboarding readiness.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-399, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Seed Validation against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Seed Validation gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-399`. If blocked, resume this task or rollback according to the recorded recovery path.
