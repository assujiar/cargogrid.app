# Prompt 405 — Production Deployment

**Prompt ID:** `CG-S16-RGL-015`  
**Package document:** `CG-AABPP-RGL-405`  
**Version:** `0.17.0`  
**Runtime build log:** `docs/build-log/release-go-live/RGL-405.md`
...
`{{TASK_ID}}` mapped to `CG-S16-RGL-015` and exactly one approved WBS/task-ledger item.
...
Workstream: Deployment Control; Epic: Production Deployment; Capability: Production Deployment; Feature slice: Release/go-live gate for Production Deployment; Atomic task: `{{WBS_TASK_ID}}`.

## 4. Objective

Execute production deployment only after go decision, with change window, backup, migrations, feature flags, monitoring and communication controls.

## 5. Business value

Move the verified release candidate into production without bypassing readiness or rollback controls.

## 6. Source requirement

Master Prompt Step 16 Production deployment; Delivery Plan cutover plan, feature flags, backup and rollback readiness.. Cite exact source sections, runtime evidence, approval records, decision/config versions and prerequisite task IDs.
...
RGL-406, Step 17 final package validation, customer readiness, support readiness and production/rollback evidence. Identify affected schemas, services, REST/GraphQL/public API/webhooks, reports/dashboards, jobs/integrations/files, AI/automation, enterprise controls, tests, docs, runbooks and go/no-go gates.
...
Execute Production Deployment against the approved release candidate at one checkpoint, collect evidence, classify failures, repair only authorized defects, and publish a clear pass/fail/no-go result.
...
- Production Deployment gate has complete evidence at one release candidate checkpoint.
...
If this task is `VERIFIED`, continue only to `RGL-406`. If blocked, resume this task or rollback according to the recorded recovery path.
