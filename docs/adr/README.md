...
| `ADR-CAND-ARCH-012` | `customers` extension-table vs. flat-column strategy | Phase 2 Commercial schema | `BLOCKED` — needs Phase 2 schema slice. *Correction (`PLT-139`): previously read "Phase 1 schema (Prompt 120+)" — `customers` is a Commercial-domain table, and Platform Core (`PLT-105..138`) closed without building any Commercial schema (confirmed: no `customers`/CRM table exists anywhere in `supabase/migrations/`), so this was always Phase 2 scope, not Phase 1 — a stale reference, not a missed Phase 1 obligation.* |
