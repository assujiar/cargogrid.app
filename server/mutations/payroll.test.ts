...
import {
  ...
  commitPayrollLoanCutoverImportJob,
...
describe("validatePayrollLoanCutoverImportRow (CG-AUDIT-2026-09-02 A4)", () => {
  test("calls validate_payroll_loan_cutover_import_row with the exact snake_case params", async () => {
    const { client, calls } = fakeClient({
      data: {
        id: ROW_ID,
        tenant_id: TENANT_ID,
        job_id: JOB_ID,
        row_number: 1,
        raw_payload: { employee_number: "EMP-1", principal_amount: "1000000", installment_amount: "100000", term_count: "12", remaining_installments: "5" },
    ...
    const row = await validatePayrollLoanCutoverImportRow(client, { stagingRowId: ROW_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester" });
  ...
  test("wraps a database error into a typed PayrollMutationError", async () => {
    const { client } = fakeClient({ data: null, error: { message: "import_export_staging_row_not_found: no staging row" } });
    await assert.rejects(() => validatePayrollLoanCutoverImportRow(client, { stagingRowId: ROW_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester" }), PayrollMutationError);
  });
});

describe("commitPayrollLoanCutoverImportJob (CG-AUDIT-2026-09-02 A4)", () => {
  test("calls commit_payroll_loan_cutover_import_job with the exact snake_case params, including client IP, defaulting allowPartial to false", async () => {
    const { client, calls } = fakeClient({
      data: {
        job_id: JOB_ID,
        tenant_id: TENANT_ID,
        ...
        idempotency_key: "idem-payroll-loan-cutover-import-job",
        import_export_schema_code: "payroll_loan_cutover_import",
    ...
    const job = await commitPayrollLoanCutoverImportJob(client, { jobId: JOB_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester", clientIp: "203.0.113.5" });
  ...
  test("classifies job_actor_unauthorized, mfa_step_up_required, and ip_not_allowed", async () => {
    ...
    await assert.rejects(
      () => commitPayrollLoanCutoverImportJob(membershipClient, { jobId: JOB_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester" }),
      (err: unknown) => {
        assert.ok(err instanceof PayrollMutationError);
    ...
    await assert.rejects(
      () => commitPayrollLoanCutoverImportJob(mfaClient, { jobId: JOB_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester" }),
      (err: unknown) => {
        assert.ok(err instanceof PayrollMutationError);
    ...
    await assert.rejects(
      () => commitPayrollLoanCutoverImportJob(ipClient, { jobId: JOB_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester", clientIp: "bad-ip" }),
      (err: unknown) => {
        assert.ok(err instanceof PayrollMutationError);
  ...
  test("classifies insufficient_authority when the actor lacks HRS:Approve (issue_payroll_loan's own gate) even with HRS:Import", async () => {
    const { client } = fakeClient({ data: null, error: { message: "insufficient_authority: actor lacks HRS:Approve" } });
    await assert.rejects(
      () => commitPayrollLoanCutoverImportJob(client, { jobId: JOB_ID, actorAuthUserId: ACTOR_ID, actorLabel: "tester" }),
      (err: unknown) => err instanceof PayrollMutationError && err.code === "insufficient_authority",
    );
  });
});
