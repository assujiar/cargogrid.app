...
describe("getDriverMobileTrackingSession", () => {
  test("returns null when no active token was ever issued", async () => {
    ...
    const session = await getDriverMobileTrackingSession(client, SLTS_ID, ACTOR_ID);
  ...
  test("parses an active session row", async () => {
    ...
    assert.equal(calls[0]?.fn, "get_driver_mobile_tracking_session");
    assert.equal(calls[0]?.args.p_shipment_leg_tracking_session_id, SLTS_ID);
  ...
  test("throws on a query error", async () => {
    ...
    await assert.rejects(() => getDriverMobileTrackingSession(client, SLTS_ID, ACTOR_ID), DriverMobileTrackingQueryError);
  });
});
