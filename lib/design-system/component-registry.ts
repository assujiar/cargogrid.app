...
export const COMPONENT_REGISTRY: readonly ComponentEntry[] = [
  ...
  { name: "Number input", category: "DataEntry", status: "IMPLEMENTED", purpose: "type=\"number\" specialization of Input — never used for money (see Currency input).", sourceFile: "components/forms/number-input.tsx", importSnippet: 'import { NumberInput } from "@/components/forms/number-input.tsx";', consumers: [], productionReady: true, citation: CHECKPOINT_4_CITATION },
  {
    ...
    consumers: [
      "commercial/contracts/[contractId]/add-component-form.tsx (base + minimum amount)",
      "commercial/costing-requests/[requestId]/costing-request-actions-panel.tsx (per-component response amounts — was NumberInput, which its own header forbids for money)",
      "commercial/costing-requests/[requestId]/{select-rate-form,calculate-margin-form}.tsx (ad-hoc quote amount, sell amount)",
      "commercial/opportunities/[opportunityId]/opportunity-actions-panel.tsx (opportunity value)",
      "(all ISS-2026-246, 2026-09-02)",
    ],
    a11y: "Sanitizes keystrokes to at most 2 decimal places; aria-invalid from the invalid prop.",
    productionReady: true,
    duplicateNote:
      "Money fields submitted through an uncontrolled Server-Action form (finance/{receipts,settlements,vendor-bills,exchange-rates}, procurement/{rates,sourcing,approvals}, commercial/{rates,accounts}, operations actual-cost) remain on Input/NumberInput by design: this primitive is controlled-only and those forms hold neither the amount nor the currency in client state. Quotation line unit price stays on Input too — it is prefilled from a margin calculation's net sell amount, which can carry more than the 2 decimals this component allows.",
    citation: CHECKPOINT_4_CITATION + "; 6 money fields migrated ISS-2026-246, 2026-09-02",
  },
  ...
  },
  {
    ...
    consumers: ["hris/overtime-timesheet/overtime-timesheet-admin-panel.tsx — EmployeePicker, shared by 3 forms, over the active employee directory (capped at 200) (ISS-2026-246, 2026-09-02)"],
    productionReady: true,
    citation: CHECKPOINT_4_CITATION + "; first consumer ISS-2026-246, 2026-09-02",
  },
  {
    name: "Multi-select",
    category: "DataEntry",
    status: "IMPLEMENTED",
    purpose: "Multiple-choice with chip display, same filtered-list pattern as Combobox.",
    sourceFile: "components/forms/multi-select.tsx",
    importSnippet: 'import { MultiSelect } from "@/components/forms/multi-select.tsx";',
    consumers: ["admin/api-keys/api-keys-admin-panel.tsx — n8n connector scopes, picked from the safe-action allowlist the RPC enforces (ISS-2026-246, 2026-09-02)"],
    productionReady: true,
    duplicateNote:
      "The plain API-key scope field beside it stays free text: its valid set is the creating actor's own current RBAC, which that component is never handed — there is no option list to pick from. The 3-value attendance channel checkbox group stays a checkbox group; a known 3-item set is what Checkbox is for.",
    citation: CHECKPOINT_4_CITATION + "; first consumer ISS-2026-246, 2026-09-02",
  },
  ...
  { name: "Radio", category: "DataEntry", status: "IMPLEMENTED", purpose: "Single choice, always-visible set — native <input type=\"radio\">; RadioGroup renders a labeled fieldset.", sourceFile: "components/forms/radio.tsx", importSnippet: 'import { Radio, RadioGroup } from "@/components/forms/radio.tsx";', consumers: [], productionReady: true, citation: CHECKPOINT_4_CITATION },
  {
    ...
    consumers: ["customer-shipments/[shipmentOrderId]/customer-shipment-alert-subscriptions-panel.tsx — the 6 \"Notify me\" alert toggles, which act on flip (ISS-2026-246, 2026-09-02)"],
    productionReady: true,
    duplicateNote:
      "Every other boolean in the tree stays a Checkbox on purpose: scheduler \"Run automatically\", vendor self-registration, leave/training/roster policy flags and the rest are all form-submit-pending booleans behind an explicit Save, which is Checkbox's half of the split, not this one's.",
    citation: CHECKPOINT_4_CITATION + "; first consumer ISS-2026-246, 2026-09-02",
  },
  ...
  { name: "Alert", category: "Feedback", status: "IMPLEMENTED", purpose: "Inline, non-persistent, dismissible emphasis block, distinct from Banner's page-level persistence.", sourceFile: "components/ui/alert.tsx", importSnippet: 'import { Alert } from "@/components/ui/alert.tsx";', variants: ["info", "warning", "success", "danger"], consumers: [], productionReady: true, citation: CHECKPOINT_4_CITATION },
  {
    ...
    importSnippet: 'import { ToastProvider, useToast } from "@/components/ui/toast.tsx";',
    consumers: [],
  ...
  { name: "Success State", category: "Feedback", status: "IMPLEMENTED", purpose: "Whole-page/whole-section confirmation, distinct from Banner's success variant and StatusBadge's success tone (both inline).", sourceFile: "components/ui/success-state.tsx", importSnippet: 'import { SuccessState } from "@/components/ui/success-state.tsx";', consumers: [], productionReady: true, citation: "Not previously named in any CargoGrid design-system document — disclosed as a gap when first requested, then implemented. " + CHECKPOINT_4_CITATION },
  { name: "Progress", category: "Feedback", status: "IMPLEMENTED", purpose: "Determinate long-running-operation indicator — native <progress>.", sourceFile: "components/ui/progress.tsx", importSnippet: 'import { Progress } from "@/components/ui/progress.tsx";', consumers: [], productionReady: true, citation: CHECKPOINT_4_CITATION },
