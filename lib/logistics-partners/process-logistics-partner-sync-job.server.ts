...
export async function processLogisticsPartnerSyncJob(client: ProcessLogisticsPartnerSyncJobRpcClient, job: ImportExportJob, workerId: string, actorLabel: string, checkUrlSafety: LogisticsPartnerDispatchUrlSafetyChecker = checkWebhookDispatchUrlIsSafe): Promise<LogisticsPartnerSyncJobOutcome> {
  ...
  // -- this worker did not: it resolved connection.tenantId and used it to record sync
  // events without ever comparing it back to job.tenantId, so a foreign connection's
  // data/credential would be polled and recorded under the wrong tenant. Fail closed before
  // any live dispatch or state-machine write, mirroring the webhook worker's own fix exactly.
  if (connection.tenantId !== job.tenantId) {
    const errorMessage = `logistics-partner connection ${connectionId} belongs to tenant ${connection.tenantId}, not the enqueuing job's tenant ${job.tenantId}`;
    await recordJobFailure(client, { jobId: job.jobId, errorMessage, actorAuthUserId, actorLabel });
    return { outcome: "failed", recordedCount: 0, skippedCount: 0, errorMessage };
  }
