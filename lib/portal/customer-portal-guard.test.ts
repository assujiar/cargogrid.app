...
import { resolveCustomerPortalAccess, type CustomerPortalGuardDeps, type TenantLookupResult } from "./customer-portal-guard.ts";
...
function deps(overrides: Partial<CustomerPortalGuardDeps> = {}): CustomerPortalGuardDeps {
  return {
...
describe("resolveCustomerPortalAccess", () => {
  test("returns unauthenticated when there is no current user", async () => {
    const result = await resolveCustomerPortalAccess(deps({ getCurrentUserId: async () => null }), "acme");
    assert.equal(result.status, "unauthenticated");
  });

  test("returns tenant_not_found when the tenant lookup returns null", async () => {
    const result = await resolveCustomerPortalAccess(deps({ findTenantBySlug: async () => null }), "does-not-exist");
  ...
  test("returns tenant_suspended for a non-active tenant, without ever checking customer_user layer", async () => {
    ...
    const result = await resolveCustomerPortalAccess(
      deps({
  ...
  test("returns allowed for a real, active customer_user membership", async () => {
    const result = await resolveCustomerPortalAccess(deps(), "acme");
  ...
  test("returns forbidden for an org_user/tenant_admin (the internal tenant workspace is a different guard/route family, never this one)", async () => {
    const result = await resolveCustomerPortalAccess(deps({ actorHoldsCustomerUserLayer: async () => false }), "acme");
